...
class Statistics(object):
    ...
    def get_field(self, name):
        for field in self.qs.model._meta.fields:
            if field.name == name:
                return field
    ...
    def calc(self):
        ...
        self._xfield = self.get_field(self.x.replace('__year', '').replace('__month', ''))
        if self._xdict == {}:
        ...
        if self.y:
            self._yfield = self.get_field(self.y.replace('__year', '').replace('__month', ''))
