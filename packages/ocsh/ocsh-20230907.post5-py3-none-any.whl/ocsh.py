...
"""
    ...
    def __init__(self, conf, destination, jumphosts=None, args=None, ssh_options=None):
        ...
        debug("XXX ssh_cmd=%s, ssh_target=%s, post=%s, conf=%s, args=%s, ssh_options=%s," % (ssh_cmd, ssh_target, post, conf, args, ssh_options))
        ...
        if 'pass' in self.conf:
            ...
            if stricthostkeychecking == 'yes':
                ...
                if check_res.returncode == 1:
                    ...
                    # XXX the above does not work when a proxy is involved, since ssh-keyscan does not respect ProxyJump / ProxyCommand
