...
def schedule_gant_chart_fig(schedule_dataframe: pd.DataFrame,
    ...
    def create_zone_row(swork, zone) -> dict:
        return {'idx': len(schedule_dataframe),
                'contractor': swork.contractor,
                'cost': 0,
                'volume': 0,
                'duration': 0,
                'measurement': 'unit',
    ...
    for swork in sworks:
        for zone in swork.zones_pre:
            schedule_dataframe = schedule_dataframe.append(create_zone_row(swork, zone), ignore_index=True)
