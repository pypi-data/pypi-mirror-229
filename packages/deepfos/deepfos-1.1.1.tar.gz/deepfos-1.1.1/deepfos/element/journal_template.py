...
from deepfos.element.datatable import (
    DataTableMySQL, DataTableSQLServer, DataTableOracle, DataTableClickHouse,
    AsyncDataTableMySQL, AsyncDataTableOracle, AsyncDataTableClickHouse, AsyncDataTableSQLServer)
...
class AsyncJournalTemplate(ElementBase):
    """单据模板"""
    ...
    def _datatable_class(
        ...
        if db_type == 'mysql':
            return AsyncDataTableMySQL
        if db_type == 'clickhouse':  # pragma: no cover
            return AsyncDataTableClickHouse
        if db_type == 'oracle':  # pragma: no cover
            return AsyncDataTableOracle
        if db_type == 'sqlserver':  # pragma: no cover
            return AsyncDataTableSQLServer

        raise TypeError(f"Unknown datatable type: {db_type}")

    @cached_property
    def approval_record(self) -> Union[AsyncDataTableMySQL,
    ...
    @cached_property
    def close_account(self) -> Union[AsyncDataTableMySQL,
                                     AsyncDataTableClickHouse,
                                     AsyncDataTableOracle,
                                     AsyncDataTableSQLServer]:
        """关账流程数据表元素"""
...
class JournalTemplate(AsyncJournalTemplate, metaclass=SyncMeta):
    ...
    @cached_property
    def approval_record(self) -> Union[DataTableMySQL,
                                       DataTableClickHouse,
                                       DataTableOracle,
                                       DataTableSQLServer]:
        """审批记录数据表元素"""
    ...
    @cached_property
    def close_account(self) -> Union[DataTableMySQL,
                                     DataTableClickHouse,
                                     DataTableOracle,
                                     DataTableSQLServer]:
        """关账流程数据表元素"""
