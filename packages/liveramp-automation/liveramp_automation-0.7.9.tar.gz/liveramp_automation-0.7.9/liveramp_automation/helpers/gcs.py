...
class GoogleCloudStorageHelper:
    ...
    def upload_files_to_bucket(self, local_file_path, cloud_blob_path):
        """Upload a local file to the cloud storage bucket.
    ...
    def upload_file(self, source_file_path, destination_blob_name):
        """Upload a file in a specific bucket.

        :param source_file_path: Path to the file update.
        :param destination_blob_name: Path to the bucket file update.
        :return: None."""

        Logger.info("Start upload_file")
        Logger.info(f"File = {source_file_path} on Path = {destination_blob_name}")
        blob = self.bucket.blob(destination_blob_name)
        result = blob.upload_from_filename(source_file_path)
        Logger.info("Finish upload_file")
        if result is None:
            result = 0
        return result
