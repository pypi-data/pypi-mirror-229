...
class Validate:
	...
	def __basic(self):
		...
		print("Chad v4.2 ( github.com/ivan-sincek/chad )")
		print("")
	...
	def __validate(self, key, value):
		...
		if len(value) > 0:
			...
			# --------------------
				...
				if not os.path.isfile(self.__args["proxies"]):
					self.__error("File with proxies does not exists")
				elif not os.access(self.__args["proxies"], os.R_OK):
