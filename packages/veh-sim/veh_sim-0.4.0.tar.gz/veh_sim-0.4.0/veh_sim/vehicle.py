...
class Vehicle(pygame.sprite.Sprite):
    def __init__(self, p_image, lane_borders, p_bottom_pos=(0,0), p_init_lat_spd=0, p_init_lng_spd=0, p_max_spd=30, is_motorcycle=False, set_top=False):
        ...
        if is_motorcycle:
            self.image = pygame.transform.scale(self.image, (veh_sim.globals.ROAD_LANE_WIDTH * 0.3, veh_sim.globals.ROAD_LANE_WIDTH * (h_height / h_width) * 0.3))  # scale the image based on given veh_sim.globals.WIDTH
        else:
            self.image = pygame.transform.scale(self.image, (veh_sim.globals.ROAD_LANE_WIDTH * 0.6, veh_sim.globals.ROAD_LANE_WIDTH * (h_height / h_width) * 0.6))  # scale the image based on given veh_sim.globals.WIDTH
    ...
    def turn_right(self, degree_step=1):
        ...
        self.steering_wheel_angle = max(self.steering_wheel_angle - degree_step, -veh_sim.globals.MAX_STEERING_ANGLE)

    def turn_left(self, degree_step=1):
        self.initial_angle = self.angle
        self.initial_radius = self.turning_radius
        self.steering_wheel_angle = min(self.steering_wheel_angle + degree_step, +veh_sim.globals.MAX_STEERING_ANGLE)
    ...
    def forward(self, speed=5):
        ...
        if self.steering_wheel_angle == 0:
            ...
            self.turning_radius = veh_sim.globals.TURNING_RADIUS_MULTIPLIER / math.radians(self.steering_wheel_angle)
