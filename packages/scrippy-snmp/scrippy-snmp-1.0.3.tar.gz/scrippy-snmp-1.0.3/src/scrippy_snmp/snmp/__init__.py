...
class Snmp:
  ...
  def __init__(self, host, port, community, version, timeout=1, retries=3):
    logging.debug("[+] SNMP agent initialization")
  ...
  def get(self, oid):
    logging.debug(f"[+] Retrieving OID {oid}")
  ...
  def walk(self, oid=".1.3.6"):
    logging.debug(f"[+] Retrieving from OID {oid}")
  ...
  def set(self, oid, value, datatype):
    logging.debug(f"[+] Setting value to OID {oid} => {value}")
  ...
  def _set_community(self, community, version):
    logging.debug(f" '-> Setting community: {community} / Version: {version}")
  ...
  def _set_transport(self, timeout=1, retries=3):
    logging.debug(f" '-> Setting transport: UDP: {self.host}:{self.port}")
