...
class AgentSyncSDK:
    ...
    def download_file(self, filename, parent_path):
        request = agent_server_pb2.DownloadRequest(filename=filename)
    ...
    def upload_file(self, filepath, filename):
        ...
        def generate_trunk(filepath, filename) -> agent_server_pb2.FileChunk:
            try:
                with open(filepath, "rb") as fp:
                    while True:
                        ...
                        yield agent_server_pb2.FileChunk(
                            buffer=chunk, filename=filename
                        )
...
class AgentSDK:
    ...
    async def upload_binary(
        self, chunks: AsyncIterable[agent_server_pb2.FileChunk], filename
    ):
        try:
    ...
    async def upload_file(self, filepath, filename):
        ...
        await self.upload_binary(generate_trunk(filepath, filename), filename)
