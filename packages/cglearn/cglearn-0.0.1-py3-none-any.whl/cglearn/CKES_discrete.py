...
class CKESAlgorithmDiscrete:
    ...
    def learnLWFCG(self, pThresh, freq_tb):
        cg = self.cg
                        ...
                        if self.hasUndirected(x, y, cg):
                            ...
                            for bdVar in range(n):
                                if bdVar != x and bdVar != y:
                                    ...
                                    if self.hasDirected(bdVar, y, cg) or self.hasUndirected(bdVar, y, cg):
                                        bdY.append(bdVar)
                            pValOp = min(multinom_ci_test(freq_tb, x, y, bdX)["pval"], multinom_ci_test(freq_tb, x, y, bdY)["pval"])
                            if pValOp > pRemove:
                                pRemove = pValOp
                                self.removeEdge(x, y, cg)
                                bestCG = cg.copy()
                            ...
                            for bdVar in range(n):
                                if bdVar != x and bdVar != y and (self.hasDirected(bdVar, y, cg) or self.hasUndirected(bdVar, y, cg)):
                                    bdY.append(bdVar)
                            pValOp = multinom_ci_test(freq_tb, x, y, bdY)["pval"]
                            if pValOp > pRemove:
                                pRemove = pValOp
                                self.removeEdge(x, y, cg)
                                bestCG = cg.copy()
                            ...
                            for bdVar in range(n):
                                if bdVar != x and bdVar != y:
                                    ...
                                    if self.hasDirected(bdVar, y, cg) or self.hasUndirected(bdVar, y, cg):
                                        bdY.append(bdVar)
                            pValX = multinom_ci_test(freq_tb, x, y, bdX)["pval"]
                            pValY = multinom_ci_test(freq_tb, x, y, bdY)["pval"]
