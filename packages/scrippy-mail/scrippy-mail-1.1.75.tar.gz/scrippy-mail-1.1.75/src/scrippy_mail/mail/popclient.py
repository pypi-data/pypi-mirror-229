...
class PopClient:
  ...
  def connect(self):
    ...
    logging.debug(f"[+] Connecting to POP server: {self.host}:{self.port}")
  ...
  def authenticate(self, username, password):
    ...
    """
    logging.debug(f"[+] Auhentication: {username}:{password}")
    buffer = BytesIO()
  ...
  def stat(self):
    ...
    Raises a ScrippyMailError error if authentication fails.
    """
    logging.debug("[+] Getting available mails number")
    buffer = BytesIO()
    buffer.write(b'STAT\r\n')
    self._send_data(buffer.getvalue())
    resp = self._recv_data()
  ...
  def retr(self, num):
    ...
    Raises a ScrippyMailError error if authentication fails.
    """
    logging.debug(f"[+] Getting email number: {num}")
    buffer = BytesIO()
  ...
  def dele(self, num):
    ...
    Raises a ScrippyMailError error if authentication fails.
    """
    logging.debug(f"[+] Deleting email number: {num}")
    buffer = BytesIO()
  ...
  def bye(self):
    ...
    logging.debug("[+] Disconnecting from email server")
    buffer = BytesIO()
    buffer.write(b'QUIT\r\n')
    self._send_data(buffer.getvalue())
