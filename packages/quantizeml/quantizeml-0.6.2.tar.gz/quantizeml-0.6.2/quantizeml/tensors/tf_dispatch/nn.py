...
def fp_qf_matmul(a: FixedPoint,
    ...
    fp_prod = tf.matmul(a, b.fp, transpose_a, transpose_b, adjoint_a, adjoint_b, name)
...
def qf_qf_matmul(a: QFloat,
    ...
    # Do matmul on a and b inner FixedPoint
    fp_prod = tf.matmul(a.fp, b.fp, transpose_a, transpose_b, adjoint_a, adjoint_b, name)
...
@tf.experimental.dispatch_for_api(tf.nn.conv2d_transpose)
def fp_fp_conv2d_transpose(
        input: FixedPoint,
        filters: FixedPoint,
        output_shape, strides=None, padding="VALID", data_format=None, dilations=None, name=None):
    ...
    Args:
        ...
        filters (:obj:`FixedPoint`): A FixedPoint with a [height, width, out_channels, in_channels]
            shape. Filter's in_channels dimension must match that of input.
    ...
    filter_values = tf.transpose(filters.values, (0, 1, 3, 2))

    # Do conv2d_transpose on values
    conv_values = tf.nn.conv2d_transpose(input.values, filter_values, output_shape, strides,
...
def fp_qf_conv2d_transpose(
    ...
    Args:
        input (:obj:`FixedPoint`): A 4-D FixedPoint of shape [batch, height, width, in_channels] for
            NHWC data format or [batch, in_channels, height, width] for NCHW data format.
        filters (:obj:`QFloat`): A QFloat of shape [height, width, out_channels, in_channels]
            shape. Filter's in_channels dimension must match that of input.
        output_shape (tf.Tensor): A 1-D Tensor representing the output shape of the
            deconvolution op.
        strides (list, optional): Sequence of N ints >= 1. Specifies the output stride.
    ...
    conv_values = tf.nn.conv2d_transpose(input, filters.fp, output_shape, strides,
                                         padding, data_format, dilations, name)

    # Return a new QFloat
    return QFloat(conv_values, filters.scales)
