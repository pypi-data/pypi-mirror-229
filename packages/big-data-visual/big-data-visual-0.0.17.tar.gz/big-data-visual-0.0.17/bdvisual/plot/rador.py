...
import bdvisual.plot.outdir as outdir
import cv2 as cv
class AbilityRadorGraph:
    ...
    def draw(title,propname, propvalue1, propvalue2, outfilename, propvalueShow = 0, gridShow = 0, gridLabelShow = 0, outline= 0, legendShow = 0):
        # 设置图形的大小
        ...
        # 新建一个子图，这个子图是本文其余子图的2倍大，不然图例显示别扭
        ...
        # ax.rcParams['axes.spines.right'] = False
        # ax.rcParams['axes.spines.top'] = False
        # ax.rcParams['axes.spines.left'] = False
        # ax.rcParams['axes.spines.bottom'] = False
        ...
        #设置网格样式
        ax.tick_params(pad=10, grid_color='k', 
                       grid_alpha=0.1 if gridShow == 1 else 0,
                       grid_linewidth=1, 
                       grid_linestyle=(0, (5, 5)),
                       labelsize = 10,
                       labelcolor = "#2454d100" if gridLabelShow == 0 else "#2454d1")
        ...
        plt.thetagrids(angles*180/np.pi, propname,color = "#2454d1",fontsize = 10)
        # plt.thetagrids(angles, labels=None,)
        ...
        # 设置图例透明度
        # for patch in plt.gca().get_legend().get_patches():
        #     patch.set_alpha(0.2)
        
        #没有给折线图添加label值，所以这个地方要手动设置

        #设置标题
        ax.set_title(title, fontdict={'color':'#2454d1'},y = -0.5)
        ...
        # 利用opencv去黑边
        image = cv.imread(outdir.get_output_dir() + outfilename + ".png")

        lower_green = np.array([0, 0, 0])
        upper_green = np.array([1, 1, 200])
        ...
        cv.imwrite(outdir.get_output_dir() + outfilename + '_no_border.png', image)