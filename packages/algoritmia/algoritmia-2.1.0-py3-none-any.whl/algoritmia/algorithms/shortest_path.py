...
from algoritmia.algorithms.traversers import traverser_bf, traverser_dijkstra_dict, traverser_dijkstra_metric_dict
...
def shortest_path_positive_weighted_graph(g: IGraph[TVertex], d: WeightingFunction,
    ...
    edges = traverser_dijkstra_dict(g, d, v_source)  # Con diccionario: O(|V|^2)
    # edges = traverser_dijkstra_heapmap(g, d, v_source)  # Con diccionario de prioridad: O(|V| + |E| log |V|)
    return path_recover(edges, v_target)
...
def shortest_path_metric_graph(g: IGraph[TVertex], d: WeightingFunction,
                               ...
                               v_source: TVertex, v_target: TVertex) -> TPath:
    edges = traverser_dijkstra_metric_dict(g, d, dist, v_source, v_target)
    return path_recover(edges, v_target)
