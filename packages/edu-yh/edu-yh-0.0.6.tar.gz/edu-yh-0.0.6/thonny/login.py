...
class UserLogin:
    ...
    def create_login_view(self):
        ...
        resized_image = original_image.resize((300, 130), Image.LANCZOS)  # Resize to 100x100
        ...
        tk.Label(image=self.login_image).place(relx=0.42, rely=0.3)


        # 添加"账号："标签和输入框
        # self.canvas.create_text(x, y + 15, text="账号：", anchor="nw")
        tk.Label(text="账号：", background='#fff').place(relx=0.42, rely=0.452)
        self.account_var = tk.StringVar()
        self.account_entry = tk.Entry(self.canvas, textvariable=self.account_var)
        self.account_entry.place(relx=0.46, rely=0.452, relwidth=0.11, relheight=0.025)

        # 添加"密码："标签和输入框
        tk.Label(text="密码：", background='#fff').place(relx=0.42, rely=0.495)
        # self.canvas.create_text(x, y + 65, text="密码：", anchor="nw")
        self.password_var = tk.StringVar()
        self.password_entry = tk.Entry(self.canvas, show="*", textvariable=self.password_var)

        self.password_entry.place(relx=0.46, rely=0.495, relwidth=0.11, relheight=0.025)

        button = tk.Button(self.canvas, text="登录", command=self.login)
        self.account_entry.configure(state="normal")
        self.password_entry.configure(state="normal")
        button.place(relx=0.49, rely=0.55, relwidth=0.03, relheight=0.04)
