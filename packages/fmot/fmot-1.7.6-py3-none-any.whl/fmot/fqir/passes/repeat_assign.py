...
def deref_assigns(arith: fqir.GraphProto, tensor: fqir.TensorProto, assigns: List[fqir.NodeProto]):
    curr_buff = None
    ...
    for node in arith.nodes:
        ...
        elif tensor in node.inputs.values() and curr_buff is not None and node.opname != 'assign':
