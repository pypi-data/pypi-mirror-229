...
from ..models.decoded_model import DecodedTransaction, DecodedTransactionDB, Proxy
...
class DecoderService:
    ...
    def decode_transaction(
        ...
        stored_decoded_tx = self.semantic_decoder.repository.get_decoded_transaction(
            tx_hash, chain_id
        )
        if stored_decoded_tx:
            return DecodedTransaction(
                balances=stored_decoded_tx.balances,
                block_metadata=stored_decoded_tx.block_metadata,
                calls=stored_decoded_tx.calls,
                events=stored_decoded_tx.events,
                metadata=stored_decoded_tx.metadata,
                status=stored_decoded_tx.status,
                transfers=stored_decoded_tx.transfers,
            )
        ...
        decoded_tx_db = DecodedTransactionDB(
            txHash=tx_hash,
            chainId=chain_id,
            balances=semantically_decoded_tx.balances,
            block_metadata=semantically_decoded_tx.block_metadata,
            calls=semantically_decoded_tx.calls,
            events=semantically_decoded_tx.events,
            metadata=semantically_decoded_tx.metadata,
            status=semantically_decoded_tx.status,
            transfers=semantically_decoded_tx.transfers,
        )
        self.semantic_decoder.repository.insert_decoded_transaction(decoded_tx_db)
