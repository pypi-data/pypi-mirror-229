...
class ParsimConductor:
    ...
    def __calculate_coil_length_with_resistance_meas(self, conductor_sample: ConductorSample, coil_name: str,
        ...
        if not bare_cable_width: raise Exception(
            'Could not find bare cable width in conductor database and in original conductor.')
        ...
        if not strand_twist_pitch: raise Exception(
            'Could not find strand twist pitch in conductor database and in original conductor.')
        ...
        if not Cu_noCu: raise Exception(
            'Could not find copper-non-copper-ratio in conductor database (nor general nor measurement specific) and in original conductor.')
    ...
    def __write_parsweep_conductors(self, dict_sweeper: Dict[str, Any], flag_optimize_fCu: bool,
        ...
        for coil_name, coil in self.data_parsim_conductor.Coils.items():
            ...
            if flag_optimize_fCu:
                ...
                if not coil.coil_resistance_room_T:
                    warnings.warn(
                        f'No room temperature resistance measurement for coil "{coil_name}" was found in database. Not optimizing fraction of copper.')
                    continue
    ...
    def __calculate_fCu_with_resistance_meas(self, conductor_sample: ConductorSample, coil_name: str,
        ...
        n_strands = conductor_sample.number_of_strands
        if not n_strands: n_strands = original_conductor.cable.n_strands
        if not n_strands: raise Exception(
            'Could not find number of strands in conductor database and in original conductor.')
        A_cond = A_strand * n_strands
