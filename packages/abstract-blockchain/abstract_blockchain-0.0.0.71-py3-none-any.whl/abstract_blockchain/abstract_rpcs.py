...
Functions:
    ...
    get_key_from_value: Fetches the key for a given value from the `get_rpc_js()` mapping.
    get_js: Provides a dictionary mapping of RPC parameters.
    get_keys: Fetches the keys from the `get_rpc_js()` mapping.
    get_values: Fetches the values from the `get_rpc_js()` mapping.
...
def rpc_win_while(event:str):
    ...
    if event == "-NETWORK_NAME-":
        rpc_add_global_bridge["window_mgr"].update_values(window=window, key="-NETWORK_NAME_CHECK-", args={"value":True})
        rpc_add_global_bridge["check_list"][event] = ''
        for each in list(get_rpc_js().values()):
            if each != event:
                rpc_add_global_bridge["check_list"][each] = ''
                rpc_add_global_bridge["window_mgr"].update_values(window=window, key=each, args={"values":rpc_add_global_bridge["keyed_lists"][get_key_from_value(each)]})
                rpc_add_global_bridge["window_mgr"].update_values(window=window, key=each[:-1]+'_CHECK-', args={"value":False})
            # Reset filters
        if each != event:
            rpc_add_global_bridge["recursed_rpc_js_list"] = rpc_add_global_bridge["get_rpc_list"]
            filter_rpc_list_by_selections()
            rpc_add_global_bridge["window_mgr"].update_values(window=window, key=each, args={"values":rpc_add_global_bridge["keyed_lists"][get_key_from_value(each)], "value":''})
            rpc_add_global_bridge["window_mgr"].update_values(window=window, key=each[:-1]+'_CHECK-', args={"value":False})
    if event == '-RESET_RPC-':
        ...
        if each !="-NETWORK_NAME-":
            rpc_add_global_bridge["window_mgr"].update_values(window=window, key=each[:-1]+'_CHECK-', args={"value":False})
    elif event in list(get_rpc_js().values()):
        rpc_add_global_bridge["check_list"][event] =rpc_add_global_bridge["window_mgr"].get_values()[event]
        rpc_add_global_bridge["window_mgr"].update_values(window=window,key=event[:-1]+'_CHECK-',args={"value":True})
    elif '_CHECK-' in event:
        if values[event] == False:
            rpc_add_global_bridge["window_mgr"].update_values(window=window,key=event,args={"value":False})
...
def filter_rpc_list_by_selections()->list:
    ...
    for key,unique_values in filtered_json_list.items():
        if get_rpc_js()[key] in rpc_add_global_bridge["check_list"][get_rpc_js()[key]]:
                value = rpc_add_global_bridge["check_list"][get_rpc_js()[key]]
            ...
            if len(unique_values)>=1:
                ...
                value = None
        window_mgr.update_values(window= window_mgr.get_last_window_method(),key=get_rpc_js()[key],args={"values":unique_values,"value":value})
def keyed_rpc_lists(event:str=None)->None:
    ...
    for each in list(get_rpc_js().keys()):
...
def get_values():
    """
    Fetches the values from the `get_rpc_js()` mapping.
    
    Returns:
    - list: A list of values.
    """
    return list(get_rpc_js().values())
...
def get_rpc_layout(RPC_list:list=None,window_mgr:type(window_mgr)=window_mgr):
    ...
    layout = []
    for key,value in get_rpc_js().items():
        layout.append([
...
def Choose_RPC_Parameters_GUI(RPC_list:list=None) -> dict or None:
    ...
    """
    layout = [[get_menu()],get_rpc_layout(RPC_list),[create_row_of_buttons("OK","Show","reset","Exit"),]]
    window = window_mgr.get_new_window(args={"title":'ADD RPC',"layout":layout,"exit_events":["OK","Exit"],"event_function":"rpc_win_while","suppress_raise_key_errors":False, "suppress_error_popups":False, "suppress_key_guessing":False,"finalize":True})
