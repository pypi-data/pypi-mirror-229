...
from deepfos.lib.utils import concat_url, retry, dict_to_key, to_version_tuple, repr_version
...
from .models.app import QueryElementInfoByTypeDto as ElementInfo
    ...
    def __call__(
        ...
        def execute(func):
            ...
            async def do_request(ins, *args, **kwargs):
                ...
                if model is None or (allow_none and obj is None):
                    return obj
...
class _DynamicAPIMixin:
    ...
    def _check_server_cache(
        self,
        element: T_DictPydanticModel
    ) -> Tuple[ElementInfo, str, str]:
        element = self._parse_element(element)
        identifier = element.dict()
        identifier.update(self.header)
        key = dict_to_key(identifier)
        return element, self.server_cache.get(key, None), key

    def _parse_element(self, element: T_DictPydanticModel) -> ElementInfo:
        if isinstance(element, dict):
            ele_info = ElementInfo.construct_from(**element)
        else:
            ele_info = ElementInfo.construct_from(element)

        ele_info.elementType = self.module_type
        return ele_info

    def get_server_name(self):
        return NotImplemented

    def get_server_name_by_element(self, element):
        return NotImplemented
...
class DynamicAPIBase(SyncAPIBase, _DynamicAPIMixin):
    ...
    def get_server_name_by_element(self, element: T_DictPydanticModel):
        element, server_name, server_key = self._check_server_cache(element)
        if server_name is None:
            from .app import AppAPI
            api = AppAPI(self.header, sync=True).element_info
            server_name = api.get_server_names([element])[0].serverName
            self.server_cache[server_key] = server_name
        return server_name

    def set_url(self, server_name):
        if OPTION.discovery.enabled:
...
class ADynamicAPIBase(AsyncAPIBase, _DynamicAPIMixin):
    ...
    async def get_server_name_by_element(self, element: T_DictPydanticModel):
        element, server_name, server_key = self._check_server_cache(element)
        if server_name is None:
            from .app import AppAPI
            api = AppAPI(self.header, sync=False).element_info
            server_meta = await api.get_server_names([element])
            server_name = server_meta[0].serverName
            self.server_cache[server_key] = server_name
        return server_name
