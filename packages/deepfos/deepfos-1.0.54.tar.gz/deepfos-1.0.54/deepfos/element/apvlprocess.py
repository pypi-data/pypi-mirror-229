from typing import List, Dict, Optional, Union, Tuple, TYPE_CHECKING, Any
...
class ApprovalProcess(ElementBase):
    """审批流"""
    api_class = ApprovalProcessAPI
    api: ApprovalProcessAPI

    @cached_property
    def meta(self) -> ProcessConfigureVo:
        """审批流的元配置信息"""
        meta = self.api.configure.query(
    ...
    def operate(
        ...
        if origin_status is None:
            origin_status = self.get_record(primary_kv, roles)[0].result_status

        return self.api.operation.record(OperationRecordDto(
            folderId=self.element_info.folderId,
    ...
    def get_record(
        ...
        return self.api.operation.get_record(QueryRecordDto(
            folderId=self.element_info.folderId,
            pcName=self.element_name,
            roles=roles,
            primaryKeyValue=primary_kv
