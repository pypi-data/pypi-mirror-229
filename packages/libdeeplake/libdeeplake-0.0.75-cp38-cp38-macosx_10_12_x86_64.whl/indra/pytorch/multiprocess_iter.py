...
class MultiprocessIterator:
    ...
    def handle_exception(self, ex):
        ...
        if isinstance(
            ...
            ex.offset = self.offset
            ex.index = self.index
            print(f"An exception happened during data handling exception: {ex} ")
        else:
            print(
                f"An exception happened during data handling exception: {ex} index {self.index} offset {self.offset}"
            )
        self.offset += 1
