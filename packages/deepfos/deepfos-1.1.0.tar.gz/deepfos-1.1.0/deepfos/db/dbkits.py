...
class DataframeSQLConvertor:
    ...
    def iter_sql(
        ...
        if chunksize is None or chunksize > nrows:
            yield self.convert(dataframe, tablename, updatecol)
        elif chunksize <= 0:
            raise ValueError("chunksize must be greater than 0.")
        else:
            for i in range(0, nrows, chunksize):
                yield self.convert(dataframe.iloc[i: i + chunksize], tablename, updatecol)
    ...
    def convert(
        ...
        return self.build_sql(columns, values, tablename, updatecol)
