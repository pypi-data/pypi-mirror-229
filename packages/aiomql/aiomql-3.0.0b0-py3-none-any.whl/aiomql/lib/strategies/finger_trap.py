...
class FingerTrap(Strategy):
    ...
    def __init__(self, *, symbol: Symbol, params: dict | None = None, trader: Trader = None):
        ...
        self.interval = self.parameters.setdefault('interval', 10)
        self.entry_interval = self.parameters.setdefault('entry_interval',
                                                         self.entry_time_frame.time / self.interval)
        self.entry_candles_count = self.parameters.setdefault('entry_candles_count',
                                                              86400 // self.entry_time_frame.time)
    ...
    async def confirm_trend(self):
        try:
            ...
            if self.entry.bullish and candles[-1].close_XA_ema:
                ...
                self.entry.update(snooze=self.entry_interval, order_type=None)
        except Exception as exe:
