...
def __detailed(
    ...
    if instanseg.metrics.constants.UNSEGMENTED_LABEL in predicted_label_to_indices:
        del predicted_label_to_indices[instanseg.metrics.constants.UNSEGMENTED_LABEL]
    if instanseg.metrics.constants.UNSEGMENTED_LABEL in gt_label_to_indices:
        del gt_label_to_indices[instanseg.metrics.constants.UNSEGMENTED_LABEL]
    ...
    for predicted_label, predicted_indices in predicted_label_to_indices.items():
        for gt_label, gt_indices in gt_label_to_indices.items():
            are_well_overlapped = tp_condition_function(
                predicted_indices,
                gt_indices,
                instanseg.metrics.constants.IOU_THRESHOLD_FULL,
            )
            ...
            are_part_overlapped = tp_condition_function(
                predicted_indices,
                gt_indices,
                instanseg.metrics.constants.IOU_THRESHOLD_PART,
            )
    ...
    for (
        predicted_label,
        part_overlapped_gt_instances,
    ) in part_overlapped_pred_to_gt.items():
        part_overlapped_gt_amount = len(part_overlapped_gt_instances)
        ...
        if has_single_only_part_overlap:
            part_overlapped_gt_label = part_overlapped_gt_instances[0]
    ...
    for gt_label, part_overlapped_pred_instances in part_overlapped_gt_to_pred.items():
        part_overlapped_pred_amount = len(part_overlapped_pred_instances)
        ...
        if has_single_only_part_overlap:
            part_overlapped_predicted_label = part_overlapped_pred_instances[0]
