...
def add_executor():
    ...
    executor.add_executor(
        name, hostname=hostname, username=username, dockerpath=dockerpath
    )
...
def run_job(executor_name: str, path: str, command: str = None):
    ...
    rich.print(
        f"You can view the logs with `radops jobs logs {executor_name} {job_id}`"
    )
...
def create_executor(
    ...
    if template_name not in gcp.list_templates():
        ...
        raise typer.Exit(1)
    if name is None:
        name = f"gcp-{settings.email.split('@')[0].replace('.', '')}-{datetime.now().strftime('%Y%m%d%H%M%S')}"
