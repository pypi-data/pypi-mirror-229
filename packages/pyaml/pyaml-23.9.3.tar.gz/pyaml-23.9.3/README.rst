...
* Pretty-print any yaml or json (yaml subset) file from the shell::
  ...
  ``pipx install pyaml`` can be a good way to only install command-line script.
...
* Control thresholds for vertical spacing of values (0 = always space stuff out),
  but clump oneliner ones at the top::
  ...
  Or same with ``pyaml -v 0/0g mydata.yaml`` cli tool option, to separate all
  but oneliner values.
