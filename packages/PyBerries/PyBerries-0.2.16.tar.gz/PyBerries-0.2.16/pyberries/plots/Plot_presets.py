...
def plot_histogram(df_in, ax, errorbars=None, **kwargs):
    ...
    if error_func:
        ...
        hist = get_histogram(df_in, col=col, binsize=binsize, density=False,
                             discrete=kwargs.get('discrete'), groupby=['Group', 'Dataset'])
        ...
        if (len(df_in[hue].unique()) > 1) and (kwargs.get('multiple') == 'dodge'):
            ...
            for grp in df_in[hue].unique():
                hist.loc[(hist.Group == grp), col] += x_offset
        ...
        for grp, data in hist.groupby('Group'):
            ...
            df = (data
                  .groupby(col)
                  .agg(bins=(col, 'mean'),
                       height=('count', lambda x: x.mean()),
                       err=('count', error_func)
                       )
