...
@click.group()
@click.option(
    "-t", "--target-dir", type=str, default=None, help="The local target path"
)
...
@click.option(
    ...
    is_flag=True,
    default=False,
    help="Print only results. Does not execute any command.",
)
def cli(target_dir: str, user: str, password: str, list: bool):
    """Gitkit - A powerful git and github tool"""
    commandContext = CommandContext()

    commandContext.localDir = (
        target_dir
        if target_dir is not None
        else environ.get("targetdir") or os.getcwd()
    )
    commandContext.user = user if user is not None else environ.get("user")
    commandContext.password = (
        password if password is not None else environ.get("password")
    )

    commandContext.filters.listOnly = list if list is not None else environ.get("list")
