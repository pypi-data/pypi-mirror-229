...
def announce(txt):
    ...
    if COMMON['CONFIG'].festival_type_voice_synth in BLANK_VALUES:
        ...
        cmd = COMMON['CONFIG'].festival_type_voice_synth.split()
        subprocess.run(
            cmd,
            text=True,
            input=txt,
...
def generate_qr_image(txt):
    ...
    if COMMON['CONFIG'].qr_code_generator in BLANK_VALUES:
        ...
        with tempfile.NamedTemporaryFile(mode='w', delete=False, suffix='.png') as unit:
            cmd = COMMON["CONFIG"].qr_code_generator.format(unit.name).split()
            cmd.append(txt)
            proc = subprocess.run(
                cmd,
                text=True,
...
class Config:
    ...
    def load(self):
        ...
        def load_globals():
            ...
            collection = self.festival_type_voice_synth.split(sep=None, maxsplit=1)
            collection += [NOT_AVAIL, NOT_AVAIL]
            (proc, parms) = collection[:2]
            if self.festival_type_voice_synth in BLANK_VALUES or (not shutil.which(proc)):
                self.festival_type_voice_synth = self.default_festival_type_voice_synth
            self.qr_code_generator = expand_vars(self.ini[sect].get('qr_code_generator', self.default_qr_code_generator))
            collection = self.qr_code_generator.split(sep=None, maxsplit=1)
            collection += [NOT_AVAIL, NOT_AVAIL]
            (proc, parms) = collection[:2]
            if self.qr_code_generator in BLANK_VALUES or (not shutil.which(proc)):
                self.qr_code_generator = self.default_qr_code_generator
