...
from util.HttpRequestUtils import MethodEnum, ContentTypeEnum,RequestParams
...
class RequestUtils:
    ...
    def requestExecute(method,session,url,datas,headers,cookies):
        ...
        # 组装formData数据格式
        formData = RequestUtils.processFormData(datas,headers)

        logging.debug("Request URL:%s",url)
        logging.debug("Request Method:%s",method)
        logging.debug("Request headers:%s",str(headers))
        logging.debug("Request cookies:%s",str(cookies))
        logging.info("form data:%s",formData)
    ...
    @staticmethod
    def processFormData(datas,headers):
        contentTypeEnum = ContentTypeEnum.getEnum(headers.get("Content-Type"))

        if contentTypeEnum is None:
            raise ValueError("Content-Type"+headers.get("Content-Type")+",form data processor not define!")

        # application/x-www-form-urlencoded
        elif contentTypeEnum == ContentTypeEnum.FormUrlencoded:
            return urllib.parse.urlencode(datas)

        # contentType:application/json
        elif contentTypeEnum == ContentTypeEnum.ApplicationJson:
...
class NormalHttpRequestProcessor(ABC):
    ...
    def getResponse(self, dynamicParams=None):
        ...
        try:
            ...
            logging.info("reponse text:%s" , response.text)
        ...
        try:
            ...
            while (1):
                ...
                for index, row in enumerate(rows):
                    ...
                    logging.debug("rowDict:%s", str(rowDict))
