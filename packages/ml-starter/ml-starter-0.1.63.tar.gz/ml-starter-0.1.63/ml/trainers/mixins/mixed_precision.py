...
class MixedPrecisionTrainerMixin(BaseTrainer[MixedPrecisionConfigT, ModelT, TaskT]):
    ...
    def load_state_dict(self, ckpt: dict) -> None:
        if self.grad_scaler is not None:
            ...
            self.grad_scaler.load_state_dict(json.loads(ckpt["grad_scaler"]))
        super().load_state_dict(ckpt)

    def update_state_dict(self, ckpt: dict) -> None:
        if self.grad_scaler is not None:
            assert "grad_scaler" not in ckpt
            ckpt["grad_scaler"] = json.dumps(self.grad_scaler.state_dict())
