...
class PopClient:
  ...
  def connect(self):
    ...
    logging.info(f"[+] Connecting to POP server: {self.host}:{self.port}")
  ...
  def authenticate(self, username, password):
    ...
    """
    logging.info(f"[+] Auhentication: {username}:{password}")
    buffer = BytesIO()
  ...
  def stat(self):
    ...
    Raises a ScrippyMailError error if authentication fails.
    """
    logging.info("[+] Getting available mails number")
    buffer = BytesIO()
    buffer.write(b'STAT\r\n')
    self._send_data(buffer.getvalue())
    resp = self._recv_data()
  ...
  def retr(self, num):
    ...
    Raises a ScrippyMailError error if authentication fails.
    """
    logging.info(f"[+] Getting email number: {num}")
    buffer = BytesIO()
  ...
  def dele(self, num):
    ...
    Raises a ScrippyMailError error if authentication fails.
    """
    logging.info(f"[+] Deleting email number: {num}")
    buffer = BytesIO()
