...
def jload_file(file):
	tmp = []
	try:
		tmp = json.loads(read_file(file, False, False))
...
class ChadExtractor:
	...
	def __extend_data(self, extract, succeeded, failed):
		with self.__data["lock"]:
			self.__data["extracted" if extract else "validated"].extend(succeeded)
			self.__data["failed_extraction" if extract else "failed_validation"].extend(failed)

	def check_extracted(self):
		return bool(self.__data["extracted"])

	def check_validated(self):
		return bool(self.__data["validated"])

	def parse_template(self, extract):
		tmp = {}
		if extract:
			for key in self.__template:
				if "extract" in self.__template[key]:
					tmp[key] = self.__template[key]
		else:
			for key in self.__template:
				if "validate" in self.__template[key]:
					tmp[key] = self.__template[key]
		self.__template = tmp

	def parse_input(self, extract, plaintext):
		tmp = []
		if extract:
			...
			for file in self.__results:
				result = self.__parse_response(True, None, read_file(file, False, False)) # passing a file content instead of a web content
				if result:
					self.__data["extracted"].append({"file": file, "results": result})
					for key in result:
	...
	def __parse_response(self, extract, record, response):
		tmp = {}
		if self.__excludes:
		...
		if extract:
			for key in self.__template:
				try:
					...
					if matches:
						tmp[key] = unique(self.__concat(key, matches), True)
				except Exception as ex:
					print(("ERROR: {0} | {1}").format(key, ex))
		elif not extract and re.search(self.__template[record["id"]]["validate"], response, self.__flags):
			tmp = True
		return tmp
	...
	def __concat(self, key, matches):
		...
		if prepend or append:
			...
			for match in matches:
				tmp.append(prepend + match + append)
			return tmp
		else:
			return matches

	def run(self, extract):
		self.__get_timestamp(("Number of URLs to be {0}: {1}").format("extracted" if extract else "validated", len(self.__results)))
		...
		with concurrent.futures.ThreadPoolExecutor(max_workers = self.__threads) as executor:
			...
			for records in self.__split_results():
				...
				subprocesses.append(executor.submit(self.__proxy_browser_requests, extract, records))
	...
	def __split_results(self):
		if self.__threads > 1:
			...
			return (self.__results[i * k + min(i, m) : (i + 1) * k + min(i + 1, m)] for i in range(self.__threads))
		else:
			return [self.__results]

	def __proxy_browser_requests(self, extract, records):
		asyncio.run(self.__browser_requests(extract, records))

	async def __browser_requests(self, extract, records):
		succeeded = []
		failed = []
		async with async_playwright() as pw:
			...
			for record in records:
				...
				if reset % 100:
					reset = 0
					context = await self.__reset_context(browser, context)
				...
				while count > 0:
					...
					if not tmp["error"] and tmp["response"]:
						count = 0
						entry["results"] = self.__parse_response(extract, record, tmp["response"])
						if entry["results"]:
			...
			await context.close()
			await browser.close()
		self.__extend_data(extract, succeeded, failed)

	async def __set_context(self, browser):
		return await browser.new_context(ignore_https_errors = True, java_script_enabled = True, accept_downloads = False)

	async def __reset_context(self, browser, context): # dirty fix to release context's cache which may lead to large resource consumption
		await context.close()
		return await self.__set_context(browser)
	...
	async def __page_get(self, context, url):
		...
		try:
			...
			await page.route("**/*", self.__block) # ignore unnecessary requests
			...
			pass
		except Exception as ex: # in case of a file or invalid domain, fallback
			tmp["error"] = True
		finally:
			if page:
				await page.close()
	...
	async def __request_get(self, context, url):
		...
		try:
			...
			pass
		except Exception as ex: # in case of an invalid domain, break
			tmp["error"] = True
		return tmp

	async def __block(self, route):
		if route.request.resource_type in ["beacon", "csp_report", "font", "image", "imageset", "media", "object", "texttrack", "manifest", "ping"]:
			await route.abort()
		else:
			await route.continue_()

	def __print_debug(self, status, url):
		with self.__debug_lock:
			print(("{0}: {1}").format(status, url))

	def save_results(self, plaintext):
		# --------------------
		...
		tmp["summary"]["extracted"] = unique(jquery(tmp["full"], self.__queries["get_results"]), True)
		# --------------------
		if not plaintext:
		...
		if self.__verbose:
			for file in unique(jquery(self.__data["extracted"], self.__queries["get_file" if plaintext else "get_files"])):
				...
				if not plaintext:
					...
					tmp["summary"]["extracted"] = unique(jquery(obj, self.__queries["get_results"]), True)
	...
	def __get_report(self, plaintext):
		if plaintext:
...
class Validate:
	...
	def __basic(self):
		...
		print("Chad Extractor v4.2 ( github.com/ivan-sincek/chad )")
		print("")
			...
			# --------------------
				...
				if os.path.isdir(self.__args["excludes"]):
					...
					if not os.access(self.__args["excludes"], os.R_OK):
						...
						self.__args["excludes"] = read_file(self.__args["excludes"])
						if not self.__args["excludes"]:
							self.__error("No regular expressions were found")
				else:
					self.__args["excludes"] = [self.__args["excludes"]]
			# --------------------
	...
	def run(self):
		...
		# --------------------
			...
			if self.__args["template"] is None or self.__args["results"] is None or self.__args["out"] is None or not self.__check(argc):
				self.__error("Missing a mandatory option (-t, -res, -o) and/or optional (-pt, -e, -th, -r, -w, -a, -v, -dbg)", True)
		# --------------------
		else:
			self.__error("Incorrect usage", True)
		# --------------------
...
def main():
	...
	if validate.run():
		...
		if validate.get_arg("plaintext"):
			chad_extractor.parse_template(True)
			if not chad_extractor.check_template():
				print("No extraction entries were found in the template file")
			else:
				chad_extractor.parse_input(False, True)
				if not chad_extractor.check_extracted():
					print("No data was extracted")
				else:
					chad_extractor.parse_template(False)
					if not chad_extractor.check_template():
						print("No validation entries were found in the template file")
					else:
						chad_extractor.run(False)
						if not chad_extractor.check_validated():
							print("No data matched the validation criteria")
					chad_extractor.save_results(True)
		else:
			chad_extractor.parse_template(True)
			if not chad_extractor.check_template():
				print("No extraction entries were found in the template file")
			else:
				chad_extractor.parse_input(True, False)
				if not chad_extractor.check_results():
					print("No results for data extraction were found")
				else:
					chad_extractor.run(True)
					if not chad_extractor.check_extracted():
						print("No data was extracted")
					else:
						chad_extractor.parse_template(False)
						if not chad_extractor.check_template():
							...
							chad_extractor.parse_input(False, False)
							if not chad_extractor.check_results():
								...
								if not chad_extractor.check_validated():
									print("No data matched the validation criteria")
						chad_extractor.save_results(False)
