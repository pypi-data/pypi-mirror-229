...
TEMPLATE_CACHE_PATH = os.getenv("TEMPLATE_CACHE_PATH", os.path.join(os.getcwd(), "template_cache"))

def get_path_from_url(url, tag = "template/"):
    pos = url.find(tag)
    if (pos < 0):
        return None
    return url[(pos + len(tag)):]
...
class ImageDefs(object):
    ...
    def __init__(self, makefile_path):
        ...
        try:
            ymldefs, alltargets, env = self.parse_yaml(self.makefile_path)
        ...
        self.environments = utils._make_environments(us.check_attribute(self.env, "image_envs", None))

    def parse_yaml(self, filename):
        # locate and verify the swbuilder.yml file
        if (filename.find("://") > 0):
            res = requests.get(filename)
            filename = os.path.join(TEMPLATE_CACHE_PATH, get_path_from_url(filename))
            us.check_path_exists(filename)
        ...
        for s in yamldefs.pop("_SOURCES_", []):
            src, _, _ = self.parse_yaml(s)  # ignore source's _ALL_ targets
        ...
        return sourcedefs, alltargets, env
    ...
    def _check_yaml_and_paths(ymlfilepath, yamldefs):
        ...
        for imagename, defn in yamldefs.items():
            if (imagename[:1] == "_") and (imagename[-1:] == "_"): # if imagename in SPECIAL_FIELDS:
                # 特殊字段处理
                if imagename == "_SOURCES_":
                    yamldefs["_SOURCES_"] = [
                        p if (p.find("://") > 0) else os.path.relpath(_get_abspath(pathroot, us.make_param(p)))
    ...
    def generate_build(
        ...
        **kwargs,
        ...
        for base_name in self.sort_dependencies(image):
            ...
            if (istep == 1) and (self.environments):
                # 添加镜像内的全局环境变量
                self.ymldefs[base_name]["build"] = self.environments + self.ymldefs[base_name]["build"]
