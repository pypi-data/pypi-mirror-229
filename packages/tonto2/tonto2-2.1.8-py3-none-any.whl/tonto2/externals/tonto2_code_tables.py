...
def get_xdg_config_home():
    ...
    result = pathlib.Path(config) if config else pathlib.Path.home() / ".config"
    return result
...
def get_xdg_data_home():
    ...
    config = os.environ.get('APPDATA') or os.environ.get('XDG_DATA_HOME')
    result = pathlib.Path(config) if config else pathlib.Path.home() / ".local"
    return result
...
PATH = get_xdg_data_home() / 'Tonto2' / FN
