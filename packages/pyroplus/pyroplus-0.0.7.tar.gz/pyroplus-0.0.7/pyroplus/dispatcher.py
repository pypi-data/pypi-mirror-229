...
from pyroplus.raw.types import (
    ...
    UpdateReadHistoryInbox, UpdateReadHistoryOutbox, UpdateUserStatus, UpdateBotInlineQuery, UpdateMessagePoll,
...
class Dispatcher:
    ...
    def __init__(self, client: "pyroplus.Client"):
        self.client = client
        ...
        async def callback_query_parser(update, users, chats):
            return (
                await pyroplus.types.CallbackQuery._parse(self.client, update, users),
                CallbackQueryHandler
            )

        async def user_status_parser(update, users, chats):
            return (
                pyroplus.types.User._parse_user_status(self.client, update),
                UserStatusHandler
            )

        async def inline_query_parser(update, users, chats):
            return (
                pyroplus.types.InlineQuery._parse(self.client, update, users),
                InlineQueryHandler
            )

        async def poll_parser(update, users, chats):
            return (
                pyroplus.types.Poll._parse_update(self.client, update),
                PollHandler
            )

        async def chosen_inline_result_parser(update, users, chats):
            return (
                pyroplus.types.ChosenInlineResult._parse(self.client, update, users),
                ChosenInlineResultHandler
            )

        async def chat_member_updated_parser(update, users, chats):
            return (
                pyroplus.types.ChatMemberUpdated._parse(self.client, update, users, chats),
                ChatMemberUpdatedHandler
            )

        async def chat_join_request_parser(update, users, chats):
            return (
                pyroplus.types.ChatJoinRequest._parse(self.client, update, users, chats),
                ...
                async with lock:
                    for group in self.groups.values():
                        for handler in group:
                            ...
                            except pyroplus.StopPropagation:
                                raise
                            except pyroplus.ContinuePropagation:
                                continue
                            except Exception as e:
                                log.exception(e)
