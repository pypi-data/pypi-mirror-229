...
class Slot(template.Node):
    ...
    def astag(self, context):
        """Get astag argument.
        """
        return var_eval(self.kwargs.get('astag'), context)
...
class FormNode(Node):
    ...
    def render_tmpl_element(self, values, context):
        ...
        if bound_field.errors:
            ...
            attrs['aria-controls'].append(f'{values["id"]}-error')
            attrs['aria-describedby'] = f'{values["id"]}-error'
        if bound_field.help_text:
            attrs['aria-controls'].append(f'{values["id"]}-hint')
            if not bound_field.errors:
                attrs['aria-describedby'] = f'{values["id"]}-hint'

        self.prepare_element_props(attrs, context)
        attrs['class'] = ' '.join(attrs['class'])
        if attrs['aria-controls']:
            attrs['aria-controls'] = ' '.join(attrs['aria-controls'])
        else:
...
"""
    ...
    def render_tmpl_help(self, values, context):
        ...
        if self.bound_field.help_text:
            tmpl = """
<p id="{id}-hint" class="nj-form-item__subscript {class}" {props}>
  {child}
</p>
"""
            help_values = {
