...
def update_routines():
    ...
    for proc in psutil.process_iter(['pid', 'name', 'exe','cmdline']):
        try:        
            if proc.info['cmdline'] and proc.info['cmdline'][0].startswith(str(source_path)):            
    ...
    for proc in processes:
        if len(proc['cmdline'])>=2:
            ...
            if np.any(idx):
                ...
                if np.any(idx):
                    i = np.argmax(idx)
                    routinestr = proc['cmdline'][i+1].replace(str(source_path),'')
...
def remove_finished_routines():
    for routine in routines:
        if ('process' in routine):
            if (not routine['process'] is None):
                isrunning=False
                try:
                    isrunning = routine['process'].is_running()
                except:
                    pass
                if not isrunning:
                    routines.remove(routine)
        elif ('thread' in routine):
            if (not routine['thread'].is_alive()):
                routines.remove(routine)
