...
class ByoGs:
  SUBCOMMANDS = ['publish-image', 'publish-version']
  DEFAULT_BUILD_PLATFORM = 'linux/amd64'

  def __init__(self, subcommand: str, token: str, base_url: str, api_key: str, sid: str, tag: str, path: Union[str, None], dockerfile: str, version: Union[str, None], http_port: Union[int, None]) -> None:
    self.subcommand: str = subcommand
    self.token: str = token
    self.token_parts: list = ByoGs.get_token_values(token)
  ...
  def publish_version(self) -> bool:
    with Progress(
      ...
      try:
        ...
        res = requests.post(f"{self.base_url}/v1/snapser-api/byogs/{self.sid}/publish-version", json=payload, headers={'api-key': self.api_key})
        if res.ok:
          return True
