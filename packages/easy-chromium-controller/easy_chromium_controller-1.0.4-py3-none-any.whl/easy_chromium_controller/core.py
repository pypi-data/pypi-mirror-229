...
class Browser(metaclass=Singleton):
    ...
    def open(
        ...
        screenshots_path=os.path.dirname(
        ...
        if self.OS == "linux":
            ...
            self.log_path = (
                binary_folder + "/bin-" + BINARY_RELEASE_VERSION + "/chromeriver.log"
            )
            check_binary_files_istalled(
                binary_folder + "/bin-" + BINARY_RELEASE_VERSION, "/linux/chromedriver"
            )
            ...
            check_binary_files_istalled(
                binary_folder + "\\bin-" + BINARY_RELEASE_VERSION,
                "\\win\\chromedriver.exe",
            )
    ...
    def findElement(self, element_locator: Locator, wait="normal") -> WebElement:
        if wait == "short":
            return self.shortWait.until(EC.presence_of_element_located(element_locator))
        else:
            return self.wait.until(EC.presence_of_element_located(element_locator))

    def findElements(self, element_locator: Locator, wait="normal") -> list[WebElement]:
        if wait == "short":
            return self.shortWait.until(
                EC.presence_of_all_elements_located(element_locator)
            )
        else:
            return self.wait.until(EC.presence_of_all_elements_located(element_locator))

    def hiddeElement(self, element_locator: Locator) -> None:
        elem = self.findElementelement_locator
        self.executeScript([elem], 'args[0].style.display = "none";')

    def click(self, element_locator: Locator, wait="normal", maybe=False) -> None:
        def func():
            if wait in "long":
                self.longWait.until(
                    EC.element_to_be_clickable(element_locator)).click()
            elif maybe:
    ...
    def clickElem(self, element: WebElement) -> None:
        self.wait.until(EC.element_to_be_clickable(element)).click()

    def switchToFrame(self, frame_locator: Locator = (By.XPATH, "/html")) -> None:
        self.wait.until(
            EC.frame_to_be_available_and_switch_to_it(frame_locator))
        """ self.driver.switch_to.frame(wait.until(EC.presence_of_element_located(frame_locator))) """
    ...
    def elementScreenshot(self, element_locator: Locator, imageName) -> None:
        """ScreenShot a un elemento y lo guarda en ``self.screenshots_path + "/" + imageName + ".png"``"""
        self.findElement(element_locator).screenshot(
            self.screenshots_path + "/" + imageName + ".png"
        )
    ...
    def markElement(self, element: WebElement, width=4, color="red") -> None:
        self.executeScript(
            [element],
            'args[0].style.border = "' +
    ...
    class FromContainer():
        ...
        def clickBetterMatch(
            ...
            self.b.clickElem(target_buttons[match_index])
            return True
        ...
        def clickBetterMatchFromCollapsedList(
            ...
            """
            self.b.markElement(self.container)
            collapsed_list = self.container.find_elements(collapsed_buttons[0], collapsed_buttons[1])
            ...
            target_buttons_text = list(map(lambda b: b.text, target_buttons_list))
            better_matches = difflib.get_close_matches(
                target_text,
                target_buttons_text,
                n=1,
            ...
            self.b.clickElem(target_button)
            return True
