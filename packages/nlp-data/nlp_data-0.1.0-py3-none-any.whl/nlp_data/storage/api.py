...
from ..document import NLUDocList, NLUDoc, EmbeddingDocList, EmbeddingDoc, APIDoc, APIDocList
import boto3
...
class APIDocStore(S3DocStore):
    ...
    def list(cls, name_space: Optional[str] = None, show_table: bool = True) -> List[str]:
        ...
        if show_table:
            ...
            table = Table(
                title=f'APIDocList in bucket s3://api',
                box=box.SIMPLE,
                highlight=True,
            )
    ...
    @classmethod
    def pull(cls, name: str) -> APIDocList[APIDoc]:
        return APIDocList[APIDoc].pull(url=f's3://{cls.bucket_name}/{name}')
    
    @classmethod
    def push(cls, docs: "EmbeddingDocList[EmbeddingDoc]", name: str) -> Dict:
        return APIDocList[APIDoc].push(docs, url=f's3://{cls.bucket_name}/{name}')
