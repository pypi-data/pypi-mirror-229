...
class PartitionMboot(PartitionBase):
    ...
    def __init__(self, terminal, device, size=None):
...
class FlashDeviceMB7707(FlashDeviceBase):
    ...
    def _write(self, fd, offset, length, callback = None):
        self.commit()
        if length % self.erase_size:
            pad = self.erase_size - (length % self.erase_size)
        self.mboot_cmd(f"eupgrade 0x{offset:x} 0x{length + pad:x}")
        es = self.magic()
        self.magic(0xa1)
    ...
    def _erase(self, offset=0, length=-1, callback = None):
        if length == -1:
            length = self.size
        self.mboot_cmd(f"mtd scrub 0x{offset:x} {length:x}")
...
class Flasher7707NAND(PartitionMboot, FlashDeviceMB7707):
    ...
    def __init__(self, terminal, device):
        ...
        FlashDeviceMB7707.__init__(self, terminal, self.device)
        PartitionMboot.__init__(self, terminal, self.device)
