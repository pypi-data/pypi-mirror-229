...
class ComputeMetrics:
    ...
    def generate_author_df(self) -> pd.DataFrame:
        ...
        return self.generate_df_factory(
            self._merged_docs_df, use_cols, use_cols[0], "; "
        )
    ...
    def generate_keyword_df(self) -> pd.DataFrame:
        ...
        if self._source in ["wos", "scopus"]:
            ...
            raise ValueError("Invalid source type")
        return self.generate_df_factory(
            self._merged_docs_df, use_cols, "DE", "; ", True
        )
    ...
    def generate_institution_df(self) -> pd.DataFrame:
        ...
        if self._source == "wos":
            ...
            raise ValueError("Invalid source type")
        return self.generate_df_factory(self._merged_docs_df, use_cols, "C3", "; ")
    ...
    def generate_journal_df(self) -> pd.DataFrame:
        ...
        if self._source in ["wos", "scopus"]:
            ...
            raise ValueError("Invalid source type")
        return self.generate_df_factory(self._merged_docs_df, use_cols, "SO")

    def generate_year_df(self) -> pd.DataFrame:
        """Return publication year DataFrame. Sort by `PY` ascending."""
        use_cols = ["PY"]
        return self.generate_df_factory(
            self._merged_docs_df, use_cols, "PY"
        ).sort_values(by="PY")
    ...
    def generate_document_type_df(self) -> pd.DataFrame:
        ...
        use_cols = ["DT"]
        return self.generate_df_factory(self._merged_docs_df, use_cols, "DT")
