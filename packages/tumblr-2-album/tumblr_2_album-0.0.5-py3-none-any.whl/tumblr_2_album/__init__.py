...
def getText(post):
    ...
    for item in soup.find_all():
        if item.name not in ['p', 'li', 'h1', 'h2', 'h3', 'h4']:
            continue
...
def getFromPost(post):
    ...
    post_body = post.get('body', '') or post.get('question', '') + post.get('answer', '')
    result.cap_html_v2 = getText(post.get('caption', '')) or getText(post_body) or post['summary']
    result.imgs = list(getImgsJson(post.get('photos', []))) or list(getImgs(post_body))
...
def getCanonical(post):
    ...
    if result.video or len(result.cap_html_v2) < 900 or (not result.img):
        return result
    post_body = post.get('body', '') or post.get('question', '') + post.get('answer', '')
    post_body = post_body[:1000]
    if result.imgs[0] in post_body:
        return result
    result.imgs = []
    return result
