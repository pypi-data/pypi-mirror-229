...
GeoIP2Fast is the fastest GeoIP2 country lookup library. A search takes l&ess than 0.00003 seconds. It has its own data file updated with Maxmind-Geolite2-CSV and is Pure Python!
...
- the number of networks and content still the same. All data was
  converted to integers and the lists was sliced a lot.
...
The file ```geoip2fast.dat.gz``` ```v1.0.3```, has ```1057489 bytes```, ```md5sum 649677fd62135280a9b75517299c7a2d``` and was created based on ```Maxmind GeoLite2 CSV files from 08/Sep/2023```, with 434271 networks from the CSV file plus 14 additions of special/private/reserved networks, totalizing ```434285 networks```.
...
'MAXMIND:GeoLite2-Country-CSV_20230908'
```
