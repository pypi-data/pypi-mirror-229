        ...
        if source["validate"] == "valid":
                        ...
                        if response_data["versions"] != []:
                            if (
                                ...
                                if  ((source['visibility'] == response_data['visibility'])
                                    and (source["description"] == response_data["description"])
            ...
            if r.status_code != 200:
                ...
                if source['owner_id'] != agent_conf['user']['id']:
                    logger.error(f"Agent cannot publish source [{source['name']}] because it is not the owner")
                    logger.error(f"Owner id of the source : {source['owner_id']}, id of the agent : {agent_conf['user']['id']}")
                invalid_count += 1
            else:
                source_id = r.json()["id"]
