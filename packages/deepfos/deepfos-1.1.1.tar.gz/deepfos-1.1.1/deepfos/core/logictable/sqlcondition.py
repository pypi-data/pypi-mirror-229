...
def _df2cond(df):
    return ' AND '.join(
        df.apply(lambda s: s.op.to_sql_template().format(s.field), axis=1))
...
class SQLCondition:
    ...
    * 取出部分字段条件
    ...
    """
    def __init__(self, fields=None, value_list=None, **kwargs):
        self.fields = []
    ...
    def static_cond(self):
        ...
        if self.__static_cond.empty:
            return ''
        return _df2cond(self.__static_cond)
    ...
    def __copy__(self):
        cp = SQLCondition(fields=self.fields, value_list=self.val_list.copy())
    ...
    def __and__(self, other):
        ...
        if val_list.empty:
            cond = SQLCondition([], pd.DataFrame())
        else:
            cond = SQLCondition(val_list.columns.values, val_list)
    ...
    def __getitem__(self, fields):
        ...
        rtn = SQLCondition(fields=valist_fields, value_list=self.val_list[valist_fields].copy())
        if fix_fields:
