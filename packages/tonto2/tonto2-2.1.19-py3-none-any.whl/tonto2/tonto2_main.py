...
class Config:
    ...
    def load(self):
        ...
        def load_globals():
            ...
            if self.festival_type_voice_synth in BLANK_VALUES or (not shutil.which(pathlib.Path(proc))):
                self.festival_type_voice_synth = self.default_festival_type_voice_synth
            ...
            collection += [NOT_AVAIL, NOT_AVAIL]
            (proc, parms) = collection[:2]
            if self.qr_code_generator in BLANK_VALUES or (not shutil.which(pathlib.Path(proc))):
                self.qr_code_generator = self.default_qr_code_generator
