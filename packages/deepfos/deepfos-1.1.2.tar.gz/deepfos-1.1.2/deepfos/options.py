...
from typing import Iterable, TYPE_CHECKING, TypeVar, Callable, Any, Tuple, Union, List
...
def _check_keys(value: dict, keys: Iterable[str] = None):
    missing = [
...
def _active_cache(value: bool):
    if value is True:
        logger.info(f'Element info cache actived.')
...
class _Option:
    ...
    def __init__(
        self,
        default: Any,
        val_type: Union[Any, Tuple[Any]] = None,
        val_choices: Tuple = (),
        write_warning: List[Tuple[Any, str]] = None,
    ...
    def __get__(self, instance, owner):
        ...
        if self.unset_action is _Action.RAISE:
            ...
            warnings.warn(f"Option '{instance}.{self}' has not yet been set.", stacklevel=2)
...
class _API(_Category):
    ...
    timeout = _Option(180, val_type=int, on_set=partial(_check_number_range, minimum=0, maximum=7200))
    ...
    cache_element_info = _Option(False, val_type=bool, on_set=_active_cache)
...
class _General(_Category):
    ...
    use_eureka = _Option(True, val_type=bool)
...
_option_ctx = ContextVar('deepfos_global_option')
...
class _OptionCTX:
    ...
    def show_options(self, category=None):
        return self._option.show_options(category)
