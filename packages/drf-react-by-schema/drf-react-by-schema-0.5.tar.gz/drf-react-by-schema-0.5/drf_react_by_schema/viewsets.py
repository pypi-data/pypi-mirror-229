...
from drf_react_by_schema.pagination import DRFReactBySchemaPagination
...
class GenericViewSet(DRFReactBySchemaBaseModelViewSet):
    serializer_list_class = None
    pagination_class = None if 'PAGINATION_MODE' not in settings.DRF_REACT_BY_SCHEMA or settings.DRF_REACT_BY_SCHEMA[
        'PAGINATION_MODE'] != 'server' else DRFReactBySchemaPagination
...
class EndpointsViewSet(viewsets.ViewSet):
    def list(self, request):
        endpoints = []
        for app in settings.DRF_REACT_BY_SCHEMA['APPS']:
            for name, model in apps.all_models['main'].items():
                endpoint = {
