...
class Plan:
    ...
    def max_workers(self, value: int):
        """
        Max workers for parallel processes.
        """
        assert value >= 1, ValueError('max_workers must be greater than 0.')
    ...
    def _validate(self):
        ...
        errors = []
        for d, txt in [(sources, 'sources'), (targets, 'targets')]:
            for key, converts in d.items():
                if len(converts) > 1:
                    errors.append(f'Error: Duplicate {txt} found: (source -> target)\n\t' + '\n\t'.join([str(c) for c in converts]))
        for convert in missing:
            errors.append(f'Error: Missing source file for {str(convert)}')
    ...
    def _tree(self) -> Tree:
        ...
        for convert in self._converts:
            ...
            tree.create_node(path.with_suffix(self.extension).name, path, parent=path.parent)
    ...
    def execute(self, postprocess_func: Optional[CustomPostprocessFunction] = None):
        """
        ...
        try:
            ...
            with tqdm(total=len(self._converts), desc=f"Converting to {self.extension}") as pbar, ThreadPoolExecutor(max_workers=self.max_workers) as pool:
                futures = [pool.submit(_execute, convert) for convert in self._converts]
                for future in as_completed(futures):
                    if exc := future.exception():
                        errors.append(exc)
                    pbar.update()
                    pbar.set_description(f'({len(errors)} errors)')

            print(f"Conversion complete with {len(errors)} errors.")
        finally:
            errors = [str(e) for e in errors if e]
            if errors:
                log = self.target_dir / LOG_FILENAME
                with open(log, 'w') as f:
                    f.write(f'There are {len(errors)} errors.\n\t')
                    f.write('\n\t'.join(errors))
                print(f"Wrote {len(errors)} errors to log at {log.as_posix()}.")
