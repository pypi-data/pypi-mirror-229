...
class AsyncJournalModel(ElementBase[JournalModelAPI]):
    ...
    async def save(
        ...
        .. admonition:: 示例
            ...
            .. code-block:: python
                ...
                # 凭证头表数据（注：type的值必须对应模型配置的凭证类型代码，
                # journal_id的值在下面的行表中必须有对应的数据）
                head_df = pd.DataFrame([
                    {
                      "type": "type_account_01", "journal_id": "head_main_id_202306080001",
                    ...
                    {
                      "type": "type_account_01", "journal_id": "head_main_id_202306080002",
                      "is_balance": "true", "scenario": "Actual","version": "Working",
                      "value": "CNY","entity": "[TotalEntity].[A]", "year": "2023",
                      "period": "12", "approve_time": "2023-05-23 15:56:00",
                      "convert_date": "2023-05-23"
    ...
    """
        ...
        unknown_type = ~head_df['type'].astype('str', errors='ignore').str.match(table_type_pattern)
        if unknown_type.any():
            raise ValueError(
                f'凭证头表数据中凭证类型:\n'
                f'{set(head_df["type"][unknown_type])}'
                f'\n在凭证模型中不存在'
            )
        ...
        head_data = head_data.assign(data=head_df.drop(columns=['type']).to_dict(orient='records'))
