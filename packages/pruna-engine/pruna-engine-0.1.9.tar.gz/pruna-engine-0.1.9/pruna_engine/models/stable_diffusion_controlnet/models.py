...
class UNet2DModel_Cnet(torch.nn.Module):
    def __init__(
            self,
            unet,
    ):
    ...
    def forward(
            self, sample, encoder_hidden_states,timestep
    ):
        noise_pred = self.unet(
            sample,
            timestep,
...
class UNet_original(BaseModel):
    def __init__(self,
        ...
        super(UNet_original, self).__init__(hf_token, fp16=fp16, device=device, verbose=verbose, path=path, max_batch_size=max_batch_size, embedding_dim=embedding_dim, text_maxlen=text_maxlen)
        self.unet_dim = unet_dim
        self.name = "UNet"
    ...
    def get_model(self):
        ...
        new_unet = UNet2DModel_Cnet(unet=pipe.unet).to('cuda')
        return new_unet

    def get_input_names(self):
        return ['sample', 'encoder_hidden_states','timestep']
