...
class FluentSqlBuilder:
    ...
    def __init__(self, connection: 'MySQLConnection|MsSqlConnection|Sqlite3Connection'):
        ...
        self._StatementIndexes_Select = []
        self._StatementIndexes_Insert = []
        self._StatementIndexes_Update = []
        self._StatementIndexes_Delete = []
        self._ClauseIndexes_Where = []
    ...
    def Select(self, *columnNames: str):
        ...
        self.Append(f"SELECT " + ', '.join(columnNames))
        self._StatementIndexes_Select.append(self._GetCurrentSegmentIndex())
        return self
    ...
    def Insert(self, tableName: str, *keyValuePairs: dict):
        ...
        self._StatementIndexes_Insert.append(self._GetCurrentSegmentIndex())
        return self
    ...
    def OnDuplicateKeyUpdate(self):
        ...
        if not self._HasStatement_Insert():
    ...
    def OnDuplicateKeyIgnore(self):
        ...
        lastInsertIndex = self._StatementIndexes_Insert[-1]
        self._segments[lastInsertIndex] = replacement + self._segments[lastInsertIndex][6:]
        return self
    ...
    def Update(self, tableName: str):
        ...
        self._StatementIndexes_Update.append(self._GetCurrentSegmentIndex())
        return self
    ...
    def Delete(self):
        ...
        self.Append("DELETE")
        self._StatementIndexes_Delete.append(self._GetCurrentSegmentIndex())
        return self

    def Join(self, table: str, onCondition: str):
        '''
        ...
        if not table or not onCondition:
            raise FluentSqlBuilderError("join", "Empty tableName/onCondition")

        self.Append(f"JOIN {table} ON {onCondition}")
        return self
    
    def InnerJoin(self, table, onCondition:str):
        self.Append("INNER")
        self.Join(table, onCondition)
        return self

    def LeftJoin(self, table, onCondition:str):
        self.Append("LEFT")
        self.Join(table, onCondition) 
        return self

    def RightJoin(self, table, onCondition:str):
        self.Append("RIGHT")
        self.Join(table, onCondition) 
        return self

    def FullJoin(self, table, onCondition:str):
        self.Append("FULL")
        self.Join(table, onCondition) 
        return self

    def CrossJoin(self, table):
        self.Append(f"CROSS JOIN {table}")
        return self
    ...
    def Where(self, condition: str, *params):
        ...
        self._ClauseIndexes_Where.append(self._GetCurrentSegmentIndex())
        return self

    def WhereIn(self, columnName: str, params:tuple):
        '''
    ...
    def WhereNotIn(self, columnName: str, params:tuple):
    ...
    def AndIn(self, columnName: str, params:tuple):
        self.GenericInClause("AND", columnName, params)
        return self

    def AndNotIn(self, columnName: str, params:tuple):
    ...
    def OrIn(self, columnName: str, params:tuple):
        self.GenericInClause("OR", columnName, params)
        return self

    def OrNotIn(self, columnName: str, params:tuple):
    ...
    def GroupBy(self, *columnName: list[str]):
    ...
    def Limit(self, maxRowCount: int, offset:int=None):
        if(self._connection_type == DBTypeEnum.MSSQLSRV): #only mssqlsrv needs to alter a previous segment
            ...
            lastSelectIndex = self._StatementIndexes_Select[-1]
            self._segments[lastSelectIndex] = replacement + self._segments[lastSelectIndex][6:] # "select..." becomes "select TOP n..."
            return self
    ...
    def Execute(self):
        ...
        if(self._HasStatement_Delete() or self._HasStatement_Insert() or self._HasStatement_Update()):
    ...
    def _ValidateBuiltQuery(self):
        ...
        if self._HasStatement_Update() or self._HasStatement_Delete():
            if not self._HasClause_Where():
    ...
    def GenericInClause(self, prefix: str, columnName: str, params:tuple):
        if not params or not columnName:
            raise FluentSqlBuilderError("generic_in_clause", "Nothing compared")

        placeholders = self.GeneratePreparedPlaceholders(len(params))
        self.Append(f"{prefix} {columnName} IN ({placeholders})", *params)

    def GenericNotInClause(self, prefix: str, columnName: str, params:tuple):
        if not params or not columnName:
            raise FluentSqlBuilderError("generic_not_in_clause", "Nothing compared")

        placeholders = self.GeneratePreparedPlaceholders(len(params))
        self.Append(f"{prefix} {columnName} NOT IN ({placeholders})", *params)
    ...
    def _GetCurrentSegmentIndex(self) -> int:
    ...
    def _HasStatement_Select(self):
        return len(self._StatementIndexes_Select) > 0
    def _HasStatement_Insert(self):
        return len(self._StatementIndexes_Insert) > 0
    def _HasStatement_Update(self):
        return len(self._StatementIndexes_Update) > 0
    def _HasStatement_Delete(self):
        return len(self._StatementIndexes_Delete) > 0
    def _HasClause_Where(self):
        return len(self._ClauseIndexes_Where) > 0
