...
def readImage(file: io.BufferedIOBase, sequential: bool, name: str,
    ...
    :type fn: Callable[[io.BufferedIOBase],partclone.imagebackup.ImageBackup]

    :raises partclone.imagebackup.ImageBackupException: Image not supported.

    :returns: A *PartClone* or *NtfsClone* instance.
    """
    try:
        return fn(file)
        ...
        if magic[:len(ImageBackup.NTFSCLONE)] == ImageBackup.NTFSCLONE:
            ...
            raise ImageBackupException(f"Image '{name}' was written with "
                                       "partimage; this tool supports ntfsclone"
                                       " and partclone images.")
        elif len(magic) >= 2:
