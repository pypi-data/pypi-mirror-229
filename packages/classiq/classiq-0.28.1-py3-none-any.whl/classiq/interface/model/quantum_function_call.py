...
from classiq.interface.model.handle_state import get_unique_handle_names
...
class QuantumFunctionCall(FunctionCall, Statement):
    ...
    inputs: Mapping[IOName, HandleBinding] = pydantic.Field(
        default_factory=dict,
        description="A mapping from the input name to the wire it connects to",
    )
    inouts: Mapping[IOName, Union[SlicedHandleBinding, HandleBinding]] = pydantic.Field(
        default_factory=dict,
        description="A mapping from in/out name to the wires that connect to it",
    )
    outputs: Mapping[IOName, HandleBinding] = pydantic.Field(
        default_factory=dict,
    ...
    def validate_handles(cls, values) -> Dict[str, Any]:
