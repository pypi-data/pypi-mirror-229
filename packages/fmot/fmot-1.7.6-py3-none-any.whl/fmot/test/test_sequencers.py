...
class TestSequencers():
    ...
    def test_DWConv1d_stack(self):
        ...
        for dilation in [1,2]:
            for bias in [False, True]:
                torch_net = DepthWiseTemporalConv1d(in_channels, out_channels, kernel_size, bias=bias)
                torch_output = torch_net(x)
    ...
    def test_DWConv1d_logic(self):
        ...
        epsilon = 10e-6
        torch.manual_seed(0)
        torch_net = TemporalConv1d(in_channels, out_channels, kernel_size, bias=False, groups=groups)
        ...
        epsilon = 10e-4
        torch.manual_seed(0)
        torch_net = TemporalConv1d(in_channels, out_channels, kernel_size, bias=True, groups=groups)
        my_net = FmotConv1dWrapper._from_torchmodule(torch_net, toplevel=torch_net)
        torch_output = torch_net(x)
