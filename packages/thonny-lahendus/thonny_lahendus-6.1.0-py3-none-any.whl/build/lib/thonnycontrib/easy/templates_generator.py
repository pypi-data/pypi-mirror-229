...
def _process_test(test):
    try:
        title, status = test['title'], test['status'].replace('FAIL', '❌').replace('PASS', '✔')
        user_inputs, actual_output = test['user_inputs'], test['actual_output']
        result = (
            f"{title}:{status}\n"
            f"Andsin programmile sisendid: {user_inputs}\n"
            f"Programmi täielik väljund oli:\n{actual_output}\n"
        )
    except KeyError:
        result = str(test)
...
def generate_exercise_html(provider, course_id, exercise_id) -> str:
    ...
    return render("exercise.mustache", {"effective_title": details.effective_title,
                                        ...
                                        "is_open": details.is_open if details.is_open else None,
                                        "points": points,
