...
def extend_args(task):
    ...
    args = [QLogM.log_q, task.init, task.end, get_config_path()]
    args.extend(QM.qs())
...
class MpDecorator:
    ...
    def __call__(self, *args, **kwargs):
        ...
        log_q, init, end, conf_path, *qs = e_args
        init_base(conf_path, log_q, qs)
...
class _MultiM:
    ...
    def _run_tasks(self):
        msg = f"start processes in {cur_pid()}"
        print(msg)
        logger.info(msg)
    ...
    def restart_child(self, sig, frame):
        ...
        try:
            ...
            if cpid == 0:
                ...
                return
            logger.info(f"child process-{cpid} exceptionally exit")
