...
def download_data(document_id):
    ...
    linsce_path = os.path.join(data_dir, 'linsce.txt')
    # Tải tệp Excel từ Google Drive
    url = f'https://drive.google.com/uc?id={document_id}'
    gdown.download(url, linsce_path, quiet=False)
    # Đọc file lưu biến
    ...
    json_data = os.path.join(data_dir, name)
    url2 = f'https://drive.google.com/uc?id={id_file}'
    gdown.download(url2, json_data, quiet=False)
    current_time = datetime.now()
    delta = timedelta(days=day)    
    time_end = current_time + delta
    time_file_path = os.path.join(chrome_dir, 'chrome_driver','download_time.txt')
    with open(time_file_path, 'w') as time_file:
        time_file.write(str(time_end))
    path_data = os.path.join(data_dir,'dsnganh.xlsx')
    df=pd.read_json(json_data)
    df.to_excel(path_data)   
    os.remove(linsce_path)
    os.remove(json_data)
    key=generate_key()
    file_key=os.path.join(chrome_dir, 'chrome_driver','snimdir.key')
...
def check_var():
    try:
        data_dir = os.path.join(os.path.dirname(__file__), 'data')
        time_file_path = os.path.join(data_dir, 'chrome_driver','download_time.txt')
        file_key=os.path.join(data_dir, 'chrome_driver','snimdir.key')
        with open(file_key, 'r') as key_save:
            key = key_save.read()
        decrypt_file(time_file_path, key)
        with open(time_file_path, 'r') as time_down:
            time_old = time_down.read()
        current_t_now=datetime.now()
        path_data = os.path.join(data_dir,'dsnganh.xlsx')
        path_nganh = os.path.join(data_dir,'ds_ngành_đã_lọc.xlsx')
        path_list = os.path.join(data_dir,'list_company.xlsx')
        time_file_path = os.path.join(data_dir, 'chrome_driver','download_time.txt')
        if current_t_now > time_old:
            os.remove(path_data)
