...
class ArbitrageSimulation:
    ...
    def run(self):
        ...
        "===========================中性==============================="
        data_with_header = pd.read_excel('D:\\量化产品跟踪\\市场中性\\中性-20230825.xlsx', sheet_name='原始净值')
        data = pd.read_excel('D:\\量化产品跟踪\\市场中性\\中性-20230825.xlsx', sheet_name='原始净值', header=1)
        data['t_date'] = data['t_date'].apply(lambda x: datetime.datetime.strftime(x, '%Y%m%d'))
        data.index = data['t_date']
        cols = data_with_header.columns.tolist()
...
if __name__ == '__main__':
    ...
    ArbitrageSimulation("20181228", "20230825", "20181228").run()