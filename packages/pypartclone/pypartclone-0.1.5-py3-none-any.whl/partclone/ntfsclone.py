...
class NtfsClone(ImageBackup):
    ...
    :type filename: str
    :raises partclone.imagebackup.WrongImageFile: if the file is not an ntfsclone image.
    :raises partclone.ntfsclone.NtfsCloneException: if the file is too short or the major version number differs.
    """
    ...
    def getBlockOffset(self, block_no: int) -> Optional[int]:
        ...
        :raises partclone.ntfsclone.NtfsCloneException: if the block number is out of range.
        """
    ...
    def blockReader(self, progress_bar: bool = True, verify_crc: bool = False,
        ...
        :raises partclone.ntfsclone.NtfsCloneException: if the image file is corrupted.
        """
        with tqdm(total=self.usedBlocks(), unit=' used blocks',
                  unit_scale=True, disable=not progress_bar) as progress:
