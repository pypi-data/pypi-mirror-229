...
def _kern_sum_group(arith: GraphProto, length: int, sums: List[NodeProto], dtype: str):
    weight = TensorProto(f'%w_sum_{length}_{dtype}', dtype=dtype, shape=(length, 1),
                        value=np.ones((length, 1), dtype=int))
    arith.add_parameter(weight)
    for sum_node in sums:
        x = sum_node.inputs['x']
        y = sum_node.outputs[0]
        mm_node = NodeProto(sum_node.name, optype=registry_v1['matmul'], 
                            inputs={'x': x, 'y': weight},
                            outputs=[y],
                            constants={'shamt_bwred': sum_node.constants['shamt_bwred'],
...
def insert_node_after_creation(graph: GraphProto, tensor: TensorProto, node_to_add: NodeProto):
...
def _kern_broadcast(arith: GraphProto, tensor: TensorProto, successors: List[Tuple[NodeProto, str]],
                    std_bmatrices: Dict[int, TensorProto], dbl_bmatrices: Dict[int, TensorProto]):
    ...
    for length, l_succs in succ_by_len.items():        
        btensor = TensorProto(f'{tensor.name}_bcast_{length}', tensor.dtype,
                              shape=[length], avg_sparsity=tensor.avg_sparsity,
                              quanta=tensor.quanta, named_dims=tensor.named_dims)

        # re-use broadcast matrix of the given length if it already exists
        if length in bmatrices:
            bmatrix = bmatrices[length]
        else:
            bmatrix = TensorProto(f'%broadcast_{length}', dtype=bm_dtype, shape=[1, length],
                                  value=np.ones((1, length), dtype=int))
            bmatrices[length] = bmatrix
            arith.add_parameter(bmatrix)


        broad_node = NodeProto(f'{tensor.name}_broadcast_{length}', registry_v1['matmul'],
