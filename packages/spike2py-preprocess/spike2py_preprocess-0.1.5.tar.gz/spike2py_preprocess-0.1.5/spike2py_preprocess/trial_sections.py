...
def _even_number_of_sections(section_names, file_name):
    if (len(section_names) % 2) != 0:
        message = f"{file_name} has an uneven number of TextMarks on the Memory channel. Please correct and rerun."
        print(message)
        sys.exit(1)
...
def _extract_waveform_section(section_data, channel_name, section_times):
    ...
    exec(f"section_data.{channel_name}.values = new_raw_values")
...
def _get_section_time_indexes(section_data, channel_name, section_times):
    ...
    for time in section_times:
        index_current_time = _find_nearest_time_index(channel_times, time)
        indexes.append(index_current_time)
    return indexes


def _find_nearest_time_index(times, time):
...
def _extract_section_data(data, section_name, section_times, channels):
    ...
    channels: List of channels [(<channel_name>, <channel_type>), ...]; e.g, [('Ds8', 'event'), ('Fdi', 'waveform')]
