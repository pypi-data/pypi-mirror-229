...
class MapPlotHelp(object):
    ...
    def make_plot_slice_3d(self,vals,min_percent=1, max_percent=1,hue="GBR",centre=True,title="Leucippus Plot 3d"):
        ...
        absmin = minv*min_percent
        absmax = maxv*max_percent   
        if maxv == minv:
            d0 = 0.5
        else:
        ...
        d1 = d0 + ((1-d0)/3)
        d2 = d0 + (2*(1-d0)/3)
        #d0,d1,d2 = d0,d0+0.2,0.9
        colorscale=[(0, "grey"), (d0, "snow"), (d1, "cornflowerblue"),(d2, "crimson"),(1.0, "rgb(100,0,0)")]
        c0 = "grey" #"rgba(119,136,153,1)"
        c1 = "snow" #"rgba(240,248,255,0)"
        c2 = "cornflowerblue" #"rgba(100,149,237,0.5)"
        c3 = "crimson" #"rgba(220,20,60,0.9)"
        c4 = "rgb(100,0,0)" #"rgba(100,0,0,1)"
    ...
    def make_plot_slice_2d(self,vals2d,points=[], naybs=[],min_percent=1, max_percent=1,hue="GBR",centre=True,title="Leucippus Plot 2d",samples=-1,width=-1):
        ...
        d1 = d0 + ((1-d0)/3)
        d2 = d0 + (2*(1-d0)/3)
        #print("levels",d0,d1,d2)
        #print('min/max',absmin,absmax)
