...
class PaymentWechatService:
    ...
    def payment_logic_processing_v2(param):
        ...
        try:
            ...
            goods_info = {}
            # # 根据支付记录用户 查询用户基本信息
            # user_set, err = DetailInfoService.get_detail(payment_message['user_id'])
            # user_platform_set, platform_err = DetailInfoService.get_detail(search_params={"nickname": sub_appid})
            # # 用户报名通知代码块
            # if user_set:
            #     goods_info['user'] = user_set
            #     if payment_message['enroll_id']:
            #         # 如果存在报名id 查询报名记录
            #         enroll_set, enroll_err = EnrollServices.enroll_detail(payment_message['enroll_id'])
            #         if enroll_set:
            #             # 根据报名记录获取 信息模块项目基本信息
            #             goods_info['enroll'] = enroll_set
            #             thread_set, thread_err = ThreadItemService.detail(enroll_set['thread_id'])
            #             if thread_set:
            #                 goods_info['thread'] = thread_set
            #                 project_name = thread_set['title']
            # summary = {
            #     "payer": user_set['full_name'],
            #     "payee": user_platform_set['full_name'],
            #     "entry_name": project_name
            # }
            # data['summary'] = summary
            # print(goods_info)
            data['goods_info'] = goods_info
            # data['goods_info'] = json.dumps(goods_info,cls=DateEncoder)
            # # TODO 拿到订单号后的操作 看自己的业务需求
            # 写入资金模块
            finance, err_txt = FinanceTransactService.finance_flow_writing(params=data, finance_type='RECHARGE')
            if err_txt:
                print(">>>>payment_logic_processing_err", err_txt)
