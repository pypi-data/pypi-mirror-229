...
class OMMStreamConnection(StreamConnection):
    ...
    def _process_message(self, message: dict) -> None:
        ...
        if message_type == MessageTypeOMM.REFRESH:
            ...
            self._emitter.emit(stream_id + StreamEventID.REFRESH, self, message)
            if message.get("Complete", True):
                self._update_usage_counter(stream_id, key, "Complete")
                self._emitter.emit(stream_id + StreamEventID.COMPLETE, self, message)

        elif message_type == MessageTypeOMM.UPDATE:
            stream_id = get_stream_id_from_message(message)
            self._emitter.emit(stream_id + StreamEventID.UPDATE, self, message)

        elif message_type == MessageTypeOMM.STATUS:
            stream_id = get_stream_id_from_message(message)
            self._emitter.emit(stream_id + StreamEventID.STATUS, self, message)
            ...
            self._emitter.emit(stream_id + StreamEventID.ERROR, self, message)
            ...
            self._emitter.emit(stream_id + StreamEventID.ACK, self, message)
