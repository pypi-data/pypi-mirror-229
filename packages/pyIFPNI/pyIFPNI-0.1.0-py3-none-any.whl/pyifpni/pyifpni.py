...
if __name__ == "__main__":
    test = IFPNI()
    # passed tests
    # o = test.supragenus(name="Orchidaceae")
    # a = test.genus(name="achlys")
    # s = test.infragenus(name="Sphenopteris")
    # c = test.species(name="Cordaites")
    # l = test.infraspecies(name="Lophozonotriletes")

    # o2a = test.supragenus_ancestors(name="orchidaceae")
    # a2a = test.genus_ancestors(name="achlys")
    # s2a = test.infragenus_ancestors(name="Stephanoporites Psilastephanoporites")
    # c2a = test.species_ancestors(name="Cordaites acadianus")
    # p2a = test.infraspecies_ancestors(name="Populus balsamoides minor")

    # o2d = test.supragenus_descendants(name='orchidaceae')
    # a2d = test.genus_descendants(name='alloberberis')
    # s2d = test.infragenus_descendants(name="Stephanoporites Psilastephanoporites")
    # c2d = test.species_descendants(name='Cordaites acadianus')
    # p2d = test.infraspecies_descendants(name='Populus balsamoides minor')

    # p = test.publication(title="Proposal to conserve the name")
    # b = test.book(title="Beiträge zur Kenntniss der fossilen Flora")
    # j = test.journal(title="Acta Palaeontologica")
    # abr = test.author(lastName="Abramova")

    # p2t = test.publication_taxon(title='A brief review of the stratigraphy and floral succession of the
    # Carboniferous in NW. Spain')
    # b2t = test.book2taxon(title='Beiträge zur Kenntniss der fossilen Flora')
    # j2t = test.journal2taxon(title="Acta Palaeontologica")

    # a2p = test.author2publication(lastName='Abramova')
    # a2t = test.author2taxon(lastName='Abramova')

    s = test.supragenus(name='balsaminaceae')
    print(s)
