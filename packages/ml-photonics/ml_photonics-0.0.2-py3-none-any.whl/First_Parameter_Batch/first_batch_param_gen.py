...
'''Generation of inital batch of parameters to test'''
x_size_bounds = np.array([.3,.6])
y_size_bounds = np.array([.4,.8])
etch_width_bounds = np.array([0.02,.05])
...
parameter_set = parameter_set_generator.get_random_parameter_set(n=50)

all_bounds = np.vstack((x_size_bounds,y_size_bounds,etch_width_bounds,etch_1_y_ratio,etch_1_etch_2_ratio))
np.save("parameter_set_batch_1.npy",parameter_set)
np.save("batch_1_bounds.npy",all_bounds)