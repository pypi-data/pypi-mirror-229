...
PADDLE_API std::tuple<Tensor, Tensor> cummax(const Tensor& x, int axis = -1, int dtype = 3);

PADDLE_API std::tuple<Tensor, Tensor> cummin(const Tensor& x, int axis = -1, int dtype = 3);
...
PADDLE_API std::tuple<Tensor, Tensor> flash_attn(const Tensor& q, const Tensor& k, const Tensor& v, const paddle::optional<Tensor>& fixed_seed_offset, const paddle::optional<Tensor>& attn_mask, float dropout = 0.0, bool causal = false, bool return_softmax = false, bool is_test = false, const std::string& rng_name = "");
...
PADDLE_API Tensor full_int_array(const IntArray& value, DataType dtype = DataType::FLOAT32, const Place& place = CPUPlace());
...
PADDLE_API Tensor shadow_output(const Tensor& x, const std::string& name);
...
PADDLE_API Tensor variable_length_memory_efficient_attention(const Tensor& query, const Tensor& key, const Tensor& value, const Tensor& seq_lens, const Tensor& kv_seq_lens, const paddle::optional<Tensor>& mask, float scale, bool causal);
...
PADDLE_API Tensor weight_only_linear(const Tensor& x, const Tensor& weight, const paddle::optional<Tensor>& bias, const Tensor& weight_scale, const std::string& weight_dtype);

PADDLE_API std::tuple<Tensor, Tensor> weight_quantize(const Tensor& x, const std::string& algo = "weight_only_int8");
...
PADDLE_API Tensor c_embedding(const Tensor& weight, const Tensor& x, int64_t start_index = 0);
...
PADDLE_API Tensor c_sync_comm_stream(const Tensor& x);

PADDLE_API Tensor& c_sync_comm_stream_(Tensor& x);
...
PADDLE_API Tensor matmul_int8(const Tensor& x, const Tensor& y, bool transpose_x = false, bool transpose_y = false);
