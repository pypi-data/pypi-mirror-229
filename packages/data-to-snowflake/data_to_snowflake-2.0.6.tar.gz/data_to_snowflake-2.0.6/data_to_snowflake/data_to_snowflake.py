...
class sharepoint:
  ...
  def upload_file(self,upload_url,modified_data):
    upload_response = requests.put(upload_url, data=modified_data, headers=self.headers)
...
class snowflake:
  def __init__(self,user,pw,database,schema,role,table_name):
  ...
  def load_data(self,sf_query,sf_tr_query,data):
    print('Table Name: {}'.format(self.database + '.' + self.schema + '.' + self.table_name))
    ...
    self.cnn.cursor().execute(
      ...
      self.table_name + "("  + sf_query + ")"
    )

    success, nchunks, nrows, _ = write_pandas(self.cnn, data, self.table_name, on_error = "CONTINUE",quote_identifiers=False)
  ...
  def get_data(self,sheet_name,search_query):
    ...
    cumm_df = pd.DataFrame(columns=columns)
    cumm_df = cumm_df.append(df, ignore_index=True)
