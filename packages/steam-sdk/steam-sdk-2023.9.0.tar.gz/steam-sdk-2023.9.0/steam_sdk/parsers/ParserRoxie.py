...
class ParserRoxie:
    ...
    def getConductorFromCableDatabase(self, cadata: pd.Cadata = None):
        ...
        if self.model_data:
            for cond_yaml in self.model_data.Conductors:
                if cond_yaml.name == self.cond_name:
                    type = cond_yaml.cable.type
                    break
        else:
            type=None
                        ...
                        for layer_nr, layer in pole.layers.items():
                            for winding_nr, winding in layer.windings.items():
                                ...
                                for block_nr, block in winding.blocks.items():
                                            ...
                                            if list_flag_ribbon[SumLayers]:
                                                for strand_nr, strand_position in strand_group.strand_positions.items():
                                                    ...
                                                    block_current.half_turns[ht_nbr_new].corners.bare = pd.Corner(
                                                        iH=pd.Coord(x=x_coord_ih,
                                                                    y=y_coord_ih),
                                                        iL=pd.Coord(x=x_coord_il,
                                                                    y=y_coord_il),
                                                        oL=pd.Coord(x=x_coord_ol,
                                                                    y=y_coord_ol),
                                                        oH=pd.Coord(x=x_coord_oh,
                                                                    y=y_coord_oh))
                                                    ...
                                                    block_current.half_turns[ht_nbr_new].strand_groups[1].strand_positions[1]= pd.Coord(x=sigDig(strand_position.x),
                                                                                                                                        y=sigDig(strand_position.y))
                                    ...
                                    data_new.coil.coils[coil_nr].poles[pole_nr].layers[layer_nr].windings[
                                    winding_nr].blocks.update(blocks_new_in_old)
                                    ...
                                    for block_nr, block_curr in blocks_new_in_old.items():
                                        ...
                                        if anticlockwise:
                                            ...
                                            block_curr.block_corners.iH = pd.Coord(x=bare_last.iH.x,
                                                                                          y=bare_last.iH.y)  # 1
                                            block_curr.block_corners.oH = pd.Coord(x=bare_last.oH.x,
                                                                                          y=bare_last.oH.y)  # 1

                                            block_curr.block_corners_ins.iH = pd.Coord(x=ins_last.iH.x,
                                                                                          y=ins_last.iH.y)  # 1
                                            block_curr.block_corners_ins.oH = pd.Coord(x=ins_last.oH.x,
                                                                                          y=ins_last.oH.y)  # 1
                                            block_curr.block_corners.iL = pd.Coord(x=bare_first.iL.x,
                                                                                      y=bare_first.iL.y)  # 1
                                            block_curr.block_corners.oL = pd.Coord(x=bare_first.oL.x,
                                                                                      y=bare_first.oL.y)  # 1

                                            block_curr.block_corners_ins.iL = pd.Coord(x=ins_first.iL.x,
                                                                                          y=ins_first.iL.y)  # 1
                                            block_curr.block_corners_ins.oL = pd.Coord(x=ins_first.oL.x,
                                                                                          y=ins_first.oL.y)  # 1
                                        else:  # First ht in new block

                                            block_curr.block_corners.iH = pd.Coord(x=bare_first.iH.x,
                                                                                          y=bare_first.iH.y)  # 1
                                            block_curr.block_corners.oH = pd.Coord(x=bare_first.oH.x,
                                                                                          y=bare_first.oH.y)  # 1

                                            block_curr.block_corners_ins.iH = pd.Coord(x=ins_first.iH.x,
                                                                                          y=ins_first.iH.y)  # 1
                                            block_curr.block_corners_ins.oH = pd.Coord(x=ins_first.oH.x,
                                                                                          y=ins_first.oH.y)  # 1
                                            block_curr.block_corners.iL = pd.Coord(x=bare_last.iL.x,
                                                                                      y=bare_last.iL.y)  # 1
                                            block_curr.block_corners.oL = pd.Coord(x=bare_last.oL.x,
                                                                                      y=bare_last.oL.y)  # 1

                                            block_curr.block_corners_ins.iL = pd.Coord(x=ins_last.iL.x,
                                                                                          y=ins_last.iL.y)  # 1
                                            block_curr.block_corners_ins.oL = pd.Coord(x=ins_last.oL.x,
                                                                                          y=ins_last.oL.y)  # 1
                            else:
                                pass
    ...
    def getData(self, dir_iron: Path = None, dir_data: Path = None, dir_cadata: Path = None,
        ...
        if path_to_yaml_model_data is not None:
            try:
                ...
                raise ValueError("Path does not exists to model_data.yaml")
        else:
