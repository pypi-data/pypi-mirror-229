...
class CTRRankTool:
    ...
    def configure_positive_and_negative_sample_ratio(self, positive_entry_id_set,negative_entry_id_set,embedding_dim):
        ...
        if 2 * ( majority_number * 0.9 ) < embedding_dim * 30:
            self.__logger.debug(f'2 * ( majority_number * 0.9 ) < embedding_dim * 30  {2 * ( majority_number * 0.9 ) } < { embedding_dim * 30}')
            return [],[]
        if len(positive_entry_id_set) < 100:
            self.__logger.debug(f'positive_entry_id_set length small than 100')
            return [],[]
        ...
        for current_x,current_y in list(zip(resampled_x,resampled_y)):
            train_tuple.append((current_x[0],current_y))
