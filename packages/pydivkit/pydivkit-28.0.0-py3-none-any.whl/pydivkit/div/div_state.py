...
class DivState(BaseDiv):
    ...
    state_id_variable: typing.Optional[typing.Union[Expr, str]] = Field(
        min_length=1, 
        description=(
            "Name of the variable where the current state ID value is "
            "stored. If the value ofa variable changes, then the active "
            "state will also be changed. The variabletakes precedence "
            "over the `default_state_id` field."
        ),
    )
