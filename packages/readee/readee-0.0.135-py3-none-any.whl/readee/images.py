...
MORE_CERTAIN_IMG_ATTRS = ['data-src-large', 'data-src', 'data-original-src', 'data-pin-media', 'data-original']
IMG_ATTRS = MORE_CERTAIN_IMG_ATTRS + ['src'] # people would put junk in src field

def isValidImgSrc(src):
	if src.endswith('pdf'):
		return False
...
def formatRawImg(raw_img, domain):
	for attr in IMG_ATTRS:
		if raw_img.get(attr) and isValidImgSrc(raw_img.get(attr)):
...
def _cleanupImages(soup, domain):
	...
	for img in soup.find_all("source"):
		if not img.get('srcset'):
			continue
		img.name = 'img'
		img['src'] = img.get('srcset').split()[-2]
