...
class CumulativeBeatenDistances(BeatenDistances):
    ...
    def to_marginal(self) -> "MarginalBeatenDistances":
        ...
        """
        marginal = []
        for i, hl in enumerate(self):
            if i == 0:
                marginal.append(hl)
            else:
                marginal.append(hl - self[i - 1])

        return MarginalBeatenDistances(marginal)  # type: ignore
...
class MarginalBeatenDistances(BeatenDistances):
    ...
    def to_cumulative(self) -> CumulativeBeatenDistances:
        ...
        for i, hl in enumerate(self):
            if i == 0:
                cumulative.append(hl)
            else:
                cumulative.append(hl + cumulative[i - 1])

        return CumulativeBeatenDistances(cumulative)  # type: ignore
