...
from classiq.interface.generator.wiring.sliced_wire import InoutSlicedWire, SlicedWire
...
WireNameOrPortBinding = Union[WireName, SlicedWire]
...
WireNamesOrInoutPortBinding = Union[WirePair, InoutSlicedWire]

SUFFIX_RANDOMIZER = random.Random()


def split_inout_port_binding_to_wires(
    inout: WireNamesOrInoutPortBinding,
) -> Tuple[WireName, WireName]:
    if isinstance(inout, WirePair):
        return inout.in_wire, inout.out_wire
    else:
        return inout.input_name, inout.output_name
...
def split_wire_names_or_inout_slice(
    ...
    return (
        wire_names_or_inout_port_binding.source_slice,
        wire_names_or_inout_port_binding.destination_slice,
    )
