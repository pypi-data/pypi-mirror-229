...
def validateDuplicates(object:DataFrame,fields:List,registersAmount: int,entity: str,threshold: int, repetitionsNumber = ""):
    ...
    if repetitionsNumber is not None:
        errorDf = object.withColumn("sum",sum(lit(1)).over(windowSpec)).filter(col("sum")>repetitionsNumber).drop("sum")
    else:
        errorDf = object.withColumn("sum",sum(lit(1)).over(windowSpec)).filter(col("sum")>One).drop("sum")
