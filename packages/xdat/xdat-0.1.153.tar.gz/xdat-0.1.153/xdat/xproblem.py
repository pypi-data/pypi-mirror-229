...
def train_cv(df, target_col, clf, n_splits=12, stratify_on=None, group_on=None, ordered_split=False, ts_split_on=None, del_cols=tuple(), feature_cols=tuple(), uid_col=None, sample_weight_col=None, eval_size=0.0, framework='auto', fit_params=None, post_fit=None, target_manipulation=None, with_confidence=False, with_tqdm=True, as_numpy=False, pred_col_prefix='', n_jobs=1):
    ...
    def calc_fold(fold_num, split_data: CVSplit):
        ...
        df_train[pred_col] = tm.post(pred_train)
        ...
        if with_confidence:
            ...
            df_train[pred_conf_col] = get_pred_conf(X_train)
            df_test[pred_conf_col] = get_pred_conf(X_test)

            if df_val is not None:
                df_val[pred_conf_col] = get_pred_conf(X_eval)
