...
_JOHNSNOWLABS_ENV_ENABLE_GPU = 'GPU_ENABLED'
...
def get_default_pip_requirements():
    ...
    if _JOHNSNOWLABS_ENV_HEALTHCARE_SECRET not in os.environ or _JOHNSNOWLABS_ENV_VISUAL_SECRET not in os.environ:
        raise Exception(
...
def _save_jars_and_lic(dst_dir, store_license=True):
    ...
    jar_type = JvmHardwareTarget('gpu' if _JOHNSNOWLABS_ENV_ENABLE_GPU in os.environ else 'cpu')
    ...
    if suite.nlp.get_java_path():
        shutil.copyfile(suite.nlp.get_java_path(), deps_data_path / f"spark_nlp_{jar_type}.jar")
...
class _PyFuncModelWrapper:
    ...
    def predict(self, text):
        ...
        """
        # TODO FUCK LOCAL TEST GO STRAIGHT TO DB+ENDPOINT?!? --> JK NEED MLFOW MERGE AND RELASE BECUASE CANNOT OVERWRITE!!!!!
        """
        TODO:
        1) support output level via first row inference and other params X DONE 
        2) support start session with GPU (ENV VAR?) X done 
        3) support image predictions (OCR MODEL) --> send img as binary??
        """
