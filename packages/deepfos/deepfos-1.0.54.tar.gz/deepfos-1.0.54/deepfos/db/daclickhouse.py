...
class ClientCH:
    ...
    def establish_conn(self):
        ...
        conf = evloop.run(self.lease.renew())
        self.lease.schedule(slow_start=True)
        evloop.register_shutdown(self.close)
...
def select(sql):
    return ClientManager().new_client().select(sql)


def execute(sql):
    return ClientManager().new_client().execute(sql)


def query_dataframe(sql):
    return ClientManager().new_client().query_dataframe(sql)


def insert_dataframe(table, dataframe):
    return ClientManager().new_client().insert_dataframe(table, dataframe)
