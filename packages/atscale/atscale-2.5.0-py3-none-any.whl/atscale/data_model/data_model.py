...
class DataModel:
    ...
    def update_query_dataset(
        ...
        """Updates aggregate settings for an existing Query Dataset in the data model.
        ...
        validation_utils.validate_required_params_not_none(
            local_vars=locals(),
            inspection=inspect.getfullargspec(self.update_query_dataset),
        )
        ...
        if not project_utils._check_if_qds(dset):
            raise atscale_errors.UserError(
                f"Invalid parameter: dataset name '{dataset_name}' is not a qds"
            )

        project_utils._update_query_dataset(
            project_dict=project_dict,
            dataset_name=dataset_name,
            cube_id=self.cube_id,
            allow_aggregates=allow_aggregates,
    ...
    def update_secondary_attribute(
        ...
        Args:
            attribute_name (str): The name of the secondary attribute to update.
            description (str, optional): The new description for the secondary attribute. Defaults to None to leave unchanged.
            caption (str, optional): The new caption for the secondary attribute. Defaults to None to leave unchanged.
            folder (str, optional): The new folder to put the secondary attribute in. Defaults to None to leave unchanged.
            publish (bool, optional): Whether or not the updated project should be published. Defaults to True.
        """
        model_utils._perspective_check(self)

        inspection = getfullargspec(self.update_secondary_attribute)
        validate_by_type_hints(inspection=inspection, func_params=locals())
        ...
        attributes = [
            x
            for x in project_json.get("attributes", {}).get("keyed-attribute", [])
            if x["name"] == attribute_name
        ]
        if len(attributes) < 1:
            if secondary_attributes.get(attribute_name, False):
                raise atscale_errors.UnsupportedOperationException(
                    f"Secondary Attribute: {attribute_name} is roleplayed but only the base name can be used to update metadata."
                )
            else:
                raise atscale_errors.UserError(
                    f"Secondary Attribute: {attribute_name} does not exist."
                )
        feature_utils._update_secondary_attribute(
    ...
    def create_dimension(
        ...
        if new_dimension_name in self.get_dimensions():
            raise atscale_errors.UserError(
                f"A dimension named '{new_dimension_name}' already exists in the given model"
            )
        if new_hierarchy_name in self.get_hierarchies():
            raise atscale_errors.UserError(
    ...
    def update_dimension(self, dimension_name: str, description: str = None, publish: bool = True):
        ...
        if dimension_name not in self.get_dimensions():
            raise atscale_errors.UserError(f"Dimension: '{dimension_name}' not found.")
        ...
        if dim:
            ...
            raise ValueError(f"No dimension {dimension_name} found")
        self.project._update_project(project_dict, publish)
    ...
    def create_hierarchy(
        ...
        if dimensions[dimension_name]["type"] == "Time":
            if level_type == TimeSteps.Regular:
                raise atscale_errors.UserError(
                    f"level_type cannot be TimeSteps.Regular for time dimensions"
                )
    ...
    def update_hierarchy(
        ...
        if hierarchy_name not in self.get_hierarchies():
            raise atscale_errors.UserError(f"Hierarchy: '{hierarchy_name}' not found.")
    ...
    def add_level_to_hierarchy(
        ...
        if hierarchy_name not in self.get_hierarchies():
            raise atscale_errors.UserError(
                f"Hierarchy '{hierarchy_name}' not associated with given model"
            )
        ...
        attributes = self.get_features(
            feature_list=[existing_level], feature_type=FeatureType.CATEGORICAL, use_published=False
        )
        if existing_level not in attributes:
            raise atscale_errors.UserError(f"Level '{existing_level}' not found")
        attribute = attributes[existing_level]
        ...
        dimensions = self.get_dimensions()
        if dimensions[attribute["base_dimension"]]["type"] == "Time":
            if level_type == TimeSteps.Regular:
                raise atscale_errors.UserError(
                    f"level_type cannot be TimeSteps.Regular for time dimensions"
                )
