...
def melon_scanner(init_host,delay=0):
    ...
    clear()
    hits = []
    interesting_bool = input("do you want to check for interesting things? y/N\n")
    vulnerabilities_bool = input("do you want to check for vulnerabilities? y/N\n")
    if interesting_bool.lower() != "y" and vulnerabilities_bool.lower() != "y":
        print(RED + "aborting")
        sys.exit()
    ...
    try:
        data = puppy_requests.status_code(init_host + "/admin")
        if data == 200:
            host_list.append(init_host + "/admin")

    except:
        pass
    
    if interesting_bool.lower() == "y":
    ...
    if vulnerabilities_bool.lower() == "y":
        ports = []
