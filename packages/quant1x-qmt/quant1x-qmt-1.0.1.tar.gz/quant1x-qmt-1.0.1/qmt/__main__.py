...
def main() -> any:
    ...
    if len(orders) != 0:
        for order in orders:
            ...
            print("order: id=%s, time=%s, code=%s, status=%s" %(order.order_id,order_time,order.stock_code, order.order_status))
            # 休市期间撤销未成交的委托订单
    ...
    if qc.sell_is_ready():
        for position in positions:
            ...
            if position.can_use_volume >= 100:
                # 卖出操作
                order_id = trader.order_stock(acc, position.stock_code, xtconstant.STOCK_SELL, position.can_use_volume,
