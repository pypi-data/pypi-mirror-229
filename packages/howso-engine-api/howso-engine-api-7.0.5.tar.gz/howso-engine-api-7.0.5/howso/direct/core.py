...
class HowsoCore:
    ...
    def __init__(  # noqa: C901
        ...
        **kwargs
        ...
        # If embedded set, its value is a package path to the amalgam library
        # files or True
        if 'embedded' in amlg_params:
            if amlg_params.get('download'):
                _logger.warning(
                    'Incompatible amalgam params \'embedded\' and \'download\' '
                    'set. Disregarding download configuration')

            # Calculate the full path to the library file
            if amlg_params['embedded'] is True:
                embedded_amlg_location = 'howso.local.lib.amlg'
                warnings.warn(
                    f'The use of a boolean for "embedded" is deprecated. '
                    f'Please update your howso.yml to use the package path '
                    f'instead. Defaulting to {embedded_amlg_location}')
            else:
                embedded_amlg_location = amlg_params['embedded']

            embedded_path = (
                f'{embedded_amlg_location}.{operating_system}.{architecture}')
            embedded_resource_filename = resource_filename(
                embedded_path, library_filename)
            ...
            if embedded_amlg_location.exists():
                ...
                _logger.debug(
                    f'Using embedded amalgam location: {embedded_amlg_location}')
            else:
                raise HowsoError(
                    f'No amalgam library found at {embedded_amlg_location}')

        # If download set (but not embedded), try and download the specified
        # version using howso-build-artifacts
        ...
        # If 'embedded' set, its value is a package path to the core library
        # files or True
        if 'embedded' in core_params:
            if core_params.get('download'):
                _logger.warn(
                    'Incompatible params "embedded" and "download" set. '
                    'Disregarding download configuration')

            # Append filenames to the embedded path
            if core_params.get('embedded') is True:
                howso_location = 'howso.local.lib.core'
                warnings.warn(
                    'The use of a boolean for "embedded" is deprecated. '
                    'Please update your howso.yml to use a package path '
                    f'instead. Using default path: {howso_location}')
            else:
                howso_location = core_params['embedded']

            howso_python_path = f'{howso_location}'
            howso_embedded_filepath = resource_filename(
                howso_python_path, 'howso.caml')
            trainee_template_embedded_filepath = resource_filename(
                howso_python_path, 'trainee_template.caml')

            howso_embedded_filepath = Path(howso_embedded_filepath)
            trainee_template_embedded_filepath = Path(
                trainee_template_embedded_filepath)

            _logger.debug(
                'Using howso embedded filepath '
                f'{howso_embedded_filepath}')
            _logger.debug(
                'Using trainee template embedded filepath '
                f'{trainee_template_embedded_filepath}')

            if not (
                howso_embedded_filepath.exists() and
                trainee_template_embedded_filepath.exists()
            ):
                raise HowsoError(
                    'Core files howso.caml and/or trainee_template.caml '
                    f'not found at {howso_embedded_filepath} '
                    f'and/or {trainee_template_embedded_filepath}')

            howso_embedded_dir = (
                howso_embedded_filepath.parent.expanduser())
            self.trainee_template_path = howso_embedded_dir
            self.howso_path = howso_embedded_dir
            _logger.debug(f'Using howso location: {self.howso_path}')
            _logger.debug(
                'Using trainee_template location: '
                f'{self.trainee_template_path}')
            self.default_save_path = Path(
                Path.home(), '.howso', 'saved_trainees')

        # If download (and not embedded), then retrieve using
