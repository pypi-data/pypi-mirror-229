...
const missionActions = async () => {
  const mappedLegs = missionForm.value.legs.map((leg) => {
    return {
      ...leg,
      arrival_datetime: dayjs(leg.arrival_datetime).format('YYYY-MM-DD[T]HH:mm:ss[Z]'),
      departure_datetime: dayjs(leg.departure_datetime).format('YYYY-MM-DD[T]HH:mm:ss[Z]')
    }
  })
  const mappedMission = {
    ...missionForm.value,
