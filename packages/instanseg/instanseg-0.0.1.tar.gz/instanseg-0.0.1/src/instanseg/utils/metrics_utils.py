...
from instanseg.utils.iou_overlap import __is_overlapped_iou
...
def __filter_unsegmented(
    ...
    :return: labels array where all unsegmented points (with label equal to instanseg.metrics.constants.UNSEGMENTED_LABEL)
     are deleted
    """
    (unsegmented_indices,) = np.where(
        label_array == instanseg.metrics.constants.UNSEGMENTED_LABEL
    )
