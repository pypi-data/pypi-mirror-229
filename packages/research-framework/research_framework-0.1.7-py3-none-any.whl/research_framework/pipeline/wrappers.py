...
class FitPredictFilterWrapper(BaseFilterWrapper):
    def __init__(self, clazz:str, params:Dict[str, Any], filter_object: BasePlugin, fly: FlyWeight):
        self.clazz:str = clazz
        self.params:Dict[str, Any] = params
        self.filter: BasePlugin = filter_object
    ...
    def fit(self, data_hashcode:str, data):
        ...
        if filter_trained_item is None:
            ...
            if callable(data):
                data = data()
            self.filter.fit(data, self.params)
            ...
            self.filter = lambda : self.fly.get_object_from_item(filter_trained_item)
            self.hashcode = trained_filter_hashcode
    ...
    def predict(self, data_hashcode:str, data_name:str, data):
        if self.hashcode is None:
            ...
            if data_item is None:
                ...
                data = self.filter.predict(data, self.params)
                ...
                data = lambda : self.fly.get_object_from_item(data_item)
...
class PassThroughFilterWrapper(BaseFilterWrapper):
    def __init__(self, clazz:str, params:Dict[str, Any], filter_object: BasePlugin, fly: FlyWeight, *args, **kwargs):
        self.clazz:str = clazz
        self.params:Dict[str, Any] = params
        self.filter: BasePlugin = filter_object
        self.fly: FlyWeight = fly
...
class InputFiterWrapper(BaseFilterWrapper):
    ...
    def predict(self, data_name:str, *args, **kwargs):
        ...
        if data_item is None:
            data = self.filter.predict(None, self.params)
            if not self.fly.set_item(
                data_name,
                data_hashcode,
                data                    
            ...
            data = lambda: self.filter.predict(None, self.params)
