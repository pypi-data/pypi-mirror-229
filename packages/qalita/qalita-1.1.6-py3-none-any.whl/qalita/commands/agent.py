...
def agent(config, name, mode, token, url):
    ...
    file_pattern = os.path.join(qalita_home, f'.env-agent-{name}' if name else '.env-agent-*')
...
@pass_config
def job_run(config, source_id, source_version_id, pack_id, pack_version_id, job_id=None):
    logger.info("------------- Job Run -------------")
...
@pass_config
def post_run(config, run_path, name, pack_id, pack_version_id, source_id, source_version_id):
...
def is_time_for_job(last_job_end_date_str, cron_expression):

    # Check if last_job_end_date_str is None or not a string
    if last_job_end_date_str is None or not isinstance(last_job_end_date_str, str):
    ...
    # Compare the current time with the next run time
    if now >= next_run:
        return True
    return False

def check_routines(config):
            ...
            if routine['status'] == "active":
                ...
                if routine['source']['id'] in source_ids:
                    ...
                    if isinstance(jobs, list) :
                        ...
                        last_job = None
                        for job in jobs:
                            if job['source']['id'] == routine['source']['id'] and job['pack']['id'] == routine['pack']['id']:
                                last_job = job
                                if job['status'] in ["running","pending"]:
                                    # logger.info(f'Job {job["id"]} already running or pending for routine {routine["id"]}')
                                    break
                        if last_job != None:
                            if is_time_for_job(last_job_end_date_str=last_job['end_date'],cron_expression=routine['schedule']):
