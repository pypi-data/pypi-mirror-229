...
def test_manntensor():
    ...
    np.testing.assert_array_almost_equal(SqrtPhi, np.load(tfp + 'manntensorsqrt_ref.npy').reshape(SqrtPhi.shape), 16)
    ...
    np.testing.assert_array_almost_equal(Phi, np.load(tfp + 'manntensor_ref.npy').reshape(Phi.shape), 16)
