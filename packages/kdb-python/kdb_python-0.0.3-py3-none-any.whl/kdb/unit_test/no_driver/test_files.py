...
from kdb import DATA_REPORT_DIR
...
def test_files(data_test, params):
    ...
    assert not os.path.isfile(os.path.join(DATA_REPORT_DIR, file_name))
    kdb_driver.files.write_as_text('a test text', file_name)
    assert os.path.isfile(os.path.join(DATA_REPORT_DIR, file_name))
