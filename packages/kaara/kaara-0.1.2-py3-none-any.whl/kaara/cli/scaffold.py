...
from kaara.cli import SubParsersAction
import logging
from kaara.shared.constants import (
    DOCS_BASE_URL,
    DEFAULT_CONFIG_PATH,
    DEFAULT_DOMAIN_PATH,
    DEFAULT_DATA_PATH,
...
def print_cancel() -> None:
    logger.info("Ok. You can continue setting up by running 'rasa init' 🙋🏽‍♀️")
    sys.exit(0)
...
def run(args: argparse.Namespace) -> None:
    ...
    if args.no_prompt and not os.path.isdir(path):
        logger.debug(f"Project init path '{path}' not found.")
