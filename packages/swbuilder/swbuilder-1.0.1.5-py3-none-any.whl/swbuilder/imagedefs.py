...
TEMPLATE_DIR = os.getenv("TEMPLATE_DIR", os.path.join(os.getcwd(), "data", "template"))
...
class ImageDefs(object):
    ...
    def parse_yaml(self, filename):
        ...
        if (filename.find("://") > 0):
            ...
            filename = os.path.join(TEMPLATE_DIR, get_path_from_url(filename))
        ...
        for imagename, defn in yamldefs.items():
            if (imagename[:1] == "_") and (imagename[-1:] == "_"): # if imagename in SPECIAL_FIELDS:
                ...
                if imagename == "_SOURCES_":
                    yamldefs["_SOURCES_"] = [
                        p if (p.find("://") > 0) else os.path.relpath(_get_abspath(pathroot, us.make_param(p, dir_dict)))
