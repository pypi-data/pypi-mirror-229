...
class Representation(Layout):
    ...
    def _add_relationship_to_previous_child(
        self, prev_child_id: str, new_child_id: str
    ):
        self.add_relationship(prev_child_id, new_child_id, NodeRelationship.NEXT)
        self.add_relationship(new_child_id, prev_child_id, NodeRelationship.PREVIOUS)

    def _add_relationship_to_parent(self, parent_id: str, child_id: str):
        self.add_relationship(parent_id, child_id, NodeRelationship.PARENT)
        self.add_relationship(child_id, parent_id, NodeRelationship.CHILD)

    def _create_single_child_message(
        self,
        child_id: str,
        part: str,
        x: float,
        ...
        coordinate = Coordinate(id=child_id, x=x, y=y, z=z, t=t, n_parts=len(part))
        return self.create_chain_message(
            child_id, part, create_time, author, part_weight, coordinate
        )
    ...
    def _create_child_messages(
        ...
        for index, part in enumerate(content_parts):
            new_child_id = self._calculate_unique_child_id(child_id, index)

            child_message = self._create_single_child_message(
                new_child_id, part, x, y, z, t, create_time, author, part_weight
            )

            if prev_child_id:
                self._add_relationship_to_previous_child(prev_child_id, new_child_id)

            self._add_relationship_to_parent(child_id, new_child_id)
    ...
    def _assign_coordinates(
        ...
        **kwargs: Dict[str, Union[str, float]],
        ...
        x, y, z, t, n_parts = self._calculate_coordinates(
        ...
        child_coordinate = Coordinate(id=child_id, x=x, y=y, z=z, t=t, n_parts=n_parts)

        child_messages = self._create_child_messages(
            child_id, x, y, z, t, content_parts, create_time, author, part_weight
        )
