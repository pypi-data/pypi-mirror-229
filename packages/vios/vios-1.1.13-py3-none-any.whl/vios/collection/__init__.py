...
class Task(object):
    ...
    def plot(self):
        """vdict = OrderedDict()
        # [vdict.update({f'test{i}': {'data': [np.arange(3000,4000), np.random.random(1000)+i]}}) for i in range(1)]
        for i in range(10000):
            import time
            time.sleep(0.1)
            try:
                vdict = OrderedDict()
                [vdict.update({f'test{j}': {'data': [np.arange(i,i+1), np.random.random(1)+j],'title':'1234'}}) for j in range(1)]
                if i == 0:
                    vs.plot([[vdict]*8]*8)
                else:
                    vs.append([[vdict]*8]*8)
