...
from zenutils.strutils import smart_get_binary_data
...
def encrypt(data, pk, envelope=PKCS1_OAEP):
    ...
    return "".join(base64utils.encodebytes(encrypted_data).decode().splitlines())


def decrypt(data, sk, envelope=PKCS1_OAEP):
...
def export_key(rsakey):
    ...
    if not isinstance(rsakey, RsaKey):
        ...
        if isinstance(rsakey, sixutils.BASESTRING_TYPES):
            rsakey = RSA.import_key(rsakey)
    return sixutils.TEXT(rsakey.export_key())


def get_hash_method(hash_method):
    if isinstance(hash_method, sixutils.BASESTRING_TYPES):
        return HASH_METHODS[sixutils.TEXT(hash_method).upper()]
    return hash_method


def sign(data, sk, hash_method="SHA", sign_method=PKCS1_PSS):
    ...
    return base64utils.encodebytes(sign_data).decode("utf-8").replace("\n", "")


def verify(data, sign_data, pk, hash_method="SHA", sign_method=PKCS1_PSS):
