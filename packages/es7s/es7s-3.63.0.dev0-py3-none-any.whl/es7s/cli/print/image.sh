...
_main() {
    ...
    [[ -z "$*" && -n "${ARGS[demo]}" ]] && set - "${ARGS[demo]}"
    [[ -z "$*" ]] && echo "ERROR: No arguments" && return 1
...
_print_status() {
    ...
    local xmk=
    [[ -n $imgw ]] && [[ -n $imgh ]] && xmk=x
    printf "\e[K%10s %s" "$imgw$xmk$imgh" "$file" >&2
}
