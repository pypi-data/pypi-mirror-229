...
class SpamAssassinClient:
  ...
  def connect(self):
    ...
    logging.info(f"[+] Connecting to SpamAssassin server: {self.host}:{self.port}")
  ...
  def close(self):
    ...
    logging.info("[+] Closing connection to SpamAssassin")
  ...
  def learn(self, mail, mail_type):
    ...
    https://svn.apache.org/repos/asf/spamassassin/trunk/spamd/PROTOCOL
    """
    logging.info(f"[+] Saving email as [{mail_type.upper()}]")
    try:
      buffer = BytesIO()
  ...
  def check_spam(self, mail):
    ...
    """
    logging.debug("[+] Checking for spam")
