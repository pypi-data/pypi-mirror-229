        ...
        with stepper.step(f'{FG_GREEN}release{NC}'):
            ...
            for component_name, component_config in self.assembly_file.components.items():
                ...
                list_deployments = kubernetes_client.list_namespaced_deployment(
                    ...
                    label_selector=f'release/component-version!={component.tag}',
                )
                if not len(list_deployments) == 0:  # We do this because if all deployments are of current service version - there's no need in service update
                    write_stdout(f'Service {release_name} with version {component.tag} already released')
                    continue
                ...
                if component_config.force_disable:
                    for deployment in list_deployments.items:
