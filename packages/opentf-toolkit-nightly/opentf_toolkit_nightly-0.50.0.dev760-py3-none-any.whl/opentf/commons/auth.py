        ...
        with open(file, 'r', encoding='utf-8') as f:
            ...
            for l in f.read().splitlines():
                if l.strip() and not l.strip().startswith('#'):
                    try:
                        authorization_policies.append(json.loads(l))
                    except json.decoder.JSONDecodeError as err:
                        raise ConfigError(
                            f'Invalid JSON entry "{l}" in policy file "{file}": {err}.'
                        )
        for policy in authorization_policies:
        ...
        for entry in authorization_trustedkeys:
            ...
            if len(entry) >= 4 and entry[3]:
                if not NAMESPACES_PATTERN.match(entry[3]):
                    raise ConfigError(
                        f'Invalid namespaces specification "{entry[3]}" in trusted key authorization file "{file}".  Namespaces names are either "*" or formed from letters, digits, and hyphens.'
                    )
