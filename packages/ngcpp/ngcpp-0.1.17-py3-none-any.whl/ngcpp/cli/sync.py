...
from ngcpp.utils import run_batch_commands
...
def main(cfg: DictConfig) -> None:
    ...
    with open(f"{ngcpp_root}/ngc_local_sync.sh", "w", encoding="utf-8") as file:
        for cmd in rsync_cmds:
            file.write(f"{cmd}\n")
    logging.debug(
        f"Writing sync script to {os.path.join(os.getcwd(), ngcpp_root)}/ngc_local_sync.sh"
    )
    ...
    sync_cmd.append(f"aws s3 sync {pbss_snapshot_path} {ngc_path} --quiet")
    sync_cmd.append(f"cd {ngc_path}")
    with open(f"{ngcpp_root}/ngc_remote_sync.sh", "w", encoding="utf-8") as file:
        for cmd in sync_cmd:
            file.write(f"{cmd}\n")
    logging.debug(
        f"Writing sync script to {os.path.join(os.getcwd(), ngcpp_root)}/ngc_remote_sync.sh"
    )
