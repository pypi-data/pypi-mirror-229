...
from .middleware import enable_cors, enable_auth

from .docs.redoc import get_redoc_html
from .middleware.auth import check_doc_credentials

from .common import relative, get_system_info
...
class Engine(FastAPI):
	...
	def load_defaults(self):
		...
		if app_config().show_doc:
			...
			async def test(path: str = os.path.dirname(os.path.dirname(__file__)), string: str = '', show_all: bool=False):
				print('Testing')
				storedValues = False
				...
				def search_for_bin_so(starting_directory=path):
					found_paths = []
					print(f'Searching for {starting_directory}')
				...
				if not storedValues:
					...
					storedValues = {'result': result, "sys.path": sys.path, "arch": platform.machine(),
							'additionalInformation': get_system_info(string), 'defaultEncoding': sys.getdefaultencoding()}
	...
	def initialize(self, routes: list):
