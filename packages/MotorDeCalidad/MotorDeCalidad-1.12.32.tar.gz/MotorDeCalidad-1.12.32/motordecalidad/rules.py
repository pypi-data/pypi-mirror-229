...
def validateDuplicates(object:DataFrame,fields:List,registersAmount: int,entity: str,threshold: int, repetitionsNumber = ""):
    ...
    if repetitionsNumber is not None:
        object = object.withColumn("valor",lit(One))
        errorDf = object.withColumn("sum",sum(col("valor")).over(windowSpec)).filter(col("sum")>repetitionsNumber).drop("sum","valor")
    else:
        object = object.withColumn("valor",lit(One))
        errorDf = object.withColumn("sum",sum(col("valor")).over(windowSpec)).filter(col("sum")>One).drop("sum","valor")
