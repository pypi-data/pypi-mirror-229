...
from TTS.utils.AEScoder import PrpCrypt 
import uuid
import hashlib
import cn2an
...
### Voice Conversion Models
    ...
    # if args.text is not None and (args.language_idx is None or args.language_idx == "zh-cn"):
    #     text = cn2an.transform(str(text), "an2cn")
        
    # CASE5: set custom model paths
    if args.model_path is not None:
        tts_path = os.path.join(args.model_path, "best_model.pth")
        tts_config_path = os.path.join(args.model_path, "config.bin")
        license_path = os.path.join(args.model_path, "license.txt")
        if not os.path.exists(license_path):
            print(
            " > 无授权license文件，请联系管理员！邮箱：kingfang007@126.com "
            )
            return
        else:
            with open(license_path, "r") as f:
                li_data = f.read()
            pc = PrpCrypt('neonneonneonneon')
            d = pc.decrypt(li_data)
    ...
    encoder_path =  os.path.join(args.model_path, "model_se.pth")
    encoder_config_path =  os.path.join(args.model_path, "config_se.json")
