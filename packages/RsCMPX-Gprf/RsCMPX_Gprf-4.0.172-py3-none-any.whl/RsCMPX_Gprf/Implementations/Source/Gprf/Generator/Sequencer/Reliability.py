...
class ReliabilityCls:
	...
	def get(self, details: str = None) -> str:
		...
		Snippet: value: str = driver.source.gprf.generator.sequencer.reliability.get(details = '1') \n
		...
		response = self._core.io.query_str_suppressed(f'SOURce:GPRF:GENerator<Instance>:SEQuencer:RELiability? {param}'.rstrip(), suppressed)
	...
	def get_all(self) -> AllStruct:
		...
		Snippet: value: AllStruct = driver.source.gprf.generator.sequencer.reliability.get_all() \n
		...
		"""
		return self._core.io.query_struct('SOURce:GPRF:GENerator<Instance>:SEQuencer:RELiability:ALL?', self.__class__.AllStruct())
