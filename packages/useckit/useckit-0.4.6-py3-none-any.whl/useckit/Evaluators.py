...
class Evaluator(ABC):
    def __init__(self, dataset: Dataset, epochs: int = None, verbose: bool = False, seed = None):
...
class AutoEvaluator(Evaluator):
    def __init__(self, dataset: Dataset, epochs: int = None, verbose: bool = False, seed: int = None):
        super().__init__(dataset, epochs=epochs, verbose=verbose, seed=seed)
    ...
    def evaluate(self):
        if self.enable_mlp:
            TSCParadigm(
                ...
                verbose=self.verbose, seed=self.seed).evaluate(self.dataset)
