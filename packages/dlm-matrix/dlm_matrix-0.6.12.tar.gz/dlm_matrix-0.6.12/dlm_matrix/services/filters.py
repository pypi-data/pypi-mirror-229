...
class ChainTreeFilter:
    ...
    def filter_conversations(
        ...
        self.conversation_trees = []
        for idx, conversation_tree in enumerate(conversation_trees):
            tree_depth = self._get_tree_depth(conversation_tree)
            if self.chain_filter.is_valid(
                idx, len(conversation_trees), conversation_tree, tree_depth
            ):
                self.conversation_trees.append(conversation_tree)
        return self.conversation_trees

    def _get_tree_depth(self, conversation_tree: ChainTreeIndex) -> int:
        if not conversation_tree.conversation.mapping:
            return 0
        return self._get_depth(conversation_tree.conversation.mapping)

    def _get_depth(self, mapping: Dict[str, ChainMap]) -> int:
        depths = []
        for node_id in mapping:
            if mapping[node_id].children:
                depths.append(self._get_depth(mapping[node_id].children))
            else:
                depths.append(0)
        return max(depths) + 1
