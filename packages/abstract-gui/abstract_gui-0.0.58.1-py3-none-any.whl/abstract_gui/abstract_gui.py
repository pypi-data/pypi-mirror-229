...
def get_gui_fun(function_name: str = '', args: dict = {}):
    ...
    if function_name not in functions:
        ...
        if name == None:
            name = get_closest_match_from_list(comp_str="frame",total_list=get_all_functions_for_instance(instance=instance))
    ...
    required_params = all_params["required"]
    for param in required_params:
        if param not in args.keys():
            ...
            if change is not None:
                print(f"{param} not in arguments, inputing {param} = {change}")
                args[param]=change
            else:
              print(f"{param} not in arguments")  
            
    if "layout" in all_params["names"]:
        args["layout"]=ensure_nested_list(args["layout"])
