...
from typing import List, Dict, Iterable, Any, Union, Type, Tuple, Optional
...
from .base import ElementBase, AsyncElementBase
from .datatable import DataTableClickHouse, DataTableMySQL
...
class Dimension(ElementBase):
    ...
    @cached_property
    def _datatable_class(
        self,
    ) -> Union[Type[DataTableMySQL], Type[DataTableClickHouse]]:
        if (server_name := self.meta.databaseServerName) is None:
            return DataTableMySQL

        if (match := self._RE_PARSE_SERVER.match(server_name)) is None:
            raise ValueError(f"{server_name} is not a datatable server.")

        db_type = match.group(1).lower()
        if db_type == 'mysql':
            return DataTableMySQL
        if db_type == 'clickhouse':
            return DataTableClickHouse

        raise TypeError(f"Unknown datatable type: {db_type}")
    ...
    @cached_property
    def members(self) -> List[DimensionMembersDto]:
        """当前维度的所有成员
        ...
        if self.api.version == self.api.default_version:
            ...
            resp = self.api.custom.select_dimension_member_list(dim_info)
            return parse_obj_as(List[DimensionMembersDto], resp['memberList'])

        dim_info = DimInfo.construct_from(
            self.element_info,
            name=self.element_name,
        )
        resp = self.api.extra.dimension_custom_select_dimension_member_list(dim_info)
        return parse_obj_as(List[DimensionMembersDto], resp['memberList'])

    @cached_property
    def meta(self) -> DimensionModel:
        """
        当前维度的元信息
        """
        if self.api.version == self.api.default_version:
            return self.api.query.open_dimension_info_by_id(self.element_info)

        return self.api.extra.dimension_query_open_dimension_info_by_id(self.element_info)

    def add_member(self, *members: DimMember, silent: bool = False) -> 'Dimension':
        """增加维度成员，
    ...
    def delete_member(self, *members: str, silent: bool = True) -> 'Dimension':
        """删除维度成员
    ...
    def reorder_members(self, memo: CIMultiDict[DimMember], complete: bool):
        """成员重排序
        ...
        Args:
            memo: 需要重拍的维度成员
            complete: memo里的成员是否可以组成完整维度树
        """
        # self._set_parent_id()  # 对于共享父节点情况的预处理
        ...
        leaf_member = all_mbr - parents

        for pname in all_mbr.intersection(parents):
            if pname in ordered_mbrs:
                continue

            for member in memo.getall(pname):
                parent_stack = [member]

                while (parent_name := member.parent_name) != ROOT and \
                        parent_name not in ordered_mbrs:
                    member = self._get_parent(member, memo)
                    if member is None:
                        break
                    parent_stack.append(member)

                for p in reversed(parent_stack):
                    ordered_mbrs.add(p.name, p)

        for name in leaf_member:
            for member in memo.getall(name):
                ordered_mbrs.add(name, member)
    ...
    def save(
        ...
        if len(dim_members) > 0:
            ...
            r = self._incr_save_members(payload)
            self._add_memo.clear()
            self._del_memo.clear()
            self._upd_memo.clear()
            return r

    def _incr_save_members(self, payload: DimensionMemberChangeSaveSw):
        if self.api.version == self.api.default_version:
            resp = self.api.save.incremental(payload)
            if resp.errors:
                raise DimensionSaveError(f"Failed to save dimension. Detail: {resp}")
        else:
            resp = self.api.compat.incremental(payload)
        return resp
    ...
    def query(
        ...
        role_name: str = None,
        role_group: str = None
    ) -> Union[List[DimensionMemberBean], List[Dict[str, Any]]]:
        """查询成员
        ...
        """
        payload = self._get_query_payload(expression, fields, role, role_name, role_group)
        if self.api.version == self.api.default_version:
            api_call = self.api.query.select_dimension_member_by_name_function
        else:
            api_call = self.api.extra.dimension_query_select_dimension_member_by_name_function
        if as_model:
            return api_call(payload)
        else:
            return api_call(payload, resp_model=List[dict])

    def _get_query_payload(self, expression, fields, role=None, role_name=None, role_group=None):
        role_params = {}

        if role is not None:
            if isinstance(role, str):
                role = RoleStrategy(element_name=role)

            rs_mapping = 1
            for dim in role.meta.dimensions:
                if dim.elementName == self.element_name and dim.folderId == self.element_info.folderId:
                    break
                rs_mapping = rs_mapping + 1
        ...
        expr = "%s{%s}" % (dim, body)
        payload = DimQueryExpr.construct_from(
    ...
    def load_dataframe(
        ...
        **langugage_keys: str
        ...
        # 合并描述列
        lan_columns = set(df.columns).intersection(ACCEPT_LANS)
        if lan_columns:
            df['multilingual'] = df[lan_columns].to_dict(orient='records')
        # pick up valid columns
        valid_columns = set(DimMember.__fields__).intersection(df.columns)

        # save replace
        if strategy is Strategy.full_replace:
            member_list = df[valid_columns].to_dict(orient='records')
            return self._save_replace(reorder, member_list)

        # split dataframe by existed data
        upd_df, add_df = self._split_dimension_df(df[valid_columns])
        ...
        with self._local_memo():
            ...
            return self.save(reorder=reorder)

    def _save_replace(self, reorder, member_list):
        if reorder:
            mbrs = CIMultiDict()
        ...
        if self.api.version == self.api.default_version:
            resp = self.api.member.save(payload)
            if not resp.success:
                raise DimensionSaveError(f"Failed to save dimension. Detail: {resp}")
        else:
            resp = self.api.compat.save(payload)
            if not resp.success:
                raise DimensionSaveError(f"Failed to save dimension. Detail: {resp}")
        return resp
    ...
    def _finalize_update(self):
        ...
        if not upd_memo:
            ...
            mbrs = self.query(";".join(upd_memo.keys()))
    ...
    def _split_dimension_df(
        ...
        if self._strict:
            ...
            name_col = getattr(self.table_dimension.table, DFLT_NAME_COLUMN)
            existed_df = self.table_dimension.select(
...
def _validate_df_for_dimension(df: pd.DataFrame):
    ...
    if SHAREDMEMBER in df.columns:
        ...
        dupliaceted = unique_df[unique_df['size'] > 1]
        if not dupliaceted.empty:
            raise ValueError(f"Duplicated member name for: {dupliaceted.to_dict(orient='record')}")
    else:
...
class AsyncDimension(AsyncElementBase, Dimension):
    async def query(
        self,
        expression: str,
        fields: Union[List[str], Tuple[str]] = None,
        ...
        payload = self._get_query_payload(expression, fields, role, role_name, role_group)
        if self.api.version == self.api.default_version:
            call_api = self.api.query.select_dimension_member_by_name_function
        else:
            call_api = self.api.extra.dimension_query_select_dimension_member_by_name_function
        if as_model:
            return await call_api(payload)
        else:
            return await call_api(payload, resp_model=List[dict])
