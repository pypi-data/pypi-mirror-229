...
def test_get_valid_credentials_using_env(monkeypatch: pytest.MonkeyPatch):
    monkeypatch.setattr(
        "glean.credentials.environ",
        {
            "GLEAN_PROJECT_ID": "test-project-from-env",
            "GLEAN_ACCESS_KEY_ID": "test-access-key-id-from-env",
            "GLEAN_SECRET_ACCESS_KEY_TOKEN": "test-access-key-token-from-env",
        },
    )

    credentials = get_credentials(
        get_fixture_path("fixtures/credentials_files/valid.json")
    )
...
def test_get_invalid_credentials_using_env(monkeypatch: pytest.MonkeyPatch):
    ...
    with pytest.raises(RuntimeError):
        get_credentials(get_fixture_path("fixtures/credentials_files/valid.json"))
