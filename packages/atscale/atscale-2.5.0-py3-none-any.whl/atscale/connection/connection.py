...
class _Connection:
    ...
    def _auth(self):
        if self._azure_oath_enabled():
            print(
                f"""Azure OAuth is enabled. Please log in at: {endpoints._endpoint_login_screen(self)}"""
            )
    ...
    # TODO do you see a user needing this John
    ...
    def _azure_oath_enabled(self) -> bool:
        """Internal function to check if Azure OAuth authentication is enabled for the current org
           so we account for it in the auth process

        Args:
            published_project_id (str): the id of the published project to unpublish

        Returns:
            bool: Whether Azure OAuth is enabled
        """
        response = self._submit_request(
