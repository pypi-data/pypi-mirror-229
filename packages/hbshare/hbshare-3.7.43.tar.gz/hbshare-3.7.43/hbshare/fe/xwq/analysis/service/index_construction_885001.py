...
class PAR:
    ...
    def get_result(self, fund_type):
        ...
        result = fund_share[['REPORT_DATE', 'PURCHASE_SHARES', 'REDEEM_SHARES', 'NET_SHARES', 'PURCHASE', 'REDEEM', 'NET', 'AUM']].groupby('REPORT_DATE').sum()
        ...
        result.columns = ['申购份额（份）', '赎回份额（份）', '净申购份额（份）', '申购金额（元）', '赎回金额（元）', '净申购金额（元）', '规模（元）']
