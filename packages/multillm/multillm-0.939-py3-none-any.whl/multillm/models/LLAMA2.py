...
class LLAMA2(BaseLLM):
    ...
    def get_content(self, response):
        ...
        try:
            resp = response["response"]["generated_text"]
    ...
    def get_response(self, prompt: Prompt):
        ...
        try:
            ...
            values = {'question':  prompt.get_string() + " , return response in markdown format"}
      
            resp = requests.post(url, data=json.dmps(values),headers=headers)
            data = resp.json()
