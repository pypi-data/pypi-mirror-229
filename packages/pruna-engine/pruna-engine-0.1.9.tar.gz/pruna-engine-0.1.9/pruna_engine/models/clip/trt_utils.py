...
def fix_fp16_network(network_definition: INetworkDefinition) -> INetworkDefinition:
    ...
    # LAYER_NORM_SEQUENCE = [trt.LayerType.REDUCE, trt.LayerType.ELEMENTWISE, trt.LayerType.CONSTANT,
    #                        trt.LayerType.SHUFFLE,
    #                        trt.LayerType.ELEMENTWISE, trt.LayerType.REDUCE, trt.LayerType.CONSTANT,
    #                        trt.LayerType.SHUFFLE,
    #                        trt.LayerType.ELEMENTWISE, trt.LayerType.UNARY, trt.LayerType.ELEMENTWISE,
    #                        trt.LayerType.CONSTANT,
    #                        trt.LayerType.SHUFFLE, trt.LayerType.ELEMENTWISE, trt.LayerType.CONSTANT,
    #                        trt.LayerType.SHUFFLE,
    #                        trt.LayerType.ELEMENTWISE]
    #
    # num_layers = network_definition.num_layers
    # for layer_index in range(num_layers - len(LAYER_NORM_SEQUENCE)):
    #     is_layer_norm = all(network_definition.get_layer(layer_index + j).type == LAYER_NORM_SEQUENCE[j]
    #                         for j in range(len(LAYER_NORM_SEQUENCE)))
    #     if is_layer_norm:
    #         # If the sequence matches, set all layers in the sequence to FP32
    #         for j in range(len(LAYER_NORM_SEQUENCE)):
    #             layer = network_definition.get_layer(layer_index + j)
    ...
    with trt.Builder(logger) as builder:  # type: Builder
        with builder.create_network(
            ...
            with trt.OnnxParser(
                ...
                # config.set_flag(trt.BuilderFlag.kDISABLE_EXTERNAL_TACTIC_SOURCES_FOR_CORE_0805, False)
                if int8:
