...
class LambdaExecutor:
    ...
    def __create_local_zarr_store(
        ...
        root.attrs["processing_software_version"] = "0.0.1"
    ...
    def execute(self, message):
        ...
        try:
            ...
            successful_file_names.extend(list(df.iloc[[ind for ind, i in enumerate(df['PIPELINE_STATUS']) if i.find('SUCCESS') >= 0]]['FILE_NAME']))
            for successful_file_name in successful_file_names:
                self.__update_processing_status(
                    file_name=successful_file_name,
                    cruise_name=cruise_name,
                    pipeline_status='PROCESSING_CREATE_EMPTY_ZARR_STORE'
                )
            print(df['CHANNELS'])
