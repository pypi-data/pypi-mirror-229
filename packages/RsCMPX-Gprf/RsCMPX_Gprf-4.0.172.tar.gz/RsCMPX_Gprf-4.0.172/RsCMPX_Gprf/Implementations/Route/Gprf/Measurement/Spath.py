...
class SpathCls:
	...
	def get_value(self) -> str:
		...
		Selects one or more signal input paths (RF connections) for the measured signal. The number of expected signal paths
		depends on the list mode settings of the power measurement. Configure them before sending this command.
			INTRO_CMD_HELP: The following situations must be distinguished: \n
			- List mode OFF: One path is expected.
			- List mode ON and connection source GLOBal: One path is expected. It is used for all list mode segments.
			- List mode ON and connection source INDex: The number of paths configured via [CONFigure:]GPRF:MEAS<i>:POWer:LIST:NIDX is expected. The order of the paths assigns them to an index (connection with index 1, index 2, index 3, ...) .
		...
		For possible signal path strings, see method RsCMPX_Gprf.Catalog.Gprf.Measurement.Spath.get_. \n
