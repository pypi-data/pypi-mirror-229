...
class SentCode(Object):
    ...
    Parameters:
        type (:obj:`~pov.enums.SentCodeType`):
            Type of the current sent code.

        phone_code_hash (``str``):
            Confirmation code identifier useful for the next authorization steps (either
            :meth:`~pov.Client.sign_in` or :meth:`~pov.Client.sign_up`).

        next_type (:obj:`~pov.enums.NextCodeType`, *optional*):
            Type of the next code to be sent with :meth:`~pov.Client.resend_code`.

        timeout (``int``, *optional*):
            Delay in seconds before calling :meth:`~pov.Client.resend_code`.
    """
