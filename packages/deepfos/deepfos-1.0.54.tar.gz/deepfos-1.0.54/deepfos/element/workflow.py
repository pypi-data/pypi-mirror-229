...
# -----------------------------------------------------------------------------
# core
class WorkFlow(ElementBase):
    """工作流"""
    api_class = WorkFlowAPI
    api: WorkFlowAPI

    @cached_property
    def all_version(self) -> List[ProcessVersionVO]:
        """所有版本信息"""
        return self.api.version_control.list(self.element_name, self.element_info.folderId)

    @cached_property
    def msg_list(self) -> List[Message]:
        """消息列表"""
        return self.api.message.list(self.element_info)

    @cached_property
    def global_params(self) -> List[Value]:
        """全局变量"""
        return self.api.instance.global_params(self.element_info)

    @cached_property
    def launch_params(self) -> List[ProcessLaunchParamsVO]:
        """启动参数"""
        return self.api.process.launch_params(self.element_name, self.element_info.folderId)
    ...
    def _send_msg(
        ...
        return self.api.process.send_message(
            SendMessageParam(
    ...
    def broadcast(self, msg_code: str, msg_body: Dict[str, Any]):
        ...
        return self._send_msg(msg_code, msg_body)
    ...
    def send_msg_to_processes(
        ...
        return self._send_msg(msg_code, msg_body, processes, ScopeType.process_instance)
    ...
    def launch_process(
        ...
        """
        return self.api.process.launch(
            ProcessLaunchDTO(
                paramMap=param,
                filePath=file_path,
                comment=comment,
    ...
    def batch_launch_process(
        ...
        return self.api.process.launch_batch(
    ...
    def get_process_by_param(
        ...
        """
        return self.api.process.process_instance_get_by_launch_params(
    ...
    def get_process_by_business_key(
        ...
        return self.api.process.process_instance_get_by_business_key(
            BusinessKeyQueryDTO(
                elementDetail=self.element_info,
    ...
    def get_task_by_param(
        ...
        """
        return self.api.process.task_instance_get_by_launch_params(
            LaunchParamsDTO(elementDetail=self.element_info, paramMap=param, version=version)
        )
    ...
    def get_task_by_business_key(
        ...
        return self.api.process.task_instance_get_by_business_key(
            BusinessKeyQueryDTO(
                elementDetail=self.element_info,
                businessKey=BusinessKey(compareType=CompareType[compare_type], key=business_key),
                version=version
    ...
    def complete_task_by_id(
        ...
        """
        return self.api.task.express_complete(
    ...
    def list_process(
        ...
        fn = partial(
            ...
            status=','.join(status),
            call_api=self.api.instance.monitor_page
        )
    ...
    def list_my_task(
        ...
        fn = partial(
            ...
            status=','.join(status),
            call_api=self.api.task.myTask_list_page
        )
    ...
    def list_claim_task(
        ...
        fn = partial(
            self._query_task_impl,
            status='',
            call_api=self.api.task.claim_list_page
        )
