...
class AsyncJournalModel(ElementBase[JournalModelAPI]):
    ...
    async def save(
        ...
        Args:
            ...
            relation_field: 用于指定凭证头、行表的关联字段，即通过该字段确定凭证头表对应的凭证行表数据，默认值是journal_id
        ...
        .. admonition:: 示例
            ...
            .. code-block:: python
                ...
                # 凭证头表数据（注：type的值必须对应模型配置的凭证类型代码，journal_id的值在下面的行表中必须有对应的数据）
                head_df = pd.DataFrame([
                    {
                        "type": "type_account_01", "journal_id": "head_main_id_202306080001","is_balance": "true",
                        "scenario": "Actual", "version": "Working","value": "CNY", "entity": "[TotalEntity].[A]",
                        "year": "2023","period": "12", "approve_time": "2023-05-23 15:56:00",
                        "convert_date": "2023-05-23"
                    },
                    {
                        "type": "type_account_01", "journal_id": "head_main_id_202306080002","is_balance": "true",
                        "scenario": "Actual","version": "Working","value": "CNY","entity": "[TotalEntity].[A]",
                        "year": "2023","period": "12", "approve_time": "2023-05-23 15:56:00",
                        "convert_date": "2023-05-23"
                    }
                ...
                line_df = pd.DataFrame([
                    {
                        "journal_id": "head_main_id_202306080001","line_no": "1","account": "100101","movement": "OPN",
                        "trx_debit": "130","debit": "130","comment_line": "line1"
                    },
                    {
                        "journal_id": "head_main_id_202306080001","line_no": "2","account": "100101","movement": "OPN",
                        "trx_credit": "130","credit": "130","comment_line": "line2"
                    },
                    {
                        "journal_id": "head_main_id_202306080002","line_no": "1","account": "100101","movement": "OPN",
                        "trx_debit": "130","debit": "130","comment_line": "line1"
                    },
                    {
                        "journal_id": "head_main_id_202306080002","line_no": "2", "account": "100101","movement": "OPN",
                        "trx_credit": "130","credit": "130","comment_line": "line2"
                    }
                ])
    ...
    """
        ...
        table_type_pattern = "|".join(
            ["(" + t.typeCode.translate(_escape_table) + ")" for t in self.config.journalModelType if t.typeCode]
        )
        errors = set()
        ...
        head_required_fields = [relation_field, 'type']
        ...
        if any(dup_ser := head_df[relation_field].duplicated()):
            raise ValueError(
                f'凭证头表数据关联字段({relation_field})存在重复的值:\n{set(head_df[relation_field][dup_ser])}'
            )

        if any(unknown_type := (
            head_df['type'].astype('str', errors='ignore').str.match(table_type_pattern) == False
        )):
            raise ValueError(
                f'凭证头表数据中凭证类型:\n{set(head_df["type"][unknown_type])}\n在凭证模型中不存在'
            )

        head_data = head_df[["type", relation_field]]
        head_data = head_data.rename(columns={"type": "journalTypeCode", relation_field: "headMainId"})
        head_data = head_data.assign(operateType="ADD")
        head_df = head_df.replace({np.nan: None})
        head_data["data"] = pd.Series(head_df.drop(columns=['type']).to_dict(orient='records'))
        main_id_pattern = "|".join(["(" + str(mid).translate(_escape_table) + ")" for mid in head_data['headMainId']])

        if any(unrelated_value := (
            line_df[relation_field].astype('str', errors='ignore').str.match(main_id_pattern) == False
            )
        ):
            errors.add(
        ...
        line_data["data"] = pd.Series(line_df.to_dict(orient='records'))
    ...
    async def check(self, where: Union[str, Term, EmptyCriterion]) -> JmPostResultVO:
        """凭证数据校验
    ...
    async def delete(self, where: Union[str, Term, EmptyCriterion]) -> CommonResultDTO:
        ...
        if not res.success:
            raise Exception(','.join([x.msg for x in res.errors]))
        return res
