...
class AsyncBusinessModel(ElementBase):
    """业务模型"""
    ...
    def set_approval_ex(
        ...
        col, where_sql, on_sql = self._create_sql(main_primary_kv)
        ...
        FROM
        {tbl_pc.table_name} a
        INNER JOIN ( SELECT `{col}`, max(line_no) AS line_no FROM {tbl_pc.table_name} 
        WHERE {where_sql} GROUP BY `{col}` ) b 
        ON {on_sql} AND a.line_no = b.line_no;
        """
        from deepfos.db.mysql import MySQLClient
        df_query = MySQLClient().query_dfs(sql)
        if origin_status is None:
            # 初始化的处理
            if df_query.empty:
        ...
        if not df_success.empty:
            ...
            main_tbl_folder_id = partition['main_tbl_folder_id']
            ...
            from .datatable import DataTableMySQL
            tbl_main = DataTableMySQL(main_tbl_name, folder_id=main_tbl_folder_id)
            if not df_failure.empty:
                _, where_sql, _ = self._create_sql(df_success[key_cols])
            sql_update = f"update {tbl_main.table_name} set `{pc_field}`={row['target_status']!r} where {where_sql}"
            with DataTableMySQL.start_transaction():
                tbl_pc.insert_df(df_insert)
                txn_support(DataTableMySQL.run_sql)(tbl_main, sql_update)
    ...
    def _create_sql(df):
        ...
        if df.shape[1] == 1:
            col = df.columns[0]
            in_val = ",".join(df[col])
            where_sql = f"`{col}` IN ({in_val})"
            on_sql = f"a.`{col}`=b.`{col}`"
        else:
            col = "`,`".join(df.columns)
            df = df.apply(lambda x: '`' + x.name + '`=' + x, axis=0)
            df.iloc[:, :-1] += ' AND '  # 除了最后一列，每列尾加AND
            data_series = "(" + df.sum(axis=1) + ")"
            where_sql = " | ".join(data_series)
            on_sql = " AND ".join([f"a.`{col}`=b.`{col}`" for col in df.columns])
