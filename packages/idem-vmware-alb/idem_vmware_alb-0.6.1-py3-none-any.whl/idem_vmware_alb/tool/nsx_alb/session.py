    ...
    async with aiohttp.ClientSession(
        ...
        if "tenant_ref" in data:
            if "name" in data["tenant_ref"]:
                ...
                tenant_uuid = data["tenant_ref"].split("/")[-1]
                tenant_name = await hub.tool.nsx_alb.session.request(
                    ctx,
                    method="get",
                    path="/tenant/{uuid}/".format(**{"uuid": tenant_uuid}),
                    data={},
                )
                tenant = tenant_name["ret"]["name"]
