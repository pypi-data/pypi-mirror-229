...
# Update requiements.in with pinned versions for configured packages
pin_requirements() {
    for package in $pinned_packages; do
        force_latest_package $package requirements.in
    done
}
...
force_latest_package() {
  local package=${1}
  local req_file=${2}
  latest_version=$(pip install --use-deprecated=legacy-resolver ${package}== 2>&1 | grep -oE '(\(.*\))' | awk -F: '{print$NF}' | sed -E 's/( |\))//g' | tr ',' '\n' | tail -n 1)
  if [ "${latest_version}" == "none" ]; then
    echo "ERROR: Could not find latest version of ${package}"
    exit 1
  fi
  # Delete any references to the package in the requirements file
  sed -i "/${package}==/d" "${req_file}"
  # Add the latest version
  echo "${package}==${latest_version} # Added by automation - please leave" >> "${req_file}"
  echo "Added ${package}==${latest_version} to ${req_file}"
}
