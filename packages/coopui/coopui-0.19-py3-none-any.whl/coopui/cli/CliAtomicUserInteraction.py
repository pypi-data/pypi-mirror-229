...
import logging
from coopui.cli.fileContentReturn import FileContentReturn
...
class CliAtomicUserInteraction(IAtomicUserInteraction):
    ...
    def request_index_from_df(cls,
        ...
        while True:
            ...
            cls.pretty_print_dataframe(print_out, prompt, prevent_newline_bool=True if additional_options else False, **kwargs)
        ...
        while True:
            try:
                ...
                if len_limit is not None and len(converted) > len_limit:
                    raise ValueError(f"Too many values provided. No more than {len_limit} expected but {len(converted)} entered")
...
if __name__ == "__main__":
    get_float = CliAtomicUserInteraction.request_float_tuple(f'Get a cool float tuple', min=0, max=100, len_limit=2)
    print(get_float)