...
class RecentSimilarPattern:
    ...
    def __init__(self, df, subsequence):
        ...
        self.subsequence = subsequence
        self.m = subsequence * df.columns.size
        self.df_stack=df.stack().reset_index(drop=True)
    ...
    def matrix_profile(self):
        ...
        condition1a = (matrix_profile_["index0"] % 4 == 0)
        # condition1b = (matrix_profile_["index0"] % 4 == 3)
        condition2a = (matrix_profile_["index1"] % 4 == 0)
    ...
    def plot_candles(self,n_plots:int=6, ahead:int=7):
        ...
        """
        assert "precio_apertura" in self.df.columns, "precio_apertura must be in the dataframe"
        assert "precio_maximo" in self.df.columns, "precio_maximo must be in the dataframe"
        assert "precio_minimo" in self.df.columns, "precio_minimo must be in the dataframe"
        assert "precio_acordado" in self.df.columns, "precio_acordado must be in the dataframe"
