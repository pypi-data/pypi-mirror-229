...
class DaoStudent:
    ...
    @Sql("select name, address from student where student_id=?;")  # 对方法使用Sql装饰器，并传入sql语句即可
    def get_student(self, student_id):  # 参数顺序与sql语句中?的传值顺序一致
        pass  # 方法本身无需实现
...
class DaoScore:
    ...
    def get(self, student_id):
        pass  # 方法本身无需实现，需要返回值的时候会自动返回（以列表嵌套具名元组形式）


# ====================================================================================================================
# 数据库类的定义形式
@Path("db_folder/test.db")  # 使用Path装饰器传递数据库的路径
...
if __name__ == '__main__':
    ...
    db.dao_student.insert(Student(name="hjh", number=1, address="www"))  # 插入一条student记录
    db.dao_score.insert(Score(score=99, student_id=1))  # 插入一条score记录
    db.commit()  # 提交改动

    result1 = db.dao_student.get_student(student_id=1)  # 获取一条student记录
    result2 = db.dao_student.get_student(student_id=2)
