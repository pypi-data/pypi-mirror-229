...
class Subtract(Step):
    ...
    Current limitations:
        - Filters can be applied to source query, but not on target query.
        - .subtract() doesn't work after .union() since it's logic relies on
          fetching "starting" table in query and union creates compound select from
          which we cannot know that. It would work if union would dump results
          in a temp table and select from it as a result
    ...
    def apply(self, query_generator):
        ...
        if self.source_uri and self.dq.source_uri:
            raise Exception("Cannot subtract two queries that were created from index")

        source_uri = self.source_uri or self.dq.source_uri

        # creating temp table that will hold subtract results
        temp_table_name = "subtract_" + _random_string(6)
        custom_columns = [
            sqlalchemy.Column(col.name, col.type)
            for col in source_query.columns
            if col.name not in DATASET_CORE_COLUMN_NAMES
        ]
        subtract_table = self.catalog.data_storage.create_dataset_rows_table(
            temp_table_name,
            custom_columns=custom_columns,
            if_not_exists=False,
        )
        # query that selects all rows from source query that are not in a target
        # query (added diff)
        subtract_q = self.catalog.data_storage.subtract_query(
            source_query, target_query, source_uri
        )

        insert_q = subtract_table.insert().from_select(
            source_query.selected_columns, subtract_q
        )

        self.catalog.data_storage.execute(insert_q)

        def q(*columns):
            return sqlalchemy.select(*columns).select_from(subtract_table)

        return step_result(q, subtract_table.c, temp_table_names=[subtract_table.name])
...
class DatasetQuery(SQLQuery):
    ...
    def source_uri(self) -> Optional[str]:
        if self.started_from_index():
            (
                source_uri,
                _,
            ) = self.starting_step.parse_path()  # type: ignore [attr-defined]
            return source_uri
        return None

    def subtract(self, dq: "DatasetQuery"):
        query = self.clone()
        # taking source_uri if indexing step, since it's needed for calculating
        # tables diff
        query.steps.append(Subtract(dq, self.catalog, self.source_uri))
        return query
