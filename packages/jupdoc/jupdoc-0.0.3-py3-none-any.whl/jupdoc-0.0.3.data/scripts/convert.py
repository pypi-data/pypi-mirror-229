...
def write_notebook(args, notebook, filename, path='./docs', format='docx', folder_id=None):
    ...
    if args.upload and args.format == 'docx' and folder_id is not None:
        ...
        logger.warning("File not uploaded to Google Drive. Please check if the file format is supported or if the folder ID is correct.")
...
def parse():
    ...
    if args.config is not None:
        ...
        parser.add_argument('--creds_path', type=str, default='./creds.json', help='path to the Google Drive credentials file')
        args = parser.parse_args()
