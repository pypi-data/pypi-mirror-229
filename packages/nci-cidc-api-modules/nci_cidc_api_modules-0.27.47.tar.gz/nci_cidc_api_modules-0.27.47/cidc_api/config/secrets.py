...
def get_secrets_manager(is_testing=False):
    ...
    if environ.get("DEV_GOOGLE_STORAGE", None):
        # if is_testing:
        client_options = ClientOptions(api_endpoint=environ.get("DEV_GOOGLE_STORAGE"))
        return CloudStorageSecretManager(secrets_bucket, client_options)
...
class CloudStorageSecretManager:
    ...
    def __init__(self, bucket_name, client_options=None, credentials=None):
        """
