...
class PageCacheMonitor(object):
    ...
    def _create_new_file(self):
        ...
        filename = str(int(time()))  # Current TimeStamp
    ...
    def _delete_files(self, existing_files, index_to_start_deletion):
        ...
        """
        logger.debug("Total files {} in list, deleting files starting from list index {} until the end of the list ".format(len(existing_files), index_to_start_deletion))
    ...
    def _get_first_expired_file(self, existing_files, now):
        ...
        for idx, file in enumerate(existing_files):
            if int(file) < now - self.max_time_window_seconds:
                logger.debug("First expired file in list: {}, list index location: {}".format(file, idx))
                return (idx, file)
        # All files are within self.max_time_window_seconds or there are not files, return a negative index
        logger.debug("Can't get first expired file ocurrence in the list, either all files are within the max_time_window_seconds or empty list")
        return (-1, None)
        ...
        for idx, file in enumerate(existing_files):
            with open("{}/{}".format(self.tmp_directory, file), "r") as fd:
                ...
                if (
                    ...
                    logger.debug("First not cached file in list: {}, list index location: {}".format(file, idx))
                    return (idx, int(file))
        # All files are cached or there are not files, return a negative index
        logger.debug("Can't get first not cached file ocurrence in the list, either all files are cached or empty list")
        return (-1, None)
    ...
    def _get_index_to_start_deletion(self, existing_files, now):
        ...
        if idx_first_expired_file != -1 and idx_first_not_cached_file != -1:
            ...
            logger.debug("Found in file list both expired and uncached files to delete, chosing the first ocurrence in the list to start deletion which is the index: {}".format(idx_to_start_deletion))
        ...
        # If only one condition of deletion is met we return that index
            ...
            if idx_first_expired_file == -1:
                ...
                logger.debug("Found in file list of uncached files for deletion, getting the index {} of the list to start deletion ".format(idx_to_start_deletion))
            else:
                idx_to_start_deletion = int(idx_first_expired_file)
                logger.debug("Found in file list of expired files for deletion, getting the index {} of the list to start deletion ".format(idx_to_start_deletion))
    ...
    def _deliver_metrics_to_dogstatsd(self, min_cached_time):
        ...
        logger.debug("Delivered metric to DogStatsD: {}:{}".format(self.dogstatsd_prefix, min_cached_time))
    ...
    def _report_metric(self, min_cached_time):
        ...
        logger.info("Current min time page is cached: {} seconds".format(min_cached_time))
