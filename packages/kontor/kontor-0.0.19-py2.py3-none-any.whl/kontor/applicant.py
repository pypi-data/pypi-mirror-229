...
class BureauApplicant:
    def __init__(
        ...
        logging.info(f"Initializing the bureau applicant of version {kontor_version}.")
    ...
    def __connect(self, server_ip_address: str, server_port: int):
        ...
        self.__address = ()
        response_json_data = wait_and_receive_response(
    ...
    def __disconnect(self):
        logging.info(f"{self.__address}: Closing the connection.")
        self.__connection.shutdown(socket.SHUT_RDWR)
        self.__connection.close()

    def __authenticate(self, username: str, password: str):
        logging.info(f"{self.__address}: Authenticating {username} user.")
        ...
        if auth_response.is_authenticated:
            logging.info(f"{self.__address}: Authentication succeed.")
        else:
            logging.error(
                f"{self.__address}: Authentication failed with error {auth_response.message}."
            )
    ...
    def __send_file_for_processing(
        ...
        if procedure_response.is_ready_for_procedure:
            logging.info(f"{self.__address}: Procedure was approved.")
        else:
            logging.error(
        ...
        if file_receiving_receipt.is_received_correctly:
            logging.info(f"{self.__address}: File was received correctly.")
        else:
            logging.error(
    ...
    def __wait_and_receive_result_file(self, file_path: str, overwrite_file: bool):
        ...
        request_json_data = wait_and_receive_response(self.__connection, self.__address)
        ...
        if procedure_receipt.is_processed_correctly:
            logging.info(
                f"{self.__address}: Procedure succeed, processed file is coming."
            )
        else:
            logging.error(
        ...
        with open(corrected_file_path, "rb") as processed_file:
            ...
            logging.info(f"{self.__address}: Received file CRC32: {data_crc32_str}.")
        ...
        logging.info(
            f"{self.__address}: Processed file was received successfully, disconnecting."
        )
        self.__disconnect()
    ...
    def process_files(
        ...
        logging.info(
            f"File list was provided, zipping files into archive: {zip_file_name}."
        )
