    ...
    for filepath in test_ntuple_path_list:
        ...
        with uproot.open(filepath) as f:
            for key, obj in f.items(cycle=False):
                ...
                result.success(
                    f"Tree {key} has {result.tree_data[filepath][key]['events']} events. "
                    f"Multiple candidates: mean = {result.tree_data[filepath][key]['mean(nCand)']}, "
                    f"max = {result.tree_data[filepath][key]['max(nCand)']}"
                )
        ...
        if len(eventNumbers_all):
            result.events_notempty[filepath] = int(eventNumbers_all.shape[0])

    if len(trees_opened) == 0:
        result.warning("No TTree objects found")
