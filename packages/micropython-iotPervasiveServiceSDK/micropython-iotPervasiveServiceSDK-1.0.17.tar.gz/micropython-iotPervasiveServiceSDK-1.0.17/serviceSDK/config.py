...
def initTopic(deviceId: str):
    ...
    INVOCATION_SERVICE_EXECUTE_TOPIC = '/client/invocation/%s/+/+' % (
        DEVICE_ID)
    global INVOCATION_SERVICE_CALLBACK_TOPIC
    INVOCATION_SERVICE_CALLBACK_TOPIC = '/server/invocation/callback/%s/${service}/${capability}/${msgid}' % (
        DEVICE_ID)
...
def initPath(basePath: str, servicePath: str, proxyPath: str):
    ...
    global SERVICE_PATH
    SERVICE_PATH = pathUtils.joinPath(basePath, servicePath)
    global PROXY_PATH
    PROXY_PATH = pathUtils.joinPath(basePath, proxyPath)
