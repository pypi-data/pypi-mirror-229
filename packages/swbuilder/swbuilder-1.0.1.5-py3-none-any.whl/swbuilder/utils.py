...
def get_build_targets(args, defs):
    ...
    elif us.check_attribute(defs.env, "name", None):
        targets = [defs.env["name"]]
    else:
...
def build_targets(args, defs, targets):
    ...
    if args.build_arg:
        ...
        buildargs = defs.env.pop("build_args", None)

    if (buildargs):
        buildargs = _make_buildargs(buildargs)
    ...
    repository = args.tag if (args.repository is not None) else us.check_attribute(defs.env, "repository", None)
    tag = args.tag if (args.tag is not None) else us.check_attribute(defs.env, "tag", "latest")

    for t in targets:
        try:
            builder = defs.generate_build(
                t,
                generate_name(t, repository, tag),
...
def _make_environments(environments):
    if (environments is None):
        return None
    environments_str = ""
    for environment in environments:
        environments_str = environments_str + "ENV " + environment + "\n"
    return environments_str
