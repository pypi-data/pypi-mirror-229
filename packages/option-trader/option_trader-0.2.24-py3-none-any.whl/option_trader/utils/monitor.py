...
def refresh_asset_basic_info(i, df, data):        
    ...
    df.at[i, '10 days max'] = round(price_history['High'].rolling(window=10).max().shift(1)[-1],2)
    df.at[i, '10 days min'] = round(price_history['Low'].rolling(window=10).min().shift(1)[-1],2)   
