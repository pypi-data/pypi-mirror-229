...
class LDA_tomopyTrainer:
    def __init__(self, dataset_handler, num_topics=50, alpha=0.1, eta=0.01, max_iters=50):
        self.model = tp.LDAModel(k=num_topics, alpha=alpha, eta=0.01)
        self.max_iters = max_iters
        self.dataset_handler = dataset_handler

    def train(self):
        for text in self.dataset_handler.train_texts:
            self.model.add_doc(text.split())
        self.model.train(self.max_iters)

    def export_beta(self):
        beta = np.stack([self.model.get_topic_word_dist(k) for k in range(self.model.k)])

        

        additional_words = [item for item in dataset.get_vocabulary()
    ...
    def export_theta(self):
        train_theta = np.stack([doc.get_topic_dist() for doc in self.model.docs])

        converted_test_docs = [self.model.make_doc(text) for text in self.dataset_handler.test_texts]
        test_theta, _ = self.model.infer(converted_test_docs)
        test_theta = np.asarray(test_theta).T
...
class LDA_tomopy(AbstractModel):
    ...
    def train_model(self, dataset, hyperparameters=None, top_words=10):
        ...
        lda = tp.LDAModel(
            k=self.hyperparameters['num_topics'],
        ...
        topic_word_matrix = np.stack(
            [lda.get_topic_word_dist(k, normalize=True)
             for k in range(lda.k)])  # topic word distribution matrix
        topic_document_matrix = np.stack(
            [doc.get_topic_dist()
             for doc in lda.docs])  # topic document distribution matrix
