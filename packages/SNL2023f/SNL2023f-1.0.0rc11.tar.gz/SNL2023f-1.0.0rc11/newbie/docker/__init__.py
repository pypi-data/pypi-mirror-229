...
def _create(container_name: str) -> str:
    ...
    command = f"docker run -d --name newbie_{container_name} -e NEWBIE_AUTHORIZED_KEYS=\"$NEWBIE_AUTHORIZED_KEYS\" -e NEWBIE_USERNAME={container_name} --hostname=newbie tklco/sparcs-newbie-2023f-linux"
    os.system(command)
