...
def deleteService(id:str)->bool:
    ...
    if (service == None):
        return True
    # 服务路径拼接    
    servicePath = pathUtils.joinPath(config.BASE_PATH,service.filePath)
    # 删除服务    
...
def downloadService(id:str):
    ...
    if existingService!=None:
        ...
        servicePath = pathUtils.joinPath("/",servicePath)
    else:
        servicePath = pathUtils.joinPath(config.SERVICE_PATH,"service"+''.join([('0'+hex(ord(os.urandom(1)))[2:])[-2:] for x in range(8)]))
    print("servicePath"+servicePath)
    serviceTargzPath = pathUtils.joinPath(servicePath,SERVICE_TARGZ_FILE_NAME)
    # 下载压缩包    
...
def downloadServiceFile(filePath:str,servicePath:str):
    ...
    try:
        ...
        sys.print_exception(e)
        raise Exception('There was a exception downloading ,url:'+downloadUrl+",filePath:"+servicePath)
