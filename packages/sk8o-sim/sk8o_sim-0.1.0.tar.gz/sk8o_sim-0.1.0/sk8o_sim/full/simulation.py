...
class FullSimulation:
    ...
    def __init__(self, cfg: FullSimulationCfg):
        ...
        # initial conditions
        self.reference_low = (
            cfg.initial_conditions_cfg.forward_velocity_range[0],
            cfg.initial_conditions_cfg.angular_velocity_range[0],
            cfg.initial_conditions_cfg.height_reference_range[0],
        )
        self.reference_high = (
            cfg.initial_conditions_cfg.forward_velocity_range[1],
            cfg.initial_conditions_cfg.angular_velocity_range[1],
            cfg.initial_conditions_cfg.height_reference_range[1],
        )
        self.body_orientation = cfg.initial_conditions_cfg.body_orientation
    ...
    def new_reference(
        ...
        reference : Tuple[float, float, float] | np.ndarray | None, default None.
            The reference (dot_x_ref, dot_psi_ref, hip_angle_ref). If None, a reference will be randomly generated based on the configuration.
        """
        if reference is None:
            reference = np.random.uniform(
                low=self.reference_low, high=self.reference_high
            )
