...
from deepfos.element.base import AsyncElementBase
...
class DimElement(AsyncElementBase):
    api_class = DimensionAPI
    api: DimensionAPI


class SmlElement(AsyncElementBase):
...
class BoundField(BaseField):
    ...
    folder_id: str = None
    path: str = None
    bound_class: Type[AsyncElementBase] = None
...
class SmartList(BoundField):
    code = 3
    bound_class: Type[AsyncElementBase] = SmlElement
...
class Dimension(BoundField):
    code = 8
    bound_class: Type[AsyncElementBase] = DimElement
