...
def compare_identical_structure_data_from_files(
    ...
    df_orig = pd.read_csv(os.path.join(folder_abs_path, structure_name + files_prefix + '.orig.' + files_suffix + '.csv'), sep=";" ,encoding='ISO-8859-1')
    df_new = pd.read_csv(os.path.join(folder_abs_path, structure_name + files_prefix + '.new.' + files_suffix + '.csv'), sep=";", encoding='ISO-8859-1')

    structure = str_catalog.get_structure_by_name(structure_name)
    compare = PandasDfSameStructureCompare(df1=df_orig, df2=df_new, df1_name='Original', df2_name='New', comparison_name='Comparison of ' + structure_name 
