...
class NpPPOReplayBuffer(PPOReplayBuffer):
    def __init__(
        ...
        *,
        ...
        self.actions_log_prob_buffer = np.zeros(
            (self.steps_per_env, self.num_envs), dtype=np.float32
        )
    ...
    def tensor_data(self, device: torch.device):
        ...
        return (
            ...
            torch.as_tensor(
                self.advantage_buffer.flatten(), dtype=torch.float32, device=device
            ),
            torch.as_tensor(
                self.return_buffer.flatten(), dtype=torch.float32, device=device
            ),
            torch.as_tensor(
                self.value_buffer.flatten(), dtype=torch.float32, device=device
            ),
            torch.as_tensor(
