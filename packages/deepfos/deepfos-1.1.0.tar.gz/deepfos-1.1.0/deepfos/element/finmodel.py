...
from typing import List, Dict, Optional, Union, Tuple, Iterable, TYPE_CHECKING, Any
import pandas as pd
...
from .datatable import (
    ...
    AsyncDataTableMySQL, AsyncDataTableClickHouse
)
...
from deepfos.api.models.financial_model import (
    ...
    TaskExecutionParam, ParameterDefineDto
)
...
class AsyncFinancialCube(ElementBase):
    """财务模型
    ...
    @future_property
    async def _meta(self) -> CubeModel:
        """财务Cube的元数据信息"""
        api = await self.wait_for('async_api')
        ele_info = await self.wait_for('element_info')
    ...
    def dim_col_map(self) -> MultiDict[str, str]:
        ...
        meta = self._meta
        for dim in meta.cubeDimensionList:
    ...
    @cached_property
    def fact_table(self) -> Union[AsyncDataTableMySQL, AsyncDataTableClickHouse]:
        """事实表对应的数据表"""
    ...
    async def save(
        ...
        return await self._save_impl(data[required_cols], pov, need_check, data_audit, chunksize)
    ...
    async def insert_null(
        ...
        """
        if not isinstance(self.fact_table, (AsyncDataTableClickHouse, DataTableClickHouse)):
            return await self.delete(expression)

        if isinstance(expression, str):
            expression = expr_to_dict(expression)

        if self.meta.cube.autoCalculation:
            expression = {**expression}
            expression.pop(VIEW, None)
