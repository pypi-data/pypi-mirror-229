...
@app.callback()
def callback(
    task_check_interval: float = typer.Option(
        default=2.0,
        min=0,
        help="Task completion check interval (set to 0 to disable waiting on tasks)",
        metavar="SECONDS",
    ),
) -> None:
    """Cyberfusion Cluster API CLI."""
    state["task_check_interval"] = task_check_interval
...
def tasks(
    ...
    wait: bool = typer.Option(default=False, help="Wait for task to complete"),
    detailed: bool = typer.Option(default=False, help=DETAILED_MESSAGE),
) -> None:
    """Show Task Collection Results

    By default it will show the current status and quit.

    If --wait is given, it will wait until the task is completed.

    When global option --task-check-interval 0 is given together with --wait, task-check-interval is set back to 2
    """
    console.print(
        get_support().get_table(
            objs=get_support().task_collection_results(
    ...
    if wait:
        # Wait even when task_check_interval is 0
        ...
        wait_for_task(
            task_collection_uuid=task_collection_uuid, detailed=detailed
        )
...
def setup(
    ...
    BASE_DIRECTORY = os.path.dirname(PATH_CONFIG_LOCAL)

    exists = os.path.exists(PATH_CONFIG_LOCAL)

    # Create directory which contains config file

    if not os.path.exists(BASE_DIRECTORY):
        os.mkdir(BASE_DIRECTORY)
        os.chmod(BASE_DIRECTORY, 0o700)
    ...
    config["clusterapi"]["apikey"] = api_user_password
