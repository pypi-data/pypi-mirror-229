...
class ToriiService(Service):
    ...
    def action(self, action=None, obj=None, params=None,
        ...
        if not isinstance(obj, self._object_class) and isinstance(obj, collections.Iterable):
    ...
    def new(self, create=True, **kwargs):
        ...
        try:
            ...
            raise ToriiException('Failed to instanciate a shell for {0}\n{1}'.format(self._object_class, e)).with_traceback(sys.exc_info()[2])
    ...
    def list(self, dir='ASC', sort=None, limit=100, start=0, timeout=60,
             filters=None, strict_filter=False, include=None, no_paging=False, count=False, raw=False, cast=True, **kwargs):
        """
    ...
    def count(self, filters=None, strict_filter=False, **kwargs):
        ...
        :param filters: the filters to apply
        :param strict_filter: filter the strings with 'eq' instead of 'like'
        :param raw_listing: use raw listing (no paging, no filtering)
        :param get_count: return the total count of elements along with the page
        :param kwargs: quick filters
        """
