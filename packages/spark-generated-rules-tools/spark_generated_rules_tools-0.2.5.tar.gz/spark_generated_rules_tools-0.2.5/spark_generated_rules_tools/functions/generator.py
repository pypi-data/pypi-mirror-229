...
def dq_generated_dataframe_json(hamu_type=None,
    ...
    with open(dir_hocons_mvp_filename, "w") as f:
        f.write(json_file)
    print(f"{get_color('HOCON JSON CREATE:')} {get_color_b(dir_hocons_mvp_filename)}")
...
def dq_generated_dataframe_conf(namespace=None,
    ...
    with open(dir_hocons_mvp_filename, "w") as f:
        f.write(txt_conf)
    print(f"{get_color('HOCON CONF CREATE:')} {get_color_b(dir_hocons_mvp_filename)}")
...
def dq_generated_mvp(table_master_name=None,
    ...
    url_raw = "http://artifactory-gdt.central-02.nextgen.igrupobbva/" \
              ...
              f"{uuaa_tag_table_raw}.input.schema"
    ...
    directory_dq_staging_conf = os.path.join(dir_hocons_mvp_name, "dq", uuaa_name_master, uuaa_tag_table_master, "staging", f"{uuaa_tag_table_master}-01.conf")
    directory_dq_raw_conf = os.path.join(dir_hocons_mvp_name, "dq", uuaa_name_master, uuaa_tag_table_master, "raw", f"{uuaa_tag_table_master}-01.conf")
    directory_dq_master_conf = os.path.join(dir_hocons_mvp_name, "dq", uuaa_name_master, uuaa_tag_table_master, "master", f"{uuaa_tag_table_master}-01.conf")

    directory_dq_staging_json = os.path.join(dir_hocons_mvp_name, "dq", uuaa_name_master, uuaa_tag_table_master, "staging", f"{uuaa_tag_table_master}-01.json")
    directory_dq_raw_json = os.path.join(dir_hocons_mvp_name, "dq", uuaa_name_master, uuaa_tag_table_master, "raw", f"{uuaa_tag_table_master}-01.json")
    directory_dq_master_json = os.path.join(dir_hocons_mvp_name, "dq", uuaa_name_master, uuaa_tag_table_master, "master", f"{uuaa_tag_table_master}-01.json")
    ...
    for index, rule_id in enumerate(rule_ids_raw):
        ...
        hamu_dict["config"]["dataValues"]["schema"]["path"] = "${ARTIFACTORY_UNIQUE_CACHE}/artifactory/${SCHEMAS_REPOSITORY}" \
                                                              ...
                                                              f"/raw/{uuaa_tag_table_raw}/latest/{uuaa_tag_table_raw}.input.schema"
        if physical_target_name_extension == "dat":
