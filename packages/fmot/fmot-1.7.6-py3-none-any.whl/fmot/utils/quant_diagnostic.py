...
def collate_saved_activations(act_history):
    ...
    for k, v in act_history.items():
        ...
        if T > 0:
            N = len(v[0])
            v_stacked = [torch.stack([v[t][n] for t in range(T)], dim=1)
                        for n in range(N)]
        else:
            v_stacked = []
...
IGNORED_LAYERS = [
    fmot.qat.nn.ObserverBase, fmot.qat.nn.Quantizer
]
...
def wrap_model_to_save_activations(model):
    ...
    for module in model.modules():
        if isinstance(module, fmot.qat.nn.AtomicModule):
            handles.append(
                module.register_forward_hook(_save_inputs_and_outputs_hook))
