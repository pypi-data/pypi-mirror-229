...
class GDPS:
	...
	async def __makeRequest(self, page, params={}, rtype='get'):
		global response
		response = ''
		async def main():
	...
	def __parse_search_user(self, raw: str):
		args = raw.split(':')
		return {'username': args[1],
				'uid': int(args[3]),
				'coins': int(args[5]),
				'userCoins': int(args[7]),
				'icon': int(args[9]),
				'color': int(args[11]),
				'secondColor': int(args[13]),
				'iconType': int(args[15]),
				'special': int(args[17]),
				'stars': int(args[21]),
				'creatorPoints': int(args[23]),
				'demons': int(args[25])
				}
	def __parse_gauntlet(self, raw: str):
		args = raw.split('|')
		print(args)
		response = []
		for arg in args:
			arg = arg.split(':')
			response.append({'gauntletID': arg[1], 'ids': arg[3].split(',')})
		return response
	async def getDaily(self):
		raw_response = await self.__makeRequest('/getGJDailyLevel.php')
		return {'id': int(raw_response.split('|')[0]), 'remaining_time': int(raw_response.split('|')[1])}
	async def getLevel(self, levelID: int):
		raw_response = await self.__makeRequest('/downloadGJLevel21.php', params={'levelID': levelID, 'secret': "Wmdf2893gb7"}, rtype='post')
		parsed_response = self.__parse_level(raw_response)
		return parsed_response
	async def getLevels(self):
		return await self.__makeRequest('/getGJLevels21.php')
	async def getSong(self, songID: int):
		raw_response = await self.__makeRequest('/getGJSongInfo.php', params={'songID': songID, 'secret': 'Wmfd2893gb7'}, rtype='post')
		return self.__parse_song(raw_response)
	async def getUser(self, userID: int):
		raw_response = await self.__makeRequest('/getGJUserInfo20.php', params={'secret': 'Wmfd2893gb7', 'targetAccountID': userID}, rtype='post')
		return self.__parse_user(raw_response)
	async def getScores(self):
		raw_response = await self.__makeRequest('/getGJScores.php', params={'secret': 'Wmfd2893gb7'}, rtype='post')
		return self.__parse_scores(raw_response)
	async def getAccComments(self, accid: int, page:int):
		raw_response = await self.__makeRequest('/getGJAccountComments20.php', params={"secret": 'Wmfd2893gb7', 'page': page, 'accountID': accid}, rtype='post')
		comments = []
		if '|' in raw_response:
		...
		print(raw_response)
		return comments
	async def getComments(self, levelID: int, page: int):
		raw = await self.__makeRequest('/getGJComments21.php', params={"secret": 'Wmfd2893gb7', 'page': page, 'levelID': levelID}, rtype='post')
		comments = []
		try:
			...
			pass
		return comments
	async def searchUser(self, usr: str):
		raw = await self.__makeRequest('/getGJUsers20.php', params={'secret': 'Wmfd2893gb7', 'str': usr}, rtype='post')
		raw = raw.split('#')[0]
		return self.__parse_search_user(raw)
	async def getGauntlets(self):
		try:
			raw = await self.__makeRequest('/getGJGauntlets21.php', params={'secret': "Wmfd2893gb7"}, rtype='post')
			raw = raw.split('#')[0]
			return self.__parse_gauntlet(raw)
		except:
			print('FruitSpaceAPI: Gauntlets error')
