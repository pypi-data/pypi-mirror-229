...
def fallback_attr(
    ...
    warn_builder = FallbackWarningBuilder(name, options)

    at = ""
    if options.fallback_lineno:
    ...
    if isinstance(attr, (types.MethodType, types.FunctionType)):
        ...
        def wrapper(*args, **kwargs):
            ...
            with warn_builder.timing("args"):
                args = _unwrap(args, reason=reason)
                kwargs = _unwrap(kwargs, reason=reason)
...
def fallback_call_packed(
    ...
    """
    method = fallback_attr(fallbacker, method, reason, stacklevel=stacklevel)
