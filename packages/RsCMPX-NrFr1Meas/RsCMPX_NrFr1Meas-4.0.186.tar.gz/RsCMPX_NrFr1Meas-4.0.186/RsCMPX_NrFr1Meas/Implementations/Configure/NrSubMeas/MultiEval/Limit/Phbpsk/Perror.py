...
class PerrorCls:
	...
	def set(self, rms: float or bool, peak: float or bool) -> None:
		...
		Snippet: driver.configure.nrSubMeas.multiEval.limit.phbpsk.perror.set(rms = 1.0, peak = 1.0) \n
		...
		self._core.io.write(f'CONFigure:NRSub:MEASurement<Instance>:MEValuation:LIMit:PHBPsk:PERRor {param}'.rstrip())
	...
	def get(self) -> PerrorStruct:
		"""SCPI: CONFigure:NRSub:MEASurement<Instance>:MEValuation:LIMit:PHBPsk:PERRor \n
		Snippet: value: PerrorStruct = driver.configure.nrSubMeas.multiEval.limit.phbpsk.perror.get() \n
		Defines symmetric limits for the RMS and peak values of the phase error for π/2-BPSK. The limit check fails if the
		absolute value of the measured phase error exceeds the specified values. \n
			:return: structure: for return value, see the help for PerrorStruct structure arguments."""
		return self._core.io.query_struct(f'CONFigure:NRSub:MEASurement<Instance>:MEValuation:LIMit:PHBPsk:PERRor?', self.__class__.PerrorStruct())
