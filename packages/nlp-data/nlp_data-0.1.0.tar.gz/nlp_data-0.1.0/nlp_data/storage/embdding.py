...
class EmbeddingDocStore(S3DocStore):
    """对docarray本地文件存储的封装
    """
    ...
    def list(cls, name_space: Optional[str] = None, show_table: bool = True) -> List[str]:
        ...
        if show_table:
            ...
            table = Table(
                title=f'EmbeddingDocList in bucket s3://embedding',
                box=box.SIMPLE,
                highlight=True,
            )
    ...
    @classmethod
    def pull(cls, name: str) -> EmbeddingDocList[EmbeddingDoc]:
        return EmbeddingDocList[EmbeddingDoc].pull(url=f's3://embedding/{name}', show_progress=True)
    
    
    
    @classmethod
    def push(cls, docs: EmbeddingDocList[EmbeddingDoc], name: str) -> Dict:
        return EmbeddingDocList[EmbeddingDoc].push(docs, url=f's3://embedding/{name}', show_progress=True)
