...
def wifi_scan_jam(sender) -> None:
    ...
    loggei.info(
        "bluetooth item value = "
        f"{dpg.get_item_theme(item='mssn_bluetooth_scan')}"
    )
    ...
    while dpg.is_mouse_button_dragging(button=0, threshold=0.05):
        ...
        if len(empty_list) > 2:
            ...
            if empty_list[-1] < empty_list[-2]:
                return True
        time.sleep(0.01)
    return "no drag detected"


def mouse_drag_callback(sender, app_data: list, user_data):
    """Scroll the window with the mouse."""
    loggei.info("%s()", mouse_drag_callback.__name__)
    loggei.debug("Mouse drag callback method called")
