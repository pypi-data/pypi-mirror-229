...
class AccessCheckAccess(OpenModuleModel):
    ...
    def check_medium_display_name_set_if_no_lpr(cls, values):
        if values["used_medium"].type != "lpr" and "medium_display_name" not in values["access_infos"]:
            raise ValueError("medium_display_name must be set if used_medium is not lpr")
        return values
