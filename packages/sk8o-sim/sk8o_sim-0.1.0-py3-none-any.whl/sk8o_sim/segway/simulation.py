...
class SegwaySimulation:
    ...
    def __init__(self, cfg: SegwaySimulationCfg):
        ...
        self.ic_cfg = cfg.initial_conditions_cfg
        self.phi0_std = cfg.phi0_std
        self.quantization = cfg.quantization
        self.view = View(cfg.view_cfg)
        self._data = None
    ...
    #     # this gets called when an attribute is not found, so the simulation data is accessible directly
    #     return getattr(self._data, name)

    def velocity_reference(self, reference: Tuple[float, float]) -> SegwaySimData:
        ...
        -------
        SegwaySimData
            New measurement with the reference included.
        """
        self._data.velocity_reference = np.array(reference)
        return self.measure()

    def position_reference(
        self, reference: Tuple[float, float, float]
    ) -> SegwaySimData:
        ...
        SegwaySimData
            New measurement with the reference included.
        """
        self._data.position_reference = np.array(reference)
        return self.measure()
    ...
    def reset(
        ...
        reference : Tuple[float, float] | np.ndarray | None, default None.
            The reference (dot_x_ref, dot_psi_ref). If None, a reference will be randomly generated based on the configuration.
        """
