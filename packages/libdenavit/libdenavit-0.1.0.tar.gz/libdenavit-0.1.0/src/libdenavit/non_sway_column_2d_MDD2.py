    ...
    def run_AASHTO_interaction(self, EI_type, num_points=10, section_factored=True, Pc_factor=0.75, beta_dns=0,
        ...
        if minimum_eccentricity:
            ...
            if buckling_load > max(P_id):
                ...
                if EI_type in ['JF-a', 'JF-b', 'ACI-c', 'new_1']:
                    def f(x):
                        EIeff = self.section.EIeff(self.axis, EI_type, beta_dns, P=Pc[-1], M=x)
                        Pc.append(pi ** 2 * EIeff / (k * self.length) ** 2)
                        return abs(Pc[-2] - Pc[-1])

                    from scipy.optimize import newton
                    newton(f, 0.0, maxiter=1000, tol=1e-6, disp=False, full_output=True)
                    buckling_load = Pc_factor * Pc[-1]
