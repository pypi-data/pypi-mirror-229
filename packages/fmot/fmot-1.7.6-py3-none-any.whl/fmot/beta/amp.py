...
def optimize_mixed_precision(cmodel, objective, eta, inputs, targets, niter=1,
                             loss_type='polynomial', device='cpu', **kwargs):
    ...
    if not cmodel.quantized:
        raise ConversionDependencyError(
            ...
            ' Call `.quantize()` first')
    model, energy, loss = auto_multiprecision.optimize_mixed_precision(cmodel, cmodel, objective=objective,
                                                         eta=eta, inputs=inputs, targets=targets, niter=niter,
                                                         ramp_eta=True, loss_type=loss_type, device=device,
                                                         **kwargs)
    return energy, loss
