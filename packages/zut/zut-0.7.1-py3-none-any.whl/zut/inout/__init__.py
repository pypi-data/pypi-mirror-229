...
from .utils import is_excel_path, split_excel_path, normalize_inout  # NOTE: kept even if not used, so that these names are available from zut root

logger = logging.getLogger(__name__)


def out_file(out: str|Path|IOBase|None = None, *, out_dir: str|Path|None = None, title: str|None = None, append: bool = False, encoding: str = 'utf-8-sig', newline: str = None, atexit: bool|Callable = None, **kwargs) -> OutFile:
    clazz: OutFile = None
    ...
    return clazz(out, out_dir=out_dir, title=title, append=append, encoding=encoding, newline=newline, atexit=atexit, **kwargs)


def out_table(out: Path|str|IOBase|None = None, *, out_dir: str|Path|None = None, title: str|None = None, append: bool = False, encoding: str = 'utf-8-sig', headers: list[str] = None, delimiter: str = None, quotechar: str = '"', tz: tzinfo = None, decimal_separator: str = None, atexit: bool|Callable = None, **kwargs) -> OutTable:
    clazz: OutTable = None
    ...
    return clazz(out, out_dir=out_dir, title=title, append=append, encoding=encoding, headers=headers, delimiter=delimiter, tz=tz, decimal_separator=decimal_separator, quotechar=quotechar, atexit=atexit, **kwargs)


def in_table(src: Path|str|IOBase, query: str = None, *args, src_dir: str|Path|None = None, title: str|None = None, encoding: str = 'utf-8', delimiter: str = None, quotechar: str = '"', formatters: dict[int|str,Callable] = None, debug: bool = False, limit: int = None, **kwargs) -> InTable:
    clazz: InTable = None
    ...
    if not clazz:
        if isinstance(src, (str,Path)) and is_excel_path(src, accept_table_suffix=True):
            clazz = InExcel
        else:
            clazz = InCsv
    ...
    return clazz(src, query=query, *args, src_dir=src_dir, title=title, encoding=encoding, delimiter=delimiter, quotechar=quotechar, formatters=formatters, debug=debug, limit=limit, **kwargs)
...
_out_file_handler: list[Callable[[Path|str|IOBase|None], type[OutFile]|None]] = []
_out_table_handler: list[Callable[[Path|str|IOBase|None], type[OutTable]|None]] = []
_in_table_handler: list[Callable[[Path|str|IOBase], type[InTable]|None]] = []

def register_out_file_handler(handler: Callable[[Path|str|IOBase|None], type[OutFile]|None]):
    _out_file_handler.append(handler)
    
def register_out_table_handler(handler: Callable[[Path|str|IOBase|None], type[OutTable]|None]):
    _out_table_handler.append(handler)
    
def register_in_table_handler(handler: Callable[[Path|str|IOBase], type[InTable]|None]):
