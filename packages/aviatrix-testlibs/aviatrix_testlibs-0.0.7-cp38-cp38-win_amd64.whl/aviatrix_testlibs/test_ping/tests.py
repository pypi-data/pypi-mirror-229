...
class Tests(test_base.TestBase):
    ...
    def test_ping(self):
        ...
        self.assertTrue(self.tu.local_ping(self.data['ec2'],allow,None))

    def test_trace_route(self):
        expectPaths = str(self.data["spoke_gw"]).split(',')
        self.assertTrue(self.tu.local_simple_trace(self.data['ec2'],self.data['traceroute'],expectPaths),'Test Failed: trace route from 10.11.0.10')
