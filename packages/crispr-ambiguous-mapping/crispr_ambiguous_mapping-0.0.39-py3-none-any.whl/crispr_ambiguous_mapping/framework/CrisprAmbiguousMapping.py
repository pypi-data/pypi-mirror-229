...
def retrieve_fastq_guide_sequences(fastq_file: str, parse_left_flank: bool = True, parse_flank_sequence: Union[None, str] = None, cores: int=1) -> Union[List[str], List[Seq]]:
    ...
    def parse_fastq_entry(seq):
        return parse_guide_sequence_p(seq.seq)

    
    read_executor_cores = int(np.sqrt(cores))
    parse_executor_cores = int(np.sqrt(cores))
    read_executor_chunk_size = 50000
    parse_executor_chunk_size = int(read_executor_chunk_size/parse_executor_cores)

    def process_fastq_chunk(chunk):
        with ProcessPoolExecutor(max_workers=parse_executor_cores) as executor:
            return list(executor.map(parse_fastq_entry, chunk, chunksize=parse_executor_chunk_size))
    
    with ProcessPoolExecutor(max_workers=read_executor_cores) as executor:
        if fastq_file.endswith('.gz'):
            print(f"Opening FASTQ.gz file with gzip, filename={fastq_file}")
            with gzip.open(fastq_file, "rt", encoding="utf-8") as handle:
                print("FASTQ opened")
                fastq_guide_sequences = list(executor.map(process_fastq_chunk, SeqIO.parse(handle, 'fastq'), chunksize = read_executor_chunk_size))
        else:
            fastq_guide_sequences = list(executor.map(process_fastq_chunk, SeqIO.parse(fastq_file, 'fastq')))
