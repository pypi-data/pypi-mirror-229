...
def run_pack(pack_file_path):
    ...
    stdout_thread = Thread(target=pack_logs, args=(process.stdout, [loggerPack, logger]))
    stderr_thread = Thread(target=pack_logs, args=(process.stderr, [loggerPack, logger], True))
            ...
            with open(file_path, "r") as f:
                ...
                if file == "properties.yaml":
                    ...
                    if "visibility" not in properties:
                        logger.warning(f"Visibility in 'properties.yaml' is not set. Defaulting to [Private]")
...
def push(config, name):
    ...
    if pack_visibility != pack_data['visibility']:
        pack_answer = ask_confirmation("Are you sure you want to publish a public pack ? Public packs are made visible for partners. Be carefull about what you share with partners.")
    ...
    if r.status_code == 200:
        ...
        if r.status_code != 200:
            logger.error(f"Pack [{pack_name}] could not be fetched. Are you the owner of the pack ? You need to login with the user id that published the pack : \n > qalita agent -n <owner> login")
            sys.exit(1)
    ...
    if pack_answer == False:
        logger.error("You have refused to publish your pack to public")
        sys.exit(1)
    ...
    if differences_detected:
        ...
        if r.status_code == 200:
            ...
            logger.error(f"Pack [{pack_name}] could not be updated. Error code {r.status_code}")
