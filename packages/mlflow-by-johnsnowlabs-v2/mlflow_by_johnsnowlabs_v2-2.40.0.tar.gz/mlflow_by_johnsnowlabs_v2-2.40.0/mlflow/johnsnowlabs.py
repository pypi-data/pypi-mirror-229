...
class _PyFuncModelWrapper:
    ...
    def predict(self, text):
        ...
        """
        TODO:
        1) support output level via first row inference and other params X DONE  hackable via nlu rc 
        2) support start session with GPU (ENV VAR?) X done hackable --> just matters which jars is in container!
        3) support image predictions (OCR MODEL) --> send img as binary?? hackable in nlu rc
        
        4) Set VISUAL_SECRET on cluster creation1
        df  with 2 cols, file_type and file_binary
        CLEAN CLUSTER TRUE!??!
        """
