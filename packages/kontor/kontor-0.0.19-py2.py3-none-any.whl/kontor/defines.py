...
def send_message(connection: socket.socket, address, json_data):
    logging.debug(f"{address}: Sending message: {json_data}.")
...
def wait_and_receive_response(connection: socket.socket, address) -> dict:
    logging.debug(f"{address}: Waiting for response.")
    ...
    while not is_message_received:
        ...
        if (
            ...
            logging.debug(f"{address}: Received message: {message_json}")
...
def send_file(connection: socket.socket, address, file: bytes):
    logging.debug(f"{address}: Sending file.")
    ...
    with open(corrected_file_path, "wb") as processed_file:
        ...
        while not file_received_fully:
            ...
            if received_percents % 10 == 0 and previous_percents != received_percents:
                ...
                logging.info(
                    f"{address}: Receiving file from server, {received_percents}% ({received_bytes} / {file_size_bytes})."
                )

            processed_file.write(data)

        file_upload_time = time.time() - file_start_time
        logging.info(f"{address}: File upload time is {file_upload_time} seconds.")
