...
def tokenize_for_masked_language_modeling(
    ...
    """
    dataset = load_dataset(file_path, output_type="Dataset", columns="text", masks=["duplicate", "pattern"])
    ...
    if output_dir:
        path_to_output = get_output_path(file_path, output_dir)
...
def generate_splits(
    ...
    if output_dir:
        for split in splits.keys():
            path_to_output = get_output_path(output_dir, split)
