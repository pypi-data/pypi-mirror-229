...
class _AbstractTask:
    def __init__(self, future: Union[asyncio.Future, Future]):
        self.future = future
...
class ThreadTask(_AbstractTask):
    ...
    def result(self, timeout: float = None):
        # noinspection PyUnresolvedReferences,PyProtectedMember
        if self.future._state == PENDING and evloop.in_same_thread():
...
class EventLoop:
    ...
    def create_task(self, coro) -> ThreadTask:
        if self.in_same_thread():
            task = AsyncIOTask(asyncio.create_task(coro))
            # raise RuntimeError("Create task inside evloop is not allowed!")
        else:
            task = ThreadTask(asyncio.run_coroutine_threadsafe(coro, self.loop))
        node = self._tasks.wrap_append(task)
        task.add_done_callback(lambda _: self._tasks.remove(node))
        return task
