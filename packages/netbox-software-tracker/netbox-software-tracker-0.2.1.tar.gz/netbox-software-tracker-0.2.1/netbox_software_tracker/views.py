...
class GoldenImageProgressListView(generic.ObjectListView):
    ...
    def get(self, request, pk: int, *args, **kwargs):
        ...
        for item in instances:
            devices = devices + list(item.device_type.instances.exclude(**{f"custom_field_data__{CF_NAME_SW_VERSION}": item.software.version}))

        table = DeviceTable(data=devices, user=request.user)
