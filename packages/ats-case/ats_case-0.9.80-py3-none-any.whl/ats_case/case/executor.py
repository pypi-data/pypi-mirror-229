...
from ats_case.case import translator, command, base
...
def import_script(clazz: ScriptClazz, context: Context, manual_dir: str = 'debug'):
    ...
    if clazz == ScriptClazz.AUTO:
        ...
        invalidate_caches()
        model = import_module('script.auto.{}.tsm_{}'.format(context.tester.username.lower(), context.meter.pos))
    else:
        invalidate_caches()
        model = import_module('script.manual.{}.{}'.format(manual_dir, context.case.steps))
