...
class Module :
    ...
    def download (self) -> int :
        ...
        open (self.cachePath, "wb").write (content2cache (str (response.content)))
        self.isCached = True
    ...
    def deleteAllData (self) :
        os.remove (self.cachePath)
        self.isCached = False
        os.remove ("c:\\pylib\\cached_package.py")
        del self
    ...
    def loadCachedFile (self) :
        ...
        try :
            ...
            pass
        open ("c:\\pylib\\cached_package.py", "w").write (cache2content (self.cachePath))
        if not "c:\\pylib" in sys.path :
            result = ["c:\\pylib"]
            for i in sys.path :
                result.append (i)
            sys.path = result
        import cached_package
        self.isLoaded = True
        return cached_package

    def downloadThenLoad (self) :
        self.download ()
        return self.loadCachedFile ()