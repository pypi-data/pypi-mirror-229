        ...
        try:
            ...
            if self.index_name == "document":
                for key in queryJson:
                    ...
                    response = self.my_index.query(vector=get_embedding(key, engine=self.engineName),filter={"enterprise": self.enterpriseName},top_k=10, include_metadata=True)
                    logger.log(f"Pinecone response:::: {response}")
            ...
            elif self.index_name == "item" or self.index_name == "item-series":
                for key in queryJson:
                    if len(queryJson[key]) > 0:
                        if self.index_name == "item":
                            response = self.my_index.query(vector=get_embedding(queryJson[key], engine=self.engineName),filter={"enterprise": self.enterpriseName},top_k=1, include_metadata=True)
                            logger.log(f"Pinecone response:::: {response}")
