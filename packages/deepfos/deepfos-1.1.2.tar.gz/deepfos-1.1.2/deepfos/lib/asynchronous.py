...
import atexit
from typing import (
    Optional, Union, TYPE_CHECKING, List,
    Callable, NamedTuple, Coroutine, Any, Dict,
    Generic, Iterable, TypeVar, Awaitable
)
...
class Node(Generic[NodeT]):
    ...
    def __init__(self, value: NodeT = None):
        self.value = value
        self.next = None
        self._prev = None
...
class _AbstractTask:
    ...
    def __repr__(self):
        if self.ensure_completed:
            ...
            lb, rb = '<', '>'
        return f"{lb}{self.__class__.__name__} {self._display_name} at 0x{hex(id(self))}{rb}"
...
class future_property(Generic[ReturnT]):
    def __init__(
        self, 
        func: Callable[[Any], Awaitable[ReturnT]] = None,
        on_demand=False
    ):
        self.func = func
    ...
    # noinspection PyProtectedMember
    def __get__(self, instance, owner=None) -> ReturnT:
        if instance is None:
            return self
