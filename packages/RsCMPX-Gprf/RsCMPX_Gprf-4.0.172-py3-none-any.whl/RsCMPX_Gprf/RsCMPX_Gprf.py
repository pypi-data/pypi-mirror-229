...
class RsCMPX_Gprf:
	"""790 total commands, 17 Subgroups, 0 group commands"""
	_driver_options = "SupportedInstrModels = CMX/CMP/CMW/PVT, SupportedIdnPatterns = CMX/CMP/CMW/PVT, SimulationIdnString = 'Rohde&Schwarz,CMX500,100001,4.0.172.0035'"
	...
	def __init__(self, resource_name: str, id_query: bool = True, reset: bool = False, options: str = None, direct_session: object = None):
		...
		self._core.driver_version = '4.0.172.0035'
	...
	def assert_minimum_version(min_version: str) -> None:
		...
		curr_version_list = '4.0.172.0035'.split('.')
		...
		for i in range(count):
			...
			if curr > minimum:
				break
			if curr < minimum:
				raise RsInstrException(f"Assertion for minimum RsCMPX_Gprf version failed. Current version: '4.0.172.0035', minimum required version: '{min_version}'")
