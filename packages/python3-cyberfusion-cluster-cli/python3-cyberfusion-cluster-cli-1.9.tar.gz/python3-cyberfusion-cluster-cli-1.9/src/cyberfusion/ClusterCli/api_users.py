...
@app.command()
@catch_api_exception
def update_trusted_ip_networks(
    username: str, trusted_ip_networks: List[str]
) -> None:
    """Update trusted IP networks."""
    api_user = get_object(get_support().api_users, username=username)

    api_user.trusted_ip_networks = trusted_ip_networks
    api_user.update()
...
def remove_trusted_ip_networks(
    ...
    # Exit code will be 64 if at least 1 given IP network could not be removed
    # Success will only be False (and exit code 1) if no IP networks were removed
