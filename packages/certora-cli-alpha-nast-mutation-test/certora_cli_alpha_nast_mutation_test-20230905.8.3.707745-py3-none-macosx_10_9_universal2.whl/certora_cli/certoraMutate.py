...
def send_get_request(web_utils: WebUtils, type: str = "") -> ServerResponse:
    ...
    return ServerResponse(resp_obj[Constants.ID],
                          resp_obj[Constants.ANONYMOUS_KEY],
                          resp_obj[Constants.PRE_SIGNED_URL])
...
def store_in_a_file(results: List[Any], ui_out: Any) -> None:
    try:
...
def submit_to_ui(results: List[Any], web_utils: WebUtils) -> None:
    ...
    upload_file_to_cloud_storage(web_utils, response.presigned_url, results)
