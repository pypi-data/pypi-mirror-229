...
class Heinzinger(SingleCommDevice, DeprecatedHeinzingerMixin):
    ...
    def __init__(self, com, dev_config=None) -> None:
        ...
        self._output_status: Optional[bool] = None
    ...
    @max_current.setter
    def max_current(self, value: Optional[Number]):
        if value is None:
            # Reset the user-defined limit to the hardware limit
            self._max_current = self.max_current_hardware
            return
        validate_number(
            "max_current", value, (0, self._max_current_hardware), logger=logger
        )
    ...
    @max_voltage.setter
    def max_voltage(self, value: Optional[Number]):
        if value is None:
            # Reset the user-defined limit to the hardware limit
    ...
    def number_of_recordings(self, value: Union[int, RecordingsEnum]) -> None:
        """
    ...
    def voltage(self) -> Number:
        ...
        :raises SerialCommunicationIOError: when communication port is not opened
        """
        value = self.com.query("MEAS:VOLT?")
        logger.info(f"Output measured voltage of the Heinzinger PNC: {value}")
        return float(value)
    ...
    def voltage(self, value: Number) -> None:
        ...
        if abs(value - set_value) > self._epsilon * self._max_voltage_hardware:
            msg = (
                ...
                f"but the device did not take the value; the value is={set_value}"
            )
            logger.error(msg)
            raise HeinzingerSetValueError(msg)
        else:
            logger.info(f"Output voltage of the Heinzinger PNC is set to: {value}")
    ...
    def set_voltage(self) -> Number:
        ...
        :raises SerialCommunicationIOError: when communication port is not opened
        """
        value = self.com.query("VOLT?")
        logger.info(f"Output set voltage of the Heinzinger PNC: {value}")
        return float(value)
    ...
    def current(self) -> Number:
        ...
        :raises SerialCommunicationIOError: when communication port is not opened
        """
        value = self.com.query("MEAS:CURR?")
        logger.info(f"Output measured current of the Heinzinger PNC: {value}")
        return float(value)
    ...
    def current(self, value: Number) -> None:
        ...
        if abs(value - set_value) > self._epsilon * self._max_current_hardware:
            ...
            logger.info(f"Output current of the Heinzinger PNC is set to: {value}")
    ...
    def set_current(self) -> Number:
        ...
        :raises SerialCommunicationIOError: when communication port is not opened
        """
        value = self.com.query("CURR?")
        logger.info(f"Output set current of the Heinzinger PNC: {value}")
        return float(value)
