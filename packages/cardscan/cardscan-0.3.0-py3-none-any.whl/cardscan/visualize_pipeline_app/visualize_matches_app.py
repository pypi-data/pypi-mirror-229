...
def run_match_app(pipeline: Pipeline):
    pygame.init()
    ...
    while running:
        ...
        contours, images = scan(
            cam_frame, results=[card_contours_transform, card_images]
        )

        thumbnail_size, (tot_rows, tot_cols) = grid_layout_tile_size(
            aspect_ratio, len(images), WINDOW_SIZE
        )
        ...
        [
            screen.blit(thumb.get_labeled_surface(), thumb.rect.topleft)
