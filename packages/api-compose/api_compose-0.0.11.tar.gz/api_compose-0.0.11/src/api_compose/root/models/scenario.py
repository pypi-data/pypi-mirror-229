...
class ScenarioModel(BaseModel):
    ...
    actions: List[Union[JsonRpcWebSocketActionModel, XmlHttpActionModel, JsonHttpActionModel, DummyActionModel]]
            ...
            for val in value:
                if isinstance(val, dict):
                    ...
                    if isinstance(model, RefResolverModel):
                        model = RefResolver(
                            model,
                            GlobalSettingsModelSingleton.get().discovery.manifests_folder_path
                        ).resolve()
                    list_.append(model)
    ...
    def validate_execution_id(self):
        ...
        if len(duplicate_keys) != 0:
            logger.error( f"Error: Actions in Scenario {self.fqn} with the same execution id found! {os.linesep} {os.linesep.join([fqn for fqn in fqns])}")
