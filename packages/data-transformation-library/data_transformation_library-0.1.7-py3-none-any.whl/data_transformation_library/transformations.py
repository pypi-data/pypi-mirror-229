...
def window1d(input_array, size, shift=1, stride=1):
    """
    Creates overlapping or non-overlapping 1D windows from an input array.
    :param input_array: a list or 1D Numpy array of real numbers
    :param size: a positive integer that determines the size (length) of the window
    :param shift: a positive integer that determines the shift (step size) between different windows
    :param stride: a positive integer that determines the stride (step size) within each window.
    ...
    if isinstance(input_array, list):
        ...
        windows = []
        for item in range(0, input_array.size - size + 1, shift):
            window = input_array[item:item + size:stride]
            windows.append(window)
