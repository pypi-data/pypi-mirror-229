...
from hashboard.constants import HB_BASE_URI
...
def create_account_and_access_key(
    ...
    if access_key_info:
        ...
        click.echo()
        click.echo(
            f"{click.style('Go to the following link to set up a database connection:', bold=True)} {HB_BASE_URI}/app/mb"
        )
        click.echo()
...
def _get_new_account_access_key_info(
    ...
    def qp_handler(query_params: dict) -> str:
        ...
        if attempted_signup_with_existing_user:
            ...
            return f"{HB_BASE_URI}/cliAuthConfirmation/error/attemptedSignupWithExistingUser?{urlencode(query_params_dict)}"
        ...
        if any(
            ...
            request_error = True
            return f"{HB_BASE_URI}/cliAuthConfirmation/error"

        access_key_info = AccessKeyInfo(**query_params)
        return f"{HB_BASE_URI}/cliAuthConfirmation/success/newUser?userFullName={access_key_info.user_full_name}"
...
def _signup_and_handle_query_params(
    ...
    base_url = f"{HB_BASE_URI}/cliAuthConfirmation/entry/newUser"
    query_params_dict = {
