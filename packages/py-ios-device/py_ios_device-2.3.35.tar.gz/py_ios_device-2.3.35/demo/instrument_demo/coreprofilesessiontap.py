...
def graphics_display(rpc):
    ...
    mach_time_factor, numer, denom = machTimeInfo
        ...
        if type(res.selector) is InstrumentRPCParseError:
            for args in kperf_data(res.selector.data):
                ...
                if time_count > NANO_SECOND:
                    print(
                        f"{((last_frame - mach_time_factor) * numer) / denom} FPS: {frame_count / time_count * NANO_SECOND} jank: {jank_count} big_jank: {big_jank_count} stutter: {jank_time_count / time_count}")
                    jank_count = 0
                    big_jank_count = 0
                    jank_time_count = 0
                    frame_count = 0
