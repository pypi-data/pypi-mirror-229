...
def writing_output(format, mol, output, forcefield, hydrogen):
    ...
    if(format == "MOL"):
        writer = Chem.MolWriter(str(output) + ".mol")
        writer.write(new_mol)
        writer.close()
        
    elif(format == "SMI"):
        writer = Chem.SmilesWriter(str(output) + ".smi")
        writer.write(new_mol)
        writer.close()
    
    elif(format == "SDF"):
        writer = Chem.SDWriter(str(output) + ".sdf")
        writer.write(new_mol)
        writer.close()
        ...
        if os.path.exists(str(output)+".gro"):
            os.remove(str(output)+".gro")
            
        if not os.path.exists(str(output)+".pdb"):
            writing_output("PDB", mol, output, forcefield, hydrogen)
        
        structure = load_file(output + ".pdb")
        structure.save(output + ".gro")
        
    elif(format == "TOP"):
        if not os.path.exists(str(output)+".pdb"):
            writing_output("PDB", mol, output, forcefield, hydrogen)
        
        command = "LigParGen -p "+str(output)+".pdb -r UNL -c 0-o  0 -l"
        ...
        new_mol = Chem.MolFromPDBFile(str(output)+".pdb")
        Chem.MolToCMLFile(new_mol, str(output) + ".cml")
...
def exportFiles(output, polymer):
    ...
    for format in output["format"]:
        writing_output(format, polymer, output.get("name", None), output.get("forcefield", None), output.get("add_hydrogen", "FALSE"))
...
if __name__=="__main__":
    ...
    for format in config["output"]["format"]:
        writing_output(format, final_mol, output.get("name", None), output.get("forcefield", None), output.get("add_hydrogen", "FALSE"))
