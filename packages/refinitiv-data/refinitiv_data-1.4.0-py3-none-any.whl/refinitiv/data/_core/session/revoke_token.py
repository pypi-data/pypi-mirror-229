...
if TYPE_CHECKING:
    from ..._core.session import PlatformSession
    from ..session.auth_manager import TokenInfo


def revoke_token(token_info: "TokenInfo", session: "PlatformSession"):
    ...
    session.debug("Access token was revoked.")
    session.http_request(refresh_token_request)
    session.debug("Refresh token was revoked.")
