...
from yaoys_checkin.aliyunpan_checkin.aliyunpan_task.aliyunpan_task import create_album, create_quick_pass, delete_file, delete_lastTime_album, delete_photo_from_album, device_room_task, follow_user, get_device_room_info, get_lucky_bittle, play_video_by_mobile2, \
    upload_file, \
    upload_photo
    ...
    def sign_in(self):
        ...
        try:
            ...
            if 'result' in data and isinstance(data['result'], dict) is True:
                ...
                if has_device_room is True and "deviceRoom_task" in self.config_file['cookieOrUser']["aliyunpan"] \
                        and self.config_file['cookieOrUser']["aliyunpan"]['deviceRoom_task'] is True:
                    is_success, device_room_task_info = device_room_task(aligo=self.aligo, access_token=self.access_token, time_sleep=10)
                    self.account_checkin_message += f', 时光设备间任务: {device_room_task_info}'
