                ...
                if key[-2] == "-":
                    ...
                    description = "{0}{1}\n".format(
                        ...
                        self.translations.get(
                            "Special Key Selects",
                            "The key combination {0} selects {1}.",
...
class UserInterface:
    ...
    def __init__(self, config_file_name):
        ...
        description_text = "\n".join(
            ...
            for paragraph in self.config.application[
                "description"
            ].splitlines()
        )
    ...
    def __config_from_file(self, config_file_name):
        ...
        supported_file_types = [("Config file", (".json", ".yaml", ".yml"))]
        while True:
            if config_file_name:
    ...
    def __build_action_frame(self):
        ...
        search_term_label = tkinter.Label(
            action_frame, text="{0}:".format(self.translated("Search Term"))
        )
        ...
        button = tkinter.Button(
            action_frame,
            text=self.translated("Clear Button", default="Clear"),
            ...
            if preferred_browser:
                try:
                    ...
                    category_label = self.translated(
                        "Opened In", default="{0} (opened in {1})"
                    ).format(current_category, preferred_browser.title())
                #            #
            if current_grid_row <= 12:
                access_key = "<KeyPress-F{0}>".format(current_grid_row)
                category_label = "{0} <F{1}>".format(
                    category_label, current_grid_row
                )
            else:
                access_key = None
            #
    ...
    def show_about(self):
        ...
        in a modal dialog
        """
        license_text = "Licensed under the MIT license, see above repository."
        metadata = "\n".join(
        ...
        gui_commons.InfoDialog(
            ...
            (
                ...
                "{0} {1} ({2})\n\n{3}".format(
                    SCRIPT_NAME, __version__, HOMEPAGE, license_text
                ),
            ),
            (
                self.translated("Config File"),
                "{0}\n{1}".format(
                    self.config.application["config_file_name"], metadata
                ),
            ),
    ...
    def show_urls_in(self, category):
        ...
        url_names = [name for (name, url) in self.config.get_items(category)]
        gui_commons.InfoDialog(
            self.main_window,
            (category, "\n".join(url_names)),
    ...
    def copy_url(self, category):
        ...
        try:
            ...
            messagebox.showerror(
                self.translated(
                    "Category Error", default="Error for category {0!r}"
                ).format(category),
                str(value_error),
