...
def download_dataset_from_bladesight_data(dataset_path_on_s3 : str) -> None:
    ...
    """
    s3 = s3fs.S3FileSystem(anon=True)
    PATH_TO_LOCAL_DB = get_path_to_local_bladesight() / "datasets"
...
class Dataset:
    ...
    def print_citation(self):
        ...
        cite_html = f"""<p>If you use this dataset in published work, please use the below citation:</p>
        <p><b>{citation['repr']}</b></p>
        """
        if 'url' in citation.keys():
            cite_html +=  f""" <p><a href="{citation['url']}">Link to paper</a></p>"""

        if 'doi' in citation.keys():
            cite_html +=  f""" <p>DOI: { citation['doi'] } </p>"""
        if is_notebook():
            display(HTML(cite_html))
        else:
            print(cite_html)
