...
from utils.errno import Error, OK, NO_M3U8, NO_TS, TRANS_MP4, check_errors
...
from utils.async_req.cli import DefaultHTTPClient

_M3u8HCli = DefaultHTTPClient()
M3u8FName = "index.m3u8"


def save_m3u8(u: str, filepath: str, timeout=3) -> M3U8:
    try:
...
async def download_m3u8_v_async(m3u8_u: str, filepath: str, con_num=10):
    m3u8_f = get_file_path(filepath, M3u8FName)
    logger.info(f"save to {m3u8_f}")
    m3u8 = save_m3u8(m3u8_u, m3u8_f)
    if not m3u8:
    ...
    for u in _get_m3u8_ts(m3u8):
        ts_f = get_file_path(filepath, get_path_last_part(u))
        if check_file_exist(ts_f):
            logger.debug(f"download_v_async {ts_f} exist")
            continue
        ts_fs.append(ts_f)
        tasks.append(async_download(u, ts_f))
...
def download_m3u8_v(m3u8_u: str, filepath: str, con_num=10):
    return call_async(download_m3u8_v_async(m3u8_u, filepath, con_num))


def merge_m3u8_ts(m3u8_dir: str) -> Error:
