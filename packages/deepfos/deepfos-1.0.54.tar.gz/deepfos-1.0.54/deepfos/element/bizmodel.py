...
from typing import (
    List, Dict, Optional, Union, Any, TYPE_CHECKING
)
...
class CopyConfig(UserDict):
    ...
    def load_config(self, conf: Dict[str, Dict]):
        for table, copy_conf in conf.items():
            ...
            if "where" not in conf:
                raise KeyError("Field 'where' is missing in copy configure.")
            tbl_conf["where"] = tbl_conf["where"]
            tbl_conf["field_map"] = tbl_conf.get("field_map")
...
class BusinessModel(ElementBase):
    """业务模型"""
    api_class = BusinessModelAPI

    if TYPE_CHECKING:
        from deepfos.api.V1_1.business_model import BusinessModelAPI as V1_1
    ...
    @cached_property
    def meta(self) -> BizModel:
        """业务模型的元数据信息"""
        return self.api.model.query(
            folderId=self.element_info.folderId,
            elementName=self.element_name
        ).businessModel

    @cached_property
    def _meta_logic_table(self) -> TableStructure:
        """内部逻辑使用：业务模型的元数据信息-主表信息(logicTable)"""
        return self.api.configure.structure_top(
            folderId=self.element_info.folderId,
            elementName=self.element_name
        )

    @cached_property
    def _meta_sub_models(self) -> List[PartitionDTO]:
        """内部逻辑使用：业务模型的元数据信息-子模型信息(subModels)"""
        return self.api.configure.partitions(
            folderId=self.element_info.folderId,
            elementName=self.element_name
        )
    ...
    @cached_property
    def permission(self) -> AuthorityDTO:
        """权限配置信息"""
        return self.api.permission.query(
            elementName=self.element_name,
            folderId=self.element_info.folderId)
    ...
    @cached_property
    def approval_process(self) -> Dict[str, LazyList[ApprovalProcess]]:
        """模型配置的审批流信息"""
        candidates = {}
    ...
    def set_approval(
        ...
        if operation_id is None or origin_status is None:
            ...
            if origin_status is None:
                origin_status = appv.get_record(primary, roles)[0].result_status
            if operation_id is None:
                operation_id = appv.get_operation_id(operation)

        return self.api.approval.operation(BusinessModelApproveDTO(
    ...
    def set_approval_ex(
        ...
        """
        actual_cli = get_client_class(tbl_pc.api.module_type)()
        df_query = actual_cli.query_dfs(sql)
        if origin_status is None:
            # 初始化的处理
            if df_query.empty:
        ...
        if not df_success.empty:
            ...
            tbl_main = self.logic_tables[main_tbl_name].datatable
            ...
            sql_update = f"update {tbl_main.table_name} set " \
                         ...
                         f"where {where_sql}"
            with tbl_main.start_transaction():
                tbl_pc.insert_df(df_insert)
                txn_support(tbl_main.__class__.run_sql)(tbl_main, sql_update)
    ...
    def save(
        ...
        return self.api.data.save(payload)  # noqa
