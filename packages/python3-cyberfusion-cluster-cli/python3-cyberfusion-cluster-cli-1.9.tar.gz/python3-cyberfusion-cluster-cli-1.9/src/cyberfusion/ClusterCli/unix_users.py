...
def usage(
    ...
    amount: Optional[int] = typer.Option(default=None, show_default="All"),  # type: ignore[arg-type]
) -> None:
    ...
    console.print(get_usages_plot(usages=usages))
...
def create(
    ...
    unix_user.create(
        ...
        virtual_hosts_directory=os.path.join(
            home_directory, virtual_hosts_subdirectory
        )
        if get_support().GROUP_CLUSTER_WEB in cluster.groups
        else None,
        mail_domains_directory=os.path.join(
            home_directory, mail_domains_subdirectory
        )
        if get_support().GROUP_CLUSTER_MAIL in cluster.groups
        else None,
        borg_repositories_directory=os.path.join(
            home_directory, borg_repositories_subdirectory
        )
        if get_support().GROUP_CLUSTER_BORG_SERVER in cluster.groups
        else None,
