...
"""
GeoIP2Dat - Version: v1.0.3 - 09/Sep/2023
...
import sys, os, gzip, pickle, io, inspect, socket, struct, json
...
def int2ip(addr):
    try:
        return socket.inet_ntoa(struct.pack("!aI", addr))
    except:
        return ""
    ...
    with elapsed_timer() as elapsed_total:
        ...
        try:
            with elapsed_timer() as elapsed:
                ...
                if not os.path.isfile(os.path.join(country_dir,MM_ASN_BLOCKS_FILENAME)):
                    raise GeoIP2DatError("Unable to access the file %s in directory %s"%(MM_ASN_BLOCKS_FILENAME,country_dir))
                ...
                if not os.path.isfile(os.path.join(country_dir,MM_CITY_BLOCKS_FILENAME)):
                    raise GeoIP2DatError("Unable to access the file %s in directory %s"%(MM_CITY_BLOCKS_FILENAME,country_dir))
                if not os.path.isfile(os.path.join(country_dir,MM_CITY_LOCATIONS_FILENAME.replace("XX",args.language))):
                    raise GeoIP2DatError("Unable to access the file %s in directory %s"%(MM_CITY_LOCATIONS_FILENAME.replace("XX",args.language),country_dir))
        ...
        # with elapsed_timer() as elapsed:
        #     try:
        #         with io.open(os.path.join(country_dir,MM_ASN_BLOCKS_FILENAME.replace("XX",args.language)), mode="rt",encoding="utf-8") as f:
        #             next(f)  # skip the first line (the CSV's header)
        #             while True:
        #                 try:
        #                     counter += 1
        ...
        #                             asn_id = LINE[1]
        #                             asn_name = LINE[2]
        #                             asnByID[asn_id] = asn_name
        #                             asnByCIDR[CIDRInfo.first_ip2int] = f"{cidr.split('/')[1]}:{asn_id}"
        #                         except Exception as ERR:
        #                             print("Error at \""+str(LINE)+"\" - "+str(ERR))
        #                         if counter % 50000 == 0:
        #                             # break
        #                             log(f"\r> Lines read: {counter}",end="")
        #                 except Exception as ERR:
        #                     logError("Falied at asn country blocks look %s"%(str(ERR)))
        #                     continue
        #         log(f"\r- Lines read: {counter} done! {timer(elapsed())}")
        #     except Exception as ERR:
        #         logError("Failed asn readline %s"%(str(ERR)))
        #         return 1 
        ##────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────
        with elapsed_timer() as elapsed:
            try:
        ...
        mainList = list(split_list(listFirstIP,5000))
        mainListCodes = list(split_list(listCountryCode,5000))
        mainListNetLength = list(split_list(listCIDR,5000))
        mainIndex = []
