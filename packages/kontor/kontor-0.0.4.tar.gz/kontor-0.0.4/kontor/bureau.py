...
class Bureau:
    ...
    def __init__(self):
        ...
        filename = timestamp + "_kontorist_server.log"
    ...
    def __serve_client(self, client: socket.socket, address):
        ...
        try:
            ...
            while not self.__is_server_shutting_down and is_connection_alive:
                message_json = wait_for_response(client)
                ...
                if message_json["type"] == TransmissionType.PROCEDURE_REQUEST:
                    ...
                    with open(received_file_path, "wb") as processed_file:
                        file_received_fully = False
                        received_bytes = 0
                        previous_percents = 0
                        chunk_size_bytes = (
                            self.__configuration.chunk_size_kilobytes * 1024
                        )

                        file_start_time = time.time()
                        while not file_received_fully:
                            data = client.recv(chunk_size_bytes)
                            if len(data) == 0:
                                break
                            ...
                            received_percents = int(
                                100 / file_size_bytes * received_bytes
                            )

                            if (
                                received_percents % 10 == 0
                                and previous_percents != received_percents
                            ):
                                previous_percents = received_percents
                                logging.info(
                                    f"Receiving file from {address}, {received_percents}% ({received_bytes} / {file_size_bytes})."
                                )

                            processed_file.write(data)

                        file_upload_time = time.time() - file_start_time
                        logging.info(f"File upload time is {file_upload_time} seconds.")
    ...
    def set_working_directory(self, folder_path: str):
        self.__working_directory = folder_path
