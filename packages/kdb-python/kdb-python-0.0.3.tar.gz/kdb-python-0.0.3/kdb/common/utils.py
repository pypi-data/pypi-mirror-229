...
import kdb
from kdb import DATA_DIR, ROOT_DIR, DATA_REPORT_DIR
...
class FileUtil:
    ...
    def get_absolute_path(file_path):
        ...
        if os.path.isabs(file_path):
            ...
            root_file = os.path.join(ROOT_DIR, file_path)
            if os.path.isfile(root_file):
                return os.path.abspath(root_file)
            # return if file is found in data report dir
            data_report_file = os.path.join(DATA_REPORT_DIR, file_path)
    ...
    def write_as_text(data, file_path):
        ...
        if os.path.isabs(file_path):
            ...
            if not os.path.exists(DATA_REPORT_DIR):
                os.makedirs(DATA_REPORT_DIR, exist_ok=True)
            full_path = os.path.join(DATA_REPORT_DIR, file_path)
