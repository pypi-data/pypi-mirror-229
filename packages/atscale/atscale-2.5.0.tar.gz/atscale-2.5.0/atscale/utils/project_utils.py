...
def _update_query_dataset(
    ...
    Args:
        ...
        allow_aggregates(bool, optional): The setting deciding if aggregates are allowed to be built off this QDS. Defaults to None for no update.
    ...
    if len(dataset_ref) > 0:
        ...
        if create_hinted_aggregate is not None:
            dataset_properties["create-hinted-aggregate"] = True
        if allow_aggregates is not None:
            dataset_properties["allow-aggregates"] = allow_aggregates
    else:
        ...
        if create_hinted_aggregate is None:
            data_set_ref = templates.create_dataset_ref_dict(
                dataset_id, [], [], create_hinted_aggregate=True, allow_aggregates=allow_aggregates
            )
        else:
            data_set_ref = templates.create_dataset_ref_dict(
                dataset_id, [], [], allow_aggregates=allow_aggregates
            )
