...
class RocketBot(RocketChat):
    ...
    def get_updates(self):
        return self.session.subscriptions_get(timeout=60).json()['update']
                ...
                for result in updates:
                    try:
                        ...
                        if chat_type == "d":
                            ...
                            # Retrieve message history for group
                            messages = self.session.groups_history(room_id).json()['messages']
                        else:
