...
def get_dset(h5name, dsetname):
    ...
    # Ensuring dset is an instance of h5py.Dataset
    if not isinstance(dset, h5py.Dataset):
        raise TypeError(f"Expected a dataset at {dsetname}, but got {type(dset)} with keys: {list(dset.keys())}")
    return dset

def choose_parallel(args):
    """reads a frame and sends back a sparse frame"""
    h5name, address, frame_num = args
    dset = get_dset(h5name, address)
