...
class SemanticsRepository:
    ...
    def insert_decoded_transaction(self, decoded_tx: DecodedTransactionDB) -> None:
        self.database.insert_into_ethtx(decoded_tx)

    def get_decoded_transaction(
        self, tx_hash: str, chain_id: str
    ) -> Optional[DecodedTransactionDB]:
        raw_decoded_tx = self.database.get_from_ethtx(tx_hash, chain_id)
        if raw_decoded_tx:
            return DecodedTransactionDB(**raw_decoded_tx)
        return None
