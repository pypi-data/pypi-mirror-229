...
from typing import Union, Iterable, Dict, Any, Sequence, List
...
from deepfos.element.datatable import DataTableClickHouse, T_DictLike  # noqa
from deepfos.lib.constant import DECIMAL_COL, STRING_COL


class FactTable(DataTableClickHouse):
    """事实表"""
    ...
    @cached_property
    def dim_cols(self):
        return [col.name for col in self.meta.datatableColumn if col.whetherPrimary]
    ...
    def select_raw(
        ...
        .. admonition:: 示例
            ...
            .. code-block:: python
                ...
                tbl = FactTable(element_name='MainTable', path='/fcube')
                t = tbl.table
    ...
    def insert_df(
        ...
        return super().insert_df(dataframe, updatecol, chunksize, auto_fit)
    ...
    def _build_query_with_valid_data(self,
        ...
        sub_query = sub_query.groupby(*self.dim_cols)
