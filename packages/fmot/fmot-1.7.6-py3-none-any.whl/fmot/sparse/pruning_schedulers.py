...
class AbstractPruningSchedule:
    ...
    """
    def __init__(self, target, ramp_time, init_level=0.):
...
class QuadrantSinePruningSchedule(AbstractPruningSchedule):
    ...
    def __call__(self, epoch_idx):
        ...
        prune_amount = (self.target - self.init_level) * math.sin(math.pi/2*t) \
                        + self.init_level
...
class ExponentialPruningSchedule(AbstractPruningSchedule):
    ...
    def __call__(self, epoch_idx):
        ...
        t = epoch_idx / tau
        prune_amount = (self.target - self.init_level) * min(1., (1 - math.exp(-t)) / scaling_factor) \
                        + self.init_level
