...
class SpecialNodeRepeatFunction:
    ...
    @staticmethod
    def kernelize(parent: fqir.NodeProto, get_tensor: Callable, num_repeats: int
...
class TemporalUnfoldRepeatFunction(SpecialNodeRepeatFunction):
    ...
    def kernelize(self, parent: fqir.NodeProto, get_tensor: Callable, num_repeats: int
                  ) -> Tuple[List[fqir.NodeProto], List[fqir.NodeProto]]:
        nodes = []
        add_to_init = []
        ...
        if num_repeats == 1 and stride == 1:
            return [create_replica_node(parent, 
                        inputs={"x": get_tensor(x, 0)}, 
                        outputs=[get_tensor(parent.outputs[0], 0)])], []

        if dilation != 1:
            raise ValueError("Dilated TCN not supported at this moment when strides != 1 are used.")
...
class TransposedFoldRepeatFunction(SpecialNodeRepeatFunction):
    ...
    def kernelize(self, parent: fqir.NodeProto, get_tensor: Callable, num_repeats: int
        ...
        for rpt in range(num_repeats//stride):
            ...
            z_chunks = [create_replica_tensor(y, tag=f".chunk.{rpt}.{idx}") for idx in range(kernel_size)]
...
def perform_stride_optimization(graph: fqir.GraphProto):
    ...
    for node in init.nodes + arith.nodes:
        ...
        if len(input_multipliers) >= 1:
            ...
            assert is_compatible, f"Had incompatible multipliers on node: {node}\ninput multipliers: {input_multipliers}"
        else:
            multiplier = None
    ...
    def get_new_tensor(parent_tensor, idx):
        ...
        if isinstance(replicas, fqir.TensorProto):
            ...
            raise ValueError(f"Tried to get the {idx}-th replica of {parent_tensor}, only have {len(replicas)}.")
    ...
    ######
    new_arith = fqir.GraphProto(name=arith.name, unbind_dim=arith.unbind_dim, stack_dim=arith.stack_dim)
    ...
    for i, x_orig in enumerate(arith.inputs):
        ...
        if repeats > 1:
            x_stacked = create_replica_tensor(x_orig, tag="_stack", shape=(x_orig.shape[0] * repeats,))
    ...
    def get_new_node(node_orig, idx):
        try:
            inputs = {name: get_new_tensor(parent, idx) for name, parent in node_orig.inputs.items()}
            outputs = [get_new_tensor(parent, idx) for parent in node_orig.outputs]
            new_node = create_replica_node(node_orig, tag=f".{idx}", inputs=inputs, outputs=outputs)
        except:
    ...
    for block, repeat in zip(blocks, block_repeats):
        ...
        if len(block) == 1 and block[0].opname in REPEAT_FUNCTIONS:
            parent = block[0]
            kern, add_to_init = REPEAT_FUNCTIONS[parent.opname].kernelize(parent, get_new_tensor, repeat)
            for node in kern:
    ...
    for i, y_orig in enumerate(arith.outputs):
        ...
        if repeats is not None and repeats > 1:
            new_out = create_replica_tensor(y_orig, tag="", shape=(repeats * y_orig.shape[0],))
            cat_node = fqir.NodeProto(
