...
def upload_and_import_videos(
    ...
    # Collect media_specs
    shared_attributes["Sail Date"] = datetime.date.max
    shared_attributes["Land Date"] = datetime.date.min
    media_specs = []
    ...
    if isinstance(section, Section):
        new_section_name = build_section_name(
            shared_attributes["Vessel Name"],
            shared_attributes["Sail Date"],
            shared_attributes["Land Date"],
        )
        try:
    ...
    try:
        ...
        logger.warning("Did not update summary image with sail and land date")
...
def main(
    ...
    if video_list:
        ...
        created_ids, shared_attrs, new_section_name = upload_and_import_videos(
            tator_api, video_list, media_type, image_type_id, shared_attrs, file_list, section_name
        )
    else:
