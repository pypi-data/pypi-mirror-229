...
float distance_to_boundary(float const &position, float const &boundary_width,
                           float const &offset) {
  float decimals, intpart;
  decimals = modf(std::abs(position), &intpart);

  float remainder = fmod(decimals + offset, boundary_width);

  if (remainder <= boundary_width / 2.0f)
    return remainder;
  else
    return boundary_width - remainder;
}
...
py::array_t<float> matrix::compute_matrix_elements(
  ...
  for (int i = 0; i < n_z - 1; i++) {
    ...
    float const dyb_1 = distance_to_boundary(y[i], slice_thickness, 0.5);
    float const dxb_1 = distance_to_boundary(x[i], pixel_width, 0.0);
    float const dzb_1 = distance_to_boundary(z[i], pixel_width, 0.0);

    float const dyb_2 = distance_to_boundary(y[i + 1], slice_thickness, 0.5);
    float const dxb_2 = distance_to_boundary(x[i + 1], pixel_width, 0.0);
    float const dzb_2 = distance_to_boundary(z[i + 1], pixel_width, 0.0);
...
#pragma omp for
    for (unsigned int b = 0; b < block_size; b++) {
      ...
      for (unsigned int i = 0; i < sub_array_size - 1; i++) {
        ...
        float const dyb_2 =
            distance_to_boundary(y[i + 1], slice_thickness, 0.5);
        float const dxb_2 = distance_to_boundary(x[i + 1], pixel_width, 0.0);
        float const dzb_2 = distance_to_boundary(z[i + 1], pixel_width, 0.0);

        // bool debug_condition = (i_1 == 0 && j_1 == 362 && k_1 == 275) ||
        //                        (i_2 == 0 && j_2 == 362 && k_2 == 275) ||
        //                        i == 1670;
        // bool debug_condition = false;
        // bool debug_condition = b == 0 && i == 0;
        // bool debug_condition = b == 0 && i >= 1344 && i <= 1350;
        bool debug_condition = (i_1 == 1 && j_1 == 139 && k_1 == 269) ||
                               (i_2 == 1 && j_2 == 139 && k_2 == 269);

        if (debug_condition) {
          std::cout << std::endl;
          std::cout << "--------------" << std::endl;
          std::cout << "i: " << i << std::endl;
          std::cout << "pos1: (" << std::setprecision(8) << x[i] << ", "
                    << std::setprecision(8) << y[i] << ", "
                    << std::setprecision(8) << z[i] << ")" << std::endl;
          std::cout << "pos2: (" << std::setprecision(8) << x[i + 1] << ", "
                    << std::setprecision(8) << y[i + 1] << ", "
                    << std::setprecision(8) << z[i + 1] << ")" << std::endl;
          std::cout << "p1: (" << i_1 << ", " << j_1 << ", " << k_1 << ")"
                    << std::endl;
          std::cout << "p2: (" << i_2 << ", " << j_2 << ", " << k_2 << ")"
                    << std::endl;
          ...
          std::cout << "dyb_1: " << dyb_1 << std::endl;
          std::cout << "dxb_1: " << dxb_1 << std::endl;
          std::cout << "dzb_1: " << dzb_1 << std::endl;
          std::cout << "dyb_2: " << dyb_2 << std::endl;
          std::cout << "dxb_2: " << dxb_2 << std::endl;
          std::cout << "dzb_2: " << dzb_2 << std::endl;
        }
        ...
        if (!in_img_space(i_1, j_1, k_1, i_max, j_max, k_max) &&
          ...
          if (debug_condition) {
            std::cout << std::endl;
            std::cout << "NOT IN IMAGE SPACE" << std::endl;
          }
          continue;
        }
        ...
        if (di == 0 && dj == 0 && dk == 0 &&
          ...
          if (debug_condition) {
            std::cout << std::endl;
            std::cout << "NO BOUNDARIES CROSSED" << std::endl;
            std::cout << "val: " << val << std::endl;
            std::cout << "i_sparse: " << i_sparse << std::endl;
            std::cout << "j_sparse: " << j_sparse << std::endl;
          }
          continue;
        }
        ...
        if (debug_condition) {
          std::cout << std::endl;
          std::cout << "chord_x: " << chord_x << std::endl;
          std::cout << "chord_y: " << chord_y << std::endl;
          std::cout << "chord_z: " << chord_z << std::endl;
        }
        ...
        if (di != 0 && dj == 0 && dk == 0) {
          ...
          if (debug_condition) {
            std::cout << std::endl;
            std::cout << "DEBUG 1 BOUNDARY" << std::endl;
            std::cout << "i: " << i << std::endl;
            std::cout << "pos1: (" << x[i] << ", " << y[i] << ", " << z[i]
                      << ")" << std::endl;
            std::cout << "pos2: (" << x[i + 1] << ", " << y[i + 1] << ", "
                      << z[i + 1] << ")" << std::endl;
            std::cout << "p1: (" << i_1 << ", " << j_1 << ", " << k_1 << ")"
                      << std::endl;
            std::cout << "p2: (" << i_2 << ", " << j_2 << ", " << k_2 << ")"
        ...
        // crossing 2 boundaries
          ...
          if (debug_condition) {
            ...
            std::cout << std::endl;
            std::cout << "if flag: " << if_flag << std::endl;
            std::cout << "chord_in_pixel1: " << chord_in_pixel1 << std::endl;
            std::cout << "chord_in_pixel2: " << chord_in_pixel2 << std::endl;
            std::cout << "xb_to_zb: " << xb_to_zb << std::endl;
        ...
        if (in_img_space(i_1, j_1, k_1, i_max, j_max, k_max)) {
          ...
          if (debug_condition || chord_in_pixel1 < 0) {
            std::cout << std::endl;
            std::cout << "ADD CHORD PIXEL 1" << std::endl;
            std::cout << "i: " << i << std::endl;
            std::cout << "p1: (" << i_1 << ", " << j_1 << ", " << k_1 << ")"
                      << std::endl;
            std::cout << "p2: (" << i_2 << ", " << j_2 << ", " << k_2 << ")"
        ...
        if (in_img_space(i_2, j_2, k_2, i_max, j_max, k_max)) {
          ...
          if (debug_condition || chord_in_pixel2 < 0) {
            std::cout << std::endl;
            std::cout << "ADD CHORD PIXEL 2" << std::endl;
            std::cout << "i: " << i << std::endl;
            std::cout << "p1: (" << i_1 << ", " << j_1 << ", " << k_1 << ")"
                      << std::endl;
            std::cout << "p2: (" << i_2 << ", " << j_2 << ", " << k_2 << ")"
