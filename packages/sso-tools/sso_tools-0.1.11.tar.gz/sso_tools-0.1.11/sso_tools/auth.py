...
class OAuth2(mixins.VerifierMixin):
    ...
    def get_authorization_url(self, scope: str = "read", code_verifier: str = None, next_page: str = None):
        """인증 URL"""
        state = None
        if code_verifier:
    ...
    def _get_authorization_code(self, code: str, code_verifier: str = None):
        ...
        code_verifier = code_verifier if code_verifier else os.getenv("SECRET_KEY")
        ...
        data = {
            ...
            "code": unquote(code),
            "redirect_uri": self.redirect_uri,
            "code_verifier": unquote(code_verifier),
        }
        resp = requests.post(
            url,
            data=data,
