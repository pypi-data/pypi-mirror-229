...
from ethtx.models.decoded_model import DecodedTransactionDB

from .base import ISemanticsDatabase
from .const import MongoCollections
from ...utils.cache_tools import cache
from decimal import Decimal, getcontext, ROUND_DOWN, Inexact
...
class MongoSemanticsDatabase(ISemanticsDatabase):
    ...
    def convert_bigints_to_str_and_decimal_to_decimal128(self, data):
        # Set decimal context
        ctx = getcontext()
        ctx.prec = 34  # Maximum precision for Decimal128 is 34 digits
        ctx.rounding = ROUND_DOWN

        if isinstance(data, dict):
            for key, value in data.items():
                if isinstance(value, Decimal):
                    # Adjust the precision of the decimal value
                    value = ctx.create_decimal(value)
                    try:
                        data[key] = Decimal128(value)
                    except Inexact:
                        # Handle error: here we are just rounding the value
                        data[key] = Decimal128(
                            value.quantize(Decimal("0.00"))
                        )  # rounding to 2 decimals
                elif isinstance(value, int):
                    # Convert big integers to string
                    data[key] = str(value)
                elif isinstance(value, (dict, list)):
                    data[key] = self.convert_bigints_to_str_and_decimal_to_decimal128(
                        value
                    )
            ...
            for i, item in enumerate(data):
                if isinstance(item, Decimal):
                    # Adjust the precision of the decimal value
                    item = ctx.create_decimal(item)
                    try:
                        data[i] = Decimal128(item)
                    except Inexact:
                        # Handle error: here we are just rounding the value
                        data[i] = Decimal128(
                            item.quantize(Decimal("0.00"))
                        )  # rounding to 2 decimals
                elif isinstance(item, int):
                    # Convert big integers to string
                    data[i] = str(item)
                elif isinstance(item, (dict, list)):
                    data[i] = self.convert_bigints_to_str_and_decimal_to_decimal128(
                        item
                    )
        return data
    ...
    def get_from_ethtx(self, tx_hash: str, chain_id: str) -> Optional[dict]:
        # Assuming you have a collection named 'ethtx' in your MongoDB database
        result: DecodedTransactionDB = self._ethtx.find_one(
            {"txHash": tx_hash, "chainId": chain_id}
        )
        return result

    def insert_into_ethtx(self, decoded_tx: DecodedTransactionDB) -> None:
        # Inserting the provided decoded transaction into the '_ethtx' collection
        decoded_tx_dict = decoded_tx.dict()
        decoded_tx_dict = self.convert_bigints_to_str_and_decimal_to_decimal128(
            decoded_tx_dict
        )
        self._ethtx.insert_one(decoded_tx_dict)
