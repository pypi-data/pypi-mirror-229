...
def simulate_price(
    ...
    """
    This function returns the adjusted option price accounting for the curve effect.
    Provide original iv or original_price, original_spot and original_time_to_expiry.
    Provide new_spot or original_spot and movement.
    Provide original time to expiry along with either time_delta_minutes or time_delta (years).
    """
    flag = flag.lower()[0]
    symbol = "NIFTY" if symbol is None else symbol.upper()
    ...
    print(f"Transforming option price with new IV: {new_iv}. Old IV: {original_iv}")
