...
class WebM3u8ByReq(_Web):
    ...
    @staticmethod
    def _save_m3u8(m3u8_dir, req, m3u8_filter) -> M3U8:
        req_u = req.url
    ...
    @staticmethod
    def _save_ts(m3u8_dir, req, ts_filter) -> str:
        req_u = req.url
        res_body = req.response.body
    ...
    @staticmethod
    def _next_miss_ts(exist_ts, m3u8: M3U8) -> tuple[Segment, bool]:
        """
        miss ts信息，是否miss
        """
        if not exist_ts:
    ...
    def download_ts(self, m3u8_dir: str,
                    ...
                    play_xpath: str,
                    forward_xpath: str) -> Error:
        if not self.click(play_xpath).ok:
        ...
        while True:
            ...
            for req in self.driver.requests:
                if exist_num >= 10 and m3u8 is not None:
                    logger.info(f"download_ts {exist_num} >= 10, break")
                    break
