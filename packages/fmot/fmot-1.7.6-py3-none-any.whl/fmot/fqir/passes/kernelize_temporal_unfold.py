...
def get_unfold(node):
    ...
    if stride != 1:
        raise ValueError(f"Cannot kernelize temporal_unfold with stride != 1, had stride = {stride}")
    ...
    buffer = fqir.TensorProto(name=f'buffer_{x.name}', dtype=x.dtype, 
        shape=[buff_size])
    node = fqir.NodeProto(name=node.name, optype=fqir.registry_v1['temporal_unfold'],
        inputs={'x': x, 'buffer': buffer},
        constants=constants, outputs=node.outputs, sourceref=node.sourceref)
