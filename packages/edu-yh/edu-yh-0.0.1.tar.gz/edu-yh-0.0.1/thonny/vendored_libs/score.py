...
class scoreListApp:
    def __init__(self, root, score_list, type):
        self.root = root
    ...
    def create_exam_list(self):
        for i, exam in enumerate(self.score_list):
            ...
            time_label = tk.Label(self.root, text=exam['score'])
            time_label.grid(row=i + 1, column=2, padx=5, pady=5)

            time_label = tk.Label(self.root, text=exam['submit_count'])
            time_label.grid(row=i + 1, column=3, padx=10, pady=5)

            # start_button = tk.Button(self.root, text=self.type, command=lambda exam=exam: self.start_exam(exam))
    ...
    #     for item in detail:
    #         # print('后端返回', item)
    #         goto_list.append({"id": item[0], "type": item[2], "title": item[1], "fraction": item[3], "options": item[4],
    #                           "difficulty": item[5], "correct": self.find_correct(item[4])})
    #
    ...
    #     self.exam_details.geometry('600x600+' + str(int(ex_w / 2) - 300) + '+' + str(int(ex_h / 2) - 300))
    #     ExamDetailsPage(self.exam_details, questions=goto_list, exam_id=exam["id"], type=self.type, time=exam["time"])
    #
