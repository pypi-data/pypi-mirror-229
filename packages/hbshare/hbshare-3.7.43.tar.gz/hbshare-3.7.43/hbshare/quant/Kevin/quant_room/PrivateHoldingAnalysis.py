...
class HoldingAnalysor:
    ...
    @staticmethod
    def _load_benchmark_weight_series_tmp(date):
        bm_dict = {"000016": 0.202, "000300": 0.053, "000905": 0.293, "000852": 0.452}
        bm_list = []
        for key, value in bm_dict.items():
            sql_script = "SELECT * FROM hsjy_gg.SecuMain where SecuCategory = 4 and SecuCode = '{}'".format(key)
            res = hbs.db_data_query('readonly', sql_script)
            index_info = pd.DataFrame(res['data'])
            inner_code = index_info.set_index('SECUCODE').loc[key, 'INNERCODE']
            ...
            weight_df = data.rename(
                columns={"SECUCODE": "consTickerSymbol", "ENDDATE": "effDate", "WEIGHT": "weight"})
            weight_df = weight_df.set_index('consTickerSymbol')['weight']
            weight_df *= value
            bm_list.append(weight_df.to_frame(key))

        a = pd.concat(bm_list, axis=1).fillna(0.).sum(axis=1)
        a /= a.sum()
        a.name = 'weight'
    ...
    def _load_data(self):
        ...
        if self.benchmark_id in ['8841425.WI']:
            ...
            # benchmark_weight_series = self._load_benchmark_weight_series_tmp(shift_date)
...
if __name__ == '__main__':
    tmp = HoldingAnalysor(
        "宽德九臻", "20230630", "000905").get_construct_result(isPlot=False)
    ...
    name_list = ['概率1000指增1号', '稳博1000指数增强1号', '磐松1000指增', '凡二量化选股3号1期']
    HoldingCompare('20230331', '000852', name_list).run()
    # concept_compare("20230331", name_list)
    # small_cap_stocks_counts("20230430", name_list)
