...
from .dimension import Strategy
...
class SmartList(ElementBase):
    ...
    def __init__(
        ...
        self.__item_memo = None
        self.__ud_memo = None

    @property
    def _item_memo(self) -> Dict[str, SmartListItem]:
        if self.__item_memo is None:
            self.__item_memo = {
                mbr.subjectValue: mbr
                for mbr in self.meta.smartList
            }

        return self.__item_memo

    @property
    def _ud_memo(self) -> Dict[str, SmartListUd]:
        if self.__ud_memo is None:
            self.__ud_memo = {
                ud.udName: ud
                for ud in self.meta.smartListUd
            }

        return self.__ud_memo

    @cached_property
    def meta(self) -> SmartListDTO:
        """值列表的元数据信息"""
        return self.api.sml.data(
    ...
    def save(self):
        ...
        return self._save_impl(payload)

    def _save_impl(self, payload: SmartListDTO):
        self.api.sml.update(payload)
        # 使memo的缓存失效
        self.__item_memo = None
        self.__ud_memo = None
    ...
    def load_dataframe(
        ...
        **langugage_keys: str
        ...
        valid_columns = set(SmartListItem.__fields__).intersection(df.columns)
        # check exist data
    ...
    def __getitem__(self, item) -> SmartListItem:
        return self.items[item]

    def __len__(self) -> int:
        return len(self._item_memo)

    def __iter__(self) -> Iterable[SmartListItem]:
