...
class ABIBalancesDecoder(ABISubmoduleAbc):
    ...
    def decode(self, transfers: List[DecodedTransfer]) -> List:
        ...
        for holder_address in balance_holders:
            tokens_list = []  # Create an empty list to store TokenInfo instances
            ...
            for token_address in balance_sheet[holder_address]:
                if balance_sheet[holder_address][token_address]:
                    ...
                    token_info = TokenInfo(
                        ...
                        balance=str(
                            balance_sheet[holder_address][token_address]
                        ),  # Convert balance to string as per TokenInfo model definition
                    )
                    tokens_list.append(
                        token_info
                    )  # Append the TokenInfo instance to the list

            if tokens_list:  # Check if the list has any items
                holder_name = balance_holders[holder_address]
                balances.append(
                    DecodedBalance(
                        holder=AddressInfo(address=holder_address, name=holder_name),
                        tokens=tokens_list,  # Assign the list of TokenInfo instances to the tokens attribute
                    )
                )
