...
PYBIND11_MODULE(DEME, obj) {
    ...
    py::class_<deme::DEMTracker, std::shared_ptr<deme::DEMTracker>>(obj, "Tracker")
        ...
        .def("GetContactForces", static_cast<std::vector<std::vector<std::vector<float>>> (deme::DEMTracker::*)(std::vector<std::vector<float>>&,
                                    std::vector<std::vector<float>>&, size_t)>(&deme::DEMTracker::GetContactForces))
        .def("GetContactAcc",
             static_cast<std::vector<float> (deme::DEMTracker::*)(size_t)>(&deme::DEMTracker::GetContactAcc), py::arg("offset") = 0)
        .def("SetGeometryWildcardValue", static_cast<void (deme::DEMTracker::*)(const std::string&, float, size_t)>(&deme::DEMTracker::SetGeometryWildcardValue))
        .def("SetGeometryWildcardValue", static_cast<void (deme::DEMTracker::*)(const std::string&, const std::vector<float>&)>(&deme::DEMTracker::SetGeometryWildcardValue))
    ...
    py::class_<deme::DEMSolver>(obj, "DEMSolver")
        ...
        .def("UpdateStepSize", &deme::DEMSolver::UpdateStepSize, "Update the time step size. Used after system initialization.", py::arg("ts") = -1.0)
        .def("SetNoForceRecord", &deme::DEMSolver::SetNoForceRecord, "Instruct the solver that there is no need to record the contact force (and contact point location etc.) in an array.", py::arg("flag") = true)
