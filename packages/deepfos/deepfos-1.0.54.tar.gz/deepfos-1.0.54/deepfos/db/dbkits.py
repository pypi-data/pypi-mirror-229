...
from deepfos.lib.asynchronous import evloop, cache_async, MAIN_TID
    ...
    async def parse(
        ...
        for table in tbl_placeholders:
            if table not in table_info:
                # todo 使用异步请求
                element = self.datatable_cls(table).element_info
...
def synchronize(method):
    ...
    evloop_id = evloop.thread.id
    main_id = (MAIN_TID, evloop_id)

    @wraps(method)
    def wrapper(self, *args, **kwargs):
        count_memo = self.__dict__.setdefault('__nested_sync__', {})
        tid = threading.get_ident()
        key = main_id if tid in main_id else tid

        nested_count = count_memo.setdefault(key, 0)
        count_memo[key] = nested_count + 1
...
class DataframeSQLConvertor:
    ...
    def build_sql(
        ...
        **opts
        ...
        update_str = ','.join([f"`{x}`=VALUES(`{x}`)" for x in updatecol])
        if not update_str:
            return f'INSERT INTO {tablename} ({columns}) VALUES {values}'

        return f'INSERT INTO {tablename} ({columns}) ' \
               f'VALUES {values} ' \
