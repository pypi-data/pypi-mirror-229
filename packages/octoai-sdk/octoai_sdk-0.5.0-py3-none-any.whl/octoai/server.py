...
class Server:
    ...
    def is_store_assets_needed(self) -> bool:
        """Detect if service requires to run store_assets."""
        return not hasattr(self.service.store_assets, STORE_ASSETS_NOT_OVERRIDDEN)
...
@server.command()
@click.pass_context
@click.option("--needed", default=False)
def store_assets(ctx, needed):
    """Run the store_assets code for the given model."""
    _LOG.info("store_assets")
    server: Server = _load_server(ctx, async_enable=False)
    if needed:
        output = {"store_assets_needed": True}
