...
class SegwaySimulationCfg:
    timestep: float = 1e-3
    initial_conditions_cfg: SegwayInitialConditionsCfg = field(
        default_factory=SegwayInitialConditionsCfg
    )
    view_cfg: ViewCfg = field(default_factory=ViewCfg)
...
class FullSimulationCfg:
    initial_conditions_cfg: FullInitialConditionsCfg = field(
        ...
        if self.P is None or self.I is None or self.D is None:
            ...
            if self.control_frequency == 200:
                ...
                raise NotImplementedError(
                    "HipController only supports 50 or 1000 Hz with unset PID values!"
                )
