...
class HTTP_Message_Factory:
    def __init__(self, connection:socket.socket,get_handler:dict, post_handler:dict, error_handler:dict) -> None:
        try:
...
class HTTP_Parser():
    ...
    def parse_first_line(self) -> tuple:
        ...
        first_line_tokens = [x for x in re.split('\s+',first_line) if x]
    ...
    def parse(self) -> None:
        ...
        try:
            ...
            path_tokens = re.split('\?(?!.+\/)',self.http_request_path)
            ...
            #print(self.http_message_factory.response_message,'\n')

        except Exception as e:
            trace = traceback.print_exc()
            log(f'[PARSE] {trace}',log_lvl='debug')
            #print('[PARSE] ERROR')