...
class input_Facture:
    def __init__(self, start,end,depassement_PS_pointe=0, depassement_PS_HPH=0, depassement_PS_HCH=0, depassement_PS_HPB=0, depassement_PS_HCB=0, heures_depassement=0,kWh_pointe=0,kWh_HPH=0,
        ...
        self.start = parse(start).date()
        self.end = parse(end).date()
...
class input_Contrat:
    def __init__(self, depassement_PS_pointe=None, domaine_tension=None, heures_depassement=None, PS_pointe=None, PS_HPH=None, PS_HCH=None, PS_HPB=None, PS_HCB=None, version_utilisation=None):
...
class TurpeCalculator:
    def __init__(self, contrat,facture, coefficients_file="coefficients.json"):
        ...
        self.euro_an_CACS=0 # intégrer après dans le calcul
        self.euro_an_CR=0 # intégrer après dans le calcul
        self.euro_an_CER=0 # intégrer après dans le calcul
        self.euro_an_CI=0 # intégrer après dans le calcul
...
# Example usage:
    def calculate_taxes_contrib(self):
    ...
    def calculate_turpe(self):
        ...
        if self.contrat.domaine_tension == "HTA":
            self.euro_an_CS_fixe = self.calculate_euro_an_CS_fixe(coeff, self.contrat,self.facture)
            self.euro_CS_variable,self.euro_CS_variable_pointe,self.euro_CS_variable_HPH, self.euro_CS_variable_HCH,  self.euro_CS_variable_HPB,   self.euro_CS_variable_HCB = self.calculate_euro_CS_variable(coeff, self.facture)
            ...
            self.euro_an_CS_fixe = self.calculate_euro_an_CS_fixe(coeff, self.contrat,self.facture)
            euro_CS_vars = self.calculate_euro_CS_variable(coeff, self.contrat)
            self.euro_CS_variable_HPH, self.euro_CS_variable_HCH, self.euro_CS_variable_HPB, self.euro_CS_variable_HCB = euro_CS_vars
            self.euro_CS_variable = sum(euro_CS_vars)
...
######################Fonctions pour le calcul du TURPE#######################################""
    def calculate_euro_an_CS_fixe(self, coeff, contrat,facture):
        b = coeff.get("b")

       
        euro_an_CS_fixe = round(
        ...
        print("euro_an_CS_fixe =",euro_an_CS_fixe," = ",b[0],"*", contrat.PS_pointe,"+",b[1], "* (",contrat.PS_HPH, "-", contrat.PS_pointe,")+",b[2], 
              "* (",contrat.PS_HCH, "-" ,contrat.PS_HPH,") +", b[3], "* (",contrat.PS_HPB, "-" ,contrat.PS_HCH,") +",  b[4], "* (",contrat.PS_HCB, "-" ,contrat.PS_HPB,")"         
            )
        return euro_an_CS_fixe
    
    # def calculate_euro_CS_fixe(self, euro_an_CS_fixe, facture):
    #     if isinstance(facture.start, str):
    #         facture.start = parse(facture.start).date()
    #     if isinstance(facture.end, str):
    #         facture.end = parse(facture.end).date()
    #     try:
    #         self.nb_jour = (facture.end - facture.start).days + 1  # Calculate the number of days between start and end
    #     except:
    #         self.nb_jour = 0.0

    #     print("self.nb_jour",self.nb_jour)

    #     self.euro_CS_fixe = round(euro_an_CS_fixe * (self.nb_jour / 365.0), 2)
    #     print("self.euro_CS_fixe",self.euro_CS_fixe)
    #     return self.euro_CS_fixe
