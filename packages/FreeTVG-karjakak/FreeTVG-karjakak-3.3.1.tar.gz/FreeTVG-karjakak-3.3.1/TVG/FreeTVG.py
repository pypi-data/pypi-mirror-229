...
from .utility import convhtml
from .utility import composemail, wrwords
...
class TreeViewGui:
    ...
    def _fold_restruct(self, size: int, pos: int, *, row: int = None):
    ...
    def _sumtot_restruct(self, plus: bool = True, ids: list = None):
        st = ParseData(self.filename, stackids=ids if ids else [], data=None)
            ...
            match len(sa) > 0:
                ...
                case True:
                    if not hasattr(self, "sumtot"):
                        ...
                        pd = ParseData(self.filename, data=None)
                        pd.record_stack()
                        self.deltots(ids=sa.getidx())
                        self.__setattr__("sumtot", True)
                        sa.sumway()
                        pd.record_stack()
                        self._sumtot_restruct()
    ...
    def deltots(self, ids: list = None):
        """Deleting all Totals"""
            ...
            if self.nonetype():
                with SumAll(self.filename, sig="+") as sal:
                    if hasattr(self, "sumtot") and self.sumtot:
                        self.__delattr__("sumtot")
                        sal.del_total()
                        self._sumtot_restruct(False, ids=ids if ids else None)
                        self.spaces()
                    else:
                        messagebox.showinfo(
