import httpx, asyncio
from .enums import API, ExceptionMsg, Joker, translator
from typing import Any, Awaitable
from .exceptions import *

class Number:
    """
    Represents a phone number.
    
        Args:
            number (str): The phone number as a string, optionally including the '+' sign.

        Attributes:
            number (str): The formatted phone number without the '+' sign.

        Example:
            number = Number("+1234567890")
            print(number.number)  # Output: "1234567890"

        Used in SDK:
            class.func( number=Number() )
    """
    def __init__(self, number: str) -> None:
        """
        Initialize a Number instance.

            Args:
                number (str): The phone number as a string.
        """
        self.number: str = number.strip("+")


class SDK:
    def __init__(
            self, 
            *args, 
            api_key: str = False, 
            **kwargs
            ) -> None:
        self.api_key: str = api_key
    
    """
    Run an asynchronous function using asyncio's event loop.
        This function takes an asynchronous function as an argument and runs it within asyncio's event loop.

        Parameters:
        -----------
        func : Awaitable[None]
            The asynchronous function to be executed.

        Returns:
        --------
        None

        Examples:
        ---------
        JokerINIT.run_async(JokerINIT(api_key="test"))

        --> run_async(async_function())
        ---> Preference Any[asyncio.run, run_async]
    """
    def run_async(
            func: Awaitable[None], *args, **kwargs
            ) -> None:
        return asyncio.run(func)

    """
    Initiates an outbound call using the JokerAPI.co voice API.

    This method creates and handles API requests to the 'voice/v1/dial' endpoint via JokerAPI.co.

    Parameters:
    -----------
    dial_to : Number
        The destination phone number to dial.

    dial_from : Number
        The caller ID to display when dialing the destination number.

    callback_url : str, required
        The URL where status change notifications are sent, corresponding to the user's API callback server.

    **kwargs : dict, optional
        Additional optional parameters:

        - request_method (str):
            The HTTP method for the API request (default is 'GET', supports 'GET' or 'POST').
        
        - proxy (str or dict):
            Proxy configuration for the API request.
            Examples: 'all', 'all://*api.jokerapi.co', 'http://', 'https://',
            {"http": "USERNAME:PASSWORD@DOMAIN:PORT", "https": "USERNAME:PASSWORD@DOMAIN:PORT"}.

    Returns:
    --------
    str
        The Session ID of the channel, which uniquely identifies the initiated call.

    Raises:
    -------
        REQUIREDAPIKEY
            If the API key is missing.

        INVALIDAPIKEY
            If the API key is invalid.

        INVALIDCOUNTRY
            If the dial_to or dial_from phone numbers aren't whitelisted.

        INVALIDPARAM
            If the keyword arguments aren't populated correctly.

    Example:
    --------
    To initiate an outbound call with the following parameters:
        - dial_to: Number("+1234567890")
        - dial_from: Number("+9876543210")
        - callback_url: "https://yourcallbackserver.com/callback"
        - request_method: 'POST'
        - proxy: {"http": "USERNAME:PASSWORD@DOMAIN:PORT", "https": "USERNAME:PASSWORD@DOMAIN:PORT"}

    The `call_sid` will contain the Session ID of the initiated call.

    Note:
    -----
        - Ensure that your JokerAPI.co API key is set using the `api_key` attribute of the `Object` instance before calling this method.
        - Make sure that the provided phone numbers are correctly formatted using the `Number` class.
    """
    async def dial(self,
                   dial_to: Number,
                   dial_from: Number,
                   callback_url: str = "https://0.0.0.0/*",
                   **kwargs
                   ) -> str:
        """
        Initiates an outbound call using the JokerAPI.co voice API.

        This method creates and handles API requests to the 'voice/v1/dial' endpoint via JokerAPI.co.

        Args:
            dial_to (Number): The destination phone number to dial.
            dial_from (Number): The caller ID to display when dialing the destination number.
            callback_url (str, optional): The URL where status change notifications are sent,
                corresponding to the user's API callback server. Default is "https://0.0.0.0/*".
            **kwargs (dict, optional): Additional optional parameters.

        Keyword Args:
            request_method (str): The HTTP method for the API request
                (default is 'GET', supports 'GET' or 'POST').
            proxy (str or dict): Proxy configuration for the API request.
                Examples: 'all', 'all://*api.jokerapi.co', 'http://', 'https://',
                {"http": "USERNAME:PASSWORD@DOMAIN:PORT", "https": "USERNAME:PASSWORD@DOMAIN:PORT"}.

        Returns:
            str: The Session ID of the channel, which uniquely identifies the initiated call.

        Raises:
            REQUIREDAPIKEY: If the API key is missing.
            INVALIDAPIKEY: If the API key is invalid.
            INVALIDCOUNTRY: If the dial_to or dial_from phone numbers aren't whitelisted.
            INVALIDPARAM: If the keyword arguments aren't populated correctly.

        Note:
            - Ensure that your JokerAPI.co API key is set using the `api_key` attribute of the `YourClass` instance before calling this method.
            - Make sure that the provided phone numbers are correctly formatted using the `Number` class.
        """
        if not self.api_key:
            raise REQUIREDAPIKEY(ExceptionMsg.REQUIRED_KEY)

        response = httpx.request(
            "GET" if not kwargs.get("request_method", False) else kwargs['request_method'] if kwargs['request_method'] in ["GET", "POST"] else "GET",
            API.__translator__("DIAL", API.URL, {"key": self.api_key, "to": dial_to.number, "from": dial_from.number, "url": callback_url}),
            proxies=None if not kwargs.get("proxy", False) else kwargs['proxy'] if isinstance(kwargs['proxy'], dict) else None
        ).json()

        if response.get("context", False) and not response['context'] == Joker.SUCCESS:
            raise translator[response['context']][0](translator[response['context']][1])
        else:
            return response['callsid']
