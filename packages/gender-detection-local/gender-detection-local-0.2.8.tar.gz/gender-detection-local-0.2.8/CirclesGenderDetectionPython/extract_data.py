...
import sys
sys.path.append('.')
from CirclesGenderDetectionPython.image_manipulation import image_2_grayscale

DATA_EXTRACTOR_COMPONET_ID = 203
DATA_EXTRACTOR_COMPONENT_NAME = "CirclesGenderDetectionPython/extract_data.py"
...
logger=Logger()
logger.init("Start logging in {DATA_EXTRACTOR_COMPONENT_NAME}", object=object_to_insert)


def process_dataset(dataset_path:str, num_classes:int, classifier:Callable[[str], int], process_image:Callable[[str], np.ndarray]=image_2_grayscale,test_size:int=0.2, random_state:int=42)->None:
    """
    ...
    Inputs:
        ...
        gender_classifier(str -> int): a function that gets a path to an image and returns the gender in binary (0 is male)
        process_image(str -> np.ndarray): a function that gets a path to a image and processes it to a numpy array (default is image_2_grayscale)
    ...
    Summary:
        ...
        Tensorflow requires us to submit inputs in the form of np arrays, so in order to train a model on our data
        we need to convert the images to np arrays and save them along with their labels.
    ...
    logger.start(PROCESS_DATASET_FUNCTION_NAME, object= {"dataset_path":dataset_path,"test_size":test_size,"random_state":random_state})
