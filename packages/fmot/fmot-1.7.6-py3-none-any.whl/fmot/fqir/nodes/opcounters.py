...
class OpCounter:
    ...
    """
    def count(self, inputs, outputs, constants, st_ld_pessimism, input_length=1,
        output_length=1):
        """
    ...
    def __call__(self, inputs, outputs, constants, st_ld_pessimism=None,
        debug=False):
        ...
        output = self.count(inputs, outputs, constants, st_ld_pessimism, input_length,
            output_length)
...
class VVCounter(OpCounter):
    ...
    def count(self, inputs, outputs, constants, st_ld_pessimism, input_length,
        output_length):
        ...
        load_first = OpCount(
            ...
            acc_write_numel=numel) * st_ld_pessimism
        ...
        write_output = OpCount(
            acc_read_numel=numel,
            mem_write_bytes=z_bytes) * st_ld_pessimism

        # multiply by input_length
        if input_length != output_length:
            warnings.warn(f'input_length != output_length ({input_length} != {output_length})')
...
class VCounter(OpCounter):
    ...
    def count(self, inputs, outputs, constants, st_ld_pessimism, input_length,
        ...
        write_y = OpCount(
            acc_read_numel=numel,
            mem_write_bytes=y_bytes) * st_ld_pessimism
...
class MatmulCounter(OpCounter):
    @staticmethod
    def matmul_count(matrix, vector, output, bias=None, st_ld_pessimism=0, mat_first=True):
        """
        Args:
            matrix: TensorProto
            vector: TensorProto
        ...
        matrix_density = matrix.nbytes(nonzero_only=True)/matrix.nbytes(nonzero_only=False)

        if vector_density == 1 and matrix_density == 1:
            return MatmulCounter.wvprod_count(matrix, vector, output, bias,
                st_ld_pessimism)
        elif vector_density == 1 and matrix_density != 1:
            return MatmulCounter.uvprod_count(matrix, vector, output, bias,
                st_ld_pessimism)
        else:
            return MatmulCounter.usprod_count(matrix, vector, output, bias,
                st_ld_pessimism, mat_first=mat_first)
    ...
    def wvprod_count(matrix, vector, output, bias=None, st_ld_pessimism=0):
        ...
        output_write = OpCount(
            acc_read_numel=output.numel(),
            mem_write_bytes=output.nbytes()) * st_ld_pessimism

        return acc_init + matmul + output_write

    @staticmethod
    def usprod_count(matrix, vector, output, bias=None, st_ld_pessimism=0, mat_first=True):
        """
        ...
        matmul = OpCount(
            mem_read_bytes=macs*matrix_bytes_per_el + vector.nbytes(nonzero_only=True),
            acc_read_numel=macs,
    ...
    def uvprod_count(matrix, vector, output, bias=None, st_ld_pessimism=0):
        ...
        pos_relu = (
            # load vector
            OpCount(
                mem_read_bytes=vector_bytes,
                acc_write_numel=vector_numel) * st_ld_pessimism +
            # perform relu op
            OpCount(
                acc_read_numel=vector_numel,
                acc_write_numel=vector_numel) + 
            # store
            OpCount(
                acc_read_numel=vector_numel,
                mem_write_bytes=0.5*vector_bytes)
            )
        ...
        acc_init = MatmulCounter.acc_init(output, bias) + MatmulCounter.acc_init(output, None)
        ...
        emacs = matrix.numel(nonzero_only=False)
        matmul = OpCount(
            mem_read_bytes=vector.nbytes(nonzero_only=False) + matrix.nbytes(nonzero_only=True),
            acc_read_numel=macs,
            acc_write_numel=macs,
            macs=macs,
            emacs=emacs)
        ...
        # writing result to memory
        output_write = OpCount(
            acc_read_numel=output.nbytes() * st_ld_pessimism)
...
class ConvCounter(OpCounter):
    @staticmethod
    def matmul_count(x, weight, output, input_length, output_length, constants,
        bias=None, st_ld_pessimism=0):
        ...
        if x.density_per_element is None:
            ...
            density_per_element = np.concatenate([x.density_per_element] * np.product(kernel_size))
        ...
        matmul = MatmulCounter.matmul_count(
            matrix_tproto, stacked_vec_tproto, output, bias, st_ld_pessimism, mat_first=True)

        return buffer_rotation * input_length + matmul * output_length

    @staticmethod
    def depthwise_count(x, weight, output, input_length, output_length, constants,
        bias=None, st_ld_pessimism=0):
        ...
        return buffer_rotation * input_length + (dw_product + output_write) * output_length
    ...
    def count(self, inputs, outputs, constants, st_ld_pessimism, input_length,
        ...
        if constants['groups'] == 1:
            return self.matmul_count(x, weight, output, input_length, output_length,
                constants, bias, st_ld_pessimism)
        elif constants['groups'] == weight.shape[0]:
            return self.depthwise_count(x, weight, output, input_length, output_length,
                constants, bias, st_ld_pessimism)
        else:
            assert False, f'input_length={input_length} was expected to be tuple or list'
...
class CopyCounter(OpCounter):
    def count(self, inputs, outputs, constants, st_ld_pessimism, input_length,
        ...
        for x in inputs.values():
            mem_reads += OpCount(
                mem_read_bytes=x.nbytes(),
                acc_write_numel=x.numel())
            N_read += 1
