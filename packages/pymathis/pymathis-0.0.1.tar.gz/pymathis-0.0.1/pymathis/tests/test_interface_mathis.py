...
def CheckRes(Probe_ID,PassiveCtrl_ID):
    with open('ProbeAndCtrlTEST.pickle', 'rb') as f:
        TesRes = pickle.load(f)
    for key in Probe_ID:
        check = [val - TesRes['Probe'][key][ii] for ii, val in enumerate(Probe_ID[key])]
        if sum(check)>0:
            return False
    for key in PassiveCtrl_ID:
        check = [val-TesRes['PassiveCtrl'][key][ii] for ii, val in enumerate(PassiveCtrl_ID[key])]
        if sum(check)>0:
            return False
    return True
...
if __name__ == '__main__':
    ...
    if CheckRes(Probe_ID, PassiveCtrl_ID):
