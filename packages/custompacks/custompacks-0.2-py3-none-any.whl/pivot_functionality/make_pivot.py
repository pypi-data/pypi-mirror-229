...
class MakePivots:
    def __init__(self, datasource_sheet: str = None):
        self.datasource_sheet = datasource_sheet
        wb = xw.Book.caller()
        self.ws = wb.sheets[self.datasource_sheet]
        self.wkb = wb
    ...
    def create_pt_table(self, pt_sheet_name: str = None, pt_name: str = None, exclude_items: tuple = None):
        target_rng = self.wkb.sheets[self.datasource_sheet].range("c11").current_region.api
        pt_cache = self.wkb.api.PivotCaches().Create(1,
                                                     self.wkb.sheets[self.datasource_sheet].api.Range(
                                                         target_rng.Address))
