...
class Dataset():
    ...
    def __init__(
        ...
        if not u_test:
            ...
            if self.creds and self.dataset_id:
                self.create_dataset_creds(self.dataset_id)
    ...
    def create_dataset_creds(self, dataset_id):
        if not dataset_id:
    ...
    def generate_embedding(self, data:pd.DataFrame, schema:RagaSchema, model:str, embedding_col_name:str):
        assert isinstance(data, pd.DataFrame) and not data.empty, "data is require and must be pandas data frame."
        assert isinstance(schema, RagaSchema), "schema is require and must be instance of RagaSchema."
        assert isinstance(model, str) and model, "model is require and must be str."
        assert isinstance(embedding_col_name, str) and embedding_col_name, "embedding_col_name is require and must be str."
