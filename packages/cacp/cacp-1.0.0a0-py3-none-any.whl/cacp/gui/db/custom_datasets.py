...
CUSTOM_DATASETS_DB = TinyDB(DB_PATH / "custom_datasets.json")
...
def _remove_code(custom_dataset_id: int):
    _code_path(custom_dataset_id).unlink(missing_ok=False)


def get_all_custom_datasets() -> List[CustomDataset]:
    return [_convert_from_document_to_custom_dataset(e) for e in CUSTOM_DATASETS_DB.all()]
