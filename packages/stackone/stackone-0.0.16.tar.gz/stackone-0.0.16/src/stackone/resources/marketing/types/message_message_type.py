...
class MessageMessageType(str, enum.Enum):
    ...
    def visit(
        ...
        if self is MessageMessageType.EMAIL:
            return email()
        if self is MessageMessageType.SMS:
            return sms()
        if self is MessageMessageType.WEB_PUSH:
            return web_push()
        if self is MessageMessageType.IOS_PUSH:
            return ios_push()
        if self is MessageMessageType.ANDROID_PUSH:
            return android_push()
        if self is MessageMessageType.APP_PUSH:
            return app_push()
        if self is MessageMessageType.OMNI_CHANNEL:
            return omni_channel()
        if self is MessageMessageType.UNKNOWN:
            return unknown()
        if self is MessageMessageType.UNMAPPED_VALUE:
