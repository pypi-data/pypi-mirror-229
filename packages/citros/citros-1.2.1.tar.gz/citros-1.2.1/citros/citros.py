...
class Citros:
    ...
    def _copy_examples_and_markdown(self):
        ...
        with self.utils.get_data_file_path('markdown', 'readme_functions.md') as md_file_path:
            destination = self.PARAMS_DIR
            shutil.copy2(md_file_path, destination)
    ...
    def git_pull(self, branch='master'):
        try:
            ...
            self.print(f"An error occurred while attempting to pull: {e}", color='red')
            self.log.error(f"pull error: {e}")
        except Exception as ex:
            self.handle_exceptions(ex, exit=True)
