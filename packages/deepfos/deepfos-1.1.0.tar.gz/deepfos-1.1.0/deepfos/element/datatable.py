...
from deepfos.db.dbkits import null, Skip, DataframeSQLConvertor
...
class _OracleDFConvertor(_DataTableDFConvertor):
    def build_sql(
        ...
        return OracleDFSQLConvertor(self.quote_char).build_sql(columns, values_in_line, tablename, updatecol)
...
class _ClickHouseDFConvertor(_DataTableDFConvertor):
    def build_sql(
        ...
        return ClickHouseConvertor(self.quote_char).build_sql(columns, values_in_line, tablename, updatecol)
...
class _SQLServerDFConvertor(_DataTableDFConvertor):
    def build_sql(
        ...
        return SQLServerDFSQLConvertor(self.quote_char).build_sql(columns, values_in_line, tablename, updatecol)
...
class _DeepEngineDFConvertor(_DataTableDFConvertor):
    def build_sql(
        ...
        return DeepEngineDFSQLConvertor(self.quote_char).build_sql(columns, values_in_line, tablename, updatecol)
...
class _PostgreSQLDFConvertor(PostgreSQLConvertor):
    def convert(
        ...
        return self.build_sql(columns, data_series, tablename, updatecol, conflict_target)
...
class AsyncDataTableMySQL(ElementBase):
    ...
    _txn_ = ContextVar('TXN', default=_TxnConfig())
    #: 事务执行结果
    ...
    def _txn_support_(self):
        return self._txn_.get().txn_support

    @_txn_support_.setter
    def _txn_support_(self, val):
        self._txn_.get().txn_support = val
    ...
    def structure(self) -> TableStructure:
        ...
        在查询和保存时对数据做类型转化的预处理。
        """
        return TableStructure(self.meta.datatableColumn)
    ...
    async def _run_sql(self, sql: str) -> Optional[CustomSqlRespDTO]:
        ...
        if ctx.get().in_txn[-1] and self._txn_support_:
            ...
            if ctx.get().async_api is None:
                ctx.get().async_api = self.async_api
            return
...
class AsyncDataTablePostgreSQL(AsyncDataTableMySQL):
    ...
    async def insert_df(
        ...
        sqls = self.convertor.iter_sql(dataframe, self.table_name, updatecol, chunksize, conflict_target)
        return await self._maybe_submit_in_txn(sqls)
