...
from cyberfusion.ClusterSupport.clusters import Cluster
...
def get_borg_server_cluster(
    ...
    for cluster in get_support().clusters:
        if get_support().GROUP_CLUSTER_BORG_SERVER not in cluster.groups:
            continue
...
def create_unix_user_archive(name: str, borg_repository_name: str) -> None:
    ...
    console.print("Creating Borg archive...\n")
    console.print(get_support().get_table(objs=[task_collection]))
...
def prune_repository(name: str) -> None:
    ...
    console.print("Pruning Borg repository...\n")
    console.print(get_support().get_table(objs=[task_collection]))
    wait_for_task(
        task_collection_uuid=task_collection.uuid, show_skip_message=True
    )
...
def check_repository(name: str) -> None:
    ...
    console.print("Checking Borg repository...\n")
    console.print(get_support().get_table(objs=[task_collection]))
    wait_for_task(
        task_collection_uuid=task_collection.uuid, show_skip_message=True
    )
...
def download_archive(
    ...
    console.print("Creating download link...\n")
    console.print(get_support().get_table(objs=[task_collection]))
    wait_for_task(
        task_collection_uuid=task_collection.uuid, show_skip_message=False
    )
...
def restore_archive(
    ...
    console.print("Restoring Borg archive...\n")
    console.print(get_support().get_table(objs=[task_collection]))
    wait_for_task(
        task_collection_uuid=task_collection.uuid, show_skip_message=True
    )
