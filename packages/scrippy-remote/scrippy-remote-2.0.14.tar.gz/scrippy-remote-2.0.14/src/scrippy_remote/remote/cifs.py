...
class Cifs:
  ...
  def get_file(self, remote_filepath, local_filepath):
    logging.debug(f"[+] Downloading file {remote_filepath} in {local_filepath}")
  ...
  def put_file(self, local_filepath, remote_filepath):
    logging.debug(f"[+] Uploading file {local_filepath} to {remote_filepath}")
...
class _CifsFileWritter:
  ...
  def write_file(self):
    logging.debug(f"[+] Writing file {self.file_path}")
