...
def get_parser():
    ...
    parser.add_argument(
        "--quiet", action="store_true", help="Changes the console log level from INFO to WARNING; defers to --verbose"
    )
    parser.add_argument(
        "--verbose", action="store_true", help="Changes the console log level from INFO to DEBUG; takes precedence over --quiet"
    )
    ...
    b3_parser.add_argument("--sail-date", type=str, required=False)
    b3_parser.add_argument("--land-date", type=str, required=False)
    b3_parser.add_argument("--hdd-recv-date", type=str, required=False)
    ...
    log_parser = cmd_parser.add_parser(
        "log-upload", help=f"Uploads the log file {LOG_FILENAME} to Tator, if it exists"
    )
    log_parser = tator.get_parser(log_parser)
    log_parser.add_argument("--log-file-type-id", type=int)
    log_parser.add_argument(
        "--log-filename", type=file_exists, required=False, default=LOG_FILENAME
    )
    return parser
