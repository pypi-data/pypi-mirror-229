...
class GithubController:
    ...
    def _get_merge_state_status(self) -> MergeStateStatus:
        ...
        if request.status_code == 200:
            return MergeStateStatus(request.json()["data"]["repository"]["pullRequest"]["mergeStateStatus"].lower())
    ...
    def deploy_to_prod(self) -> None:
        ...
        # Make sure the PR is up-to-date with the target branch and is in a state that can be merged.
        merge_status = self._get_merge_state_status()
        if merge_status.is_behind:
            raise CICDBotError("PR is behind target branch. Please merge/rebase and try again.")
        if merge_status.is_draft:
            raise CICDBotError("PR is a draft. Please mark as ready for review and try again.")
        if merge_status.is_dirty:
            raise CICDBotError("Merge commit cannot be cleanly created. Please check PR and resolve any issues.")
        if merge_status.unknown:
            raise CICDBotError("Merge status could not be determined. "
                               "If this persists, please contact Tobiko Data team on Slack since this is unexpected.")
