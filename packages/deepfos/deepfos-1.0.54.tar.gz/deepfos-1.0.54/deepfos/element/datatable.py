...
from typing import Iterable, List, Dict, Union, Type, Tuple, TypeVar, Any, Sequence, Optional
...
from deepfos.api.datatable import MySQLAPI, ClickHouseAPI, SQLServerAPI, OracleAPI, KingBaseAPI, GaussAPI, DaMengAPI, \
    PostgreSQLAPI, DeepEngineAPI
...
from deepfos.lib.concurrency import ThreadLocal
from deepfos.lib.decorator import flagmethod
from deepfos.lib.constant import UNSET
from deepfos.db import damysql, daclickhouse
...
class TableStructure:
    ...
    def fit_single(self, df: pd.DataFrame, column: str):
        """
...
class _DataTableDFConvertor(DataframeSQLConvertor):
    def convert(
        ...
        **opts
        ...
        return self.build_sql(columns, data_series, tablename, updatecol, **opts)


class _OracleDFConvertor(_DataTableDFConvertor):
    build_sql = staticmethod(OracleDFSQLConvertor.build_sql)
...
class _ClickHouseDFConvertor(_DataTableDFConvertor):
    build_sql = staticmethod(ClickHouseConvertor.build_sql)


class _SQLServerDFConvertor(_DataTableDFConvertor):
    build_sql = staticmethod(SQLServerDFSQLConvertor.build_sql)


class _DeepEngineDFConvertor(_DataTableDFConvertor):
    build_sql = staticmethod(DeepEngineDFSQLConvertor.build_sql)


class _PostgreSQLDFConvertor(PostgreSQLConvertor):
    build_sql = staticmethod(PostgreSQLConvertor.build_sql)
...
class APIBasedDataTableMySQL(ElementBase):
    ...
    def __init__(
        ...
        self._txn_support_ = False

    @cached_property
    def meta(self):
        """数据表的元配置信息"""
        return self.api.dml.table_info_field([self.element_info])[0]  # noqa
    ...
    def select_raw(
        ...
        return self.run_sql(sql).selectResult
    ...
    def insert_df(
        ...
        sqls = self.convertor.iter_sql(dataframe, self.table_name, updatecol, chunksize)
        return self._maybe_submit_in_txn(sqls)

    def _maybe_submit_in_txn(self, sqls: Iterable[str]):
        if self.__class__._txn_.in_txn[-1]:
            for sql in sqls:
                self.run_sql(sql)
        else:
            return self._trxn_execute(self, list(sqls))
    ...
    def update(
        ...
        if where is not None:
            q = q.where(where)
        return self.run_sql(q.get_sql(quote_char=self.quote_char))
    ...
    def update_from_dataframe(
        ...
        .. admonition:: 示例
            ...
            .. code-block:: python
                ...
                from deepfos.elements.datatable import Skip
        ...
        for dataframe in split_dataframe(source, chucksize):
            ret.append(self._maybe_submit_in_txn(yield_sql(dataframe)))
        return ret
    ...
    def count(
        ...
        resp = self.run_sql(sql).selectResult[0]
    ...
    def run_sql(self, sql: str) -> Optional[CustomSqlRespDTO]:
        ...
        """
        if self.__class__._txn_.in_txn[-1] and self._txn_support_:
            self.__class__._txn_.sql[-1].append(sql)
            self.__class__.api = self.api
            return
        ...
        return self.api.dml.run_sql(sql)
    ...
    def start_transaction(cls, flatten: bool = False):
        ...
        """
        cls._txn_.in_txn.append(True)

        if flatten and not cls._txn_.flatten:
            cls._txn_.flatten = True
            force_submit = True
        else:
            force_submit = False

        if not cls._txn_.flatten:
            cls._txn_.sql.append([])

        try:
            yield
            if force_submit or not cls._txn_.flatten:
                cls.__submit_txn()
        finally:
            cls._txn_.in_txn.pop()
            cls._txn_.flatten = False

    @classmethod
    def __submit_txn(cls):
        if cls._txn_.sql[-1]:
            sql = cls._txn_.sql.pop()
            resp = cls._trxn_execute(cls, sql)
            cls.transaction_result = resp

    @staticmethod
    def _trxn_execute(self, sqls: List[str]):
        return self.api.dml.execute_batch_sql(sqls)


class DirectAccessDataTableMySQL(APIBasedDataTableMySQL):
    def select(
        self,
        columns: Iterable[Union[str, Term]] = None,
        where: Union[str, Term, EmptyCriterion] = None,
        ...
        return evloop.run(damysql.query_dataframe(sql))
    ...
    def run_sql(self, sql: str):
        ...
        if len(sql) > SQL_LOG_MAX_LEN:
            ...
            sql_log = sql
        logger.debug(f"Execute SQL: [{sql_log}].")
        return evloop.run(damysql.execute(sql))

    @staticmethod
    def _trxn_execute(cls, sqls: List[str]):
        return evloop.run(damysql.trxn_execute(sqls))

    def count(
        self,
        where: Union[str, Term, EmptyCriterion],
    ) -> int:
        sql = f"SELECT COUNT(1) FROM {self.table_name} WHERE {self._parse_where(where)};"
        return evloop.run(damysql.select(sql))[0][0]


class APIBasedDataTableClickHouse(APIBasedDataTableMySQL):
    __doc__ = DOC_TEMPLATE.format(DB='ClickHouse')
    api_class = ClickHouseAPI
    api: ClickHouseAPI
    query = ClickHouseQuery
    convertor = _ClickHouseDFConvertor(quote_char=APIBasedDataTableMySQL.quote_char)
    ...
    def delete(self, where: Dict[str, Union[VT, List[VT]]]):
        """
        删除数据表的数据
        ...
        return self.api.dml.delete_data(DatatableDataDeleteDTO.construct_from(
            self.element_info,
            columnList=del_cols
        ))
...
class DirectAccessDataTableClickHouse(DirectAccessDataTableMySQL):
    __doc__ = DOC_TEMPLATE.format(DB='ClickHouse')
    api_class = ClickHouseAPI
    api: ClickHouseAPI
    query = ClickHouseQuery
    ...
    def run_sql(self, sql: str):
        if len(sql) > SQL_LOG_MAX_LEN:
            ...
            sql_log = sql
        logger.debug(f"Execute SQL: [{sql_log}].")
        return daclickhouse.execute(sql)
    ...
    def insert_df(
        ...
        for df in split_dataframe(dataframe, chunksize):
            r.append(daclickhouse.insert_dataframe(self.table_name, df))
        return r
    ...
    def select(
        ...
        return daclickhouse.query_dataframe(sql)
    ...
    def count(
        ...
        sql = f"SELECT COUNT(1) FROM {self.table_name} WHERE {self._parse_where(where)};"
        return daclickhouse.select(sql)[0][0]
...
if OPTION.general.db_direct_access:
    ...
    DataTableMySQL = APIBasedDataTableMySQL
    DataTableClickHouse = APIBasedDataTableClickHouse
...
class DataTableOracle(APIBasedDataTableMySQL):
    __doc__ = DOC_TEMPLATE.format(DB='Oracle')
    api: OracleAPI
    api_class = OracleAPI
    quote_char = '"'
    convertor = _OracleDFConvertor(quote_char=None)
    query = OracleQuery
    ...
    def copy_rows(
        ...
        return super().copy_rows(where, new_field_map, distinct)
    ...
    def insert(
        ...
        if value_map is not None:
            ...
            insert = '\n'.join(insert_list)
        return self.run_sql(f"INSERT ALL {insert} SELECT 1 FROM DUAL")


class DataTableSQLServer(APIBasedDataTableMySQL):
...
class DataTablePostgreSQL(APIBasedDataTableMySQL):
...
class DataTableKingBase(DataTablePostgreSQL):
    __doc__ = DOC_TEMPLATE.format(DB='KingBase')
    api: KingBaseAPI
    api_class = KingBaseAPI


class DataTableGauss(DataTablePostgreSQL):
    __doc__ = DOC_TEMPLATE.format(DB='Gauss')
    api: GaussAPI
    api_class = GaussAPI


class DataTableDaMeng(DataTableOracle):
    __doc__ = DOC_TEMPLATE.format(DB='DaMeng')
    api: DaMengAPI
    api_class = DaMengAPI


class DataTableDeepEngine(APIBasedDataTableClickHouse):
    __doc__ = DOC_TEMPLATE.format(DB='DeepEngine')
    api: DeepEngineAPI
    api_class = DeepEngineAPI
    convertor = _DeepEngineDFConvertor(quote_char=APIBasedDataTableClickHouse.quote_char)
...
def get_table_class(
    ...
    """
    table = {
        MySQLAPI.module_type: DataTableMySQL,
        ClickHouseAPI.module_type: DataTableClickHouse,
        SQLServerAPI.module_type: DataTableSQLServer,
        OracleAPI.module_type: DataTableOracle,
        KingBaseAPI.module_type: DataTableKingBase,
        GaussAPI.module_type: DataTableGauss,
        DaMengAPI.module_type: DataTableDaMeng,
        PostgreSQLAPI.module_type: DataTablePostgreSQL,
        DeepEngineAPI.module_type: DataTableDeepEngine,
