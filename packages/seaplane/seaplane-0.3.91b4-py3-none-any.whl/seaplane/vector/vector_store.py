...
class VectorStore:
    ...
    def delete(self, index_name: str, id: List[str]) -> str:
        ...
        result = client.delete_vectors(
            collection_name=index_name, vectors=[""], points=id
        )
    ...
    def update(self, index_name: str, vectors: Vectors) -> Dict[str, Any]:
        ...
        result = client.update_vectors(
            collection_name=index_name,
            vectors=[
                PointVectors(id=vector.id, vector=vector.vector) for vector in vectors
            ],
        )
    ...
    def knn_search(
        self, index_name: str, vector: Vector, neighbors: int = 10
    ) -> object:
        client = self.check_connection()

        return client.search(
            collection_name=index_name, query_vector=vector, limit=neighbors
        )
