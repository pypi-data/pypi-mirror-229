...
def get_lane_borders(start_point, num_lane):
    return [(start_point + i * (veh_sim.globals.ROAD_LANE_WIDTH + veh_sim.globals.WHITE_LANE_WIDTH), start_point + (i + 1) * veh_sim.globals.ROAD_LANE_WIDTH + i * veh_sim.globals.WHITE_LANE_WIDTH) for i in range(0, num_lane)]
...
def is_on_screen(p_x_coord, p_y_coord):
    result = False
    if 0 <= p_x_coord <= veh_sim.globals.WIDTH and 0 <= p_y_coord <= veh_sim.globals.HEIGHT:
        result = True
    return result
