...
from .datatable import DataTableMySQL, DataTableClickHouse
...
from deepfos.lib.constant import DFLT_DATA_COLUMN, VIEW, VIEW_DICT, HIERARCHY, DECIMAL_COL, STRING_COL
...
from deepfos.api.models.financial_model import (
    ...
    PcParams, CopyCalculateDTO, FinancialDataDto, TaskExecutionParam, ParameterDefineDto
)
...
# -----------------------------------------------------------------------------
# core classes
class WorkBook(ElementBase):
    """电子表格"""
    element_type = 'GRD'


class FinancialCube(ElementBase):
    """财务模型
    ...
    Note:
        ...
        .. code-block:: python
            ...
            cube = FinancialCube(element_name='test_cube', entry_object='Script for: {script_name}')
    ...
    """
    api_class = FinancialModelAPI
    api: FinancialModelAPI
    ...
    @cached_property
    def meta(self) -> CubeModel:
        """财务Cube的元数据信息"""
        return self.api.cube.data(
            folderId=self.element_info.folderId,
            cubeName=self.element_name
        )

    @cached_property
    def _meta(self) -> FinancialDataDto:
        return self.api.cube.find_cube_data(
            folderId=self.element_info.folderId,
            cubeName=self.element_name
        )
    ...
    @cached_property
    def dim_col_map(self) -> MultiDict[str, str]:
        """维度名 -> 数据列名的字典"""
        dc_map = MultiDict()
    ...
    @cached_property
    def fact_table(self) -> Union[DataTableMySQL, DataTableClickHouse]:
        """事实表对应的数据表"""
    ...
    def query(
        ...
        rslt = self.api.data.query(query_info)
        ...
        if pivot_dim is not None:
            ...
            if pivot_col in full_pov:
                ...
                # 不开启校验将默认autoCalculation为True
                if (
                    (not validate_expr or self._meta.autoCalculation)
                    and (view_mbr := get_ignore_case(full_pov, VIEW, None))
                    and (view := VIEW.lower()) in data.columns
                ):
                    # 在一些特殊情况下，view可能会同时出现在data和commonScript中
                    # data中的view并不一定完整，需要删除以免影响后续逻辑
                    data = data.drop(columns=view).assign(view=view_mbr)
    ...
    def save(
        ...
        """
        if data.empty:
            logger.info(f"Will not save to cube because dataframe is empty.")
            return
        ...
        return self._save_impl(data[required_cols], pov, need_check, data_audit, chunksize)
    ...
    def save_unpivot(
        ...
        return self._save_impl(data, pov, need_check, data_audit, chunksize)
    ...
    def _save_impl(
        ...
        for batch_data in split_dataframe(data, chunksize):
            ...
            resp.append(self.api.reactspreadsheet.save(payload.dict(exclude_unset=True)))  # noqa
        return resp
    ...
    def delete(
        ...
        data, pov = self.query(expression)
        data[DFLT_DATA_COLUMN] = None
        return self.save(data, pov, data_audit=True, chunksize=chunksize)
    ...
    def queries(
        ...
        if isinstance(expressions, str):
            return self.query(expressions, compact=False)

        expressions = list(expressions)
        if len(expressions) == 1:
            return self.query(expressions[0], compact=False)

        with ThreadCtxExecutor() as exc:
            jobs = [
                exc.submit(self.query, expr, compact=False)
                for expr in expressions
            ]

            df_list = [job.result() for job in jobs]
    ...
    def pc_init(
        self,
        process_map: Dict[str, str] = None,
        data_block_map: Dict[str, str] = None,
        block_name: str = None,
        ...
        """
        return self.api.block.pc_init(PcParams(
    ...
    def pc_update(
        ...
        """
        return self.api.block.pc_status(PcParams(
            blockList=block_list,
            blockName=block_name,
            cubeFolderId=self.element_info.folderId,
            cubeName=self.element_name,
    ...
    def copy_calculate(self, formula: str, fix_members: str):
        ...
        """
        return self.api.extra.copyCalculate(
            CopyCalculateDTO(
                cubeFolderId=self.element_info.folderId,
                cubeName=self.element_name,
                cubePath=self._path,
    ...
    def insert_null(
        ...
        """
        if not isinstance(self.fact_table, DataTableClickHouse):
            return self.delete(expression)
        ...
        with ThreadCtxExecutor(max_workers=len(expression)) as exe:
            for dim, expr in expression.items():
                if isinstance(expr, list):
                    member_dict[dim] = expr
                else:
                    if query_all or need_query(expr):
                        future = exe.submit(
                            self.dim_elements[self.col_dim_map.get(dim, dim)].query,
                            expr, fields=['name'], as_model=False
                        )
                        jobs.append((dim, future))
                    else:
                        member_dict[dim] = expr.split(';')

            for dim, j in jobs:
                member_dict[dim] = [item['name'] for item in j.result()]
    ...
    def mdx_execution(
        ...
        path = self._path or AppAPI().folder.get_folder_full(self.element_info.folderId)
        ...
        res = self.api.mdxtask.execution(
            TaskExecutionParam(
