...
class SileroTTSPlugin(TTS):
    ...
    def __init__(self, lang="en-us", config=None):
        ...
        lang = lang.split('-')[0]
        self.model = self.config.get("model") or next(iter(lang_config[lang]))
        self.voice = self.config.get("voice") or next(
            iter(lang_config[lang][model]))
        self.sample_rate = self.config.get("sample_rate") or SAMPLE_RATE
    ...
    def get_tts(self, sentence, wav_file, lang=None, speaker=None):
        ...
        wav_file = model.save_wav(text=sentence,
                                  speaker=self.voice,
...
SileroTTSPluginConfig = {
    lang: [
        ...
        for model, voices in SileroTTSPlugin.lang_config[lang].keys
        for voice in voices
    ]
...
if __name__ == "__main__":
    ...
    print(SileroTTSPluginConfig["en"])
    e = SileroTTSPlugin()
