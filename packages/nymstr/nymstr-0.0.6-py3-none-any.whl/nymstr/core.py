...
from ispec.utils import (funkws, kwsopt, kwsadd)
...
class nymstr(fuzstr, DynamicAttributesMixin):    
    ...
    def __new__(cls, name: StrLikeQ,  **kwargs):        
        # NOTE: `aggattr` gets all kws, whereas `cls.prepall(cls)` only gets annotated kws
        kwsall, supnew = aggattrs(cls, -1), getnew(getsup(cls))
        kws = kwsopt(kwsall, **kwargs)
        ...
        dattrs = ('itemattr', )
        obj.setdynattrs(dattrs, **kwsadd(kws, **kwargs))
        return obj
