import jinja2
import yaml
from jinja2 import Environment

from api_compose import JinjaFunctionsRegistry, JinjaFunctionType
from api_compose.core.utils.transformers import parse_json_with_jsonpath


@JinjaFunctionsRegistry.set(name='double_quote', type=JinjaFunctionType.Filter)
def add_double_quote(value):
    """
    Add double quotations to a string. Similar to Helm `quote`.
    Example Usage in Jinja: {{ value | double_quote }}
    """
    return f'"{value}"'

@JinjaFunctionsRegistry.set(name='single_quote', type=JinjaFunctionType.Filter)
def add_single_quote(value):
    """
    Add single quotations to a string. Similar to Helm `quote`.
    Example Usage in Jinja: {{ value | single_quote }}
    """
    return f"'{value}'"

@JinjaFunctionsRegistry.set(name='acp.utils.tpl', type=JinjaFunctionType.Global)
@jinja2.pass_context
def render_template_in_yaml_template(
        context: jinja2.runtime.Context,
        json_path: str,
):
    """
    Evaluate a template inside a yaml template. See Helm `tpl`.

    Example Usage in Jinja: {{ acp.utils.tpl('$.path.to.variable') }}
    """
    template: str = dict(context).get('template')
    dict_ = yaml.load(template, Loader=yaml.FullLoader)

    return parse_json_with_jsonpath(dict_, json_path, get_all_matches=False)
