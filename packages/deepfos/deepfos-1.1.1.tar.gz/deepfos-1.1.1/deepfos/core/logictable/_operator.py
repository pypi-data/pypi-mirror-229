...
class BaseOperator:
    ...
    def to_sql_template(self):
        return f"`{{0}}`{repr(self)}"
...
class Interval:
    ...
    def __to_sql(self, quote_field=True, force_repr=False):
        ge_or_gt = '>=' if self.cl else '>'
        le_or_lt = '<=' if self.cu else '<'
        field = "`{0}`" if quote_field else "{0}"
        if self.upper == PINF:
    ...
    def to_sql_template(self):
        if self.is_point:
            return OpEqual(self.lower).to_sql_template()
        return self.__to_sql(quote_field=True, force_repr=False)
...
class OpMultiInterval:
    ...
    def __to_sql(self, engine='mysql'):
        if engine == 'mysql':
            conctr = ' OR '
            caller = operator.methodcaller('to_sql_template')
        elif engine == 'pandas':
            conctr = ' or '
    ...
    def to_sql_template(self):
        return self.__to_sql(engine='mysql')
