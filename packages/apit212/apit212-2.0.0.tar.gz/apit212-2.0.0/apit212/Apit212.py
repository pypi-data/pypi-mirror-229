...
class Apit212:
    ...
    def __init__(self, timeout: int = 2, interval: float = 0.5, Longinterval: float = 10) -> None:
    ...
    def _reconnect(self) -> None:
        """
        Reconnect to the trading212 platform using saved cookie files. (If failed it will reconnect using selenium)
        """
    ...
    def setup(self, username: str, password: str, _beauty: bool = True, mode: str = "demo") -> None:
        """
        This function will log into the trading212 platform. set _saveCookies to True to enable faster reconnection times
    ...
    def _build_driver(self) -> None:
        ...
        while self.timeout > 0:
            ...
            try:
                d.find_element(By.NAME, "email").send_keys(self.username)
            ...
            try:
                d.find_element(By.NAME, "password").send_keys(self._password)
            ...
            while attempt != 0:
                ...
                try:
                    ...
                    self.logger.info(em)
                try:
                    if str(self.username) == str(user_name):
    ...
    def _cleanup_driver(self, driver: webdriver.Firefox, _end: bool = False) -> None:
        """
        """
...
class CFD(Apit212):
    ...
    def __init__(self, dealer: str = "AVUSUK", lang: str = "EN") -> None:
        ...
        if str(authAccount) != str(accountId):
            # switch accound    
            print(self._switch(account_id=accountId))

        else:
            pass

    def _switch(self, account_id: int) -> dict:
        """
        """
