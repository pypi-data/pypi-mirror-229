...
def validate_add_lvol_func(name, size, host_id_or_name, pool_id_or_name,
    ...
    if pool.lvol_max_size < size:
        return False, f"Pool Max LVol size is: {pool.lvol_max_size}, LVol size: {size} must be below this limit"

    if pool.pool_max_size > 0:
        total = pool_controller.get_pool_total_capacity(pool.get_id())
        if total + size > pool.pool_max_size:
            return False, f"Invalid LVol size: {size} " \
                          f"Pool max size has reached {total} of {pool.pool_max_size}"
...
def add_lvol(name, size, host_id_or_name, pool_id_or_name,
             use_comp, use_crypto, replicated,
             max_rw_iops, max_rw_mbytes, max_r_mbytes, max_w_mbytes):
