...
class MinMaxOutputCalibrater(MinMaxCalibrater):
    ...
    def augment_graph(self):
        ...
        def add_reduce_min_max(tensor_name, reduce_op_name):
            ...
            tensor_shape = get_tensor_shape(tensor_name, model.graph)
            is_tensor_input = tensor_name in self.model_original_inputs
            if len(tensor_shape) == 4 and not (self.per_tensor_activations or is_tensor_input):
                tensor_out_shape = tensor_shape[1:2]
                # Depending of the onnx opset version, axes in ReduceMin/ReduceMax
                # are in attribute or inputs:
                if onnx.defs.onnx_opset_version() < 18:
