...
class Env:
    def __init__(self, number_of_lanes, show_horizontal_line=False):
        ...
        self.roadside_area_widths = calculate_roadside_area_width(
            veh_sim.globals.WIDTH
            - (
                veh_sim.globals.ROAD_LANE_WIDTH * self.number_of_lanes
                + self.number_of_white_lines * veh_sim.globals.WHITE_LANE_WIDTH
            ),
            veh_sim.globals.GRASS_AREA_RATIO,
            veh_sim.globals.SIDEWALK_AREA_RATIO,
        )
        ...
        self.sidewalk_border_right = (
            veh_sim.globals.WIDTH - self.roadside_area_widths.grass_area_width * 2
        )
        self.road_border_left = (
            self.roadside_area_widths.grass_area_width
            + self.roadside_area_widths.sidewalk_area_width
        )
        self.road_border_right = veh_sim.globals.WIDTH - 2 * (
            self.roadside_area_widths.grass_area_width
            + self.roadside_area_widths.sidewalk_area_width
        )
        ...
        if show_horizontal_line:
            self.horizontal_line_x_loc = (
                ...
                - veh_sim.globals.HORIZONTAL_LANE_WIDTH
            )
            self.horizontal_line_y_loc = range(
                veh_sim.globals.HORIZONTAL_LANE_START_Y,
                veh_sim.globals.HEIGHT,
                veh_sim.globals.HORIZONTAL_LANE_GAP + veh_sim.globals.HORIZONTAL_LANE_HEIGHT,
            )
            if veh_sim.globals.DEBUG:
                print(self.horizontal_line_y_loc)
        else:
            self.white_y_loc = range(
                -veh_sim.globals.WHITE_LANE_HEIGHT,
                veh_sim.globals.HEIGHT,
                math.floor(veh_sim.globals.WHITE_LANE_HEIGHT + veh_sim.globals.WHITE_LANE_GAP),
            )
            self.white_x_loc = range(
                math.floor(
                    self.roadside_area_widths.grass_area_width
                    + self.roadside_area_widths.sidewalk_area_width
                    + veh_sim.globals.ROAD_LANE_WIDTH
                ),
                math.floor(
                    veh_sim.globals.WIDTH
                    - (
                ...
                veh_sim.globals.ROAD_LANE_WIDTH + veh_sim.globals.WHITE_LANE_WIDTH,
            )
        ...
        for tree_idx in range(
            veh_sim.globals.TREES_START_POINT,
            veh_sim.globals.HEIGHT,
            math.floor(veh_sim.globals.TREES_WIDTH + veh_sim.globals.TREES_GAP),
        ):
            tree_right = Tree(
                veh_sim.globals.TREES_IMAGES[random.randint(0, 3)],
                veh_sim.globals.TREES_WIDTH,
                (self.roadside_area_widths.grass_area_width, tree_idx),
            )
            tree_left = Tree(
                veh_sim.globals.TREES_IMAGES[random.randint(0, 3)],
                veh_sim.globals.TREES_WIDTH,
                (veh_sim.globals.WIDTH - self.roadside_area_widths.grass_area_width, tree_idx),
            )
        ...
        for house_idx in range(
            veh_sim.globals.HOUSE_START_POINT,
            veh_sim.globals.HEIGHT,
            math.floor(veh_sim.globals.HOUSES_WIDTH + veh_sim.globals.HOUSES_GAP),
        ):
            house_right = House(
                veh_sim.globals.HOUSES_IMAGES[random.randint(0, 3)],
                veh_sim.globals.HOUSES_WIDTH,
                (0, house_idx),
            )
            house_left = Tree(
                veh_sim.globals.HOUSES_IMAGES[random.randint(0, 3)],
                veh_sim.globals.HOUSES_WIDTH,
                (veh_sim.globals.WIDTH, house_idx),
            )
        ...
        for sidewalk_idx in range(
            veh_sim.globals.SIDEWALK_OBJECTS_START_POINT,
            veh_sim.globals.HEIGHT,
            math.floor(
                veh_sim.globals.SIDEWALK_OBJECTS_WIDTH
                + random.randint(
                    veh_sim.globals.SIDEWALK_OBJECTS_GAP_MIN, veh_sim.globals.SIDEWALK_OBJECTS_GAP_MAX
                )
            ),
        ):
            ...
            swo_right = SidewalkObject(
                veh_sim.globals.SIDEWALK_OBJECTS_IMAGES[right_idx],
                veh_sim.globals.SIDEWALK_OBJECTS_WIDTH,
            ...
            swo_left = SidewalkObject(
                veh_sim.globals.SIDEWALK_OBJECTS_IMAGES[left_idx],
                veh_sim.globals.SIDEWALK_OBJECTS_WIDTH,
                0 if right_idx < 4 else 1,
                (
                    get_sidewalk_x_loc(
    ...
    def update(self, speed):
        ...
        if not self.show_horizontal_line:
            ...
            if self.distance_pointer_white_lines > (
                veh_sim.globals.WHITE_LANE_GAP + veh_sim.globals.WHITE_LANE_HEIGHT
            ):
                self.distance_pointer_white_lines = 0
                temp_y_list.append(-veh_sim.globals.WHITE_LANE_HEIGHT)

            for y_loc in self.white_y_loc:
                temp = y_loc + speed
                if temp <= veh_sim.globals.HEIGHT:
        ...
        # update trees
        if self.distance_pointer_trees > (veh_sim.globals.TREES_GAP + veh_sim.globals.TREES_WIDTH):
            self.distance_pointer_trees = 0
            tree_right = Tree(
                veh_sim.globals.TREES_IMAGES[random.randint(0, 3)],
                veh_sim.globals.TREES_WIDTH,
                (self.roadside_area_widths.grass_area_width, veh_sim.globals.TREES_START_POINT),
            )
            tree_left = Tree(
                veh_sim.globals.TREES_IMAGES[random.randint(0, 3)],
                veh_sim.globals.TREES_WIDTH,
                (
                    veh_sim.globals.WIDTH - self.roadside_area_widths.grass_area_width,
                    veh_sim.globals.TREES_START_POINT,
                ),
            )
            self.trees_group.add(tree_right)
        ...
        # update houses
        if self.distance_pointer_houses > (veh_sim.globals.HOUSES_WIDTH + veh_sim.globals.HOUSES_GAP):
            self.distance_pointer_houses = 0
            house_right = House(
                veh_sim.globals.HOUSES_IMAGES[random.randint(0, 3)],
                veh_sim.globals.HOUSES_WIDTH,
                (0, veh_sim.globals.HOUSE_START_POINT),
            )
            house_left = House(
                veh_sim.globals.HOUSES_IMAGES[random.randint(0, 3)],
                veh_sim.globals.HOUSES_WIDTH,
                (veh_sim.globals.WIDTH, veh_sim.globals.HOUSE_START_POINT),
            )
            self.houses_group.add(house_right)
            self.houses_group.add(house_left)
    ...
    def render(self, window, show_sidewalk_objects=True):
        ...
        pygame.draw.rect(
            window, veh_sim.globals.GRASS_COLOR, (0, 0, veh_sim.globals.WIDTH, veh_sim.globals.HEIGHT)
        )
        pygame.draw.rect(
            window,
            veh_sim.globals.SIDEWALK_COLOR,
            (self.sidewalk_border_left, 0, self.sidewalk_border_right, veh_sim.globals.HEIGHT),
        )
        pygame.draw.rect(
            window,
            veh_sim.globals.ASPHALT_COLOR,
            (self.road_border_left, 0, self.road_border_right, veh_sim.globals.HEIGHT),
        )
        ...
        if not self.show_horizontal_line:
            for x_loc in self.white_x_loc:
                for y_loc in self.white_y_loc:
                    if y_loc < veh_sim.globals.HEIGHT:
                        pygame.draw.rect(
                            window,
                            veh_sim.globals.WHITE,
                            (
                                x_loc,
                                y_loc,
                                veh_sim.globals.WHITE_LANE_WIDTH,
                                veh_sim.globals.WHITE_LANE_HEIGHT,
                            ),
                        )
        else:
            for y_loc in self.horizontal_line_y_loc:
                pygame.draw.rect(
                    window,
                    veh_sim.globals.YELLOW,
                    (
                        self.horizontal_line_x_loc,
                        y_loc,
                        veh_sim.globals.HORIZONTAL_LANE_WIDTH,
                        veh_sim.globals.HORIZONTAL_LANE_HEIGHT,
                    ),
                )
...
class Tree(pygame.sprite.Sprite):
    ...
    def update(self, p_spd):
        self.vy = p_spd
        self.rect.y += self.vy
        if self.rect.top >= veh_sim.globals.HEIGHT:
            self.kill()
...
class SidewalkObject(pygame.sprite.Sprite):
    ...
    def update(self, p_spd):
        ...
        if self.object_type == 0:
            self.vy = p_spd + veh_sim.globals.PEDESTRIAN_SPEED * ratio
        else:
            self.vy = p_spd + veh_sim.globals.BIKE_SPEED * ratio
        self.rect.y += self.vy
        if self.rect.top >= veh_sim.globals.HEIGHT:
            self.kill()
