...
def read_lpicola(path, h, boxsize):
    ...
    with open(path, mode="rb") as fh:
        ...
        while True:
            ...
            n_rows_current = header[1]
            block_size = n_rows_current * 7
            data[n_rows_read : n_rows_read + n_rows_current] = np.fromfile(
                fh, dtype=np.float32, count=block_size
            ).reshape(-1, 7)[:, :3]
...
def read_pkdgrav(path, boxsize, n_rows_per_block=int(1e6)):
    ...
    with open(path, mode="rb") as f:
        while True:
            block = np.fromfile(f, dtype=np.float32, count=n_block).reshape(-1, 7)[
                :, :3
            ]
...
def read_file(path, boxsize, cosmo, file_format="l-picola"):
    ...
    if file_format == "l-picola":
        ...
        raise ValueError(
            'Data format {} is not supported, choose either "l-picola" or "pkdgrav"'
        )
...
def construct_shells(dirpath, z_shells, boxsize, cosmo, nside, file_format="l-picola"):
    ...
    if file_format == "l-picola":
        filelist = list(
            filter(
                lambda fn: "_lightcone." in fn and os.path.splitext(fn)[1] != ".info",
                os.listdir(dirpath),
            )
        )
    elif file_format == "pkdgrav":
    ...
    for i, filename in enumerate(filelist):
        ...
        for i_shell in range(shells.shape[0]):
            ...
            counts_shell = thetaphi_to_pixelcounts(
                coord[i_low:i_up, 1], coord[i_low:i_up, 2], nside
            )
