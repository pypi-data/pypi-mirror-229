...
class ReplaceFieldsWithValues(object):
    ...
    def choose_option(self, message_index: int, first_param: str) -> (str, int):
        ...
        logger.end(object={'rand_option_sel': rand_option_sel, 'index': index})
        return (rand_option_sel, index)
    ...
    def get_variable_values_and_chosen_option(self) -> (list, str):
        ...
        for variable_name in variable_names_list:
            ...
            variable_value = VariablesLocal.get_variable_value_by_id(
                language=self.language, variable_id=variable_id)
            variable_value_list.append(variable_value)
        formatted_massage = message_without_variable_names.format(
            ", ".join(variable_value_list))
        logger.end(object={'formatted_massage': formatted_massage})
        return formatted_massage
