...
def all_vs_all_blastn_by_mmseqs_clusters(mmseqs_tsv, ffn_dir, min_cluster_size, max_subset_seq_mum, num_threads, blast_parameters, ffn_dir_filtered, combined_blastn_op, pwd_log_file):
...
def PI(MetaCHIP_wd, pwd_tmp_dir, input_genome_basename_list, gbk_dir, gbk_ext, GTDB_output_file, grouping_levels, grouping_file, previous_blast_op,
    ...
    if grouping_levels == 'x':
        ...
        if sorted(input_genome_basename_list) != sorted(genomes_in_grouping_file):
            print(input_genome_basename_list)
            print(genomes_in_grouping_file)
            report_and_log(('Genome ids provided in %s do not match genome files in %s, program exited!' % (pwd_grouping_file, gbk_dir)), pwd_log_file, keep_quiet)
            report_and_log(('Please note that file extension (e.g., gbk) of the input genomes should NOT be included in the grouping file.'), pwd_log_file, keep_quiet)
            exit()
    else:
    ...
    if previous_blast_op is None:
        ...
        if use_mmseqs is False:
            ...
            all_vs_all_blastn_by_mmseqs_clusters(pwd_mmseqs_tsv_file, pwd_prodigal_output_folder, 5, 20000, num_threads, blast_parameters, pwd_blast_result_folder_mmseqs, pwd_blast_result_folder, pwd_log_file)
...
def BP(MetaCHIP_wd, pwd_tmp_dir, gbk_dir, op_prefix, species_tree_file, grouping_file, detection_ranks, num_threads, No_Eb_Check,
    ...
    if grouping_file is not None:
        ...
        pwd_MetaCHIP_op_folder_re = '%s/%s_HGT_x*' % (MetaCHIP_wd, op_prefix)
        ...
        detect_wd                   = '%s/%s'                   % (MetaCHIP_wd, MetaCHIP_op_folder)
        pwd_detected_HGT_PG_txt     = '%s/HGTs_PG.txt'          % MetaCHIP_wd
...
def detect(args):
    ...
    MetaCHIP2_wd = '%s_MetaCHIP_wd' % output_prefix
    ...
    if (grouping_file is not None) and (GTDB_output_file is None) and (grouping_levels is None):
        ...
        print('Please group your genomes either with "-g" or with "-c -r", program exited!')
        exit()
    ...
    BP(MetaCHIP2_wd, pwd_tmp_dir, gbk_dir, output_prefix, pwd_species_tree, grouping_file, grouping_levels, num_threads,
       No_Eb_Check, previous_blast_op, keep_quiet, plot_flk_region, keep_temp, rank_abbre_dict, circos_HGT_R,
...
if __name__ == '__main__':
    ...
    detect_parser.add_argument('-v', required=False, action="store_true",   help='visualize flanking region of identified HGTs')
    detect_parser.add_argument('-e', required=False, action="store_true",   help='disable end-break and contig-match check, not recommend for metagenome-assembled genomes')
    detect_parser.add_argument('-t', required=False, type=int, default=1,   help='number of threads, default: 1')
    detect_parser.add_argument('-k', required=False, action="store_true",   help='keep temporary files')
...
cd /Users/songweizhi/Desktop/metachip_test
python3 ~/PycharmProjects/MetaCHIP2/MetaCHIP2/detect.py -i S2_sal_fde_gbk -x gbk -c S2_sal_fde_taxon.tsv -s S2_sal_fde_rooted.tree -t 10 -f -r pco -p S2_sal_fde_pco_blastn
python3 ~/PycharmProjects/MetaCHIP2/MetaCHIP2/detect.py -i S2_sal_fde_gbk -x gbk -c S2_sal_fde_taxon.tsv -s S2_sal_fde_rooted.tree -t 10 -f -r pco -p S2_sal_fde_pco_mmseqs -m
python3 ~/PycharmProjects/MetaCHIP2/MetaCHIP2/detect.py -i S2_sal_fde_gbk -x gbk -c S2_sal_fde_taxon.tsv -s S2_sal_fde_rooted.tree -t 10 -f -b S2_sal_fde_blastn_results -r pco -p S2_sal_fde_pco
python3 ~/PycharmProjects/MetaCHIP2/MetaCHIP2/detect.py -i S2_sal_fde_gbk -x gbk -c S2_sal_fde_taxon.tsv -s S2_sal_fde_rooted.tree -t 10 -f -b S2_sal_fde_blastn_results -r pcofg -p Demo
...
cd /Users/songweizhi/Desktop/metachip_test
python3 ~/PycharmProjects/MetaCHIP2/MetaCHIP2/detect.py -i S2_sal_fde_gbk -x gbk -c S2_sal_fde_taxon.tsv -s S2_sal_fde_rooted.tree -t 12 -f -r p -p S2_sal_fde_p_mmseqs -m -v
python3 ~/PycharmProjects/MetaCHIP2/MetaCHIP2/detect.py -i S2_sal_fde_gbk -x gbk -c S2_sal_fde_taxon.tsv -s S2_sal_fde_rooted.tree -t 12 -f -r pcofg -p S2_sal_fde_pcofg_mmseqs -m -v
