        ...
        for page in page_iter:
            for file_obj in page:
                if file_obj.name == base_filename:
                    file_id = file_obj.id
                    logger.debug("Found existing file %d with same name, skipping upload", file_id)
                    return file_id
    except RuntimeError:
...
def import_metadata(
    *, host: str, token: str, project_id: int, directory: str, meta_ext: str, file_type: int
    ...
    results = []
    for filename in tqdm(file_list, total=len(file_list), desc="GPS Imports", dynamic_ncols=True):
        try:
