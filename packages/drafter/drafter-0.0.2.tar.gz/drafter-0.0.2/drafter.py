...
class Table(PageContent):
    ...
    def reformat_as_single(self):
        ...
        for field in fields(self.rows):
            ...
            result.append([f"<code>{field.name}</code>", f"<code>{field.type}</code>", f"<code>{value}</code>"])
        self.rows = result
        if not self.header:
...
class Server:
    ...
    def debug_information(self):
        ...
        for original_route, function in self.original_routes:
            page.append(f"<li><code>{original_route}/</code></li>")
        ...
        for page_history in reversed(self._page_history):
            button_pressed = f"Pressed <code>{page_history.button_pressed}</code> button &rarr;" if page_history.button_pressed else ""
