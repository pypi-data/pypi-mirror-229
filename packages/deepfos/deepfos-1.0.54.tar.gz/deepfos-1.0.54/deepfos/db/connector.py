...
class ClickHouseDirectAccess(AbstractConnector):
    ...
    def execute_many(self, sqls: List[str]):
        return [daclickhouse.execute(sql) for sql in sqls]

    def trxn_execute(self, sqls: List[str]):
        raise NotImplementedError('ClickHouse does not support transaction')

    def select(self, sql: str):
        return daclickhouse.select(sql)

    def query_dataframe(self, sql: str) -> pd.DataFrame:
        return daclickhouse.query_dataframe(sql)

    def insert_dataframe(self, table: str, dataframe: pd.DataFrame):  # noqa
        return daclickhouse.insert_dataframe(table, dataframe)
