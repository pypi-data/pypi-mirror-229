...
from kdb import REPORT_TEMPLATE_DIR, HTML_REPORT_DIR, XML_REPORT_DIR
...
class ReportManager:
    ...
    def create_test_case_report(test_case_name, is_passed, duration, file_name):
        ...
        # read html layout
        html = ET.parse(os.path.join(REPORT_TEMPLATE_DIR, 'test_case_layout.html'))
        ...
        # write to html file
        html_file_path = os.path.join(HTML_REPORT_DIR, test_case_name + ".html")
    ...
    def create_index_report(xml_report_name=None):
        ...
        if xml_report_name is None:
            xml_report = ET.parse(os.path.join(XML_REPORT_DIR, 'xml_report_main.xml'))
        else:
            xml_report = ET.parse(os.path.join(XML_REPORT_DIR, xml_report_name))
        ...
        if num_of_tests > 0:
            ...
            # read html layout
            html = ET.parse(os.path.join(REPORT_TEMPLATE_DIR, 'index_layout.html'))
            # get body tag
            body = html.find('body')
