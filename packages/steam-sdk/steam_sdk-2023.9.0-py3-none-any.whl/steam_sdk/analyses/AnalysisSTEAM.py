...
class AnalysisSTEAM:
    ...
    def step_postprocess(self, step, verbose: bool = False): # run sequence variable
        ...
        if step.sources is not None and len(step.sources)!=0:
    ...
    def steam_analyze_lhc_event(self, input_csv_file: str, local_folder: str, flag_run_software: bool, software: str):
        circuit_name = self.__get_first_circuit_name(os.path.join(os.getcwd(), input_csv_file))
        if circuit_name.startswith("RCD"):
            self.analyze_RCD_RCO_event(os.path.join(os.getcwd(), input_csv_file), local_folder, flag_run_software, software)
        elif circuit_name.startswith(("RD1", "RD2", "RD3", "RD4")):
            circuit_type = "IPD"
            self.analyze_circuit_event(os.path.join(os.getcwd(), input_csv_file), local_folder, circuit_type, flag_run_software, software)
    ...
    def analyze_circuit_event(self, input_csv_file: str, local_folder: str, circuit_type: str, flag_run_software: bool, software: str):
        file_counter = 1
        ...
        if software == 'PSPICE':
            aSTEAM.settings.local_PSPICE_folder = local_folder
        elif software == 'XYCE':
            aSTEAM.settings.local_XYCE_folder = local_folder
    ...
    def compare_PSPICE_XYCE_simulations(self, circuit_type: str, path_folder_model_PSPICE: str, path_folder_model_XYCE: str):
        ...
        df_PSPICE = get_signals_from_csd(full_name_file=os.path.join(path_folder_model_PSPICE, f'{circuit_type}.csd'), list_signals=['time']+list_signals_PSPICE)
        df_XYCE = get_signals_from_csd(full_name_file=os.path.join(path_folder_model_XYCE, f'{circuit_type}.csd'), list_signals=['time']+list_signals_XYCE)
    ...
    def analyze_RCD_RCO_event(self, input_csv_file: str, local_folder: str, flag_run_software: bool, software: str):
        # arrange
        file_counter = 1
        file_name_analysis = os.path.join(os.getcwd(), "analysisSTEAM_settings.yaml") #file containing settings paths
    ...
    def __get_circuit_type(self, circuit_name: str):
        if circuit_name.startswith("RCBH") or circuit_name.startswith("RCBV"):
            ...
            magnet_number = self.__find_magnets(os.path.abspath(os.path.join(os.getcwd(), "../../../steam_models/circuits/circuit_parameters/RQ_circuit_parameters.csv")), circuit_name_temp)
