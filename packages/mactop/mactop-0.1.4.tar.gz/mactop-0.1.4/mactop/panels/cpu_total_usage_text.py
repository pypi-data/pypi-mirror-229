...
class CPUTotalUsageTextPanel(BaseStatic):
    ...
    def watch_percentages(self, percentages):
        if percentages:
            user_legend = self.query_one("CPUTotalUsageTextPanel .cpu-legend-user")
            user_legend.current_value = self._render_legend_value(percentages[0])

            nice_legend = self.query_one("CPUTotalUsageTextPanel .cpu-legend-nice")
            nice_legend.current_value = self._render_legend_value(percentages[1])

            system_legend = self.query_one("CPUTotalUsageTextPanel .cpu-legend-system")
            system_legend.current_value = self._render_legend_value(percentages[2])

            idle_legend = self.query_one("CPUTotalUsageTextPanel .cpu-legend-idle")
            idle_legend.current_value = self._render_legend_value(percentages[3])

    def compose(self) -> ComposeResult:
        yield Horizontal(
            Legend(self.color_user, "user", classes="cpu-legend-user"),
            Legend(self.color_nice, "nice", classes="cpu-legend-nice"),
            Legend(self.color_system, "system", classes="cpu-legend-system"),
            Legend(self.color_idle, "idle", classes="cpu-legend-idle"),
        )
