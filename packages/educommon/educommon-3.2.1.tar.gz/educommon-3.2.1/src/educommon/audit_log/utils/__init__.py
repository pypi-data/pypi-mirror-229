...
def _set_audit_log_tables():
    ...
    for table_name in table_names_for_deleting:
        table_id = Table.objects.filter(name=table_name).values_list('id', flat=True).first()
        if table_id:
            ...
            logs_to_delete = AuditLog.objects.filter(table_id=table_id)
