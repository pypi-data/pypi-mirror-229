...
def test_association(network: str, genesets_a: str, output: str,
                     ...
                     high_memory_parallel: bool = False):
    ...
    if not high_memory_parallel:
        ...
        with Pool(processes=min(processes, len(geneset_pairs))) as pool:
            results = pool.starmap(partial(run_comparison_on_genesets,
                sc_comparison=sc_comparison, permutations=permutations,
                processes=1, verbose=True), geneset_pairs)
    pd.DataFrame(
    ...
    if results_figure:
        if not genesets_b:
            ...
            plot_test_results(null_dists,
                              observed.drop(('Gene set A', 'Gene set B')),
                              results_figure,
