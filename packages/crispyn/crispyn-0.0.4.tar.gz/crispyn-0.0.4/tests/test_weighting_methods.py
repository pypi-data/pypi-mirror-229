...
class Test_SWARA(unittest.TestCase):
    ...
    def test_swara(self):
        ...
        criteria_descending_order = np.array([0, 1, 2, 3, 4, 5, 6])
        s = np.array([0, 0.35, 0.2, 0.3, 0, 0.4])

        test_result = mcda_weights.swara_weighting(criteria_descending_order, s)
...
class Test_LBWA(unittest.TestCase):
    ...
    def test_lbwa1(self):
        ...
        lbwa_influence_values = [
            [0, 2, 3, 4, 4, 5],
            [1, 2]
        ]

        test_result = mcda_weights.lbwa_weighting(lbwa_influence_values, lbwa_indexes)
