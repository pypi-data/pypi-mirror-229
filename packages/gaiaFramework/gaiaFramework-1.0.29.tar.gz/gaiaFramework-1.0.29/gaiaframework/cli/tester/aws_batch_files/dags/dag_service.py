...
class Dag(ConfigBase):
    ...
    def get_new_cluster_dag_code(self) -> str:
        ...
        with DAG(
            ...
            schedule_interval=schedule_interval,  # Set to None to prevent automatic scheduling
            # schedule_interval=timedelta(days=1),  # Set to None to prevent automatic scheduling
    ...
    def create_stage(self, job_flow_id, name):
        archives = ['''bucket_path + "/dist/" + whl_file_name''']
        py_files = ['''bucket_path + "/dist/" + whl_file_name''']
        conf = []
        # if not self.use_cluster_bootstrap:
        #     py_files.append('''bucket_path + "/dist/pyspark_venv.tar.gz#venv"''')
        #     conf.append('''"spark.yarn.appMasterEnv.PYSPARK_PYTHON=venv/bin/python"''')
        return '''
                ...
                steps=[{
                    ...
                    "HadoopJarStep": {
                        ...
                        "Args": [
                            ...
                            ''' + ('"--conf", ' + ', "--conf", '.join(conf) + ',' if len(conf) else '') + '''
                            "--archives", ''' + ", ".join(archives) + ''',
                            "--py-files", ''' + ", ".join(py_files) + ''',
                            bucket_path + "/aws_batch_files/stages/''' + name + '''/main.py",
                            stage_config, "{{ execution_date }}", "{{ next_execution_date }}", extra_job_params
                        ]
                    }
                }],
...
'''
    ...
    def get_dag_conf(self):
        ...
        extra_job_params = \'''' + json_dumps(self.extra_job_params) + '''\'
    ...
    def run_cli_dag(self, command):
        ...
        if response.status_code == 200:
            print(f'{command} dag successfully: {dag_id}')
            # print("Response content:", response.text)
        else:
            print(f"Failed to {command} dag '{dag_id}'.")
            # print("Response content:", response.text)
...
if __name__ == "__main__":
    ...
    if dag_action == 'instantiate':
        ...
        # dag.run_cli_dag('dags unpause')
        dag.run_cli_dag('dags trigger')
        # dag.run_cli_dag('dags pause')
