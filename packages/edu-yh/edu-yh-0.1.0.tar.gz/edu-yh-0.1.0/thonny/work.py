...
class WorkDialog(CommonDialog):
    def __init__(self, master):
        ...
        self.statString.set(self.get_previous_year(365))
    ...
    def select_click(self):
        ...
        # print(start, end, '我是时间啊，哈哈哈哈哈')
        # if not self.validate_datetime_format(start) or not self.validate_datetime_format(
        #     end):
        #     return messagebox.showinfo('错误', '请输入%Y-%m-%d %H:%M:%S的数据格式哦')
        history_job_list = view_history_job_list(
            token=self.token,
            hpc_prefix=self.enable_url,
            job_cluster=self.job_cluster,
