...
def wait_and_receive_response(connection: socket.socket) -> dict:
...
def wait_and_receive_file(
    connection: socket.socket, corrected_file_path: str, file_size_bytes: int
):
    with open(corrected_file_path, "wb") as processed_file:
        ...
        while not file_received_fully:
            ...
            if received_percents % 10 == 0 and previous_percents != received_percents:
                ...
                logging.info(
                    f"Receiving file from server, {received_percents}% ({received_bytes} / {file_size_bytes})."
                )
