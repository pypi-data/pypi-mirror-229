...
def get_table_rows_by_keys2(con: MySQLConnection, database, table, table_keys: list[tuple[str, str]], table_keys_vals: list[dict]) -> str:
    whereval = []
    for coln, colt in table_keys:
        if colt in ["int", "bigint", "varchar", "char"]:
...
def get_table_checksum(con: MySQLConnection, database, table):
    with con.cursor() as cur:
        cur.execute(f"CHECKSUM TABLE {database}.{table}")
        _, checksum = cur.fetchone()
        return checksum
...
class MysqlTableCompare:
    ...
    def get_full_table_orderby_keys(self, limit_size: int, keycols: list[tuple[str, str]], keyval: dict = None):
        ...
        while True:
                ...
                for j in range(0, len(keycols)):
                    ...
                    for i in range(0, len(_kcs)):
                        ...
                        if colt in ["int", "bigint"]:
                            unit.append(f"{coln} {soy} {_keyval[coln]}")
                        elif colt == "varchar":
                            unit.append(f'{coln} {soy} "{_keyval[coln]}"')
                        else:
                            raise Exception(f"data type: {colt} not suppert yet.")
            ...
            self.logger.debug(f"get_full_table_orderby_keys: {statement}")
    ...
    def process_rows(
        ...
        try:
            _s_ts = time.time()
            _target_rows = get_table_rows_by_keys2(target_con, self.database, self.table, self.source_table_keys, source_key_vals)
            ...
            _s_ts = time.time()
            _source_rows = get_table_rows_by_keys2(source_con, self.database, self.table, self.source_table_keys, source_key_vals)
            ...
            with self.thread_lock:
                if len(_diff_rows) >= 1:
                    self.logger.info(f"discover diff repair rows: {len(_diff_rows)}.")
                    self.write_diff_row(_diff_rows)
    ...
    def write_diff_row(self, rows):
        with open(f"{self.database}.{self.table}.diff.log", "a", encoding="utf8") as f:
            for row in rows:
                f.write(f"{row}\n")

    def write_checkpoint(self, row: dict, processed: int):
        with open(f"{self.database}.{self.table}.ckpt.json", "w", encoding="utf8") as f:
            json.dump({"checkpoint": row, "processed": processed}, f)

    def read_checkpoint(self):
        if os.path.exists(f"{self.database}.{self.table}.ckpt.json"):
            with open(f"{self.database}.{self.table}.ckpt.json", "r", encoding="utf8") as f:
                return json.load(f)
        else:
            return None

    def run(self):
        print("start", self.database, self.table)
        if os.path.exists(f"{self.database}.{self.table}.done"):
            print(f"{self.database}.{self.table} done")
            return True
        ...
        if self.checksum and not os.path.exists(f"{self.database}.{self.table}.ckpt.json"):
            _s_ts = time.time()
            source_checksum = get_table_checksum(self.source_con, self.database, self.table)
            self.logger.info(f"checksum source table checksum:{source_checksum}, time:{get_elapsed_time(_s_ts, 2)}s.")

            _s_ts = time.time()
            target_checksum = get_table_checksum(self.target_con, self.database, self.table)
            self.logger.info(f"checksum target table checksum:{target_checksum}, time:{get_elapsed_time(_s_ts, 2)}s.")

            if source_checksum == target_checksum and source_checksum != "0":
                self.logger.info("no difference between the two tables.")
                return True
        ...
        if not (len(source_table_struct) == len(target_table_struct) and len(table_struct_diff) == 0):
            self.logger.info(f"source and target table structure diff.")
            return False
        ...
        if len(self.source_table_keys) == 0:
            self.logger.error(f"source table: {self.database}.{self.table} not exist keys.")
            return False
        ...
        _thread_container: list[tuple[int, threading.Thread, MySQLConnection, MySQLConnection, list[dict], int, float]] = []
        ...
        for end, rows in self.get_full_table_orderby_keys(6000, self.source_table_keys, _checkpoint_row):
            ...
            _t = threading.Thread(target=self.process_rows, args=(_i, _sc, _tc, rows.copy()))

            _thread_container.append((_i, _t, _sc, _tc, rows.copy(), _cnt + 1, time.time()))
            _t.start()

            while (len(_thread_container) == self.parallel) or (end and len(_thread_container) >= 1):
                _i, _t, _sc, _tc, _rows, _cnt, _s_ts = _thread_container.pop(0)
                _t.join()
        ...
        # self.logger.info(f"difference rows number: {len(self.diff_rows)}.")

        with open(f"{self.database}.{self.table}.done", "w", encoding="utf8") as f:
            pass

        if os.path.exists(f"{self.database}.{self.table}.ckpt.json"):
            os.remove(f"{self.database}.{self.table}.ckpt.json")
