...
def jupyter_notebook_generic(__import__, input_jupyter_notebook_file_path):
    ...
    try:
        ...
        python_file_name, _ = os.path.splitext(
            input_jupyter_notebook_file_path)
        python_file = python_file_name + ".py"
        convert_notebook_to_python(
            input_jupyter_notebook_file_path, python_file)
        ...
        functions_with_decorator = get_functions_with_decorator(
            module, dp_function)
        ...
        with open(zip_file, 'rb') as zip_file_opened:
            files = [
                (
                    "file",
                    (f"{dict_args['name']}.zip",
                     zip_file_opened, "application/zip"),
                )
            ]
            response = requests.request(
...
@click.argument('_import_', required=1, type=click.Choice(['import']))
@click.option("-f", "--input_share_folder_path", callback=check_empty_value, prompt="Enter input share folder path", help="The input share folder path")
def notebook_share(_import_, input_share_folder_path):
    tmpFolder = "notebook_share_tmp"

    if not is_credential_imported():
        return

    # Zip Process
    try:
        file_or_folder_path_org = input_share_folder_path.replace(
            "\\", "/").strip('"')
        if not os.path.exists(file_or_folder_path_org):
            raise Exception(
                f"'{file_or_folder_path_org}' does not exist")

        base_file_name, file_extension = os.path.splitext(
            file_or_folder_path_org)
        ...
        with zipfile.ZipFile(zip_file, 'w', zipfile.ZIP_DEFLATED) as zf:
            # Walk through the folder and its contents
            for root, dirs, files in os.walk(file_or_folder_path_org):
                # Exclude ignored folders from further traversal
                dirs[:] = [d for d in dirs if d not in ignore_folders]
                for file in files:
                    # Create the relative path
                    relative_path = os.path.relpath(
                        os.path.join(root, file), file_or_folder_path_org)
                    # Add the file to the zip file
                    zf.write(os.path.join(root, file), arcname=relative_path)
        click.echo(
            f"Folder '{file_or_folder_path_org}' zipped successfully")
    except Exception as exc:
        click.echo(
            f"Error: Folder '{input_share_folder_path}' can not zip because '{exc}'")
        return

    # Import process
    try:
        invoked_url = f'http://{host}:{port}/api/v1/team-collaborations/upload/'
        dt_conf_file = "credentials.json"
        with open(dt_conf_file, "r") as conf_file:
            dt_config = json.load(conf_file)
        access_key = dt_config["access_key"]
