...
class CommonBase(object):
    def __new__(cls, *args, **kwargs):
        log.info("Creating %s instance" % cls.__name__)
    ...
    def __del__(self):
        ...
        """
        log.debug("Destroying %s instance", self.__class__.__name__)
    ...
    def signal_wait(self):
        ...
        while not os.path.exists(signal_file):
            log.info("Bad Request,look before , Waiting for signal...")
            time.sleep(3)
        log.info("Signal received, continue processing...")
        os.remove(path=signal_file)
        log.info("del Signal file")
        ...
        try:
            with open(file=file_path,mode="w+",encoding="utf-8") as fp:
                ...
                if file_extension == '.json':
                    ...
                    pass
            state = True
            log.info("success write file :%s",file_path)
