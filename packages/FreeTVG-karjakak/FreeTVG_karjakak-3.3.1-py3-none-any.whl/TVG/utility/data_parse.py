...
class ParseData(TreeView):
    ...
    pos: int = 0
    size: int = 0
    stackids: list[int] = field(default_factory=list)
    ...
    def record_stack(self) -> None:
        """Initial stack creation before new one created"""

        if stack := tuple(self.add_stacks()):
            with open(Path(self.filename).parent.joinpath("old_stack.dat"), "wb") as sw:
                sw.write(f"{stack}".encode())

    def stack_exist(self) -> bool:
        """Checking Stack Record existence"""

        if Path(self.filename).parent.joinpath("old_stack.dat").exists():
            return True
        else:
            return False

    def analyze_stack(self) -> int | None:
        """Getting stack that invoke differences in stacking order"""

        if self.stack_exist() and self.stackids:
            try:
                with open(
                    Path(self.filename).parent.joinpath("old_stack.dat"), "rb"
                ) as rw:
                    get_old_stack = leval(rw.read().decode().strip("\n"))
                return self.analyze_stack_diff(self.stackids, get_old_stack[-1][-1])
            finally:
                del rw, get_old_stack

    def analyze_stack_diff(self, stack_diff: list, last_key: int) -> int:
        """Seperate the stacks that invoke differences in stacking order"""

        try:
            get_injected_stack = 0
            for i in stack_diff:
                if i < last_key:
                    get_injected_stack += 1
            return get_injected_stack
        finally:
            del get_injected_stack, stack_diff, last_key

    def del_stack_record(self) -> None:
        """Delete stack record"""

        if self.stack_exist():
            os.remove(Path(self.filename).parent.joinpath("old_stack.dat"))
            ...
            try:
                ...
                for n, s in self.getdata():
                    if s[0] == "+":
                        ...
                        if pos or pos == 0:
                            sequence.append((n - 1, pos))
                            pos = None
                    elif n == self.getdatanum() - 1:
                        if pos or pos == 0:
                            sequence.append((n, pos))
                if sequence:
    ...
    def update_data_sum(self, plus: bool = True) -> tuple | None:
        ...
        if self.check_exist():
            try:
                ...
                dat = []
                stack = []
                stack_invoke = self.analyze_stack()
                for i, p in self.add_stacks():
                    for k, v in self.getkv():
                        ...
                        if not stack:
                            ...
                            if stack_invoke and (p + num) > self.stackids[-1]:
                                k += -stack_invoke if plus else stack_invoke
                            if k <= i:
                                if k not in dat:
                                    dat.append(k)
                            else:
