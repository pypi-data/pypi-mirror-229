...
def dq_path_workspace(user_sandbox=None):
    ...
    pj_dq_dir_cookie_name = os.path.join(pj_dir_workspace, pj_dq_dir_name, "cookie")
    ...
    if is_windows:
        ...
        pj_dq_dir_cookie_name = pj_dq_dir_cookie_name.replace("\\", "/")
    ...
    dq_creating_directory_sandbox(path=pj_dq_dir_cookie_name)
    ...
    os.environ['pj_dq_dir_cookie_name'] = pj_dq_dir_cookie_name
...
def dq_generated_mvp(table_master_name=None,
    ...
    dir_hocons_mvp_name = os.getenv('pj_dq_dir_mvp_name')
    dir_cookie_name = os.getenv('pj_dq_dir_cookie_name')
    ...
    s = requests.Session()
    filename_cookie = "cookie_artifactory.json"
    dir_cookie_name_filename = os.path.join(dir_cookie_name, filename_cookie)
    artifactory_gdt = "http://artifactory-gdt.central-02.nextgen.igrupobbva/artifactory/"

    if cookie_value not in ("", " ", None, np.nan, np.NaN):
        with open(dir_cookie_name_filename, "w+") as f:
            cookie_au_json = {cook.get("name"): cook.get("value")
                              for cook in json.loads(cookie_value)}
            cookie_au_json2 = json.dumps(cookie_au_json, indent=4)
            f.write(cookie_au_json2)

        cookies = json.loads(Path(dir_cookie_name_filename).read_text())
        cookies = requests.utils.cookiejar_from_dict(cookies)
        s.cookies.update(cookies)
        artifactory_gdt = "https://artifactory.globaldevtools.bbva.com/ui/native/"
