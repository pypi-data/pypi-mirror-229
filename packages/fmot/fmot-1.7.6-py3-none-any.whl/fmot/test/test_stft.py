...
class Model(nn.Module):
    ...
    def __init__(self, n_fft=512):
        ...
        N_1 = int(3/4 * n_fft)
        N_2 = int(1/4 * n_fft)
        self.asym_window = nn.Parameter(torch.tensor([np.sin(np.pi/2 * n/N_1) for n in range(N_1)] + \
                      [np.cos(np.pi/2 * n/N_2) for n in range(N_2)], dtype=torch.float32), requires_grad=False)
        self.stft_kwargs = {'n_fft': n_fft,
