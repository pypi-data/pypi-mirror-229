############################################################################
######################## Print Tricks & More - version 0.3.132 #############
############################################################################
...
TODO_QUICK = """
    ...
    - easy_imports:
        - Create option to search for main.py and then use that folder for the path. 
        - Create a core1k version that does the main.py but by default if you just do core1k.easy_import
        
        - Should check to see if the incoming file that imported this, is running as "main" or not. 
            - The whole purpose of easy_imports is to allow any file to become the main with relative folders etc. 
                So we don't want it to do the easy_imports statement if it is not main because it won't make sense otherwise. 
        - Should consider easy folder finding as well:
            - If doesn't find a folder/file that you specify, can backtrack up to the project folder you passed/specified or where your main.py is located. 
                - Then it finds the folder/file you specified. 

        - if main.py cannot be found, use the saved topmost __init__.py. 
        - if a source file/direcotry is passed and can't be found, check to see if it's possibly
        been mistyped. If so, return and let the user know. 
...
class pt:
    ...
    @staticmethod
    def easy_imports(source=None):
        '''
            source - Can put in the name of your project folder / main app folder name or
            you can put main.py or whatever file is located where your main folder is also 
            located. 
            '''
        ...
        main_file_path = Path(sys.argv[0]).resolve()
        
        # pt(current_file, file_path, parent, main_file_path)


        ### - If calling file is not main, skip it because this is irrelevant
        
        if file_path != main_file_path:
            print(f"{C.t1}{C.es}{filename}{C.er} is NOT running as __main__")
            return
        
        
        ### - If caller file is already in path, skip it
        
        file_dir = str(file_path.parent)
        source_dir = str(Path(source).parent) if source else None

        if file_dir.lower() in [path.lower() for path in sys.path] or (source_dir and source_dir.lower() in [path.lower() for path in sys.path]):
            print("Already in the system path.")
            return
        ...
        if source is not None and source != '':
            if os.path.isfile(source):
                ...
                while parent != parent.parent:
                    if filename in os.listdir(parent):
                        break
                    elif possible_mistyped_path is None:
                        possible_mistyped_path = os.path.join(parent, filename)
                    parent = parent.parent
                else:  # This else clause corresponds to the while loop
                    source_not_found = True

            elif os.path.isdir(source):
                source = source.lower()
                while parent.name.lower() != source and parent != parent.parent:
                    parent = parent.parent
                else:  # This else clause corresponds to the while loop
                    source_not_found = True
            else:
            ...
            # If source is None or a filename, look for the file in parent directories
            filename = source if source else "main.py"
            while parent != parent.parent:
                if filename in os.listdir(parent):
                    break
                parent = parent.parent
        ...
        if source_not_found:
            if possible_mistyped_path is not None:
                print(f"Possible mistyped path: {possible_mistyped_path}")
                return 
        ...
        import_str = f"pt.easy_import() {'successful' if str(parent) in sys.path else 'unsuccessful'}" \
            f"- The directory {parent} {'is now' if str(parent) in sys.path else 'is not'} in the system path."
            
        print(
            f"{NAM.inst_nam}.easy_imports({formatted_args}): "
            f"(Line :{line_no}, {func_name_fmt}{file_name})"
            f"{C.t1}{C.eu}{filename}{C.er}, is running as __main__ ({file_path})"
            f"{import_str}"
        )
        ...
        # for i, file_name in enumerate(os.listdir(parent)):
