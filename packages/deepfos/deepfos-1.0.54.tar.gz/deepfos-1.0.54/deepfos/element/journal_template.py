...
from typing import Union, Type, Dict, List, Any, Tuple, Set
...
from deepfos.element.datatable import DataTableMySQL, DataTableSQLServer, DataTableOracle, \
    DataTableClickHouse
...
class JournalTemplate(ElementBase):
    """单据模板"""
    api_class = JournalTemplateAPI
    api: JournalTemplateAPI
    _RE_PARSE_SERVER = re.compile(r"data[-]?table-(.*?)-server[\d]-[\d]")

    @cached_property
    def meta(self) -> JournalTemplateDetail:
        """元信息"""
        return self.api.journaltemplate.query_detail(
    ...
    def _datatable_class(
            self, server_name
    ) -> Union[Type[DataTableMySQL], Type[DataTableClickHouse], Type[DataTableOracle], Type[DataTableSQLServer]]:
    ...
    def approval_record(self):
        ...
        datatable = self._datatable_class(self.basic_info.approveRecord.serverName)
        return datatable(
    ...
    def close_account(self):
        ...
        datatable = self._datatable_class(self.basic_info.closeAccountDB.serverName)
        return datatable(
    ...
    def approval_process(self):
        ...
        return ApprovalProcess(element_name=approval_ele.elementName,
    ...
    def role_strategy(self):
        """角色方案元素"""
        if (rs_deltail := self.basic_info.rsElementDetail) is None:
            raise ValueError('无角色方案元素信息')

        return RoleStrategy(
            element_name=rs_deltail.elementName,
            folder_id=rs_deltail.folderId,
            path=rs_deltail.path,
            server_name=rs_deltail.serverName
        )

    @cached_property
    def posting_params(self):
        """过账参数信息"""
        return self.api.posting.query_posting_param(
            PostingParamQueryDto.construct_from(self.element_info)
        ).params
    ...
    def delete(self, where: Union[pd.DataFrame, Dict[str, List]]):
        """按条件删除日记账数据
        ...
        return self.api.journal.delete_by_filter(
            JournalDataBatchDel(
                templateCode=self.element_name,
                path=self._path,
                folderId=self.element_info.folderId,
    ...
    def get_posting(
        ...
        """
        return self.api.posting.get_posting(self._get_posting_payload(where))
    ...
    def cancel_posting(
        ...
        """
        return self.api.posting.cancel_posting(self._get_posting_payload(where))
    ...
    def batch_query_raw(
        ...
        return self.api.journal.get_batch_data(payload).selectResult
    ...
    def batch_query(
        ...
        """
        return pd.DataFrame(self.batch_query_raw(query, show_detail))
