...
def melon_scanner(host,delay=0):
    ...
    # check for open ports/banners
    print("checking for open ports/banners")
    banners = dolphin_scanner(urllib.parse.urlparse(host).netloc,True)
    ...
    print("checking for python injection")
    for mal in mal_python:
        time.sleep(delay)
        try:
    ...
    for password in password_list:
        ...
        try:
            ...
            ftp.quit()
            hits.append(f"ftp weak credentials allowed- admin:{password}")
            break
    ...
    for password in password_list:
        ...
        try:
            ...
            ftp.login("admin",password)
            ftp.quit()
            hits.append(f"secure ftp weak credentials allowed- admin:{password}")
            break
