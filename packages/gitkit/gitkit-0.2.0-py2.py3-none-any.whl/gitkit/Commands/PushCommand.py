...
class PushCommand:
    ...
    # @click.option('-p/-np', '--private/--no-private', is_flag=True, default=None, help='Default with
    # private and public. -p = Private only. -np = Public only')
    # @click.option('-f/-nf', '--forks/--no-forks', is_flag=True, default=None, help='Default with forks.
    # -f = Forks only. -nf = Without forks')
    # @click.option('-o/-no', '--owner/--no-owner', is_flag=True, default=None, help='Default owned and not owned.
    # -o = Owned only. -no = Not owned only.')
    # @click.option('-d/-nd', '--dirty/--no-dirty', is_flag=True, default=None, help='Default clean and dirty.
    # -d = Dirty only. -nd = Clean only')
