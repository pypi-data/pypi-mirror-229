...
from circles_local_aws_s3_storage_python import CirclesStorage
import json
...
def create_model(input_shape: np.shape, learning_rate: float) -> Sequential:
    ...
    logger.start(CREATE_MODEL, object={"input_shape": input_shape, "learning_rate": learning_rate})
    ...
    summary_str = []
    CNNModel2.summary(print_fn=lambda x: summary_str.append(x))
    summary_str = "\n".join(summary_str)

    logger.end(CREATE_MODEL, object={"CNNModel2": summary_str})
    return summary_str, CNNModel2
...
def capture_write(filename: str = "image.jpeg", port: int = 0, ramp_frames: int = 30, x: int = 178, y: int = 218) -> None:
    ...
    Args:
        ...
        y (int): The height of the image in pixels. Default is 2 
    Returns:
        numpy.ndarray: The captured image as a NumPy array.
    """

    CAPTURE_WRITE_METHOD_NAME = "capture_write()"
    logger.start(CAPTURE_WRITE_METHOD_NAME, object={
    ...
    del (camera)
    logger.end(CAPTURE_WRITE_METHOD_NAME)
...
class GenderClassifier:
    ...
    def _get_prediction(self, model_prediction: np.array) -> str:
        GET_PREDICTION_METHOD_NAME = "_get_prediction()"

        #you can't serielize a np.array
        logger.start(GET_PREDICTION_METHOD_NAME)
        ...
        logger.end(GET_PREDICTION_METHOD_NAME,
    ...
    def predict_gender(self, model_path: str, image_path: str = None) -> str:
        ...
        PREDICT_GENDERER_METHOD_NAME = "predict_gender()"
        logger.start(PREDICT_GENDERER_METHOD_NAME, object={
        ...
        if INTERFACE_MODE == "BATCH":
            ...
            if inquiry.lower() != "y" and inquiry.lower != "yes":
                self.logger.info("\nAborting\n")
                self.logger.end(PREDICT_GENDERER_METHOD_NAME)
                exit()
            ...
            logger.end(PREDICT_GENDERER_METHOD_NAME)
            return None
        ...
        logger.end(PREDICT_GENDERER_METHOD_NAME, object={
    ...
    def train_model2(self, learning_rate: float = 5e-5,
                     ...
                     save: bool = True, verbose=1, path_to_model="models/gender_detection.h5") -> Sequential:
        """
        ...
        TRAIN_MODEL_METHOD_NAME = "train_model()"
        logger.start(TRAIN_MODEL_METHOD_NAME, object={"learning_rate": learning_rate,
        ...
        try:
            ...
            logger.end(TRAIN_MODEL_METHOD_NAME)
            raise
        ...
        model_data, model = create_model((100, 100, 1), learning_rate=learning_rate)

        model.fit(train_data, train_labels, epochs=epochs, batch_size=batch_size,
                  validation_data=(test_data, test_labels), verbose=verbose)
        if save:
            model.save(path_to_model, save_format='h5')
            model_json = {
            ...
            self.save_model_to_storage(model_json)
    ...
    def load_model(self, path_to_model: str = "models/gender_detection.h5"):
        LOAD_MODEL_METHOD_NAME = "load_model()"
        logger.start(LOAD_MODEL_METHOD_NAME, object={
        ...
        logger.end(LOAD_MODEL_METHOD_NAME)
        return model

    def save_model_to_storage(model_json):

        with open("model.json", "w") as f:
            json.dump(model_json, f)

        CirclesStorage.AwsS3Storage.upload_file(
            file_path='model.json', bucket_name="circles-local-aws-s3-storage-python"
        )