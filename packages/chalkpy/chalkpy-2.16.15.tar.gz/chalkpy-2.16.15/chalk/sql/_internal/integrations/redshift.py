...
class RedshiftSourceImpl(BaseSQLSource):
    ...
    def __init__(
        ...
        # TODO figure out how we want to pass these in once this is called by customers and not just engine
        self._s3_client = s3_client
        self._s3_bucket = s3_bucket or load_integration_variable(
            name="REDSHIFT_UNLOAD_S3_BUCKET", integration_name=name
        )
        self._s3_prefix = s3_prefix or load_integration_variable(
            name="REDSHIFT_UNLOAD_S3_PREFIX", integration_name=name
        )
    ...
    def execute_query_efficient(
        ...
        if self._s3_client is None:
            raise UnsupportedEfficientExecutionError(
                "Unable to run efficient Redshift query execution (via UNLOAD) since an S3 Client was not provided. Falling back to default execution.",
                logging.WARN,
            )
        ...
        def _do_query():
            with self.get_engine().connect() if connection is None else contextlib.nullcontext(connection) as cnx:
                with cnx.begin():
                    ...
                    try:
                        unload_destination = f"{self._s3_prefix}/{temp_table_name}/"
