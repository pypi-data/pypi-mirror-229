...
def parse_row_into_object(db_cursor: pyodbc.Cursor, result_row: pyodbc.Row, obj: object, strict_attributes: bool = True,
    ...
    for column in db_cursor.description:
        ...
        if not hasattr(obj, column_name) and not add_attributes:
            continue
        setattr(obj, column_name, result_row[ci(db_cursor, column_name)])
...
class query_result_class(afg_freezable_object_class):
    def __init__(self, db_cursor: pyodbc.Cursor = None, query_text: str = None, query_params: tuple = None,
        ...
        db_cursor.execute(query_text, query_params)

        if db_cursor.rowcount > 0:
            result_row = db_cursor.fetchone()
            parse_row_into_object(db_cursor, result_row, self, strict_attributes, add_attributes)
