...
class SQLiteDataStorage(AbstractDataStorage):
    ...
    def subtract_query(
        ...
        source_table = source_query.get_final_froms()[0]
        target_table = target_query.get_final_froms()[0]

        # complex condition logic needed because we have differences between index and
        # dataset table. It won't be needed when we unite schemas of those two tables
        if source_table.name.startswith(self.LISTING_TABLE_NAME_PREFIX):
            source_expression = target_table.c.source == source_uri
        elif target_table.name.startswith(self.LISTING_TABLE_NAME_PREFIX):
            source_expression = source_table.c.source == source_uri
        else:
            # both are dataset originated queries
            source_expression = source_table.c.source == target_table.c.source

        return source_query.select_from(
            sqlalchemy.join(
                source_table,
                target_table,
                source_expression
                & (source_table.c.parent == target_table.c.parent)
                & (source_table.c.name == target_table.c.name),
                isouter=True,
            )
        ).where(
