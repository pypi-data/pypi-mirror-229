...
class TestLogUpload(TestCase):
    ...
    def setUp(self):
        ...
        self.root.mkdir()
        os.chdir(self.root)
        self._patch_wait_for_request(deepcopy(SUITE))
        self._patch_http_request()

    def _patch_wait_for_request(self, suite):
        ...
        response = Mock()
        response.json.return_value = json.loads(json.dumps(suite), object_pairs_hook=OrderedDict)
    ...
    def test_log_upload(self):
        ...
        self._patch_wait_for_request(suite)
        with self.environ(environment):
            ...
            for method_call in self.http_request.call_args_list:
                ...
                self.assertEqual(verb, suite["log_area"]["upload"]["method"])
                self.assertEqual(
                    url,
                    suite["log_area"]["upload"]["url"].format(
    ...
    def test_log_upload_rename_file_too_long(self):
        ...
        with self.environ(environment):
            ...
            response.json.return_value = json.loads(
                json.dumps(suite), object_pairs_hook=OrderedDict
            )
            self.wait_for_request.return_value = [response]
            etr.run_etr()
            test_suite_finished = self.get_event(
