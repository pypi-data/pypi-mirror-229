...
def add_hook(module: nn.Module, output_idx: int, fqir_name: str,
            state_dump: dict, src_types: dict):
...
@torch.no_grad()
def compare_to_fqir(cmodel: fmot.ConvertedModel, graph: fmot.fqir.GraphProto,
                    inputs: List[torch.Tensor]):
    if not hasattr(cmodel, 'tsrc_dict'):
        raise RuntimeError('No tsrc_dict to source FQIR tensors to their qat counterparts')
...
def run_fqir_autocompare(cmodel: fmot.ConvertedModel, graph: fmot.fqir.GraphProto,
                    inputs: List[torch.Tensor]):
