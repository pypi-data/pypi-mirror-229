from histcite.parse_reference import ParseReference, WosField, CssciField, ScopusField
...
def test_parse_one_cell():
    ...
    assert isinstance(parsed_wos_ref, WosField)
    ...
    assert isinstance(parsed_cssci_ref, CssciField)
    ...
    assert isinstance(parsed_scopus_ref, ScopusField)
...
def test_parse_ref_cell():
    parsed_ref_list = ParseReference().parse_ref_cell(wos_ref_cell, "wos")
    assert isinstance(parsed_ref_list, list)
    assert len(parsed_ref_list) == 4
    assert isinstance(parsed_ref_list[0], WosField)
    assert isinstance(parsed_ref_list[1], WosField)
    assert isinstance(parsed_ref_list[2], WosField)
    assert isinstance(parsed_ref_list[3], WosField)
    assert parsed_ref_list[0].FAU == "Bengio Y"
    assert parsed_ref_list[1].PY == "2003"
    assert parsed_ref_list[2].J9 == "6 INT C SPOK LANG PR"
    assert parsed_ref_list[3].VL == "20"
