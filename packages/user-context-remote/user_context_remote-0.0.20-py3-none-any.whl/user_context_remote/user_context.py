...
# TODO: add mini logger from sdk package
    ...
    def _authenticate_by_email_and_password(self, email: str, password: str) -> str:
        ...
        try:
            ...
            if output.status_code != HTTPStatus.OK:
                raise Exception(output.text, file=sys.stderr)
            self.user_jwt_token = output.json()["data"]["token"]
            # logger.end(object={"user_jwt_token": user_jwt_token })
            return output
        except Exception as e:
            print(
                "Error(Exception): user-context-remote-python _authenticate() " + e, file=sys.stderr)
