    ...
    for or_condition in decombine_condition(  # type: ignore[var-annotated]
        ...
        for comparison_condition in comparison_conditions:
            ...
            if comparison_condition.operator == "ne":
                and_list.append(
                    {"$not": {column_name: comparison_condition.target.value}}
                )
            else:
                operator = (
                    "lte"
                ...
                and_list.append(
                    {column_name: {f"${operator}": comparison_condition.target.value}}
                )

        and_list.extend(
            {
                isin_condition.subject.column_name: {
                    "$in": [element.value for element in isin_condition.elements]
                }
            }
