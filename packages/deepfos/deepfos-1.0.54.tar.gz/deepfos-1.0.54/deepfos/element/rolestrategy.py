...
class RoleStrategy(ElementBase):
    """权限方案"""
    api_class = RoleStrategyAPI
    api: RoleStrategyAPI
    ...
    def meta(self):
        ...
        if self.__meta is None:
            self.query(user=self.current_user)
        return self.__meta
    ...
    def _query_impl(
        ...
        return self.api.rolestrategy.info_list(payload)

    def _query_records(self, user):
        if user is None or user == self.current_user:
            return self.meta.records
        else:
            return self.query(user=user).records

    def query_roles(self, user: str = None) -> List[RoleDetailVO]:
        """查询用户所属的角色
        ...
        """
        records = self._query_records(user)
        return sum((r.roles for r in records), [])

    def query_role_groups(self, user: str = None) -> List[RoleGroupDetailVO]:
        """查询用户所属的角色组
