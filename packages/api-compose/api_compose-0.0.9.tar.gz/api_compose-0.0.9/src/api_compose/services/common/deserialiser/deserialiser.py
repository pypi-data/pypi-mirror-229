...
from api_compose.services.common.deserialiser.checks import get_file_paths_relative_to
from api_compose.services.common.deserialiser.exceptions import ManifestMissingModelNameException, \
    NonUniqueManifestNameException, ManifestNotFoundException
...
def deserialise_manifest_to_dict(
    ...
    context_merged = merge_dict(overlayed_dict=GlobalSettingsModelSingleton.get().env_vars, overlaying_dict=context)
...
def get_manifest_relative_path(
    ...
    if len(relative_paths) == 0:
        raise ManifestNotFoundException(manifests_folder_path, stem)
