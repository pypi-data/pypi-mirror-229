...
def update_php_versions(
    ...
    cluster.php_versions = php_versions
    cluster.update()
...
@app.command(rich_help_panel=HELP_PANEL_UPDATE)
@catch_api_exception
def update_custom_php_modules(
    name: str,
    custom_php_modules_names: List[PHPExtension],
) -> None:
    """Update custom PHP modules."""
    cluster = get_object(get_support().clusters, name=name)

    cluster.custom_php_modules_names = custom_php_modules_names
    cluster.update()
...
def update_nodejs_versions(
    ...
    cluster.nodejs_versions = nodejs_versions
    cluster.update()
