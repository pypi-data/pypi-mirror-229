...
def WeightedReliabilityScoring(base_data, reliability_scores, WL_columns = None ):
    '''
    Returns an array on weighted reliability lable matching the index order of base_data. 
    Calculates weighted average of weak labels, where reliability scores of a label are assumed as respective label weights. This approach helps build a relatively strong label from multiple weak labels.
    This approach requires some quantifiable sense of the underlying business objective. Reliabiality score of a weak label can simply be the coverage of the label across the expected outcome. 
    Example: In revenue size estimation, reliability score of a label could the market share on business objective covered by the label. 
    
    Wt.Reliability Scpre = (w1*L1 + w2*L2 +...+ wn*Ln)/(w1+w2+...+wn)
    
    base_data: pandas dataframe with weak labels  
    WL_columns: Weak label column names to be used in the calculation 
    reliability_scores :  data types acceptable are list and dict. 
                  When "list" type scores are proided WL_columns parameter is required and fucntion assumes index order when matching WL_columns to a reliability_score.
                  When dict type score are provided function expected Keys of dict to contain names of the weak labels and values to be their respective reliability scores. When reliability_scores is dict WL_columns parameter is no longer requierd. 
    '''
    
    if isinstance(reliability_scores, dict):
        missing_cols = [col for col in list(reliability_scores.keys()) if col not in base_data.columns ]
        assert len(missing_cols) == 0,  'Dataframe provided does not contain following features to calculated weighted scoring :: {}'.format(', '.join(missing_cols))
        
    elif isinstance(reliability_scores, list):
        assert WL_columns != None, 'reliability_scores are provided as a list. WL_columns parameter cannot be left empty.'
        missing_cols = [col for col in WL_columns if col not in base_data.columns ]
        assert len(missing_cols) == 0,  'Dataframe provided does not contain following features to calculated weighted scoring :: {}'.format(', '.join(missing_cols))
        reliability_scores = {k:v for k,v in zip(WL_columns, reliability_scores)}
        
    WL = base_data[list(reliability_scores.keys())]
    numerator_op =  np.array([WL[k].values*v for k,v in reliability_scores.items()]).T
    wtRel_label = np.sum(numerator_op, axis=1) / sum(reliability_scores.values())
