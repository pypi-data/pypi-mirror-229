...
def validateDuplicates(object:DataFrame,fields:List,registersAmount: int,entity: str,threshold: int, repetitionsNumber = 1):
    ...
    print(fields)
    print(windowSpec)
    object2 = object.withColumn("valor",lit(One))
    object3 = object2.withColumn("sum",sum(col("valor")).over(windowSpec))
    errorDf = object3.filter(col("sum")>repetitionsNumber).drop("sum","valor")
