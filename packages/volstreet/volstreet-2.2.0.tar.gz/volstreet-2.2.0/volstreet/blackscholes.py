...
def simulate_iv(
    ...
    bs_logger.debug(
        f"simulate_iv inputs condensed to: original_iv={original_iv}, original_spot={original_spot}, "
        f"original_time_to_expiry={original_time_to_expiry}, new_spot={new_spot}, "
        f"new_time_to_expiry={new_time_to_expiry}"
    )
    ...
    bs_logger.debug(
        f"simulate_iv: called iv_multiple_to_atm with original_time_to_expiry={original_time_to_expiry}, "
        f"original_spot={original_spot}, strike={strike}, symbol={symbol} "
        f"and got current_iv_multiple={current_iv_multiple}."
    )

    bs_logger.debug(
        f"simulate_iv: adjusted original_iv={original_iv} to atm_iv={atm_iv} "
        f"using current_iv_multiple={current_iv_multiple}"
    )
    ...
    bs_logger.debug(
        ...
        f"using maximum_increase={maximum_increase} and new_time_to_expiry={new_time_to_expiry}"
    )

    iv_multiple = iv_multiple_to_atm(new_time_to_expiry, new_spot, strike, symbol)

    bs_logger.debug(
        f"simulate_iv: called iv_multiple_to_atm with new_time_to_expiry={new_time_to_expiry}, "
        f"new_spot={new_spot}, strike={strike}, symbol={symbol} "
        f"and got iv_multiple={iv_multiple}."
    )
    return new_atm_iv * iv_multiple
