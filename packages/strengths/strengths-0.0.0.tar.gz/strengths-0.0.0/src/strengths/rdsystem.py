...
class RDSystem : 
    ...
    def state(self, v) :
        if isarray(v) :
            v = UnitArray(v, Units(self.units_system, quantity_units_dimensions()))
        if type(v) != UnitArray : 
            raise ValueError("state must be a UnitArray")
        else : 
            raise ValueError("invalid type for state. must be a UnitArray or an array.")
            
        if v.units.dim != {"space" : 0, "time" : 0, "quantity" : 1} : 
...
def rdsystem_from_dict(d):
    ...
    return RDSystem(rdn, space=space, units_system=units)
