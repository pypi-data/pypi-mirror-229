...
def __predict__(pipe, data, output_level, positions, keep_stranger_features, metadata, multithread,
                drop_irrelevant_cols, return_spark_df, get_embeddings, isdb=False):
    '''
    ...
    if 'DB_ENDPOINT_ENV' in os.environ:
        ...
        """
        if not isdb:
            params = PredictParams.maybe_from_pandas_df(data).dict()
            files = []
            if 'file' in data.columns and 'file_type' in data.columns:
                ...
                data = files
            return __predict__(pipe, data, **params, isdb=True)
