...
class MyMetaAngledClass(type):
    def __new__(cls, name: str, bases: tuple, namespace: dict, **kwargs) -> type:
    ...
    @staticmethod
    def _create_new_namespace(namespace: dict, **kwargs) -> dict:
...
class MyMetaAngledClassModel(PydanticMetaClass, MyMetaAngledClass):
    def __new__(cls, name, bases, namespace, **kwargs) -> PydanticMetaClass:
