from base.structural.edge_id import *
from base.mapping.signal_function import *
from sim.dgcm import *
from sim.random_mlp import *
import yaml
import json
...
class PatientSampler(DGCM):
    ...
    def __init__(self, graph=None, config_path=None, rng=None, *args, **kwargs):
        self._is_init = False
        
        
        self._info = None # get something like  {v: {'C': int, 'offset': }, }, indim: int, hdim:int}  lag_window: int
        if config_path is not None:
            graph = self.read_config(config_path) 
        ...
        if self._info is not None:
            ...
            for v in self.nodes:
                ...
                if not is_discrete:
                    ...
                    sfunc = SignalFunction.ConstantSignal(offset)
                else:
                    values = node_info['values']
                    
                    efunc = RandomDiscreteMLP(v, self.nodes, indim, values, hdim)
                    sfunc = DiscreteSignal.UniformDiscrete(values)
    ...
    def simulate_perturb(self, T, n_max=5, pgv=None, *args, **kwargs):
        ...
        data = self.__step_based_generation(T, sfs, efs, eids_for_nodes, pre_len, traverse_order, 
                                           safe_mode=False, 
                                           noisy_signal=True, 
                                           full_offset=False,
