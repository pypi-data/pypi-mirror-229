...
class ExtendedModel(AbstractObject):
    ...
    def initialize(self):
        ...
        if self.mc_name is None:
            ...
            if not mc_names:
                raise RuntimeError(f"no ModelConfig object found in the workspace: {self.ws_name}")
            if len(mc_names) > 1:
                self.stdout.warning(f"Found multiple ModelConfig instances from the workspace: {self.ws_name}. "
