...
list_company_path = os.path.join(data_path, 'list_company.xlsx')
save_list = os.path.join(data_path, 'ds_ngành_đã_lọc.xlsx')

def load_list_company():
    try:
        path_nganh_check = os.path.join(data_path, 'dsnganh.xlsx')
        ...
        return 'Giớn hạn thành viên, nếu bạn đã được cấp phép. Sử dụng hàm key_id(Mã id) để kích hoạt'
...
def list_company_24h():
    try:
        try:
            save_list = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'data', 'ds_ngành_đã_lọc.xlsx')
