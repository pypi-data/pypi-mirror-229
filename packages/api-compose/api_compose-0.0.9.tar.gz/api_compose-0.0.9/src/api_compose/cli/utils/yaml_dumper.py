...
def dump_collection_to_multiple_yaml_files(
        collection: Union[Dict, List, Tuple],
        folder_path: Path,
        key_to_path: str,
):
    if isinstance(collection, dict):
        for key, value in collection.items():
            if key == key_to_path and isinstance(value, Path):
                dump_dict_to_single_yaml_file(dict_=collection, file_path=folder_path.joinpath(value))
            else:
                dump_collection_to_multiple_yaml_files(value, folder_path, key_to_path)  # Recursively call the function for nested dictionaries
    elif isinstance(collection, tuple) or isinstance(collection, list):
        for val in collection:
            dump_collection_to_multiple_yaml_files(val, folder_path, key_to_path)  # Recursively call the function for nested dictionaries
