...
class Zabbix(object):
        ...
        try:
            ...
            match True:
                ...
                case True if not utils.v_true(name, str):
                    logger.error('ERROR!! name is not string')
                    return False
                case True if not utils.v_true(proc_name, str):
                    logger.error('ERROR!! proc_name is not string')
                    return False
                case True if not utils.v_true(proc_cmdline, str):
                    logger.error('ERROR!! proc_cmdline is not string')
                    return False
                case _:
                    next
            ...
            proc_num_trigger_name: str = f'{name} not running'
            ...
            for ip in ips:
                ...
                logger.warning(f'create item response:{_}') if utils.v_true(self.debug, bool) else next
                if isinstance(_, utils.NoneType) or (utils.v_true(_, dict) and _.get('error')):
                    logger.error('create item error')
                    logger.error(_.get('error', {}).get('data'))
                    continue
                else:
                ...
                logger.warning(f'create trigger response:{_}') if utils.v_true(self.debug, bool) else next
                if isinstance(_, utils.NoneType) or (utils.v_true(_, dict) and _.get('error')):
    ...
    def create_ip_port_object(self, ips: list, name: str, target_ip: str, target_port: int, item_type: int = 0) -> bool:
        '''
        创建进程对象
        ...
        try:
            ...
            match True:
                ...
                case True if not utils.v_true(target_ip, str):
                    logger.error('ERROR!! ip is not string')
                    return False
                case True if not utils.v_true(target_ip, int):
                    logger.error('ERROR!! port is not integer')
                    return False
