...
def query_to_json(
    ...
    def expand_data(in_data):
        ...
        if drop_irrelevant_cols:
            datas.append(drop_irrelevant_cols)
        if get_embeddings:
            datas.append(get_embeddings)
        if keep_stranger_features:
            datas.append(keep_stranger_features)
        if multithread:
            datas.append(multithread)
        return datas

    positions = str(positions)
    metadata = str(metadata)
    drop_irrelevant_cols = str(drop_irrelevant_cols)
    get_embeddings = str(get_embeddings)
    keep_stranger_features = str(keep_stranger_features)
