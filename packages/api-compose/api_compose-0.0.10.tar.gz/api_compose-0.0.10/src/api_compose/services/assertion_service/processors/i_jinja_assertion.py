...
class InteractiveJinjaAssertion(cmd2.Cmd):
    ...
    def do_examples(self, arg):
        ...
        self.poutput(
            f"""\t\t render {{ acp.actions.output_body('{self.example_action_model.execution_id}', '$.field_one') }} == '1234'  """)
        self.poutput(f"""\t Assert Output Headers:""")
        self.poutput(
            f"""\t\t render {{ acp.actions.output_headers('{self.example_action_model.execution_id}', '$.Server') }} == 'Google Frontend'  """)
        self.poutput(f"""\t Assert Output Status Code:""")
        self.poutput(
            f"""\t\t render {{ acp.actions.output_status_code('{self.example_action_model.execution_id}' }} == 200  """)
