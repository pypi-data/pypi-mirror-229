...
class Probe:
    def __init__(self, dirpath):
        Probe.validate_probe_structure(dirpath)
    ...
    def read_conf(path) -> dict:
        with open(path, 'r') as pbconffile:
            ...
            _pbconf.update({'hash': dirhash(path, "sha256")})
            return _pbconf

    @staticmethod
    def validate_probe_structure(dirpath):
        """

        :param dirpath: Dir path pointing to a probe dir
        :return: True if dirpath points to a valid probe dir. False otherwise
        """
        assert (os.path.isfile(os.path.join(dirpath, 'main.py')) and
                os.path.isfile(os.path.join(dirpath, '.conf'))), ("Not parser directory. Missing 'parser.py' and/or "
                                                                  "template files")
...
class AnalysisMatrix(pd.DataFrame):
    ...
    def __init__(self, *args, **kwargs):
        super().__init__(columns=AnalysisMatrix.columns, *args, **kwargs)
    ...
    def add_subjects(self, *args):
        ...
        for fpath in args:
            try:
                ...
                # df['Address'] = address
                # ["Pending"] * len("hola")
                self.insert(-1, Subject(fpath), ["Pending"] * len(self), False)
    ...
    def load_probes(dirpath):
        ...
        for i in next(os.walk(dirpath))[1]:
            try:
                _loaded.loc[len(_loaded)] = [Probe(os.path.join(dirpath, i))]
                # _loaded.append(Probe(os.path.join(dirpath, i)))
            except ValueError:
                continue
        logging.info(f"({len(_loaded)}) Probes loaded: {[x.name for x in _loaded['Probe']]}")
        return _loaded
    ...
    def filter_by_id(self, include=None, exclude=None) -> list:
        ...
        if include is not None:
            ...
            return list(filter(lambda x: x.conf.get('id') in set(include), list(self['Probe'])))
            ...
            return list(filter(lambda x: x.conf.get('id') not in set(exclude), list(self['Probe'])))
        else:
            return list(self)
