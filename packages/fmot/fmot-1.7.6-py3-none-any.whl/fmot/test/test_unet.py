...
class UNetEncoder(nn.Module):
    def __init__(self, in_channels: int, expansion_factor: int,
                kernel_size: int, stride: int, num_layers: int):
        super().__init__()
        ...
        for _ in range(num_layers):
            self.layers.append(TemporalConv1d(
...
class UNetDecoder(nn.Module):
    def __init__(self, out_channels: int, expansion_factor: int,
                kernel_size: int, stride: int, num_layers: int):
        super().__init__()
        ...
        for _ in range(num_layers):
            self.layers.append(TemporalConvTranspose1d(
                in_channels = in_channels,
                out_channels = in_channels//expansion_factor,
                kernel_size = kernel_size,
                stride = stride
...
class UNet(nn.Module):
    def __init__(self, channels, expansion_factor, kernel_size, stride, rnn_hidden, num_layers):
        super().__init__()
        ...
        self.encoder = UNetEncoder(channels, expansion_factor, kernel_size, stride,
                                   num_layers)
        ...
        self.decoder = UNetDecoder(channels, expansion_factor, kernel_size, stride,
                                   num_layers)
...
def test_unet():
    ...
    y_orig = model(x)
    print(f"Called original model on input of shape {x.shape}, got output of shape {y_orig.shape}")
