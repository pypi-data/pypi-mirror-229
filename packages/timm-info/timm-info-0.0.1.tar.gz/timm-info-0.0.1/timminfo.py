...
def info(name: str):
    ...
    click.echo(f'Number of params:                {num_params:,}')
    click.echo(f'Estimated model size:            {model_size:0.3f} MB')
    click.echo(f'Number of feature layers:        {len(result)}')
    click.echo(f'Number of channels per feature:  {[tensor.shape[1] for tensor in result]}')
    if not can_be_pretrained:
        return
    click.echo(f'Pretrained Input Size: {model.pretrained_cfg["input_size"]}')
    click.echo()
