...
class Database:
    ...
    def open(self) -> Query:
        ...
        if not file_header.startswith("53514c69746520666f726d6174203300"):
            sqlite3.DatabaseError(f'{self.path} does not appear to be a valid database')
    ...
    def create(self, data_dir: PathLike, max_workers: int = 4, custom_header_func: Optional[CustomHeaderFunction] = None, *additional_dicom_tags: str):
        ...
        try:
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                if readers_len == 0:
                    raise sqlite3.DataError(f'No DICOM data found in {data_dir} ({len(self._errors)} errors occurred)')
                ...
                print(f"Database created at {self.path} with {len(self._errors)} errors.")
        finally:
            self._errors = [str(e) for e in self._errors if e]
            if self._errors:
                log = self.path.with_suffix('.log')
                with open(self.path.with_suffix('.log'), 'w') as f:
                    f.write(f'There are {len(self._errors)} errors.\n')
                    f.write([err for err in self._errors])
                print(f"Wrote {len(self._errors)} errors to log at {log.as_posix()}.")
    ...
    def _collect_data_walk(self, subdirectory: Path):
        readers = []
        for directory, _, filenames in os.walk(subdirectory, onerror=lambda err: self._errors.append(traceback.format_exception(err))):
                ...
                with tqdm(total=len(filenames), desc='Scanning files') as pbar, ThreadPoolExecutor(
                    ...
                    for future in as_completed(futures.keys()):
                        try:
                            ...
                            self._append_error('file scan', e, filenames=futures[future])
                        pbar.update(chunk_size)
                        pbar.set_description(f'({len(self._errors)} errors)')
                ...
                with tqdm(total=len(subdirectories), desc='Scanning directories') as pbar, ThreadPoolExecutor(
                    ...
                    for future in as_completed(futures):
                        try:
                            ...
                            self._append_error('dir scan', e, subdir=futures[future])
                        pbar.update()
                        pbar.set_description(f'({len(self._errors)} errors)')
                break
    ...
    def _create_columns(self, reader: DICOMImageReader):
        ...
        if dicom_metadata is None or len(dicom_columns) == 0:
            raise sqlite3.DataError(f'No DICOM data found')
    ...
    def _create_data_dir(self, data_dir: Path, readers: List[DICOMImageReader], temp_db: Path, max_workers: int):
        existing_errors_len = len(self._errors)
        chunk_size = max_workers
        ...
        with tqdm(total=len(readers)) as pbar, ThreadPoolExecutor(max_workers=max_workers) as pool:
            ...
            for future in as_completed(futures):
                try:
                    ...
                    self._append_error('create', e, DICOMImageReaders=futures[future])
                pbar.update(chunk_size)
                pbar.set_description(f'({len(self._errors) - existing_errors_len} errors)')
    ...
    def _extract_metadata(self, data_dir: Path, reader: DICOMImageReader):
        ...
        try:
            ...
            self._errors.append(traceback.format_exception(e))
        if metadata:
            return metadata
    ...
    def _append_error(self, name: str, exc: Exception, **kwargs):
        text = [f'Error during {name}:']
