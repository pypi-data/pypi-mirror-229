...
FACIAL_ANALYSIS_LOCAL_COMPONENT_NAME = "CirclesGenderDetectionPython/gender_detection.py"
...
class GenderClassifier:
    ...
    def _get_prediction(self, model_prediction: np.array) -> str:
        ...
        logger.end(GET_PREDICTION_FUNCTION_NAME, object={"prediction": prediction})
        return prediction
    ...
    def train_model2(self, learning_rate: float = 5e-5, epochs: int = 30, batch_size: int = 16, save: bool = True, verbose=1) -> Sequential:
        """
        This function trains the model on the training data and saves the model to a file.
        Inputs:
            learning_rate (float): the learning rate of the Adam optimizer. Defaults to 1e-3.
            ...
            verbose (int): a number indicating whether to print the training progress or not. Defaults to 2.
        ...
        try:
            ...
            logger.exception(
                f"Please make sure all the data files exists:\ntrain_data.npy\ntrain_labels.npy\ntest_data.npy\ntest_labels.npy\n\nGot error: {e}"
            )
        ...
        if save:
            model.save("./CirclesGenderDetectionPython/gender_detection",
                       save_format='h5')
    ...
    def load_model(path_to_model="CirclesGenderDetectionPython/gender_detection"):
