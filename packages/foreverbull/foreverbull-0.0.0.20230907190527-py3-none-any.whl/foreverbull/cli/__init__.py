...
def run(
    ...
    def socket_runner(fb: Foreverbull, socket_config: models.service.SocketConfig, stop_event: Event):
        ...
        router = broker.socket.MessageRouter()
        router.add_route(fb.info, "info")
        router.add_route(fb.configure_execution, "configure_execution", models.backtest.Execution)
        router.add_route(fb.run_execution, "run_execution")
        while not stop_event.is_set():
            ...
            try:
                ...
                rsp = router(req)
                ctx.send(rsp.dump())
                ctx.send(router.route(ctx.recv()))
                if req.task == "stop":
                    break
