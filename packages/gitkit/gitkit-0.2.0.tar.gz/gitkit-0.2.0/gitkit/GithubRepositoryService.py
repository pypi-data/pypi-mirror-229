...
class GithubRepositoryService:
    ...
    def __init__(self, username: str, password: str, preload: bool = False) -> None:
        ...
        ghUser = gh.get_user()
        self._user = ghUser  # cast(AuthenticatedUser, ghUser)
    ...
    # def CloneRepo(self, clone_url: str, repoPath: str):

    #    localRepo = Repo(repoPath)
    #    for branch in localRepo.branches:

    #       print(f'Clone branch {branch} from {clone_url} to {repoPath}')
    #       Repo.clone_from(clone_url, repoPath, branch)

    def CloneOrPullRepo(self, clone_url: str, repoPath: str, pull: bool):
        ...
        if not os.path.exists(repoPath):
            print(f'Clone {clone_url} to {repoPath}')
        ...
        if pull:
            for branch in localRepo.branches:
                print(f'Pull {branch.name} from {clone_url} to {repoPath}')
                o = localRepo.remotes.origin
                options = '--tags'
    #         o.pull(all=True)
                o.pull(options)
        else:
            print(f'Skipping pull from {clone_url} to {repoPath}')
    ...
    def __extractRepoName(self, repo: Repo) -> str:
        if repo.remotes.origin is None:
            return ""
