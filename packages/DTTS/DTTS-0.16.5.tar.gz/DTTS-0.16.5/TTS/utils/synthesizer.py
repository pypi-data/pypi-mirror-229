    ...
    def tts(
        ...
        **kwargs,
        ...
        if speaker_wav is not None and self.tts_model.speaker_manager is not None:
            wavname = os.path.splitext(speaker_wav[0])[0]
            ext = os.path.splitext(speaker_wav[0])[1]
            wav = None
            speaker_wav1=speaker_wav
            if ext != ".wav" :
                speaker_wav1 = wavname + ".wav"
                (
                 ffmpeg
                 .input(speaker_wav[0])
                 .output(speaker_wav1, format='wav', acodec='pcm_s16le', ac=2, ar='44100')
                 .overwrite_output()
                 .run()
                 )
            
            speaker_embedding = self.tts_model.speaker_manager.compute_embedding_from_clip(speaker_wav1)
