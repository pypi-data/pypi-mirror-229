...
def _line_to_attrs(line):
    try:
        message = pynmea2.parse(f"$GPRMC,{line}")
        return {
        ...
        logger.warning(f"Could not parse gps entry: '{line}'", exc_info=True)
    return None
...
def generate_states_main(
    ...
    media_list = tator_api.get_media_list(project=project_id, section=section.id)
    ...
    for media in media_list:
        ...
        try:
            ...
            datetime_lookup[media.id] = {"fps": media.fps, "toc_start": toc_start, "toc_end": toc_start + duration}
            ...
            if start_datetime != datetime.datetime.min:
                multi_lookup[start_datetime].append((filename, media.id))
        except Exception:
            logger.error(
    ...
    for start_dt, media_lst in tqdm(
        list(multi_lookup.items()), desc="Creating Multis", dynamic_ncols=True, position=0
    ):
