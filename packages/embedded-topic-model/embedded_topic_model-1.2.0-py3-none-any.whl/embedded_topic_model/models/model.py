...
class Model(nn.Module):
    ...
    def get_theta(self, normalized_bows, debug=False):
        mu_theta, logsigma_theta, kld_theta = self.encode(normalized_bows)
        if debug:
            print(f'get_theta - mu_theta: {mu_theta}, logsigma_theta: {logsigma_theta}, kld_theta: {kld_theta}')
        z = self.reparameterize(mu_theta, logsigma_theta)
        if debug:
            print(f'get_theta - z: {z}')
        theta = F.softmax(z, dim=-1)
        if debug:    
            print(f'get_theta - theta: {theta}')
