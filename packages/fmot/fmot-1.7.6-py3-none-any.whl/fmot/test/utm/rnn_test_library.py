...
rnn_library['temporal_conv1d'] = TestSet(
    ...
    par_sets=[dict(
        ...
        kernel_size=K,
        dilation=D)
    for T, Cin, Cout, K, D in
    iterprod([16], [32], [32], [3, 4], [1, 2])])
...
rnn_library['big_conv1d'] = TestSet(
    ...
    par_sets=[dict(
        ...
        dilation=D)
    for T, Cin, Cout, K, D in
    iterprod([16], [64, 128], [64, 128], [3], [1, 2])])
...
class LSTMReuseUTM(SUTM):
    def __init__(self, nb_timesteps: int, hidden_size:int, num_layers: int,
...
gru_set = TestSet(
    ...
    par_sets=[dict(nb_timesteps=nb_timesteps,
                   ...
                   bias=bias,
                   batch_first=True)
              for nb_timesteps, hidden_size, num_layers, bias in 
              iterprod([6], [32, 64], [1, 2], [True])]
    )
...
rnn_library['unfold_conv1d'] = TestSet(
    utm=UnfoldTCNUTM,
    par_sets=[dict(nb_timesteps=10, channels=ch, dilation=dil, 
                   depthwise=dw, kernel_size=k) 
