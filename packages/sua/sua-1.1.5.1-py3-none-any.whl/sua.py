...
def get_report_st(my_portfolio, rf=0.0, sigma_value=1, confidence_value=0.95, filename: str = "report.pdf"):
    try:
        # we want to get the dataframe with the dates and weights
        rebalance_schedule = my_portfolio.rebalance
    ...
    html = create_download_link(pdf.output(dest="S", name=filename).encode("latin-1"), "test")
...
def prediction_st(portfolio, start_date, end_date, weights=None, prediction_days=None, based_on='Adj Close'):
	st.write("Collecting datas...")
    # define weights
	if weights == None:
		weights = [1.0 / len(portfolio)] * len(portfolio)
	...
	for asset in portfolio:
		...
		st.write("Evaluating the models for " + str(asset) + "...")
		...
		st.write("Making the predictions for " + str(asset) + "...")
		predict(ExponentialSmoothing())
		predict(Prophet())
		predict(AutoARIMA())
		predict(Theta())
	...
	st.write("\n")
	st.write("Assets MAPE (accuracy score)")
	with pd.option_context('display.max_rows', None, 'display.max_columns', None) and pd.option_context(
            'expand_frame_repr', False):
		st.write(mape_df.iloc[:-1, :])
	mape_df = pd.DataFrame(mape_df.iloc[:-1, :])
	st.write("\n")
	st.write("Assets returns prediction for the next " +
          str(prediction_days) + " days")
	with pd.option_context('display.max_rows', None, 'display.max_columns', None) and pd.option_context(
            'expand_frame_repr', False):
		st.write(final_df.iloc[:-1, :])
	final_df = pd.DataFrame(final_df.iloc[:-1, :])
	...
	st.write("\n")
	st.write("Portfolio returns prediction for the next " +
          str(prediction_days) + " days")
	st.write(portfolio_pred.iloc[0])
