...
class Meter(object):
    ...
    def encode(self, context: Context):
        ...
        self._element = base.replace_global_result(context, self._element)
        if isinstance(self._parameter, dict):
            self._parameter = _replace(context, self._parameter)
            self._parameter = base.replace_global_result(context, self._parameter)
    ...
    def acv(self, context: Context):
        ...
        if self._func is not None:
            if type(self._func_parameter) is dict:
                ...
                self._func_parameter = base.replace_global_result(context, self._func_parameter)
            try:
                if self._expect_result is None:
                    expect_result = context.runtime.sos[context.runtime.step - 1]
                else:
                    if str(self._expect_result).find(',') > 0:
                        expect_result = []
                        rs = str(self._expect_result).split(',')
                        for r in rs:
...
class Bench(object):
    ...
    def encode(self, context: Context):
        ...
        if type(self._parameter) is dict:
            self._parameter = base.replace_context(context, self._parameter)
            self._parameter = base.replace_global_result(context, self._parameter)
    ...
    def acv(self, context: Context):
        ...
        if self._func is not None:
            if type(self._func_parameter) is dict:
                data['parameter'] = base.replace_context(context, self._func_parameter)
            try:
                if self._expect_result is None:
    ...
    def _times(self, context: Context):
        self._interval = base.replace_global_result(context, self._interval)
        if self._interval > 0:
...
class ATS(object):
    ...
    def jump(self, context: Context):
        if isinstance(self._jp, dict) and len(self._jp) > 0:
            ...
            times = base.replace_global_result(context, times)

            logger.info('~ @ATS:JUMP-> context:{} step:{} times:{}'.format(self._ctx, step, times))

            context.runtime.jump_times += 1

            base.send(context, todo={'app:show': {'msg': base.JUMP_MESSAGE.format(
                context.runtime.jump_times, times, step)}})
