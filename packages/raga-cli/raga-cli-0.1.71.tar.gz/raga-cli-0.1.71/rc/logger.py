"""Manages logging configuration for RC"""
...
from rc.env import RC_SHOW_TRACEBACK
from rc.progress import Tqdm
...
def _is_verbose():
    return (
        logging.NOTSET < logging.getLogger("rc").getEffectiveLevel() <= logging.DEBUG
    )
...
def setup(level: int = logging.INFO, log_colors: bool = True) -> None:
    ...
    show_traceback = bool(os.environ.get(RC_SHOW_TRACEBACK))
