...
class Database:
        ...
        def insert_table(value):
            try: 
               cursorobj = con.cursor()
               cursorobj.execute('INSERT INTO Epizode (candel_num , Type , point_Pattern , point_5 , point_5_time  , command , candel_coler , price_candel_open , price_candel_close , gap_point , gap_amount , gap_pip ,gap_word , tension , status , chek , time_start_search , time_end_Pattern , timestamp , times , ticket) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)', value )
               con.commit()
        ...
        def update_table_chek(point_5 , point_5_time  , command , chek , ticket , candel_num ):
            try:
               cursorobj = con.cursor()
               sqlite_update_query = """UPDATE Epizode SET point_5 = ? , point_5_time =? , command = ? , chek = ? , ticket = ?  where candel_num = ?"""
               cursorobj.execute(sqlite_update_query , (point_5 , point_5_time , command , chek , ticket  , candel_num) )
               record = con.commit()
               print("Record Update successfully")
               cursorobj.close()
               return record
