...
__all__ = ["AccountingEngines", "BillEngines", "CallbackInfo"]
...
class AccountingEngines(ElementBase):
    """会计引擎"""

    api_class = AccountingEnginesAPI
    api: AccountingEnginesAPI

    @cached_property
    def meta(self) -> AccountingEnginesDTO:
        """会计/单据引擎的元数据信息"""
        return self.api.engines.get_accounting_info_by_name_type(
    ...
    def execute(
        ...
        if sync:
            return self.api.engines.exec_by_id(request_body)
        else:
            return self.api.engines.sync_exec_by_id(request_body)
    ...
    def update_status(self, key: List[str], status: Union[Status, str]) -> None:
        ...
        return self.api.engines.update_control_status(
            AccountingControlRequestDTO(
                elementName=self.element_name,
                folderId=self.element_info.folderId,
                elementType=self.element_info.elementType,
...
class AccountingEventType(ElementBase):
    """会计事件类型"""

    api_class = AccountingEventTypeAPI
    api: AccountingEventTypeAPI

    @cached_property
    def meta(self) -> AccEventTypeElementDTO:
        """会计事件类型元数据信息"""
        return self.api.engines.get_info_by_name(
            elementType=self.element_info.elementType,
            elementName=self.element_name,
            folderId=self.element_info.folderId
        )
    ...
    def insert_to_event_table(self, object_id_list: List[str]) -> None:
        ...
        return self.api.engines.insert_to_event_table(
            AccEventTypeTableDTO(
                elementName=self.element_name,
                folderId=self.element_info.folderId,
