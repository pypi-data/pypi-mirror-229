...
## databases
## diat.barcode
def create_database_diat_barcode(makeblastdb_exe, diatbarcode_xlsx, project_folder, databases_path):
...
def create_database_from_fasta(makeblastdb_exe, fasta_file, project_folder, databases_path):
...
## midori2, pr2 database
def create_database_silva(makeblastdb_exe, fasta_file, project_folder, databases_path):
    ' Create new FASTA database using makeblastdb '
    ...
    db_name = Path(db_folder).joinpath('db')
    subprocess.call([makeblastdb_exe, '-in', str(tmp_fasta), '-dbtype', 'nucl', '-out', str(db_name)])
...
## local NCBI-mined DB
def create_database_NCBI(makeblastdb_exe, genbank_file, project_folder, databases_path):
