...
class Base(object):
    ...
    def create_stats(self,
        ...
        df["nxt1_ret"] = np.log(1. +
                                df["nxt1_ret"]) if is_log else df["nxt1_ret"]
        if not no_code:
            df = df.groupby("code").rolling(window=horizon + 1)['nxt1_ret'].sum() \
            .groupby(level=0)
        else:
            df = df.rolling(window=horizon)['nxt1_ret'].sum()
        return df.shift(-(horizon + offset - 1)).dropna().reset_index(
        ) if is_shift else df.shift(-(0)).dropna().reset_index()

    def create_train_data(self, horizon, offset, total_data):
        yields_data = self.create_stats(
            df=total_data[['trade_date', 'code', 'nxt1_ret']].copy(),
            horizon=int(horizon),
            offset=offset)
        return total_data.drop(['nxt1_ret'],
                               axis=1).merge(yields_data,
                                             on=['trade_date', 'code'])

    def create_predict_data(self, horizon, offset, total_data):
        yields_data = self.create_stats(
            df=total_data[['trade_date', 'code', 'nxt1_ret']].copy(),
            horizon=int(horizon),
            offset=offset,
