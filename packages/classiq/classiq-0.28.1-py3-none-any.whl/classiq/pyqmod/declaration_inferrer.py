...
from classiq.interface.generator.functions import PortDeclaration, unmangle_keyword
...
def python_type_to_qmod(py_type: type) -> Optional[ConcreteClassicalType]:
    if py_type == int:
        return Integer()
    elif py_type == float:
        return Real()
        ...
        return ClassicalList(element_type=python_type_to_qmod(get_args(py_type)[0]))
    return None
...
def _extract_param_decls(args: Dict[str, Any]) -> Dict[str, ConcreteClassicalType]:
    ...
    for name, py_type in args.items():
        ...
        qmod_type = python_type_to_qmod(py_type)
