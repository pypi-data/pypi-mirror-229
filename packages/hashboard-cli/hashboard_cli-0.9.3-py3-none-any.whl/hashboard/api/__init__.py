...
from hashboard.constants import FILE_SIZE_LIMIT_MB, HB_BASE_URI
...
preview_uri = lambda build_results, query_name="createBuild": click.style(
    f"{HB_BASE_URI}/app/?build={build_results['data'][query_name]['id']}",
    underline=True,
)

build_details_uri = lambda build_results, query_name="createBuild": click.style(
    f"{HB_BASE_URI}/app/p/builds/{build_results['data'][query_name]['id']}",
    underline=True,
)

preview_model_uri = (
    lambda model_id, build_results, query_name="modelPreviewBuildFromGleanDb": f"{HB_BASE_URI}/app/m/{model_id}?build={build_results['data'][query_name]['id']}"
)
...
def _graphql_query(session: Session, query: str, variables: dict):
    ...
    if r.status_code == 504:
        raise ClickException(
            f"The Hashboard CLI client has timed out but your request is still running on our server. Please check your project's build page in a few minutes to see build results: {HB_BASE_URI}/app/p/data-ops"
        )
