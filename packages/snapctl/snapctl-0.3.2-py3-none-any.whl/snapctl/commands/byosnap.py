...
class ByoSnap:
  SUBCOMMANDS = ['publish-image', 'publish-version', 'upload-docs']
  DEFAULT_BUILD_PLATFORM = 'linux/arm64'

  def __init__(self, subcommand: str, token: str, base_url: str, api_key: str, sid: str, tag: str, path: Union[str, None], dockerfile: str, prefix: str, version: Union[str, None], http_port: Union[int, None]) -> None:
  ...
  def upload_docs(self) -> bool:
    ...
    with Progress(
      ...
      try:
        ...
        test_res = requests.post(f"{self.base_url}/v1/snapser-api/byosnaps/{self.sid}/upload/{self.tag}/openapispec", files = {"attachment": dfile}, headers={'api-key': self.api_key})
        if test_res.ok:
    ...
    with Progress(
      ...
      try:
        ...
        test_res = requests.post(f"{self.base_url}/v1/snapser-api/byosnaps/{self.sid}/upload/{self.tag}/markdown", files = {"attachment": dfile}, headers={'api-key': self.api_key})
        if test_res.ok:
  ...
  def publish_version(self) -> bool:
    with Progress(
      ...
      try:
        ...
        res = requests.post(f"{self.base_url}/v1/snapser-api/byosnaps/{self.sid}/publish-version", json=payload, headers={'api-key': self.api_key})
        if res.ok:
          return True
