...
def schedule_gant_chart_fig(schedule_dataframe: pd.DataFrame,
    ...
    for swork in schedule_dataframe['scheduled_work_object']:
        for zone in swork.zones_pre:
            zone_series = pd.Series()
            zone_series['task_name'] = zone.name
            zone_series['start'] = zone.start_time
            zone_series['finish'] = zone.end_time
            schedule_dataframe.append(zone_series)
