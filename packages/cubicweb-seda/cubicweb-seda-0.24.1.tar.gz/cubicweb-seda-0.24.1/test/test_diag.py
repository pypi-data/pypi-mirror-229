...
class CompatAnalyzerTC(CubicWebTC):
    def test_rules(self):
        with self.admin_access.cnx() as cnx:
            ...
            self.assertFailingRules(
                doctor, ["SEDA 2.1", "SEDA 1.0", "SEDA 0.2", "simplified"]
            )
            ...
            cnx.commit()
            self.assertFailingRules(
                doctor, ["SEDA 2.1", "SEDA 0.2", "simplified"], "seda1_need_access_rule"
            )

            access_rule = create("SEDAAccessRule", seda_access_rule=unit_alt_seq)
            unit_alt_seq.cw_clear_all_caches()
            cnx.commit()
            self.assertFailingRules(doctor, ["SEDA 2.1"], "rule_without_rule")
            ...
            cnx.commit()
            self.assertFailingRules(
                doctor, ["SEDA 2.1"], "rule_unsupported_card", "rule_need_start_date"
            )
            ...
            cnx.commit()
            self.assertFailingRules(doctor, ["SEDA 2.1"], "rule_start_unsupported_card")
            ...
            access_rule.cw_clear_all_caches()
            cnx.commit()
            self.assertFailingRules(doctor, ["SEDA 2.1"], "rule_with_too_much_rules")
            ...
            access_rule.cw_clear_all_caches()
            cnx.commit()
            self.assertFailingRules(doctor, ["SEDA 2.1"], "rule_ref_non_rule_id")
    ...
    def test_archive_unit_reference_in_transfer(self):
        with self.admin_access.cnx() as cnx:
            ...
            doctor = transfer.cw_adapt_to("ISEDACompatAnalyzer")
            cnx.commit()
            self.assertFailingRules(doctor, ["SEDA 2.1"], "use_archive_unit_ref")
    ...
    def test_archive_unit_reference_in_archive_unit(self):
        with self.admin_access.cnx() as cnx:
            ...
            cnx.commit()
            self.assertFailingRules(
                doctor, ["SEDA 2.1"], "use_archive_unit_ref", "seda1_need_access_rule"
            )
