    ...
    for i in range(3):
        try:
            ...
            if response.status_code == 200 and len(uels) > 0:
                # B2Cloudに遷移
                session.get('https://newb2web-s2.kuronekoyamato.co.jp/b2/d/_html/index.html?oauth&call_service_code=A')
                return session
        except:
            pass
...
def get_history(session:requests.Session, params:dict):
    ...
    """
    url = 'https://newb2web-s2.kuronekoyamato.co.jp/b2/p/history'
...
def put_tracking(session:requests.Session, feed:dict):
    ...
    headers = {
        ...
        'Origin': 'https://newb2web-s2.kuronekoyamato.co.jp',
    }
    # 配送状況を更新して取得する。配送状況の名称(tracking_status_name)はこの処理でしかとれない
    response = session.put(
        'https://newb2web-s2.kuronekoyamato.co.jp/b2/p/history?tracking',
        headers=headers,
        data=data,
    )
...
def post_new_checkonly(session:requests.Session, shipments:list):
    ...
    # postに必須
    headers = {'Origin': 'https://newb2web-s2.kuronekoyamato.co.jp'}

    # 内容の事前チェック
    res = session.post('https://newb2web-s2.kuronekoyamato.co.jp/b2/p/new?checkonly', headers=headers, json=json_data)
...
def post_new(session:requests.Session, checked_feed:dict):
    ...
    headers = {'Origin': 'https://newb2web-s2.kuronekoyamato.co.jp'}
    # 登録
    res = session.post('https://newb2web-s2.kuronekoyamato.co.jp/b2/p/new', headers=headers, json=checked_feed)
    return json.loads(res.text)
...
def get_new(session:requests.Session, params=None):
    ...
    """
    headers =  {'Origin': 'https://newb2web-s2.kuronekoyamato.co.jp'}
    url = 'https://newb2web-s2.kuronekoyamato.co.jp/b2/p/new'
...
def delete_new(session:requests.Session, feed:dict):
    ...
    response = session.delete(
        'https://newb2web-s2.kuronekoyamato.co.jp/b2/p/new',
        headers=headers,
        data=data,
    )
    res = json.loads(response.text)
...
def print_issue(session:requests.Session, print_type:str, entry_feed:dict):
    ...
    if entry_feed['feed']['entry'][0]['shipment']['tracking_number'].startswith('VMN'):
        # 新規印刷
        response = session.post(f'https://newb2web-s2.kuronekoyamato.co.jp/b2/p/new?issue&print_type={print_type}&sort1=service_type&sort2=created&sort3=created&print_position=1&print_position=2&print_position=3&print_position=4&print_position=5&print_position=6&print_position=7&print_position=8', headers=headers, json=json_data)
    else:
        # 再印刷
        response = session.put(f'https://newb2web-s2.kuronekoyamato.co.jp/b2/p/history?reissue&print_type={print_type}&sort1=service_type&sort2=created&sort3=created&print_position=1&print_position=2&print_position=3&print_position=4&print_position=5&print_position=6&print_position=7&print_position=8',headers=headers, json=json_data)

    issue_no = json.loads(response.text)['feed']['title']
    # _res['feed']['title'] == "Success"になるまでループする
    for i in range(100):
        res = session.get(f'https://newb2web-s2.kuronekoyamato.co.jp/b2/p/polling?issue_no={issue_no}&service_no=interman')
    ...
    _ = session.get(f'https://newb2web-s2.kuronekoyamato.co.jp/b2/p/B2_OKURIJYO?checkonly=1&issue_no={issue_no}')

    # 生成されたPDFデータをダウンロードする
    res = session.get(f'https://newb2web-s2.kuronekoyamato.co.jp/b2/p/B2_OKURIJYO?issue_no={issue_no}&fileonly=1')
...
def put_history_delete(session:requests.Session, feed:dict)->dict:
    ...
    url = 'https://newb2web-s2.kuronekoyamato.co.jp/b2/p/history?display_flg=0'
    headers = {
    'Origin': 'https://newb2web-s2.kuronekoyamato.co.jp',
    }
...
def put_history_display(session:requests.Session, feed:dict):
    ...
    for entry in feed['feed']['entry']:
        ...
        entries.append(_entry)
    _feed = {'feed':{'entry':entries}}
    url = 'https://newb2web-s2.kuronekoyamato.co.jp/b2/p/history?display_flg=1'
    headers = {
    'Origin': 'https://newb2web-s2.kuronekoyamato.co.jp',
    }
    response = session.put(url, headers=headers, json=_feed)
...
def get_dm_number_print(session:requests.Session, params:dict):
    ...
    response = session.get('https://newb2web-s2.kuronekoyamato.co.jp/b2/p/history', params=_params)
    ...
    for i in range(100):
        ...
        raise Exception("PDFデータ生成に失敗")
    # 完了後にcheckを読み込むレスポンスは空
    res = session.get(f'https://newb2web-s2.kuronekoyamato.co.jp/b2/p/B2_OKURIJYO?checkonly=1&issue_no={issue_no}')
...
def __compress_feed(session, feed):
    ...
    def __create_template():
        ...
        response = session.get('https://newb2web-s2.kuronekoyamato.co.jp/b2/d/_settings/template')
