...
def main():
    ...
    if (not args.session) and (not args.sequence) :
        ...
        print('remember to close your sessions using the `/done` command !\n')
        ...
        if args.session:
            ...
            if (not args.new) and (not args.resume) and (not args.join) and (not args.url) :
                if (not args.multiplayer):
                    ...
                    # shell2 --session --multiplayer
                    cmd_live = f'shell2_cli_live --sandbox session --action new --timeout {timeout} --multiplayer'
                    os.system(cmd_live)                
            elif (not args.new) and (not args.resume) and (not args.join) and (args.url):
                # shell2 --session --join {url}
