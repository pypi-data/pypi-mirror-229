...
class Variable(ElementBase):
    ...
    @cached_property
    def meta(self) -> VariableVO:
        """系统中的变量列表的元数据信息"""
        return self.api.variable.query(
    ...
    def save(self):
        ...
        将当前元素内变量信息保存至系统
        """
        payload = UpdateVariavlesDTO.construct_from(self.meta, description=self._description,
                                                    globalVariables=list(self._gv_memo.values()),
                                                    userVariables=list(self._uv_memo.values()),
                                                    moduleId=self.api.module_id
                                                    )
        return self._update_impl(payload=payload)

    def _update_impl(self, payload: UpdateVariavlesDTO):
        self.api.variable.update(payload)
        # 使memo的缓存失效
        self.__gv_memo = None
        self.__uv_memo = None
        self.__description = None
