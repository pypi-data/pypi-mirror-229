...
def module(name: str):
    ...
    This command creates a new directory with the given name and scaffolds a submodule of the app. Following files are created:\n
    - __init__.py\n
    - router.py\n
    - model.py\n
    - service.py\n
    
    Example: aiofauna module users
    """
    ...
    with open(os.path.join(name, "__init__.py"), "w") as f:
        f.write(
            f"""
from .router import app as {name}_app
from .model import {Name}Model
from .service import {Name}Service
            """
            )
    with open(os.path.join(name, "router.py"), "w") as f:
        f.write(
            f"""
from aiofauna import APIRouter

app = APIRouter()

@app.get("/")
async def {name}_home():
    return {{ "message": "Hello from {name}!" }}
            """
        )
    with open(os.path.join(name, "model.py"), "w") as f:
        f.write(
            f"""
from aiofauna import FaunaModel, Field

class {Name}Model(FaunaModel):
    pass
            """
        )
    with open(os.path.join(name, "service.py"), "w") as f:
        f.write(
            f"""
from dataclasses import field
from aiofauna import APIClient

class {Name}Service(APIClient):
    base_url:str = field(default="http://jsonplaceholder.typicode.com")
