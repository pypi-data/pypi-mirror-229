...
class BaseApplication(ApplicationInterface, metaclass=BaseApplicationMeta):
    ...
    def emit(self, data: Union[Message, Sequence[Message]]) -> None:
        ...
        for message in data:
            ...
            if not message_name and not message.resource:
                self.logger.warning(
                    "Emitted messages require a name or resource", message_type=message_type
                )
                continue
    ...
    def push_buffer(self, message: Message, expiry: Optional[int] = None) -> None:
        ...
        if expiry is None:
            try:
                ...
                expiry = int(message.timestamp.utcnow().timestamp())
    ...
    def make_recommendation(
        ...
        expiration_date : datetime, optional
            Optional absolute date when the recommendation expires. If not present, reccommendation won't ever expire.
        alarms : List[AlarmModel], optional
            Optional list of alarms triggered by this recommendation.
        control_changes : List[RecommendationControlChangeModel], optional
            Optional list of control changes triggered by this recommendation.
        datalabels : List[DataLabelModel]
            Optional list of data labels triggered by this recommendation.
        emit : bool, optional
            Emit the message
        """
        source = KRNWorkload(
