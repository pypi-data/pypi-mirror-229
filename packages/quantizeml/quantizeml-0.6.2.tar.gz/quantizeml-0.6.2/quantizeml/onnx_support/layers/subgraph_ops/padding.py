...
def get_padding_ops(in_name, out_name, pad_value_name=None):
    ...
    Args:
        ...
        pad_value_name (str, optional): name of padding value.
            Takes a zero value when not specified. Defaults to None.

    Returns:
        list of NodeProto: the operation chain.
    """
    nodes = [make_node("Pad", inputs=[in_name, "pads"], outputs=[out_name])]
    if pad_value_name is not None:
        # Include padding value as one Pad input
        nodes[-1].input.append(pad_value_name)
    return nodes
