...
def run_pack(pack_file_path):
    ...
    if stdout:
        loggerPack.info(stdout)
        logger.info(stdout)

    if stderr:
        for line in stderr.split('\n'):
            if line:  # Only print if there's output
                log_level = handle_stderr_line(line)
                if log_level == 'INFO':
                    loggerPack.info(line)
                    logger.info(line)
                else:
                    loggerPack.error(line)
                    logger.error(line)
                    has_errors = True  # set the flag if an error is encountered
