...
def submit(app: App, block: bool = False, path: str | Path = Path.cwd(), encoding: bool = True, suffix: str = '0', dry_run: bool = False):
    ...
    Args:
        ...
        encoding (bool, optional): 是否序列化. Defaults to True.
        suffix (str, optional): 线路文件后缀, 用于多个任务循环时避免文件覆盖.
        dry_run (bool, optional): 跳过设备执行
    ...
    >>> metainfo:
      ...
      > user: 实验者代号. 默认为usr. 
      > tid: 任务id, 全局唯一, 如不指定, 则由系统生成. 
      > priority: 优先级, 任务同时提交时, 优先级数值小的先执行. 默认为0. 
      > other: 其他参数, 如shots、signal等, 作为kwds传递给ccompile(见envelope.assembler)
    >>> taskinfo:
      > STEP: 大写, 描述任务执行的变量(即for循环变量)与执行步骤(即for循环体)
      > CIRQ: 大写, 描述任务线路, 长度等于STEP中for循环变量长度. 可为空. 
      > INIT: 大写, 任务初始化设置. 可为空. 
      > RULE: 大写, 变量关系列表, 可为表达式或空, 如[f'<gate.rfUnitary.{q}.params.frequency>=<freq.{q}>']. 可为空. 
      > LOOP: 大写, 定义循环执行所用变量, 与STEP中main的值对应, STEP中main所用变量为LOOP的子集
    """
    app.toserver = 'ready'
