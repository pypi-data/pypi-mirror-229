...
class MetaNodeMixin(type):
    ...
    def set_parent(cls, node):
        ...
        if cls.parent is not node:
            cls.__check_loop(node)
            cls.__detach(cls.parent)
            cls.__attach(node)

    def __check_loop(cls, new_parent):
...
class NodeMixin:
    ...
    def add_child(self, node):
        ...
        if node not in self.children:
            node.__check_loop(self)
            node.__detach(node.parent)
            node.__attach(self)

    def __check_loop(self, new_parent):
        if new_parent is None:
            return
