...
class ActivationCounter(nn.Module):
    ...
    def collect(self, reset=True):
        out = ActivationCount(L1=self._L1, L0=self._L0,
            numel=self._numel)
        if reset:
            self.reset_counter()
        return out
    ...
    for name, module in model.named_modules():
        if isinstance(module, ActivationCounter):
            ...
            itemized[name] = ActivityScore(
                L1=count_obj.L1/count_obj.numel,
                L0=count_obj.L0/count_obj.numel)
    ...
    if isinstance(obj, nn.Module):
        ...
        def wrapped(*args, **kwargs):
            ...
            if not isinstance(self, nn.Module):
                raise ValueError('reset_counters must wrap a class method of a torch.nn.Module')
            reset_counters(self, _from_wrapper=True)
            return f(*args, **kwargs)
        return wrapped
    else:
        raise ValueError('reset_counters expects either a nn.Module or function to wrap')
