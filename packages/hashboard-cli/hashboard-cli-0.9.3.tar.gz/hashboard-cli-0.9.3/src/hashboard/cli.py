...
from hashboard.constants import DEFAULT_CREDENTIALS_FILEPATH, HB_DEBUG
...
HB_VERBOSE_DEBUG_UNSAFE = getenv_bool("HB_VERBOSE_DEBUG_UNSAFE")
...
def main():
    ...
    with cli_error_boundary(debug=HB_DEBUG):
...
@click.option(
    ...
    type=str,
    help="Path to your Hashboard access key credentials. You can also control this by setting a HB_CREDENTIALS_FILEPATH environment variable.",
)
@click.pass_context
def cli(ctx: click.Context, credentials_filepath: str):
    """A command-line interface for interacting with Glean."""
    if HB_DEBUG or HB_VERBOSE_DEBUG_UNSAFE:
...
def upload(ctx, database, filepath):
    """Pushes a file to a Hashboard duckdb database connection that can be queried using duckdb or used as the basis for a data model."""
    ...
    click.echo("📄 Uploading file to Hashboard...")
    filename = upload_files_to_hashboard(s, project_id, database, fp)
