...
class DataPreprocessor:
    ...
    def save_processed_data(self):
        self.data.to_csv("output/output.csv", index=False)
...
class ImageProcessor:
    def __init__(self, folder_path):
        ...
        self.csv_file_path = os.path.join(folder_path, 'output.csv')
    ...
    def plot_and_save(self, data, x, title, order=None, rotation=45):
        fig, ax = plt.subplots(figsize=(10, 5))
        sns.countplot(x=x, data=data, order=order)
        plt.title(title)
        plt.xticks(rotation=rotation)
        for i in ax.patches:
            ax.text(i.get_x() + 0.1, i.get_height() + 0.1, str(round((i.get_height()))), fontsize=7, color='black')
