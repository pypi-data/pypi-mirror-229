...
class SysCube(CubeBase):
    ...
    def __init__(
        ...
        **options
        ...
        self.cube = cube = FinancialCube(cube_name, folder_id, path)
        ...
        with ThreadCtxExecutor(max_workers=len(cube.dim_elements)) as executor:
            for dim_name in cube.dim_elements.keys():
                executor.submit(
                    self._dim_init, dim_name, cube,
                    self.dim_type, extra_dim_attrs.get(dim_name, [])
                )

            executor.submit(self._dim_view_init, cube)

    def _dim_init(self, dim_name: str, cube: FinancialCube, dim_type: DimensionType, extra_attr: List[str]):
        dim_column = cube.dim_col_map[dim_name]
        dim_element = cube.dim_elements[dim_name]

        if dim_type == DimensionType.complete:
            data = dim_element.table_dimension.select_raw(
    ...
    def _dim_view_init(self, cube: FinancialCube):
        """初始化View维度"""
        ...
        api = DimensionAPI(module_id=period.moduleId, sync=True)
        view_info = api.query.get_view_by_period(ElementQueryBaseDto(
    ...
    def load_expr(self, cube_expr: str):
        ...
        jobs = []
        with ThreadCtxExecutor(max_workers=len(self.dimensions)) as executor:
            for dim_expr in cube_expr.split('->'):
                ...
                executor.submit(self.dimensions[dimname].load_expr, expr)
        return self
