...
def get_request(endpoint: str, api_key: str, **kwargs) -> dict | str:
    ...
    base_endpoints = {
        ...
        "odds": "https://api.the-odds-api.com/v4/sports/{sports}/odds/?apiKey={api_key}",
        "scores": "https://api.the-odds-api.com/v4/sports/{sports}/scores/?apiKey={api_key}",
        "historical_odds": "https://api.the-odds-api.com/v4/sports/{sports}/odds-history/?apiKey={api_key}",
        "event_odds": "https://api.the-odds-api.com/v4/sports/{sports}/events/{event_id}/?apiKey={api_key}",
    }
    ...
    if endpoint == "sports":
        ...
        call_endpoint = call_endpoint.format(
            api_key=api_key, sports=",".join(kwargs.get("sports"))
        )
    else:
        call_endpoint = call_endpoint.format(
            api_key=api_key,
    ...
    for arg_name, arg_value in locals()["kwargs"].items():
        if (
            arg_name not in ["endpoint", "api_key", "sports", "event_id"]
