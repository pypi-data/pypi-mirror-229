...
class Base(object):
    ...
    def create_data(self, total_data, name=None, returns=None, **kwargs):
        models = {}
    ...
    def predict(self, total_data, returns, models):
        total_data['trade_date'] = pd.to_datetime(total_data['trade_date'])
        begin_date = total_data['trade_date'].dt.strftime('%Y-%m-%d').min()
        end_date = total_data['trade_date'].dt.strftime('%Y-%m-%d').max()
    ...
    def train(self, total_data, is_save=1):  # 0 不保存   1. 保存本地  2.保存OSS
        models = self.educate(
