...
class RemoteMount:
    ...
    settings : Remote
        ...
        and user preferences. Please see ``Remote``
    remote_path : Path | str
    ...
    def mount(self):  # noqa: DOC502
        ...
        def _monitor():
            while self._heart is not None and self._heart.is_alive():
                self._heart.join(self._refresh_interval)
            self._heart = None
    ...
    def __write_settings(self):
        ...
        self._config_path = Path(tmp_dir) / ".rmount.conf"
        descriptor = os.open(
            path=self._config_path.as_posix(),
            flags=(
