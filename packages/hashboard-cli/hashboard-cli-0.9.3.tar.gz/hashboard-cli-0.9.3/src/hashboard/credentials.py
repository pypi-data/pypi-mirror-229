...
def get_credentials(credentials_filepath: str) -> CliCredentials:
    ...
    project_id_from_env = env_with_fallback("HB_PROJECT_ID", "GLEAN_PROJECT_ID")
    access_key_id_from_env = env_with_fallback("HB_ACCESS_KEY_ID", "GLEAN_ACCESS_KEY_ID")
    access_key_token_from_env = env_with_fallback("HB_SECRET_ACCESS_KEY_TOKEN", "GLEAN_SECRET_ACCESS_KEY_TOKEN")
    if project_id_from_env and access_key_id_from_env and access_key_token_from_env:
        ...
        raise RuntimeError(
            'Either all or none of these environment variables must be set: "HB_PROJECT_ID", "HB_ACCESS_KEY_ID", "HB_SECRET_ACCESS_KEY_TOKEN"'
        )
