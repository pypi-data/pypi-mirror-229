...
from src.addons_installer.git_addons import (
    GitOdooAddons,
    GitOdooAddonsResult,
    LegacyDepotGit,
)
...
class TestLegacyDepotGit(unittest.TestCase):
    ...
    def test_legacy_depot_git_numbered(self):
        ...
        env = {
            ...
            "DEPOT_GIT": "-b master --single-branch --depth=1 git@gitlab.ndp-systemes.fr:odoo/v15/issues.git",
            "DEPOT_GIT_2": "-b master --single-branch --depth=1 git@gitlab.ndp-systemes.fr:odoo/v15/issues.git",
            "DEPOT_GIT_3": "-b master --single-branch --depth=1 git@gitlab.ndp-systemes.fr:odoo/v15/issues.git",
            "DEPOT_GIT_4": "-b master --single-branch --depth=1 git@gitlab.ndp-systemes.fr:odoo/v15/issues.git",
            "DEPOT_GIT_5": "-b master --single-branch --depth=1 git@gitlab.ndp-systemes.fr:odoo/v15/issues.git",
            "DEPOT_GIT_6": "-b master --single-branch --depth=1 git@gitlab.ndp-systemes.fr:odoo/v15/issues.git",
        }
        for number in [""] + list(map(str, range(2, 10))):
            with self.subTest(f"For debot_git => [{number}]"):
                ...
                name = "depot_git"
                if number:
                    self.assertEqual(git_addons.NAME.full_key, "_".join(["DEPOT_GIT", number]))
                    name = "_".join([name, number])
                else:
